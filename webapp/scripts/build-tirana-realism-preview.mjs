...
import {SIGN_REFERENCES} from '../src/games/tirana-street-life/signReferences.mjs';
...
const artwork=[];for(const r of SIGN_REFERENCES){const file=path.join(root,'public',r.logo),mime=r.logo.endsWith('.svg')?'image/svg+xml':r.logo.endsWith('.jpg')?'image/jpeg':'image/png';artwork.push({...r,logo:'data:'+mime+';base64,'+(await readFile(file)).toString('base64')});}
const result=await build({entryPoints:[path.join(root,'src/games/tirana-environment/RealismReview.tsx')],write:false,bundle:true,minify:true,format:'esm',platform:'browser',plugins:[{name:'realism-in-chat',setup(b){
 ...
 b.onLoad({filter:/tirana-street-life\/signReferences\.mjs$/},()=>({contents:'export const SIGN_REFERENCES=['+artwork.map(r=>'({...'+JSON.stringify(r)+',match:'+r.match.toString()+'})').join(',')+'];export const signReferenceFor=text=>SIGN_REFERENCES.find(r=>r.match.test(text));export const referencedAdvertising=s=>s;',loader:'js'}));
}}]});
