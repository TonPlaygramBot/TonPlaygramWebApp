...
import {CITY_BUSINESS_BUILDING_PROFILES} from '../src/games/tirana-city-source/cityBusinessProfiles.mjs';
...
const cityExpansion=process.argv.includes('--city'),reviewProfiles=cityExpansion?CITY_BUSINESS_BUILDING_PROFILES:BUSINESS_BUILDING_PROFILES;
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');
const centres=WORLD.buildings.filter(b=>reviewProfiles[b.id]).map(b=>[b.p.reduce((s,p)=>s+p[0],0)/b.p.length,b.p.reduce((s,p)=>s+p[1],0)/b.p.length]);
const near=(x,z)=>centres.some(p=>Math.hypot(x-p[0],z-p[1])<(cityExpansion?130:180));
...
const template=await readFile(path.join(root,'scripts/tirana-business-preview.fragment.html'),'utf8');const html=template.replace('id="tirana-business-review-root"','id="tirana-business-review-root" data-edition="'+(cityExpansion?'city':'business')+'"').replace('/*__BUSINESS_REVIEW_BUNDLE__*/',result.outputFiles[0].text.replace(/<\/script/gi,'<\\/script'));
if(Buffer.byteLength(html)>1000000)throw Error('Preview exceeds 1 MB: '+Buffer.byteLength(html));
await writeFile(process.argv.slice(2).find(a=>!a.startsWith('--'))||(cityExpansion?'/workspace/tirana-city-businesses.html':'/workspace/tirana-business-landmarks.html'),html);console.log({bytes:Buffer.byteLength(html),buildings:subset.buildings.length,roads:subset.roads.length,trees:trees.length,businesses:businesses.length,shops:shops.storefronts.length,brands:artwork.length});
