...
import json,base64,struct,hashlib,gzip,sys
operator = "--operator" in sys.argv
...
for m in j['materials']:
 ...
 if operator:
  c={'Suit':[.045,.095,.15,1],'Tie':[.12,.15,.19,1],'White':[.55,.59,.62,1],'Hair':[.055,.032,.02,1]}.get(m.get('name'))
 if c:m['pbrMetallicRoughness']['baseColorFactor']=c
# The hair is a separate primitive. Keep the complete scalp from Skin.
for m in j['meshes']:
 if not operator and m.get('name')=='Cube.006':m['primitives']=[p for p in m['primitives'] if j['materials'][p.get('material',0)]['name']!='Hair']
...
j['extras']={'source':'https://quaternius.com/packs/ultimatemodularcharacters.html','license':'CC0-1.0','sourceSHA256':hashlib.sha256(source).hexdigest(),'heightMetres':1.78,'appearance':'Tirana operator, navy jacket and original hair' if operator else 'Bald suited agent with red tie'}
binary=bytearray()
...
clips=['Idle','Walk','Run','Idle_Gun','Idle_Gun_Pointing','Run_Shoot','Gun_Shoot'] if operator else ['Idle','Walk','Run']
...
p=ROOT/'webapp/public/assets/tirana-streets/living'/('operator.glb' if operator else 'suited-agent.glb');p.write_bytes(out);print({'bytes':len(out),'animations':len(j['animations']),'sha256':hashlib.sha256(out).hexdigest()})
