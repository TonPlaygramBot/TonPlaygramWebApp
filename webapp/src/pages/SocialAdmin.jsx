...
export default function SocialAdmin() {
  ...
  return <main className="social-admin">
    ...
    {tab === 'accounts' && <section className="social-stack"><h2>Accounts</h2><p className="social-note">OAuth credentials stay on the server. Official provider adapters are required before leaving mock mode.</p><div className="social-cards">{platforms.map((platform) => { const account = overview.accounts.find((a) => a.platform === platform); return <article key={platform}><div className={`platform-dot ${platform}`}>{label(platform)[0]}</div><div><strong>{label(platform)}</strong><span>{account?.accountName || 'Not connected'}</span><small>{account?.status || 'Official OAuth setup required'}</small></div><button disabled>{account ? 'Test' : 'Connect'}</button></article>; })}</div></section>}
