...
export default function SocialAdmin() {
  ...
  return <main className="social-admin">
    ...
    {tab === 'overview' && <section className="social-stack"><h2>Overview</h2><div className="social-metrics">{[['Connected accounts', overview.accounts.filter((a) => a.status === 'CONNECTED').length], ['Scheduled posts', overview.stats.scheduled || 0], ['Published posts', overview.stats.published || 0], ['Failed posts', overview.stats.failed || 0], ['Automatic tasks', overview.stats.tasks || 0]].map(([text, value]) => <article key={text}><strong>{value}</strong><span>{text}</span></article>)}</div><h2>Connected Accounts</h2><div className="social-cards">{platforms.map((platform) => { const account = overview.accounts.find((a) => a.platform === platform); return <article key={platform}><div className={`platform-dot ${platform}`}>{label(platform)[0]}</div><div><strong>{label(platform)}</strong><span>{account?.status?.replaceAll('_', ' ') || 'Not connected'}</span></div></article>; })}</div><h2>Recent activity</h2><PostCards posts={overview.recent} /></section>}
    ...
    {tab === 'accounts' && <section className="social-stack"><h2>Connected accounts</h2><p className="social-note">Connect with each social app. You will approve the requested permissions on the provider's secure authorization screen—no credentials are entered here.</p>{message && <p className={message.includes('successfully') ? 'social-success' : 'social-error'} role="status">{message}</p>}<div className="social-cards account-cards">{platforms.map((platform) => { const account = overview.accounts.find((a) => a.platform === platform && a.status === 'CONNECTED'); return <article key={platform}><div className={`platform-dot ${platform}`}>{label(platform)[0]}</div><div><strong>{label(platform)}</strong><span>{account?.accountName || 'Not connected'}</span><small>{account ? 'Authorized permissions active' : `Authorize TonPlaygram with ${label(platform)}`}</small></div><button className={account ? 'connected' : ''} disabled={connecting === platform} onClick={() => connectAccount(platform)}>{connecting === platform ? <RefreshCw className="spin" size={15} /> : account ? <Check size={15} /> : <Link2 size={15} />}{connecting === platform ? 'Opening…' : account ? 'Reconnect' : 'Connect'}</button></article>; })}</div><p className="oauth-privacy"><strong>Secure OAuth</strong><br />Passwords are never shared with TonPlaygram. Access can be revoked at any time from your social account settings.</p></section>}
