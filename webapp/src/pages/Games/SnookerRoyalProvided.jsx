...
const SNOOKER_CHAMPION_BALL_MATERIAL_VARIANT = 'pool';
const BALL_VISIBLE_LIFT = 0.045 * 3.5;
const BALL_CENTER_Y = 0.045 * 3.5 + BALL_VISIBLE_LIFT;
const HDRI_RESOLUTION_ORDER = Object.freeze(['4k', '2k', '1k']);
...
function createBall(number, color, isCue = false, kind = 'red', value = 1, spot = null) {
  const material = getBilliardBallMaterial({
    ...
    number: null,
    variantKey: SNOOKER_CHAMPION_BALL_MATERIAL_VARIANT
  });
  const mesh = new THREE.Mesh(new THREE.SphereGeometry(CFG.ballR, 48, 48), material);
  mesh.renderOrder = 30;
...
function snookerRackPositions() {
  ...
  const out = [{ n: 0, c: 0xf8fafc, p: new THREE.Vector3(-CFG.tableW * 0.16, BALL_CENTER_Y, baulkZ), cue: true, kind: 'cue', value: 0 }];
  let idx = 1;
  for (let row = 0; row < 5; row += 1) {
    for (let i = 0; i <= row; i += 1) {
      out.push({
        n: idx++,
        c: 0xdc2626,
        p: new THREE.Vector3((i - row / 2) * CFG.ballR * 2.04, BALL_CENTER_Y, redApexZ - row * CFG.ballR * 1.78),
        kind: 'red',
  ...
  const spots = {
    yellow: new THREE.Vector3(-CFG.tableW * 0.24, BALL_CENTER_Y, baulkZ),
    green: new THREE.Vector3(CFG.tableW * 0.24, BALL_CENTER_Y, baulkZ),
    brown: new THREE.Vector3(0, BALL_CENTER_Y, baulkZ),
    blue: new THREE.Vector3(0, BALL_CENTER_Y, 0),
    pink: new THREE.Vector3(0, BALL_CENTER_Y, redApexZ + CFG.ballR * 2.5),
    black: new THREE.Vector3(0, BALL_CENTER_Y, -CFG.tableL * 0.36)
  };
  out.push(
...
async function resolveSnookerChampionHdriUrl(config = {}) {
  ...
  if (config?.assetUrls && typeof config.assetUrls === 'object') {
    const direct = preferred.map((res) => config.assetUrls[res]).find(Boolean);
    if (direct) return direct;
  }
  if (config?.assetUrl) return config.assetUrl;
  return `https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/${fallbackRes}/${config?.assetId ?? 'neon_photostudio'}_${fallbackRes}.hdr`;
}
async function applyGroundedHdriEnvironment(scene, renderer, config = {}) {
  ...
  const url = await resolveSnookerChampionHdriUrl(config);
  ...
  loader.load(
    ...
    (texture) => {
      ...
      const cameraHeightM = Math.max(1.45, config?.cameraHeightM ?? 1.58);
      const unitsPerMeter = CFG.scale * 1.55;
      const skyboxHeight = cameraHeightM * unitsPerMeter;
      const skyboxRadius = Math.max(CFG.tableL * (config?.groundRadiusMultiplier ?? 5.1), skyboxHeight * 2.5);
      try {
        skybox = new GroundedSkybox(texture, skyboxHeight, skyboxRadius, Math.max(32, config?.groundResolution ?? 112));
        skybox.position.y = skyboxHeight;
        skybox.rotation.y = Number.isFinite(config?.rotationY) ? config.rotationY : 0;
        skybox.material.depthWrite = false;
        scene.background = null;
        scene.add(skybox);
      } catch (error) {
        console.warn('Snooker Champion grounded HDRI failed; using equirectangular background.', error);
        scene.background = envMap;
      }
      scene.environment = envMap;
      if ('backgroundIntensity' in scene && typeof config?.backgroundIntensity === 'number') {
        scene.backgroundIntensity = config.backgroundIntensity;
      }
      if ('environmentIntensity' in scene && typeof config?.environmentIntensity === 'number') {
    ...
    undefined,
    (error) => {
      console.warn('Snooker Champion HDRI load failed', error);
    }
  );
  return dispose;
}
...
function updateBalls(balls, dt, tmpA, tmpB, pocketPositions = [], rulesState = null) {
  ...
  for (const ball of balls) {
    ...
    if (pocket && ball.vel.lengthSq() < (8.5 * CFG.scale) ** 2) {
      if (ball.isCue) {
        ball.pos.set(-CFG.tableW * 0.16, BALL_CENTER_Y, CFG.tableL * 0.29);
        ball.vel.set(0, 0, 0);
        ball.spin?.set(0, 0);
        if (rulesState) {
...
export default function SnookerRoyalProvided({ gameTitle = 'Snooker Royal Provided' } = {}) {
  ...
  const giftPlayers = useMemo(() => [
    { id: 'snooker-champion-player', index: 0, name: playerName, avatar: playerAvatar },
  ...
  useEffect(() => {
    ...
    scene.add(sun);
    let disposeHdri = () => {};
    applyGroundedHdriEnvironment(scene, renderer, activeHdri).then((dispose) => {
      if (typeof dispose === 'function') disposeHdri = dispose;
    });
  ...
  return (
    <div className="fixed inset-0 bg-[#0b0b0b] text-white">
      ...
      <div className="pointer-events-none fixed inset-0 font-sans">
        ...
        <div className="pointer-events-auto">
          <BottomLeftIcons
            ...
            className="fixed left-0 z-50 flex -translate-x-2 flex-col gap-2.5"
            style={{ bottom: '12px' }}
        ...
        <div className="pointer-events-none fixed left-4 top-24 z-50 flex flex-col gap-2">
          {chatBubbles.slice(-3).map((bubble) => (
            <div key={bubble.id} className="rounded-full border border-white/15 bg-black/70 px-3 py-2 text-xs font-semibold text-white shadow-xl backdrop-blur">
              {bubble.text}
            </div>
          ))}
        </div>
      ...
      <QuickMessagePopup
        ...
        onSend={(text) => setChatBubbles((prev) => [...prev.slice(-2), { id: `${Date.now()}-${text}`, text }])}
        title="Quick Chat"
        showCloseButton
        overlayClassName="fixed inset-0 z-[70] flex items-center justify-center bg-black/70"
        panelClassName="w-72 space-y-3 rounded-3xl border border-emerald-400/45 bg-slate-950/95 p-4 text-white shadow-2xl backdrop-blur"
        messageGridClassName="grid max-h-44 grid-cols-2 gap-2 overflow-y-auto"
        ...
        sendButtonClassName="w-full rounded-full bg-emerald-300 px-3 py-2 text-sm font-black uppercase tracking-[0.18em] text-black"
      />
      <GiftPopup
        open={showGift}
        ...
        showCloseButton
        overlayClassName="fixed inset-0 z-[70] flex items-center justify-center bg-black/70"
        panelClassName="w-80 space-y-3 rounded-3xl border border-amber-300/45 bg-slate-950/95 p-4 text-white shadow-2xl backdrop-blur"
        playerButtonClassName="flex w-full items-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-2 py-1 text-sm"
        playerButtonActiveClassName="border-amber-200 bg-amber-200/20"
        giftButtonClassName="flex items-center justify-center gap-1 rounded-2xl border border-white/15 bg-white/10 px-2 py-1 text-xs"
      />
    </div>
  );
}
