...
const BALL_VISIBLE_LIFT = 0.115 * WORLD_SCALE;
const BALL_CENTER_Y = () => CFG.ballR + BALL_VISIBLE_LIFT;
...
function createBall(number, color, isCue = false, kind = 'red', value = 1, spot = null) {
  const mesh = new THREE.Mesh(
    new THREE.SphereGeometry(CFG.ballR, 64, 64),
    getBilliardBallMaterial({
      color,
  ...
  gltfTools.loader.load(
    ...
    (gltf) => {
      ...
      model.position.set(
        ...
        targetTopY - scaledFullBounds.max.y - BALL_VISIBLE_LIFT * 0.72,
      ...
      proceduralTableMeshes.forEach((mesh) => {
        ...
        if (mesh.visible) mesh.renderOrder = 1;
      });
...
function buildPolyHavenHdriUrl(variant) {
  const assetId = variant?.assetId || 'neon_photostudio';
  const preferred = variant?.preferredResolutions?.[0] || variant?.fallbackResolution || '2k';
  const resolution = preferred === '8k' ? '4k' : preferred;
  return `https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/${resolution}/${assetId}_${resolution}.hdr`;
}
function applyPoolRoyaleHdriEnvironment(scene, renderer, variant, tableTopY) {
  ...
  loader.load(
    ...
    (texture) => {
      ...
      try {
        skybox = new GroundedSkybox(texture, variant?.placement?.cameraHeightM ?? 1.58, CFG.tableL * 2.7);
        skybox.name = 'snooker-champion-grounded-hdri-skybox';
        skybox.position.y = tableTopY - CFG.ballR * 2.15;
        skybox.rotation.y = variant?.rotationY ?? 0;
        scene.add(skybox);
      } catch {
...
export default function SnookerRoyalProvided({ gameTitle = 'Snooker Royal Provided' } = {}) {
  ...
  useEffect(() => {
    ...
    const disposeHdri = applyPoolRoyaleHdriEnvironment(scene, renderer, activeHdri, CFG.tableTopY);
  ...
  return (
    <div className="fixed inset-0 bg-[#0b0b0b] text-white">
      ...
      <QuickMessagePopup
        ...
        title="Quick Chat"
        showCloseButton
        overlayClassName="fixed inset-0 z-[80] flex items-center justify-center bg-black/70"
        panelClassName="w-72 rounded-3xl border border-emerald-400/35 bg-slate-950/95 p-4 text-white shadow-2xl"
        messageButtonClassName="rounded-xl border border-white/15 bg-white/10 px-2 py-1 text-xs text-white/80"
      ...
      <GiftPopup
        ...
        players={[{ id: 'snooker-ai', index: 1, name: opponentName, photoUrl: opponentAvatar }]}
        senderIndex={0}
        title="Send Gift"
        showCloseButton
        overlayClassName="fixed inset-0 z-[80] flex items-center justify-center bg-black/70"
        panelClassName="w-80 rounded-3xl border border-emerald-400/35 bg-slate-950/95 p-4 text-white shadow-2xl"
        playerButtonClassName="flex w-full items-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-2 py-1 text-sm"
        playerButtonActiveClassName="border-emerald-300 bg-emerald-300/20"
        giftButtonClassName="flex items-center justify-center gap-1 rounded-xl border border-white/15 bg-white/10 px-2 py-1 text-xs"
      />
    </div>
  );
}
