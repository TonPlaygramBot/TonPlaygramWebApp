...
// Config
// --------------------------------------------------
// Scale factors to match real-world snooker dimensions
...
// align ball centers with cloth by offsetting from the table's top surface
const BALL_Y = -TABLE.THICK + BALL_R;
...
function Table3D(scene) {
  ...
  // Markings on cloth
  const markingMat = new THREE.LineBasicMaterial({ color: COLORS.mark });
  ...
  // Cushions: extended with straight undercut for right-triangle profile
  const cushionRaiseY = -TABLE.THICK + 0.02;
  const cushionW = TABLE.WALL * 0.9;
  const cushionExtend = POCKET_VIS_R * 0.7;
...
export default function NewSnookerGame() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const sph = new THREE.Spherical(3, 1.05 /* slightly lower angle */, Math.PI);
      const updateCamera = () => {
        const target =
      ...
      spot.target.position.set(0, TABLE_H, 0);
