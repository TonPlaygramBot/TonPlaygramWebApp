...
// --------------------------------------------------
const pocketCenters = () => [
  new THREE.Vector2(-TABLE.W/2, -TABLE.H/2), new THREE.Vector2(0, -TABLE.H/2), new THREE.Vector2(TABLE.W/2, -TABLE.H/2),
  new THREE.Vector2(-TABLE.W/2,  TABLE.H/2), new THREE.Vector2(0,  TABLE.H/2), new THREE.Vector2(TABLE.W/2,  TABLE.H/2),
];
...
function Guret(scene, id, color, x, y){
  ...
  mesh.position.set(x, BALL_R, y); mesh.castShadow=true; mesh.receiveShadow=true; scene.add(mesh);
  return { id, color, mesh, pos: new THREE.Vector2(x,y), vel: new THREE.Vector2(), active:true };
}
...
function Table3D(scene){
  ...
  pocketCenters().forEach(p=>{ const h=new THREE.Path(); h.absellipse(p.x,p.y, POCKET_R_VIS*0.85, POCKET_R_VIS*0.85, 0, Math.PI*2, false, 0); shape.holes.push(h); });
  const extrude = new THREE.ExtrudeGeometry(shape, { depth: TABLE.THICK, bevelEnabled:false });
  const cloth = new THREE.Mesh(extrude, new THREE.MeshStandardMaterial({ color: COLORS.cloth, roughness:0.95 }));
  cloth.rotation.x = -Math.PI/2; cloth.position.y = -TABLE.THICK; cloth.receiveShadow=true; scene.add(cloth);
  // Rails
  const railMat = new THREE.MeshStandardMaterial({ color: COLORS.rail, metalness:0.12, roughness:0.7 });
  const railTop=new THREE.Mesh(new THREE.BoxGeometry(TABLE.W+8,4,4),railMat); railTop.position.set(0,0,halfH+2);
  const railBot=railTop.clone(); railBot.position.z=-halfH-2;
  const railL  =new THREE.Mesh(new THREE.BoxGeometry(4,4,TABLE.H+8),railMat); railL.position.set(-halfW-2,0,0);
  const railR  =railL.clone(); railR.position.x=halfW+2; scene.add(railTop,railBot,railL,railR);
  // Pocket rings (vizuale sipërfaqe)
  const ringGeo = new THREE.RingGeometry(POCKET_R_VIS*0.6, POCKET_R_VIS, 48);
  const ringMat = new THREE.MeshStandardMaterial({ color:0x000000, side:THREE.DoubleSide, metalness:0.05, roughness:0.4 });
  pocketCenters().forEach(p=>{ const m=new THREE.Mesh(ringGeo, ringMat); m.rotation.x=-Math.PI/2; m.position.set(p.x,-0.99,p.y); scene.add(m); });
  // Markings: baulk, D, spots
  const BAULK_RATIO_FROM_LEFT=0.2014; const D_R=(11.5/72)*TABLE.H; const baulkX=-halfW + BAULK_RATIO_FROM_LEFT*TABLE.W;
  const markMat=new THREE.LineBasicMaterial({ color:COLORS.mark, transparent:true, opacity:0.65 });
  const lineGeo=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(baulkX,-0.98,-halfH), new THREE.Vector3(baulkX,-0.98,halfH)]);
  scene.add(new THREE.Line(lineGeo, markMat));
  const dPts=[]; for(let i=0;i<=64;i++){ const t=Math.PI*(i/64); dPts.push(new THREE.Vector3(baulkX+Math.cos(t)*D_R, -0.98, Math.sin(t)*D_R)); }
  scene.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(dPts), markMat));
  const spot=(x,z)=>{ const r=new THREE.Mesh(new THREE.RingGeometry(0.6,1.0,24), new THREE.MeshBasicMaterial({ color:COLORS.mark })); r.rotation.x=-Math.PI/2; r.position.set(x,-0.99,z); scene.add(r); };
  spot(baulkX,0); spot(baulkX, TABLE.H*0.22); spot(baulkX, -TABLE.H*0.22); spot(0,0); spot(TABLE.W*0.25,0); spot(halfW - TABLE.W*0.09, 0);
...
export default function NewSnookerGame(){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      renderer.setPixelRatio(Math.min(2, window.devicePixelRatio||1)); renderer.setSize(host.clientWidth, host.clientHeight, false); host.appendChild(renderer.domElement);
      ...
      // Lights
      scene.add(new THREE.HemisphereLight(0xffffff, 0x222233, 0.95)); const key=new THREE.DirectionalLight(0xffffff,1.05); key.position.set(-60,90,40); key.castShadow=true; scene.add(key);

      // Table
      const { centers, baulkX } = Table3D(scene);

      // Balls (ONLY Guret)
      const balls=[]; const add=(id,color,x,y)=>{ const b=Guret(scene,id,color,x,y); balls.push(b); return b; };
      let cue = add('cue', COLORS.cue, -TABLE.W*0.32, 0);
      // reds triangle
      let rid=0; const bx=TABLE.W*0.1, by=0; for(let r=0;r<5;r++) for(let c=0;c<=r;c++){ const x=bx + r*(BALL_R*2+0.6); const y=by - r*BALL_R + c*(BALL_R*2+0.2); add(`red_${rid++}`, COLORS.red, x, y); }
      // colours
      const SPOTS={ yellow:[-TABLE.W*0.38, TABLE.H*0.22], green:[-TABLE.W*0.38, -TABLE.H*0.22], brown:[-TABLE.W*0.30,0], blue:[0,0], pink:[TABLE.W*0.25,0], black:[TABLE.W/2 - TABLE.W*0.09, 0] };
      const colors = Object.fromEntries(Object.entries(SPOTS).map(([k,[x,y]])=>[k, add(k, COLORS[k], x, y)]));
      ...
      const aim=new THREE.Line(aimGeom, aimMat); aim.visible=false; aim.computeLineDistances(); scene.add(aim);
      const tickGeom=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(), new THREE.Vector3()]);
      const tick=new THREE.Line(tickGeom, new THREE.LineBasicMaterial({ color:0xffffff })); tick.visible=false; scene.add(tick);

      // Pointer → XZ plane
      const pointer=new THREE.Vector2(); const ray=new THREE.Raycaster(); const plane=new THREE.Plane(new THREE.Vector3(0,1,0),0);
      ...
      // In-hand placement
      const free=(x,z)=> balls.every(b=> !b.active || b===cue || new THREE.Vector2(x,z).distanceTo(b.pos) > BALL_R*2.1);
      const onPlace=(e)=>{ if(!hud.inHand) return; const p=project(e); if(p.x<=baulkX && Math.abs(p.y)<=TABLE.H/2 - BALL_R*2 && free(p.x,p.y)){ cue.active=true; cue.mesh.visible=true; cue.pos.set(p.x,p.y); cue.mesh.position.set(p.x,BALL_R,p.y); setHud(s=>({...s, inHand:false })); } };
      dom.addEventListener('pointerdown', onPlace);

      // Shot lifecycle
      let shooting=false; let potted=[]; let foul=false; let firstHit=null;
      const legalTarget=()=> hud.phase==='reds' ? (hud.next==='red' ? 'red':'colour') : hud.next;
      ...
      // Resolve shot
      function resolve(){
        const me=hud.turn===0?'A':'B', op=hud.turn===0?'B':'A'; let gain=0; let swap=true;
        if(!cue.active) foul=true; const target=legalTarget();
        if(firstHit){ if(target==='red' && !isRedId(firstHit)) foul=true; else if(target==='colour' && isRedId(firstHit)) foul=true; else if(target!=='red' && target!=='colour' && firstHit!==target) foul=true; } else { foul=true; }
        const reds=potted.filter(isRedId), cols=potted.filter(id=>!isRedId(id));
        if(hud.phase==='reds'){
          if(hud.next==='red'){
            if(cols.length>0) foul=true; gain += reds.length; if(reds.length>0 && !foul){ setHud(s=>({...s, next:'colour'})); swap=false; }
          } else {
            if(reds.length>0) foul=true; if(cols.length>0 && !foul){ cols.forEach(id=>{ gain+=val(id); const b=colors[id]; if(b){ const [sx,sy]=SPOTS[id]; b.active=true; b.mesh.visible=true; b.pos.set(sx,sy); b.mesh.position.set(sx,BALL_R,sy);} }); setHud(s=>({...s, next:'red'})); swap=false; }
          }
          const redsLeft=balls.some(b=> b.active && isRedId(b.id)); if(!redsLeft) setHud(s=>({...s, phase:'colors', next:'yellow'}));
        } else {
          if(cols.length===1 && reds.length===0 && cols[0]===hud.next && !foul){ gain+=val(hud.next); const order=['yellow','green','brown','blue','pink','black']; const idx=order.indexOf(hud.next); const nxt=order[idx+1]; if(nxt){ setHud(s=>({...s, next:nxt })); swap=false; } else { setHud(s=>({...s, over:true })); } }
          else if(cols.length>0 || reds.length>0){ foul=true; }
        }
        if(foul){ const foulPts=Math.max(4, ...potted.map(id=>val(id)), cue.active?0:4); setHud(s=>({ ...s, [op]: s[op]+foulPts, inHand:true, next: s.phase==='reds'? 'red' : s.next })); cue.active=false; cue.mesh.visible=false; cue.vel.set(0,0); }
        else if(gain>0){ setHud(s=>({ ...s, [me]: s[me]+gain })); }
        if(swap||foul) setHud(s=>({ ...s, turn: 1-s.turn })); shooting=false; potted=[]; foul=false; firstHit=null;
      }
      ...
      const step=()=>{
        ...
        // Fizika
        balls.forEach(b=>{ if(!b.active) return; b.pos.add(b.vel); b.vel.multiplyScalar(FRICTION); if(b.vel.length()<STOP_EPS) b.vel.set(0,0); reflectRails(b); b.mesh.position.set(b.pos.x, BALL_R, b.pos.y); });
        // Kolizione + regjistro firstHit
        for(let i=0;i<balls.length;i++) for(let j=i+1;j<balls.length;j++){
          const a=balls[i], b=balls[j]; if(!a.active||!b.active) continue; const dx=b.pos.x-a.pos.x, dy=b.pos.y-a.pos.y; const d2=dx*dx+dy*dy; const min=(BALL_R*2)**2; if(d2>0&&d2<min){ const d=Math.sqrt(d2)||1e-4; const nx=dx/d, ny=dy/d; const overlap=(BALL_R*2-d)/2; a.pos.x-=nx*overlap; a.pos.y-=ny*overlap; b.pos.x+=nx*overlap; b.pos.y+=ny*overlap; const avn=a.vel.x*nx+a.vel.y*ny; const bvn=b.vel.x*nx+b.vel.y*ny; const at=a.vel.clone().sub(new THREE.Vector2(nx,ny).multiplyScalar(avn)); const bt=b.vel.clone().sub(new THREE.Vector2(nx,ny).multiplyScalar(bvn)); a.vel.copy(at.add(new THREE.Vector2(nx,ny).multiplyScalar(bvn))); b.vel.copy(bt.add(new THREE.Vector2(nx,ny).multiplyScalar(avn))); if(!firstHit){ if(a.id==='cue' && b.id!=='cue') firstHit=b.id; else if(b.id==='cue' && a.id!=='cue') firstHit=a.id; } } }
        // Kapje në xhepa
        balls.forEach(b=>{ if(!b.active) return; for(const c of centers){ if(b.pos.distanceTo(c) < CAPTURE_R){ b.active=false; b.mesh.visible=false; b.vel.set(0,0); if(b!==cue) potted.push(b.id.startsWith('red')? 'red' : b.id); break; } } });
        // Fund i goditjes
        if(shooting){ const any=balls.some(b=> b.active && b.vel.length()>=STOP_EPS); if(!any) resolve(); }

        renderer.render(scene, camera); rafRef.current=requestAnimationFrame(step);
      };
      step();
      ...
      return ()=>{
        cancelAnimationFrame(rafRef.current); window.removeEventListener('resize', onResize);
        try{ host.removeChild(renderer.domElement);}catch{}
        dom.removeEventListener('mousedown',down); dom.removeEventListener('mousemove',move); window.removeEventListener('mouseup',up);
        dom.removeEventListener('touchstart',down); dom.removeEventListener('touchmove',move); window.removeEventListener('touchend',up); dom.removeEventListener('wheel',wheel);
