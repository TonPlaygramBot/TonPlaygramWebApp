...
function Table3D(parent) {
  ...
  const SHORT_CUSHION_HEIGHT_SCALE = 1.12; // raise short rail cushions to match the remaining four rails
  ...
  const chromePlateThickness = railH * 0.24; // extend chrome trim farther downward so the pocket arches are fully wrapped
  const chromePlateInset = TABLE.THICK * 0.02;
  const chromePlateExpansionX = TABLE.THICK * 0.72;
  const chromePlateExpansionZ = TABLE.THICK * 0.74;
  ...
  const sideChromePlateWidth = Math.max(
    MICRO_EPS,
    Math.min(sidePlatePocketWidth, sidePlateMaxWidth) - TABLE.THICK * 0.12
  );
  ...
  const sideInset = SIDE_POCKET_RADIUS * 0.76 * POCKET_VISUAL_EXPANSION;
  ...
  const SIDE_CUSHION_CENTER_PULL =
    POCKET_VIS_R * 0.3 * POCKET_VISUAL_EXPANSION; // pull green side cushions slightly farther from the wooden rails
