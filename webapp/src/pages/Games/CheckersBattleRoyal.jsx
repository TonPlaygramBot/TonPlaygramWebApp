...
function applyChairThemeMaterials(chairMaterials, theme) {
  if (!chairMaterials) return;
  if (theme?.preserveMaterials) return;
  const seatColor = theme?.seatColor ?? '#8b0000';
  const legColor = theme?.legColor ?? '#111827';
  [chairMaterials.seat, ...(chairMaterials.upholstery || [])].forEach((mat) => {
    if (!mat?.color) return;
    mat.color.set(seatColor);
    mat.needsUpdate = true;
  });
  [chairMaterials.leg, ...(chairMaterials.metal || [])].forEach((mat) => {
    if (!mat?.color) return;
...
function resolveCheckersTableShapeOption(tableId) {
  ...
  if (!tableId) return TABLE_SHAPE_OPTIONS[0];
  let hash = 0;
  for (let i = 0; i < tableId.length; i += 1) {
...
export default function CheckersBattleRoyal() {
  ...
  useEffect(() => {
    ...
    const buildSceneAssets = async () => {
      ...
      chairMaterialSetsRef.current = [];
      try {
        tableRef.current?.dispose?.();
        const activeTableId =
          appearance.tableId ||
          CHESS_TABLE_OPTIONS.find((opt) => opt?.id)?.id ||
          null;
        const shapeOption = resolveCheckersTableShapeOption(activeTableId);
        ...
        currentTableIdRef.current = activeTableId;
      } catch (error) {
      ...
      try {
        ...
        const chairTheme = {
          ...
          seatColor: chairOption?.primary || chairOption?.seatColor || '#8b0000',
          legColor: chairOption?.legColor || '#111827',
        ...
        const makeChair = (z, ry) => {
          ...
          const chairMaterials = extractChairMaterials(g);
          applyChairThemeMaterials(chairMaterials, chairTheme);
          chairMaterialSetsRef.current.push(chairMaterials);
        ...
        const makeFallback = (z, ry) => {
          const g = createProceduralChairFallback(
            chairTheme.seatColor,
            chairTheme.legColor
          );
          g.position.set(0, CHAIR_BASE_HEIGHT, z);
          g.rotation.y = ry;
          scene.add(g);
          chairMaterialSetsRef.current.push(extractChairMaterials(g));
          return g;
        };
        chairsRef.current = [
  ...
  useEffect(() => {
    ...
    const selectedTableId =
      appearance.tableId || CHESS_TABLE_OPTIONS.find((opt) => opt?.id)?.id;
    if (selectedTableId && currentTableIdRef.current !== selectedTableId) {
      tableRef.current?.dispose?.();
      const shapeOption = resolveCheckersTableShapeOption(selectedTableId);
      ...
      currentTableIdRef.current = selectedTableId;
    }
    ...
    (chairMaterialSetsRef.current || []).forEach((materials) =>
      applyChairThemeMaterials(materials, chairTheme)
    );
