...
export default function SnakeAndLadder() {
  ...
  const [playerDiceCounts, setPlayerDiceCounts] = useState([1]);
  const [gameOver, setGameOver] = useState(false);
  const [ai, setAi] = useState(0);
  const [aiPositions, setAiPositions] = useState([]);
  const [aiTokenShapes, setAiTokenShapes] = useState([]);
  const [aiWeaponLoadout, setAiWeaponLoadout] = useState([]);
  ...
  useEffect(() => {
    ...
    const next = Array.from({ length: Math.max(0, ai) }, (_, i) => {
      const randomIndex = Math.floor(Math.random() * CAPTURE_WEAPON_OPTIONS.length);
      return CAPTURE_WEAPON_OPTIONS[randomIndex]?.id || CAPTURE_WEAPON_OPTIONS[i % CAPTURE_WEAPON_OPTIONS.length]?.id;
    });
    setAiWeaponLoadout(next);
  }, [ai, isMultiplayer]);
  ...
  const getPlayerName = useCallback(
    ...
    [aiAvatars, isMultiplayer, mpPlayers, myName]
  );
  ...
  const fastForward = (elapsed, state) => {
    ...
    setCurrentTurn(turn);
    setDiceCount(playerDiceCounts[turn] ?? 1);
    setRanking(rank);
    setGameOver(over);
    if (over) {
  ...
  useEffect(() => {
    ...
    if (total === 1) {
      ...
      setTurnMessage('Your turn');
      setCurrentTurn(0);
      setDiceCount(playerDiceCounts[0] ?? 1);
      return;
    }
    ...
    const rollNext = (idx) => {
      if (idx >= rollOrder.length) {
        ...
        setTimeout(() => {
          ...
          setDiceCount(playerDiceCounts[first.index] ?? 1);
        }, 2000);
        return;
      }
    ...
    : [
        {
          ...
          seatIndex: 0,
          weaponType: selectedCaptureWeaponId,
