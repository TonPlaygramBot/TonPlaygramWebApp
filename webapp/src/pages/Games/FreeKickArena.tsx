  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < 26; col++) {
      ...
      if (characterIndex < MURLAN_CHARACTER_THEMES.length) {
        ...
        const spectator = createActor(scene, loader, "wall", new THREE.Vector3(x, y + stands.position.y + 0.04, z + 0.08), characterIndex, kit);
...
export default function FreeKickGame() {
  ...
  useEffect(() => {
    ...
    loop();
    (window as any).__setFreeKickCameraYaw = (dir: number) => { cameraYaw = THREE.MathUtils.clamp(cameraYaw + dir * CAMERA_YAW_STEP, -CAMERA_YAW_LIMIT, CAMERA_YAW_LIMIT); };
    (window as any).__resetFreeKickCameraYaw = () => { cameraYaw = 0; };
    ...
    return () => { cancelAnimationFrame(frame); window.removeEventListener("resize", resize); canvas.removeEventListener("pointerdown", onDown); canvas.removeEventListener("pointermove", onMove); canvas.removeEventListener("pointerup", onUp); canvas.removeEventListener("pointercancel", onUp); audio.dispose(); delete (window as any).__setFreeKickCameraYaw; delete (window as any).__resetFreeKickCameraYaw; renderer.dispose(); };
  }, []);
  ...
  const moveCamera = (dir: number) => {
    (window as any).__setFreeKickCameraYaw?.(dir);
  };

  const resetCamera = () => {
    (window as any).__resetFreeKickCameraYaw?.();
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#07110b", overflow: "hidden", touchAction: "none", userSelect: "none" }}>
      ...
      <div style={{ position: "fixed", left: 12, right: 12, bottom: 76, display: "grid", gridTemplateColumns: "0.9fr 1.2fr 0.9fr", gap: 8, pointerEvents: "auto" }}>
        <button onClick={() => moveCamera(-1)} style={cameraButtonStyle}>View left</button>
        <button onClick={resetCamera} style={cameraButtonStyle}>Behind player</button>
        <button onClick={() => moveCamera(1)} style={cameraButtonStyle}>View right</button>
      </div>
