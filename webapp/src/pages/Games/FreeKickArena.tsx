...
type StadiumSpectator = {
  root: THREE.Group;
  model: THREE.Object3D;
  bones: SpectatorBones;
  seatedPose: Partial<Record<keyof SpectatorBones, THREE.Euler>>;
  celebrateTime: number;
  waveSeed: number;
};
...
const MAX_STADIUM_SPECTATORS = MURLAN_CHARACTER_THEMES.length;
...
function findBoneByHints(root: THREE.Object3D, hints: string[]) {
  let matched: THREE.Bone | undefined;
  root.traverse((obj) => {
    const bone = obj as THREE.Bone;
    if (matched || !bone.isBone) return;
    const name = cleanName(bone.name);
    if (hints.some((hint) => name.includes(cleanName(hint)))) matched = bone;
  });
  return matched;
}

function captureRotation(bone?: THREE.Bone) {
  return bone ? bone.rotation.clone() : new THREE.Euler();
}

function addRotation(bone: THREE.Bone | undefined, x = 0, y = 0, z = 0) {
  if (!bone) return;
  bone.rotation.x += x;
  bone.rotation.y += y;
  bone.rotation.z += z;
...
function findSpectatorBones(model: THREE.Object3D): SpectatorBones {
  return {
    hips: findBoneByHints(model, ["hips", "pelvis", "pelvisjoint", "hip_joint"]),
    spine: findBoneByHints(model, ["spine", "chest", "torso"]),
    head: findBoneByHints(model, ["head", "neck", "headjoint", "head_joint"]),
    leftUpperArm: findBoneByHints(model, ["leftarm", "arm.l", "l_upperarm", "leftshoulder", "armjointl", "arm_joint_l_1", "shoulderl"]),
    leftForeArm: findBoneByHints(model, ["leftforearm", "l_forearm", "leftlowerarm", "forearml", "elbowl", "arm_joint_l_2"]),
    rightUpperArm: findBoneByHints(model, ["rightarm", "arm.r", "r_upperarm", "rightshoulder", "armjointr", "arm_joint_r_1", "shoulderr"]),
    rightForeArm: findBoneByHints(model, ["rightforearm", "r_forearm", "rightlowerarm", "forearmr", "elbowr", "arm_joint_r_2"]),
    leftThigh: findBoneByHints(model, ["leftupleg", "leftthigh", "l_thigh", "legjointl1", "leg_joint_l_1"]),
    leftCalf: findBoneByHints(model, ["leftleg", "leftcalf", "l_calf", "legjointl2", "leg_joint_l_2"]),
    rightThigh: findBoneByHints(model, ["rightupleg", "rightthigh", "r_thigh", "legjointr1", "leg_joint_r_1"]),
    rightCalf: findBoneByHints(model, ["rightleg", "rightcalf", "r_calf", "legjointr2", "leg_joint_r_2"]),
  };
}

function applyMurlanSeatedPose(model: THREE.Object3D) {
  const bones = findSpectatorBones(model);
  model.position.y -= 0.09;
  addRotation(bones.hips, THREE.MathUtils.degToRad(-9), 0, 0);
  addRotation(bones.spine, THREE.MathUtils.degToRad(-3), 0, 0);
  addRotation(bones.head, THREE.MathUtils.degToRad(2), 0, 0);
  addRotation(bones.leftUpperArm, THREE.MathUtils.degToRad(-53), THREE.MathUtils.degToRad(-6), THREE.MathUtils.degToRad(-2));
  addRotation(bones.leftForeArm, THREE.MathUtils.degToRad(40), THREE.MathUtils.degToRad(-3), THREE.MathUtils.degToRad(-2));
  addRotation(bones.rightUpperArm, THREE.MathUtils.degToRad(-57), THREE.MathUtils.degToRad(6), THREE.MathUtils.degToRad(2));
  addRotation(bones.rightForeArm, THREE.MathUtils.degToRad(44), THREE.MathUtils.degToRad(3), THREE.MathUtils.degToRad(2));
  addRotation(bones.leftThigh, THREE.MathUtils.degToRad(-90.5), THREE.MathUtils.degToRad(9.2), THREE.MathUtils.degToRad(2.9));
  addRotation(bones.rightThigh, THREE.MathUtils.degToRad(-90.5), THREE.MathUtils.degToRad(1.7), THREE.MathUtils.degToRad(-1.1));
  addRotation(bones.leftCalf, THREE.MathUtils.degToRad(-95.1), THREE.MathUtils.degToRad(1.1), THREE.MathUtils.degToRad(0.6));
  addRotation(bones.rightCalf, THREE.MathUtils.degToRad(-95.1), THREE.MathUtils.degToRad(-1.1), THREE.MathUtils.degToRad(-0.6));
  const seatedPose: StadiumSpectator["seatedPose"] = {};
  Object.entries(bones).forEach(([key, bone]) => {
    seatedPose[key as keyof SpectatorBones] = captureRotation(bone);
  });
  return { bones, seatedPose };
}

function restoreSpectatorSeatedPose(spectator: StadiumSpectator) {
  Object.entries(spectator.bones).forEach(([key, bone]) => {
    const target = spectator.seatedPose[key as keyof SpectatorBones];
...
function applyKickerPose(kicker: Actor) {
  if (kicker.celebrateTime > 0) {
    ...
    kicker.root.position.y += Math.abs(Math.sin(t * 1.7)) * 0.18;
    kicker.root.rotation.y += Math.sin(t * 0.75) * 0.22;
    if (kicker.bones.hips) kicker.bones.hips.rotation.y += Math.sin(t) * 0.28;
    if (kicker.bones.spine) {
      kicker.bones.spine.rotation.x += Math.sin(t * 1.3) * 0.12;
      kicker.bones.spine.rotation.z += Math.sin(t * 0.9) * 0.22;
    }
    if (kicker.bones.leftArm) {
      kicker.bones.leftArm.rotation.x += -1.35 + Math.sin(t) * 0.28;
      kicker.bones.leftArm.rotation.z += -0.75 + Math.cos(t * 1.2) * 0.32;
    }
    if (kicker.bones.rightArm) {
      kicker.bones.rightArm.rotation.x += -1.35 + Math.cos(t) * 0.28;
      kicker.bones.rightArm.rotation.z += 0.75 + Math.sin(t * 1.1) * 0.32;
    }
    if (kicker.bones.leftUpLeg) kicker.bones.leftUpLeg.rotation.x += Math.sin(t * 1.6) * 0.35;
    if (kicker.bones.rightUpLeg) kicker.bones.rightUpLeg.rotation.x += Math.cos(t * 1.6) * 0.35;
    return;
  }
...
function makeChair(color: number) {
  ...
  const seat = new THREE.Mesh(new THREE.BoxGeometry(0.44, 0.09, 0.43), plastic);
  seat.position.y = 0.30;
  const frontLip = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.05, 0.08), plastic);
  frontLip.position.set(0, 0.31, 0.25);
  const back = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.52, 0.08), plastic);
  back.position.set(0, 0.58, -0.18);
  back.rotation.x = THREE.MathUtils.degToRad(-7);
  const legMat = material(0x334155, 0.45, 0.18);
  const legs = [
    [-0.17, 0.14, -0.13], [0.17, 0.14, -0.13], [-0.17, 0.14, 0.15], [0.17, 0.14, 0.15],
  ].map(([x, y, z]) => {
    const leg = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.28, 0.04), legMat);
    leg.position.set(x, y, z);
    return leg;
  });
  chair.add(shadow(seat), shadow(frontLip), shadow(back), ...legs.map((leg) => shadow(leg)));
  return chair;
}
...
function makeBillboardsAndStands(scene: THREE.Scene) {
  ...
  stands.position.z = GOAL_LINE_Z - GOAL_D - 2.15;
  const concrete = material(0x7c8798, 0.9, 0.02);
  const riserMat = material(0x64748b, 0.86, 0.03);
  const aisleMat = material(0xdbe4ee, 0.76, 0.02);
  const railMat = material(0xf8fafc, 0.38, 0.2);
  const rows = 7;
  const rowDepth = 0.82;
  const rowRise = 0.34;
  const width = FIELD_W + 5.2;
  const seatAnchors: Array<{ position: THREE.Vector3; rotationY: number; themeIndex: number }> = [];

  for (let row = 0; row < rows; row++) {
    const tread = new THREE.Mesh(new THREE.BoxGeometry(width, 0.15, rowDepth), concrete);
    tread.position.set(0, 0.20 + row * rowRise, -row * rowDepth);
    stands.add(shadow(tread));

    const riser = new THREE.Mesh(new THREE.BoxGeometry(width, rowRise, 0.12), riserMat);
    riser.position.set(0, 0.04 + row * rowRise, -row * rowDepth + rowDepth * 0.45);
    stands.add(shadow(riser));

    const stepNose = new THREE.Mesh(new THREE.BoxGeometry(width, 0.055, 0.09), railMat);
    stepNose.position.set(0, 0.32 + row * rowRise, -row * rowDepth + rowDepth * 0.42);
    stands.add(shadow(stepNose));
  }

  const stairXs = [-7.1, -2.35, 2.35, 7.1];
  stairXs.forEach((x) => {
    for (let row = 0; row < rows; row++) {
      const stair = new THREE.Mesh(new THREE.BoxGeometry(0.74, 0.08, rowDepth * 0.72), aisleMat);
      stair.position.set(x, 0.36 + row * rowRise, -row * rowDepth + 0.03);
      stands.add(shadow(stair));
      const edgeL = new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.08, rowDepth * 0.72), railMat);
      const edgeR = edgeL.clone();
      edgeL.position.set(x - 0.39, 0.43 + row * rowRise, -row * rowDepth + 0.03);
      edgeR.position.set(x + 0.39, 0.43 + row * rowRise, -row * rowDepth + 0.03);
      stands.add(shadow(edgeL), shadow(edgeR));
    }
  });

  const chairColors = [0x1d4ed8, 0xef4444, 0xfacc15, 0x16a34a, 0xffffff, 0x0ea5e9];
  let themeCursor = 0;
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < 28; col++) {
      const x = (col - 13.5) * 0.73;
      if (stairXs.some((aisleX) => Math.abs(x - aisleX) < 0.52)) continue;
      const chair = makeChair(chairColors[(row + col) % chairColors.length]);
      chair.position.set(x, 0.37 + row * rowRise, -row * rowDepth + 0.06);
      chair.rotation.y = Math.PI;
      stands.add(chair);
      if (seatAnchors.length < MAX_STADIUM_SPECTATORS && (row + col) % 2 === 0) {
        seatAnchors.push({
          position: new THREE.Vector3(x, 0.42 + row * rowRise, stands.position.z - row * rowDepth + 0.03),
          rotationY: Math.PI,
          themeIndex: themeCursor++ % Math.max(1, MURLAN_CHARACTER_THEMES.length),
        });
      }
    }
  }

  const backWall = new THREE.Mesh(new THREE.BoxGeometry(width + 1.1, rows * rowRise + 1.2, 0.32), material(0x334155, 0.72, 0.06));
  backWall.position.set(0, 1.15 + rows * rowRise, -(rows - 1) * rowDepth - 0.72);
  stands.add(shadow(backWall));

  const rail = new THREE.Mesh(new THREE.BoxGeometry(width + 0.8, 0.08, 0.08), railMat);
  rail.position.set(0, 1.06, 0.62);
  stands.add(shadow(rail));
  const upperRail = new THREE.Mesh(new THREE.BoxGeometry(width + 0.8, 0.08, 0.08), railMat);
  upperRail.position.set(0, 1.25 + rows * rowRise, -(rows - 1) * rowDepth - 0.08);
  stands.add(shadow(upperRail));

  scene.userData.freeKickSpectatorSeats = seatAnchors;
  scene.add(stands);
}
...
function normalizeSpectatorModel(model: THREE.Object3D, height = 1.18) {
  model.scale.setScalar(1);
  model.updateMatrixWorld(true);
  const box = new THREE.Box3().setFromObject(model);
  const h = Math.max(0.001, box.max.y - box.min.y);
...
function makeSeatedFallbackSpectator(color: number) {
  const group = new THREE.Group();
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.11, 0.38, 6, 10), material(color));
  torso.position.y = 0.78;
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.095, 12, 10), material(0xf1d6bd));
  head.position.y = 1.14;
  const leftArm = new THREE.Mesh(new THREE.CapsuleGeometry(0.025, 0.34, 5, 8), material(0xf1d6bd));
  const rightArm = leftArm.clone();
  leftArm.position.set(-0.14, 0.78, 0.02);
  rightArm.position.set(0.14, 0.78, 0.02);
  leftArm.rotation.z = 0.2;
  rightArm.rotation.z = -0.2;
  const legMat = material(0x1f2937);
  const leftLeg = new THREE.Mesh(new THREE.CapsuleGeometry(0.035, 0.38, 5, 8), legMat);
  const rightLeg = leftLeg.clone();
  leftLeg.position.set(-0.07, 0.48, 0.15);
  rightLeg.position.set(0.07, 0.48, 0.15);
  leftLeg.rotation.x = Math.PI / 2;
  rightLeg.rotation.x = Math.PI / 2;
  group.add(shadow(torso), shadow(head), shadow(leftArm), shadow(rightArm), shadow(leftLeg), shadow(rightLeg));
  return group;
}

async function loadThemeModel(loader: GLTFLoader, theme: any) {
  const urls = Array.isArray(theme?.modelUrls) && theme.modelUrls.length ? theme.modelUrls : theme?.url ? [theme.url] : [];
  ...
  throw lastError || new Error("Missing Murlan character model URL");
}

async function addMurlanStadiumSpectators(scene: THREE.Scene, loader: GLTFLoader) {
  const seats = (scene.userData.freeKickSpectatorSeats || []) as Array<{ position: THREE.Vector3; rotationY: number; themeIndex: number }>;
  const spectators: StadiumSpectator[] = [];
  await Promise.all(seats.map(async (seat, index) => {
    const theme = MURLAN_CHARACTER_THEMES[seat.themeIndex % Math.max(1, MURLAN_CHARACTER_THEMES.length)];
    let model: THREE.Object3D;
    try {
      const template = await loadThemeModel(loader, theme);
      model = cloneSkeleton(template) as THREE.Object3D;
      normalizeSpectatorModel(model, 1.16);
    } catch (_) {
      model = makeSeatedFallbackSpectator(MURLAN_CHARACTER_COLORS[index % MURLAN_CHARACTER_COLORS.length]);
    }
    shadow(model);
    const { bones, seatedPose } = applyMurlanSeatedPose(model);
    const root = new THREE.Group();
    root.position.copy(seat.position);
    root.rotation.y = seat.rotationY;
    root.scale.setScalar(0.78);
    root.add(model);
    scene.add(root);
    spectators.push({ root, model, bones, seatedPose, celebrateTime: 0, waveSeed: Math.random() * Math.PI * 2 });
  }));
  return spectators;
}

function triggerSpectatorGoalCelebration(spectators: StadiumSpectator[]) {
  spectators.forEach((spectator, index) => {
    spectator.celebrateTime = 2.8 + (index % 4) * 0.16;
  });
}

function updateSpectators(spectators: StadiumSpectator[], dt: number) {
  spectators.forEach((spectator) => {
    if (spectator.celebrateTime <= 0) {
      restoreSpectatorSeatedPose(spectator);
      spectator.root.position.y = THREE.MathUtils.lerp(spectator.root.position.y, spectator.root.userData.baseY ?? spectator.root.position.y, 0.12);
      return;
    }
    if (spectator.root.userData.baseY == null) spectator.root.userData.baseY = spectator.root.position.y;
    spectator.celebrateTime = Math.max(0, spectator.celebrateTime - dt);
    const t = performance.now() * 0.006 + spectator.waveSeed;
    const stand = THREE.MathUtils.smoothstep(Math.min(1, spectator.celebrateTime), 0, 0.65);
    spectator.root.position.y = (spectator.root.userData.baseY as number) + 0.36 * stand + Math.abs(Math.sin(t * 2.2)) * 0.04;
    const b = spectator.bones;
    restoreSpectatorSeatedPose(spectator);
    addRotation(b.hips, THREE.MathUtils.degToRad(18) * stand, 0, 0);
    addRotation(b.spine, THREE.MathUtils.degToRad(10) * stand, 0, Math.sin(t) * 0.06);
    addRotation(b.leftUpperArm, THREE.MathUtils.degToRad(-118) * stand, 0, Math.sin(t) * 0.42);
    addRotation(b.rightUpperArm, THREE.MathUtils.degToRad(-118) * stand, 0, -Math.sin(t + 0.7) * 0.42);
    addRotation(b.leftForeArm, THREE.MathUtils.degToRad(-32) * stand, 0, Math.sin(t * 1.7) * 0.28);
    addRotation(b.rightForeArm, THREE.MathUtils.degToRad(-32) * stand, 0, -Math.sin(t * 1.5) * 0.28);
    addRotation(b.leftThigh, THREE.MathUtils.degToRad(58) * stand, 0, 0);
    addRotation(b.rightThigh, THREE.MathUtils.degToRad(58) * stand, 0, 0);
    addRotation(b.leftCalf, THREE.MathUtils.degToRad(58) * stand, 0, 0);
    addRotation(b.rightCalf, THREE.MathUtils.degToRad(58) * stand, 0, 0);
  });
}
...
export default function FreeKickGame() {
  ...
  useEffect(() => {
    ...
    const kicker = createActor(scene, loader, "kicker", new THREE.Vector3(0, 0, 6), 0, actorKitColor(0));
    const keeper = createActor(scene, loader, "keeper", new THREE.Vector3(0, 0, GOAL_LINE_Z + 0.36), 0, actorKitColor(1));
    const wall = Array.from({ length: 5 }, (_, i) => createActor(scene, loader, "wall", new THREE.Vector3(0, 0, 0), i, actorKitColor(i + 2)));
    let spectators: StadiumSpectator[] = [];
    let disposed = false;
    void addMurlanStadiumSpectators(scene, loader).then((loadedSpectators) => {
      if (disposed) {
        loadedSpectators.forEach((spectator) => scene.remove(spectator.root));
        return;
      }
      spectators = loadedSpectators;
    });
    ...
    const loop = () => {
      ...
      updateBall(ball, dt); updateNetShake(netRig, dt); updateSpectators(spectators, dt);
      if (state === "flight") {
      ...
      if (state === "replay") {
        ...
        if (replayIndex >= replayFrames.length - 1 || replayFrames.length === 0) { state = "result"; resultTimer = pendingDecision === "GOAL" ? 2.85 : 1.05; if (pendingDecision === "GOAL") { audio.goal(); kicker.celebrateTime = 2.65; triggerSpectatorGoalCelebration(spectators); score.goals += 1; setHud((h) => ({ ...h, goals: score.goals, state: "VAR: GOAL confirmed" })); } else { score.saves += 1; setHud((h) => ({ ...h, saves: score.saves, state: `VAR: ${pendingDecision}` })); } }
      }
      ...
      if (state !== "replay") {
        const cameraSide = cameraOrbitRef.current;
        if (ball.flying || state === "var") { const toGoal = new THREE.Vector3(0, 0, GOAL_LINE_Z).sub(ball.object.position).setY(0).normalize(); const side = new THREE.Vector3(toGoal.z, 0, -toGoal.x).normalize(); const followPos = ball.object.position.clone().addScaledVector(toGoal, -4.3).addScaledVector(side, cameraSide * 1.7).add(new THREE.Vector3(0, 1.55, 0)); camera.position.lerp(followPos, 0.11); lookPoint = ball.object.position.clone().addScaledVector(toGoal, 6.0).add(new THREE.Vector3(cameraSide * 0.35, 0.55, 0)); camera.lookAt(lookPoint); } else { const toGoal = new THREE.Vector3(0, 0, GOAL_LINE_Z).sub(kicker.pos).setY(0).normalize(); const side = new THREE.Vector3(toGoal.z, 0, -toGoal.x).normalize(); const behind = kicker.pos.clone().addScaledVector(toGoal, -2.95).addScaledVector(side, cameraSide * 2.2).add(new THREE.Vector3(0, 1.35, 0)); camera.position.lerp(behind, 0.18); lookPoint.copy(kicker.pos).addScaledVector(toGoal, 8.5).add(new THREE.Vector3(cameraSide * 0.65, 1.16, 0)); camera.lookAt(lookPoint); }
    ...
    return () => { disposed = true; cancelAnimationFrame(frame); window.removeEventListener("resize", resize); canvas.removeEventListener("pointerdown", onDown); canvas.removeEventListener("pointermove", onMove); canvas.removeEventListener("pointerup", onUp); canvas.removeEventListener("pointercancel", onUp); audio.dispose(); renderer.dispose(); };
  }, []);
  ...
  const shiftCamera = (amount: number) => {
    cameraOrbitRef.current = THREE.MathUtils.clamp(cameraOrbitRef.current + amount, -1.25, 1.25);
  };

  const centerCamera = () => {
    cameraOrbitRef.current = 0;
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#07110b", overflow: "hidden", touchAction: "none", userSelect: "none" }}>
      ...
      <div style={{ position: "fixed", left: 12, bottom: 76, right: 12, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, pointerEvents: "auto" }}>
        <button onClick={() => shiftCamera(-0.35)} style={buttonStyle}>Camera<br />Left</button>
        <button onClick={centerCamera} style={buttonStyle}>Camera<br />Center</button>
        <button onClick={() => shiftCamera(0.35)} style={buttonStyle}>Camera<br />Right</button>
      </div>
