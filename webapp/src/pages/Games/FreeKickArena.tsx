...
const STADIUM_CHAIR_COLORS = [0x1d4ed8, 0xffffff];
...
function shuffledMurlanRoster() {
  const count = Math.max(7, MURLAN_CHARACTER_THEMES.length || 0);
  const roster = Array.from({ length: count }, (_, index) => ({
    theme: MURLAN_CHARACTER_THEMES[index % Math.max(1, MURLAN_CHARACTER_THEMES.length)] ?? null,
    color: MURLAN_CHARACTER_COLORS[index % MURLAN_CHARACTER_COLORS.length],
    index,
  }));
...
function makeChair(color: number) {
  ...
  const seat = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.08, 0.5), plastic);
  seat.position.y = 0.3;
  const back = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.58, 0.08), plastic);
  back.position.set(0, 0.63, -0.21);
  const legMat = material(0x334155, 0.45, 0.18);
  const legs = [
    [-0.2, 0.14, -0.16], [0.2, 0.14, -0.16], [-0.2, 0.14, 0.18], [0.2, 0.14, 0.18],
  ].map(([x, y, z]) => {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < 26; col++) {
      ...
      if (characterIndex < MURLAN_CHARACTER_THEMES.length) {
        const theme = MURLAN_CHARACTER_THEMES[characterIndex];
        const kit = MURLAN_CHARACTER_COLORS[characterIndex % MURLAN_CHARACTER_COLORS.length];
        const spectator = createActor(scene, loader, "wall", new THREE.Vector3(x, y + stands.position.y + 0.04, z + 0.08), characterIndex, kit, theme?.modelUrls?.[0] ?? theme?.url ?? HUMAN_URL);
        spectator.dir.set(-x * 0.08, 0, 1).normalize();
        spectator.wallIndex = characterIndex;
        spectator.root.scale.setScalar(0.78);
...
export default function FreeKickGame() {
  ...
  const cameraDragRef = useRef<CameraDragState>({ active: false, lastX: 0 });
  ...
  useEffect(() => {
    ...
    const characterRoster = shuffledMurlanRoster();
    const actorAppearance = (index: number) => characterRoster[index % characterRoster.length] ?? { color: MURLAN_CHARACTER_COLORS[index % MURLAN_CHARACTER_COLORS.length], index };
    const kickerLook = actorAppearance(0);
    const keeperLook = actorAppearance(1);
    const kicker = createActor(scene, loader, "kicker", new THREE.Vector3(0, 0, 6), kickerLook.index, kickerLook.color);
    const keeper = createActor(scene, loader, "keeper", new THREE.Vector3(0, 0, GOAL_LINE_Z + 0.36), keeperLook.index, keeperLook.color);
    const wall = Array.from({ length: 5 }, (_, i) => { const look = actorAppearance(i + 2); return createActor(scene, loader, "wall", new THREE.Vector3(0, 0, 0), look.index, look.color); });
    ...
    const onDown = (e: PointerEvent) => {
      audio.start();
      const cameraZone = e.clientY < host.clientHeight * 0.42 || state !== "aim";
      if (cameraZone) {
        cameraDragRef.current = { active: true, lastX: e.clientX };
        swipeRef.current.active = false;
        hideAimLine(aimLine);
        return;
      }
      swipeRef.current = { active: true, startX: e.clientX, startY: e.clientY, endX: e.clientX, endY: e.clientY };
      setAimCurve(aimLine, ball.object.position, swipeRef.current);
    };
    const onMove = (e: PointerEvent) => {
      if (cameraDragRef.current.active) {
        const dx = e.clientX - cameraDragRef.current.lastX;
        cameraDragRef.current.lastX = e.clientX;
        cameraYaw = THREE.MathUtils.clamp(cameraYaw + dx * CAMERA_DRAG_SENSITIVITY, -CAMERA_YAW_LIMIT, CAMERA_YAW_LIMIT);
        return;
      }
    ...
    const loop = () => {
      ...
      if (state === "flight") {
        ...
        if (!shotSaved && keeperSaveCheck(keeper, ball, dt)) { shotSaved = true; ball.flying = false; audio.save(); }
        ...
        if (!ball.flying && !shotFinalized) {
          ...
          if (pendingDecision === "GOAL") {
            ...
            state = "result";
            resultTimer = pendingDecision === "SAVE" ? 0.7 : 0.85;
            score.saves += 1;
            setHud((h) => ({ ...h, saves: score.saves, state: pendingDecision }));
          }
        }
      }
      if (state === "var") { varTimer -= dt; if (varTimer <= 0) { state = "replay"; replayIndex = 0; replayClock = 0; setHud((h) => ({ ...h, state: "SLOW REPLAY: GOAL" })); } }
      if (state === "replay") {
        ...
        if (replayIndex >= replayFrames.length - 1 || replayFrames.length === 0) { state = "result"; resultTimer = 2.65; audio.goal(); triggerCrowdCheer(stadium); kicker.celebrateTime = 2.45; score.goals += 1; setHud((h) => ({ ...h, goals: score.goals, state: "VAR: GOAL confirmed" })); }
      }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#07110b", overflow: "hidden", touchAction: "none", userSelect: "none" }}>
      ...
      <div style={{ position: "fixed", left: 14, top: 54, color: "white", fontFamily: "system-ui,sans-serif", fontSize: 11, lineHeight: 1.3, maxWidth: 300, pointerEvents: "none", textShadow: "0 2px 8px #000" }}>
        Drag the upper screen left/right to adjust the behind-player camera. Slow replay is shown only for goals.
      </div>
    </div>
  );
}
