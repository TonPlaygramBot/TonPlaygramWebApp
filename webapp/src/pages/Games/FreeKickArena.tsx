...
const HUMAN_STYLE_PALETTES: ActorStyle[] = [
  { kit: 0x1d69ff, accent: 0xffffff, skin: 0xf1d6bd },
  { kit: 0xdc2626, accent: 0xfacc15, skin: 0xd7a77f },
  { kit: 0x16a34a, accent: 0xe0f2fe, skin: 0x8d5524 },
  { kit: 0x7c3aed, accent: 0xf8fafc, skin: 0xc68642 },
  { kit: 0x0891b2, accent: 0x111827, skin: 0xffdbac },
  { kit: 0xf97316, accent: 0x172554, skin: 0xb06c49 },
];

function randomActorStyles(count: number) {
  const styles = HUMAN_STYLE_PALETTES.map((style) => ({ ...style }));
  for (let i = styles.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [styles[i], styles[j]] = [styles[j], styles[i]];
  }
  return Array.from({ length: count }, (_, i) => styles[i % styles.length]);
}
...
function tintModel(model: THREE.Object3D, kind: ActorKind, index = 0, style?: ActorStyle) {
  const baseKit = kind === "keeper" ? 0x111111 : kind === "wall" ? (index % 2 ? 0x174ea6 : 0xfacc15) : 0x1d69ff;
  const kitColor = new THREE.Color(style?.kit ?? baseKit);
  const accentColor = new THREE.Color(style?.accent ?? (kind === "wall" ? 0x174ea6 : 0xffffff));
  model.traverse((obj) => {
    const mesh = obj as THREE.Mesh;
    if (!mesh.isMesh) return;
    const mats = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    mats.forEach((raw, matIndex) => {
      const m = raw as THREE.MeshStandardMaterial;
      if (m.color) m.color.lerp(matIndex % 2 ? accentColor : kitColor, kind === "keeper" ? 0.28 : 0.36);
      m.needsUpdate = true;
    });
  });
}

function makeFallback(kind: ActorKind, index = 0, style?: ActorStyle) {
  const group = new THREE.Group();
  const kit = kind === "keeper" ? 0x111111 : (style?.kit ?? (kind === "kicker" ? 0x1d69ff : 0xfacc15));
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.19, 0.72, 8, 14), material(kit));
  torso.position.y = 1.05;
  const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.06, 0.02), material(style?.accent ?? (kind === "wall" ? 0x174ea6 : 0xffffff)));
  stripe.position.set(0, 1.25, -0.16);
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.13, 18, 12), material(style?.skin ?? 0xf1d6bd));
...
function createActor(scene: THREE.Scene, loader: GLTFLoader, kind: ActorKind, pos: THREE.Vector3, index = 0, style?: ActorStyle): Actor {
  const root = new THREE.Group();
  root.position.copy(pos).setY(GROUND_Y);
  scene.add(root);

  const fallback = makeFallback(kind, index, style);
  root.add(fallback);
  ...
  loader.setCrossOrigin("anonymous").load(HUMAN_URL, (gltf) => {
    ...
    tintModel(model, kind, index, style);
    shadow(model);
    root.add(model);
...
function resetShot(ball: BallState, kicker: Actor, keeper: Actor, wall: Actor[], spot: KickSpot) {
  ...
  ball.lift = 1;
  ball.flying = false;
  ball.freeFlight = false;
  ball.goalCrossed = false;
  ball.postHit = false;
...
function beginRunup(kicker: Actor, wall: Actor[], keeper: Actor, ball: BallState, swipe: SwipeState) {
  ...
  const targetHeight = THREE.MathUtils.clamp(final.y, 0.35, GOAL_H + 0.25);
  const quality = Math.random();
  keeper.diveDir = quality < 0.04 ? -targetSide || (Math.random() > 0.5 ? 1 : -1) : targetSide;
  keeper.diveHeight = targetHeight < 0.72 ? 0.52 : targetHeight > 1.56 ? 1.74 : 1.12;
  keeper.diveDelay = quality > 0.76 ? 0.05 : quality > 0.34 ? 0.1 : 0.16;
  keeper.diveTime = 0;
}
...
function rollBall(ball: BallState, move: THREE.Vector3, dt: number) {
  ...
  if (dist > 0.0001) {
    ...
    QTMP.setFromAxisAngle(rollAxis, dist / BALL_R);
    ball.object.quaternion.premultiply(QTMP);
  }
  const curveSpin = Math.abs(ball.curve) * 0.08 + 0.04;
  QTMP.setFromAxisAngle(new THREE.Vector3(0, Math.sign(ball.curve || 1), 0), curveSpin * Math.max(0.35, dt * 60));
  ball.object.quaternion.premultiply(QTMP);
}

function isInsideGoalMouth(p: THREE.Vector3) {
  return Math.abs(p.x) <= GOAL_W / 2 - BALL_R && p.y >= BALL_R && p.y <= GOAL_H - BALL_R * 0.25;
}

function applyPostCollisions(ball: BallState, prev: THREE.Vector3) {
  const p = ball.object.position;
  const crossedFront = prev.z > GOAL_LINE_Z && p.z <= GOAL_LINE_Z + POST_R * 2;
  if (!crossedFront) return;

  const posts = [-GOAL_W / 2, GOAL_W / 2];
  for (const postX of posts) {
    if (p.y <= GOAL_H + BALL_R && Math.hypot(p.x - postX, p.z - GOAL_LINE_Z) <= POST_R + BALL_R) {
      ball.postHit = true;
      p.x = postX + Math.sign(p.x - postX || ball.vel.x || 1) * (POST_R + BALL_R + 0.01);
      ball.vel.x = Math.sign(p.x - postX || 1) * Math.max(2.4, Math.abs(ball.vel.x) * 0.55 + 1.1);
      ball.vel.z = Math.abs(ball.vel.z) * 0.34;
      ball.vel.y *= 0.72;
      ball.freeFlight = true;
      return;
    }
  }

  if (Math.abs(p.x) <= GOAL_W / 2 + BALL_R && Math.abs(p.y - GOAL_H) <= POST_R + BALL_R && p.y > GOAL_H - 0.22) {
    ball.postHit = true;
    p.y = GOAL_H + POST_R + BALL_R + 0.01;
    ball.vel.y = Math.abs(ball.vel.y) * 0.48 + 1.2;
    ball.vel.z = Math.abs(ball.vel.z) * 0.28;
    ball.freeFlight = true;
  }
}

function updateFreeBall(ball: BallState, dt: number) {
  const prev = ball.object.position.clone();
  ball.vel.y -= 7.8 * dt;
  ball.vel.x += Math.sin(ball.shotAge * 8 + ball.knuckleSeed) * Math.abs(ball.curve) * 0.02;
  ball.vel.multiplyScalar(Math.pow(0.992, dt * 60));
  ball.object.position.addScaledVector(ball.vel, dt);
  applyPostCollisions(ball, prev);

  if (!ball.goalCrossed && isGoalPosition(ball.object.position)) ball.goalCrossed = true;

  if (ball.object.position.y <= BALL_R) {
    ball.object.position.y = BALL_R;
    if (Math.abs(ball.vel.y) > 1.2) ball.vel.y = -ball.vel.y * 0.34;
    else ball.vel.y = 0;
    ball.vel.x *= 0.9;
    ball.vel.z *= 0.9;
  }
  ...
  const beyondNet = ball.object.position.z < GOAL_LINE_Z - GOAL_D - 4.2;
  const outWide = Math.abs(ball.object.position.x) > FIELD_W / 2 + 8;
  const resting = ball.object.position.y <= BALL_R + 0.001 && ball.vel.length() < 0.28;
  if (beyondNet || outWide || resting) ball.flying = false;
}
...
function updateBall(ball: BallState, dt: number) {
  ...
  const prev = ball.lastPos.clone();
  ball.object.position.copy(next);
  rollBall(ball, next.clone().sub(prev), dt);
  if (!ball.goalCrossed && isGoalPosition(ball.object.position)) ball.goalCrossed = true;
  applyPostCollisions(ball, prev);
  ball.lastPos.copy(ball.object.position);
  ...
  if (t >= 1) {
    ball.freeFlight = true;
    ball.vel.z = Math.min(ball.vel.z, -10.5);
  }
}
...
function keeperSaveCheck(keeper: Actor, ball: BallState, dt: number) {
  ...
  const ballGoalX = THREE.MathUtils.clamp(ball.object.position.x, -GOAL_W / 2 + 0.32, GOAL_W / 2 - 0.32);
  const plannedX = keeper.diveDir === 0 ? ballGoalX * 0.42 : keeper.diveDir * Math.min(2.55, Math.max(1.05, Math.abs(ballGoalX)));
  const urgency = ball.object.position.z < GOAL_LINE_Z + 5.5 ? 0.18 : 0.1;
  keeper.pos.x = THREE.MathUtils.lerp(keeper.pos.x, plannedX, urgency);
  keeper.diveHeight = THREE.MathUtils.lerp(keeper.diveHeight, THREE.MathUtils.clamp(ball.object.position.y, 0.58, 1.92), 0.08);

  const nearGoal = ball.object.position.z < GOAL_LINE_Z + 2.25 && ball.object.position.z > GOAL_LINE_Z - 0.62;
  const bodyCenter = keeper.pos.clone().add(new THREE.Vector3(0, 1.05, 0));
  const handCenter = keeper.pos.clone().add(new THREE.Vector3(keeper.diveDir * 1.62, keeper.diveHeight, 0));
  const gloveReach = keeper.diveDir === 0 ? 1.02 : 1.48;
  const bodyReach = 0.66;
  const savedByGlove = handCenter.distanceTo(ball.object.position) < gloveReach;
  const savedByBody = bodyCenter.distanceTo(ball.object.position) < bodyReach;

  if (nearGoal && (savedByGlove || savedByBody)) {
    const deflectSide = Math.sign(ball.object.position.x - keeper.pos.x || keeper.diveDir || (Math.random() > 0.5 ? 1 : -1));
    ball.freeFlight = true;
    ball.netCaught = false;
    ball.vel.set(deflectSide * (4.2 + Math.random() * 1.6), Math.max(1.6, ball.vel.y * 0.22 + 1.2), 5.2 + Math.random() * 1.4);
    ball.lastPos.copy(ball.object.position);
    return true;
  }
  return false;
...
function wallBlockCheck(wall: Actor[], ball: BallState) {
  ...
  for (const actor of wall) {
    ...
    if (chest.distanceTo(ball.object.position) < 0.55) {
      ball.freeFlight = true;
      ball.vel.set((ball.object.position.x - actor.pos.x) * 3.2, Math.max(1.2, ball.vel.y * 0.2 + 1.0), 5.6);
      return true;
    }
  }
  return false;
...
function isGoalPosition(p: THREE.Vector3) {
  const crossedLine = p.z <= GOAL_LINE_Z - 0.05;
  return crossedLine && isInsideGoalMouth(p);
}

function decideShot(ball: BallState, blocked: boolean, saved: boolean): Decision {
  if (blocked) return "BLOCKED";
  if (saved) return "SAVE";
  return ball.goalCrossed || ball.netCaught ? "GOAL" : "NO GOAL";
}
...
export default function FreeKickGame() {
  ...
  useEffect(() => {
    ...
    const actorStyles = randomActorStyles(7);
    const kicker = createActor(scene, loader, "kicker", new THREE.Vector3(0, 0, 6), 0, actorStyles[0]);
    const keeper = createActor(scene, loader, "keeper", new THREE.Vector3(0, 0, GOAL_LINE_Z + 0.36), 0, actorStyles[1]);
    const wall = Array.from({ length: 5 }, (_, i) => createActor(scene, loader, "wall", new THREE.Vector3(0, 0, 0), i, actorStyles[i + 2]));
    let spot: KickSpot = "center"; let state: ShotState = "aim"; let resultTimer = 0; let varTimer = 0; let replayIndex = 0; let replayClock = 0; let pendingDecision: Decision = "NO GOAL"; let shotBlocked = false; let shotSaved = false; let shotFinalized = false; let netTriggered = false; let postTriggered = false; const replayFrames: ReplayFrame[] = []; let score = { goals: 0, saves: 0 }; resetShot(ball, kicker, keeper, wall, spot);
    ...
    const loop = () => {
      ...
      if (state === "flight") {
        ...
        if (!postTriggered && ball.postHit) { postTriggered = true; audio.save(); setHud((h) => ({ ...h, state: "Post hit · ball still live" })); }
        if (!netTriggered && !shotBlocked && !shotSaved && ball.object.position.z <= GOAL_LINE_Z - GOAL_D + 0.24 && isInsideGoalMouth(ball.object.position)) { netTriggered = true; catchBallInNet(ball, netRig); audio.net(); setHud((h) => ({ ...h, state: "Back net hit · checking VAR" })); }
      ...
      if (state === "result") { resultTimer -= dt; if (resultTimer <= 0) { state = "aim"; shotBlocked = false; shotSaved = false; shotFinalized = false; netTriggered = false; postTriggered = false; replayFrames.length = 0; resetShot(ball, kicker, keeper, wall, spot); setHud((h) => ({ ...h, state: "Swipe to aim and shoot" })); } }
      if (state !== "replay") {
