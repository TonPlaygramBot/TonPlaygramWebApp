...
type PinResetState = "idle" | "lowering" | "sweeping" | "lifting";
...
const RETURN_SAFE_WAYPOINT = new THREE.Vector3(1.62, CFG.laneY, 5.48);
const RETURN_PICKUP_POINT = new THREE.Vector3(CFG.rackEdgeX, CFG.laneY, CFG.rackStopZ);
const PLAYER_READY_POINT = new THREE.Vector3(0.46, CFG.laneY, CFG.playerStartZ);
const PLAYER_SEATS = [
  { pos: new THREE.Vector3(-4.06, CFG.laneY, 7.24), yaw: 0, stand: PLAYER_READY_POINT.clone() },
  { pos: new THREE.Vector3(-3.18, CFG.laneY, 8.34), yaw: Math.PI, stand: new THREE.Vector3(-0.46, CFG.laneY, CFG.playerStartZ) },
];
...
type BowlingArenaDecor = {
  ...
  pinResetRig: THREE.Group;
  sweepHead: THREE.Mesh;
};
...
function createEnvironment(scene: THREE.Scene, loader: THREE.TextureLoader, tableFinishId: string, chromeColorId: string): BowlingArenaDecor {
  ...
  const pinResetRig = new THREE.Group();
  const sweepHead = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 }));
  const decor: BowlingArenaDecor = { returnBalls: [], scoreboardPanels: [], crowdPulseLights: [], pinResetRig, sweepHead };
  scene.add(group);
  ...
  // Keep the arena open so the HDRI supplies depth; only the active lane, pit, and player area are modeled.
  const carpetMat = new THREE.MeshStandardMaterial({ color: 0x21162c, roughness: 0.88, metalness: 0.01 });
  const sideFloorMat = new THREE.MeshStandardMaterial({ color: 0x141313, roughness: 0.78, metalness: 0.02 });
  const pinBackdropMat = new THREE.MeshStandardMaterial({ color: 0x10141c, roughness: 0.72, metalness: 0.08 });
  const rubberMat = new THREE.MeshStandardMaterial({ color: 0x05070a, roughness: 0.86, metalness: 0.01 });
  const ledCyan = new THREE.MeshBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.34, toneMapped: false });
  const ledAmber = new THREE.MeshBasicMaterial({ color: 0xffb86b, transparent: true, opacity: 0.38, toneMapped: false });

  const sideFloor = new THREE.Mesh(new THREE.PlaneGeometry(7.6, 29.2), sideFloorMat);
  sideFloor.rotation.x = -Math.PI / 2;
  sideFloor.position.set(0, CFG.laneY - 0.026, -3.55);
  group.add(sideFloor);
  const loungeCarpet = new THREE.Mesh(new THREE.PlaneGeometry(2.9, 5.85), carpetMat);
  loungeCarpet.rotation.x = -Math.PI / 2;
  loungeCarpet.position.set(-3.62, CFG.laneY - 0.014, 7.82);
  ...
  const pinBackdrop = new THREE.Mesh(new THREE.BoxGeometry(5.4, 1.8, 0.16), pinBackdropMat);
  pinBackdrop.position.set(0, 0.94, CFG.backStopZ - 0.82);
  group.add(pinBackdrop);
  const pinsetter = new THREE.Mesh(new THREE.BoxGeometry(4.45, 0.62, 1.12), new THREE.MeshStandardMaterial({ color: 0x202833, roughness: 0.54, metalness: 0.28 }));
  pinsetter.position.set(0, 0.62, CFG.backStopZ + 0.08);
  group.add(pinsetter);
  const pinFocus = new THREE.Mesh(new THREE.BoxGeometry(3.96, 0.055, 0.05), new THREE.MeshBasicMaterial({ color: 0x92e7ff, transparent: true, opacity: 0.3, toneMapped: false }));
  pinFocus.position.set(0, 0.98, CFG.pinDeckZ + 0.38);
  group.add(pinFocus);
  const pitCurtain = new THREE.Mesh(new THREE.BoxGeometry(4.05, 0.92, 0.1), new THREE.MeshStandardMaterial({ color: 0x03050a, roughness: 0.95, metalness: 0.02 }));
  pitCurtain.position.set(0, CFG.laneY + 0.38, CFG.backStopZ - 0.2);
  group.add(pitCurtain);
  decor.pinResetRig = pinResetRig;
  pinResetRig.visible = false;
  pinResetRig.position.set(0, CFG.laneY + 1.16, CFG.pinDeckZ + 0.6);
  const resetCarriage = new THREE.Mesh(new THREE.BoxGeometry(4.05, 0.14, 0.34), metalMat);
  resetCarriage.position.set(0, 0, -0.16);
  pinResetRig.add(resetCarriage);
  const sweep = new THREE.Mesh(new THREE.BoxGeometry(3.78, 0.18, 0.18), metalMat);
  sweep.position.set(0, -0.42, 0.28);
  pinResetRig.add(sweep);
  decor.sweepHead = sweep;
  for (const x of [-1.78, 1.78]) {
    const arm = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.72, 0.08), metalMat);
    arm.position.set(x, -0.32, 0.04);
    pinResetRig.add(arm);
  }
  group.add(pinResetRig);
  ...
  const hood = new THREE.Mesh(new THREE.BoxGeometry(4.9, 1.22, 1.36), housingMat);
  hood.position.set(0, 1.22, CFG.backStopZ + 0.05);
  machineryHousing.add(hood);
  const fascia = new THREE.Mesh(new THREE.BoxGeometry(4.72, 0.28, 0.06), new THREE.MeshStandardMaterial({ color: 0x070b12, roughness: 0.7, metalness: 0.2 }));
  fascia.position.set(0, 1.78, CFG.backStopZ + 0.82);
  ...
  slimScore.position.set(0, 1.8, CFG.backStopZ + 0.86);
  machineryHousing.add(slimScore);
  decor.scoreboardPanels.push(slimScore);
  for (const x of [-1.34, 0, 1.34]) {
    const servicePanel = new THREE.Mesh(new THREE.BoxGeometry(0.94, 0.42, 0.04), new THREE.MeshStandardMaterial({ color: 0x283242, roughness: 0.66, metalness: 0.24 }));
    servicePanel.position.set(x, 1.17, CFG.backStopZ + 0.84);
  ...
  for (const [x, z] of [[-4.34, 5.82], [-3.28, 9.72], [3.16, 6.22]]) {
  ...
  returnBase.position.set(0.34, CFG.laneY + 0.15, 6.46);
  ...
  returnCover.position.set(0.34, CFG.laneY + 0.39, 6.44);
  ...
  returnMouth.position.set(0.34, CFG.laneY + 0.45, 6.36);
  ...
  animatedReturnColors.forEach((colors, i) => {
    ...
    rb.position.set(0.0 + i * 0.34, CFG.laneY + 0.32, 4.76 + i * 0.58);
  ...
  group.add(leftRack);
  for (const [x, z, color] of [[-3.75, 7.6, 0x38bdf8], [0, CFG.pinDeckZ - 0.4, 0xffb86b]]) {
...
function createDominoBowlingCrowd(scene: THREE.Scene, playerCharacterId: string): BowlingCrowdMember[] {
  ...
  const spots = [
    { pos: [-4.0, 0, 7.02], yaw: Math.PI / 2, seated: true },
    { pos: [-3.28, 0, 8.78], yaw: 0.04, seated: true },
    { pos: [-3.0, 0, 6.05], yaw: Math.PI * 0.78, seated: false },
    { pos: [-3.58, 0, 9.62], yaw: 0.18, seated: false },
    { pos: [3.28, 0, 6.25], yaw: -Math.PI * 0.72, seated: false },
    { pos: [3.2, 0, 8.9], yaw: -0.2, seated: false },
  ];
...
function updateArenaDecor(decor: BowlingArenaDecor, elapsed: number, criticalPulse: boolean) {
  decor.returnBalls.forEach((ball, i) => {
    ...
    ball.position.z = lerp(4.0, 7.02, phase);
    ball.position.x = lerp(-0.02, 0.66, phase) + Math.sin(phase * Math.PI * 2 + i) * 0.026;
...
function updateBallReturn(ball: BallState, dt: number) {
  ...
  if (ball.returnState === "hidden") {
    ...
    if (ball.returnT >= 1) {
      ...
      ball.pos.set(0.22, CFG.laneY + 0.22, -0.35);
      ball.mesh.position.copy(ball.pos);
    }
    return false;
  }
  if (ball.returnState === "returning") {
    ...
    ball.pos.set(lerp(0.22, 0.74, t), CFG.laneY + lerp(0.22, 0.42, t), lerp(-0.35, 6.88, t));
    ball.mesh.position.copy(ball.pos);
...
function getHumanFaceCameraPose(player: HumanRig) {
  ...
  const cameraOffset = new THREE.Vector3(-0.42 + sway, 1.68, 6.35);
...
function updateCamera(camera: THREE.PerspectiveCamera, ball: BallState, player: HumanRig, dt: number) {
  ...
  if (ball.rolling) {
    ...
    desired = ball.pos.clone().addScaledVector(lead, -5.9).add(new THREE.Vector3(-0.08, lerp(1.36, 1.12, speed01), 1.12));
    ...
    desired = new THREE.Vector3(-0.28, 2.08, 13.35);
...
export default function MobileBowlingRealistic() {
  ...
  useEffect(() => {
      ...
      const tryLoad = (idx: number) => {
        ...
        new RGBELoader().setCrossOrigin("anonymous").load(
          ...
          (hdr) => {
            ...
            if ("backgroundIntensity" in scene) scene.backgroundIntensity = graphicsQuality === "ultra" ? 0.64 : graphicsQuality === "performance" ? 0.48 : 0.56;
            if ("environmentIntensity" in scene) scene.environmentIntensity = graphicsQuality === "performance" ? 0.5 : graphicsQuality === "ultra" ? 0.72 : 0.62;
          },
          undefined,
          () => tryLoad(idx + 1)
        );
    ...
    scene.add(new THREE.HemisphereLight(0xffecd6, 0x08070b, 0.12));
    const key = new THREE.DirectionalLight(0xffefd8, 1.35);
    ...
    scene.add(fill);
    const rim = new THREE.DirectionalLight(0xff9a4f, 0.82);
    rim.position.set(3.8, 4.6, -12.4);
    scene.add(rim);
    for (let i = 0; i < 3; i++) {
      const z = lerp(4.6, -12.8, i / 2);
      const light = new THREE.PointLight(0xffd8a6, i === 2 ? 0.58 : 0.28, 7.6, 2.1);
      light.position.set(i % 2 === 0 ? -0.95 : 0.95, 2.38, z);
      scene.add(light);
    }
    const pinSpot = new THREE.SpotLight(0xfff0c8, 2.05, 11, Math.PI / 8.5, 0.44, 1.35);
    ...
    const laneSpot = new THREE.SpotLight(0x9fe8ff, 0.5, 17, Math.PI / 9, 0.58, 1.2);
    ...
    let replayTimer = 0;
    let pinResetState: PinResetState = "idle";
    let pinResetT = 0;
    let pendingKnockedPins: number | null = null;
    let fallenPinsCleared = false;
    ...
    const completeShotAfterPinReset = () => {
      const knocked = pendingKnockedPins ?? 0;
      pendingKnockedPins = null;
    ...
    const updatePinResetMechanism = (dt: number) => {
      if (pinResetState === "idle") return;
      const rig = arenaDecor.pinResetRig;
      rig.visible = true;
      if (pinResetState === "lowering") {
        pinResetT = clamp01(pinResetT + dt / 0.42);
        const k = easeInOut(pinResetT);
        rig.position.y = lerp(CFG.laneY + 1.16, CFG.laneY + 0.58, k);
        rig.position.z = CFG.pinDeckZ + 0.62;
        arenaDecor.sweepHead.position.z = 0.34;
        if (pinResetT >= 1) { pinResetState = "sweeping"; pinResetT = 0; }
        return;
      }
      if (pinResetState === "sweeping") {
        pinResetT = clamp01(pinResetT + dt / 0.68);
        const k = easeInOut(pinResetT);
        arenaDecor.sweepHead.position.z = lerp(0.42, -2.28, k);
        if (!fallenPinsCleared && pinResetT > 0.46) {
          clearFallenPins(pins);
          fallenPinsCleared = true;
          for (const pin of pins) {
            if (!pin.root.visible || !pin.standing) continue;
            pin.pos.copy(pin.start).add(new THREE.Vector3(0, -0.14, 0));
            pin.vel.set(0, 0, 0);
            pin.angularVel = 0;
            pin.tilt = 0;
            pin.root.rotation.set(0, 0, 0);
        ...
        if (pinResetT >= 1) { pinResetState = "lifting"; pinResetT = 0; }
        return;
      }
      pinResetT = clamp01(pinResetT + dt / 0.48);
      const k = easeOutCubic(pinResetT);
      rig.position.y = lerp(CFG.laneY + 0.58, CFG.laneY + 1.16, k);
      arenaDecor.sweepHead.position.z = lerp(-2.28, 0.34, k);
      for (const pin of pins) {
        if (!pin.root.visible || !pin.standing) continue;
        pin.pos.y = lerp(pin.start.y - 0.14, pin.start.y, k);
        pin.root.position.copy(pin.pos);
      }
      if (pinResetT >= 1) {
        pinResetState = "idle";
        pinResetT = 0;
        fallenPinsCleared = false;
        rig.visible = false;
        completeShotAfterPinReset();
      }
    };

    const finalizeShot = () => {
      const afterStanding = standingPinsCount(pins);
      pendingKnockedPins = clamp(lastShotStandingBefore - afterStanding, 0, 10);
      shotResolved = true;
      pinResetState = "lowering";
      pinResetT = 0;
      fallenPinsCleared = false;
      setHud((prev) => ({ ...prev, status: "Pinsetter lowering · preserving standing pins" }));
    };
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      if (control.active || ball.rolling || waitingForBallReturn || replayActive || pinResetState !== "idle") return;
    ...
    function animate() {
      ...
      if (activePlayer === 1 && !control.active && !ball.rolling && !waitingForBallReturn && !replayActive && pinResetState === "idle" && player.action === "idle" && nextAction !== "gameOver") {
      ...
      const mechanismBusy = ball.returnState !== "idle" || pinResetState !== "idle" || ball.rolling || pinsMoving;
