...
function bowlingReleaseFootPoint(laneCenter = 0) {
  // Left-handed bowling finish: mirror the existing delivery logic so the
  // opposite hand is now the pickup and shooting hand. Plant the body slightly
  // left of lane center so the held ball stays on the same-hand release line.
  return new THREE.Vector3(
    laneCenter - 0.18,
    CFG.laneY,
...
function applyBowlingDeliveryPose(model: THREE.Object3D | null, t: number) {
  ...
  const rightPlant = easeInOut(clamp01(t / CFG.releaseT));
  const leftTrailSettle = easeOutCubic(follow);

  applyRotationOffset(
    bowlingPoseBone(model, 'hips'),
    deg(lerp(0, 13, rightPlant) - follow * 4),
    deg(lerp(4, -12, drive) + follow * 5),
    deg(lerp(-2, 9, drive) - follow * 12)
  );
  applyRotationOffset(
    bowlingPoseBone(model, 'spine'),
    deg(lerp(2, 20, rightPlant) + releaseSnap * 4 - follow * 7),
  ...
  // Four-step left-handed release: the right arm balances while the left
  // hand carries the ball into the same pendulum backswing and forward release.
  applyRotationOffset(
    bowlingPoseBone(model, 'rightUpperArm'),
    deg(lerp(-12, -34, rightPlant) + follow * 6),
    deg(lerp(8, 18, rightPlant)),
    deg(lerp(6, 31, rightPlant) - follow * 6)
  );
  applyRotationOffset(
    bowlingPoseBone(model, 'rightForeArm'),
    deg(lerp(8, 24, rightPlant)),
    deg(8),
    deg(10)
  );
  applyRotationOffset(
  ...
  // Right foot plants at the foul line and the left foot trails back for balance.
  applyRotationOffset(
    bowlingPoseBone(model, 'rightThigh'),
    deg(lerp(3, -29, rightPlant)),
    deg(lerp(0, 6, rightPlant)),
    deg(lerp(0, 4, rightPlant))
  );
  applyRotationOffset(
    bowlingPoseBone(model, 'rightCalf'),
    deg(lerp(0, 18, rightPlant)),
    0,
    deg(-1)
  );
  applyRotationOffset(
    bowlingPoseBone(model, 'leftThigh'),
    deg(lerp(-5, 24, rightPlant) - leftTrailSettle * 4),
    deg(lerp(0, -10, rightPlant)),
    deg(lerp(0, -16, rightPlant))
  );
  applyRotationOffset(
    bowlingPoseBone(model, 'leftCalf'),
    deg(lerp(0, -22, rightPlant) - leftTrailSettle * 3),
    deg(-1.5),
    deg(-6)
  );
}
