...
function getHeldBallWorldPosition(rig: HumanRig) {
  let local = new THREE.Vector3(0.22, 0.88, -0.02);
  if (rig.action === "approach") {
    const s = Math.sin(rig.walkCycle);
    local = new THREE.Vector3(0.26, 0.84 + Math.abs(s) * 0.04, -0.08 + s * 0.04);
  } else if (rig.action === "throw") {
    const t = clamp01(rig.throwT);
    if (t < 0.38) {
      const k = easeInOut(t / 0.38);
      local = new THREE.Vector3(lerp(0.24, 0.34, k), lerp(0.88, 0.52, k), lerp(-0.02, -0.78, k));
    } else if (t < CFG.releaseT) {
      const k = easeInOut((t - 0.38) / (CFG.releaseT - 0.38));
      local = new THREE.Vector3(lerp(0.34, 0.16, k), lerp(0.52, 0.36, k), lerp(-0.78, 1.28, k));
    } else {
...
function createEnvironment(scene: THREE.Scene, loader: THREE.TextureLoader) {
  ...
  const sideChannel = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.2, 9.9), metalMat);
  sideChannel.position.set(1.9, 0.18, 1.05);
  group.add(sideChannel);
  const returnRailTop = new THREE.Mesh(new THREE.BoxGeometry(0.56, 0.05, 9.9), new THREE.MeshPhysicalMaterial({ color: 0xf8fafc, metalness: 1, roughness: 0.08, clearcoat: 1 }));
  returnRailTop.position.set(1.9, 0.29, 1.05);
  group.add(returnRailTop);
  const hoodLip = new THREE.Mesh(new THREE.TorusGeometry(0.35, 0.03, 16, 48, Math.PI), metalMat);
  hoodLip.rotation.set(Math.PI / 2, 0, Math.PI / 2);
  hoodLip.position.set(1.67, 0.61, 5.92);
  group.add(hoodLip);
...
function updateHuman(rig: HumanRig, ball: BallState, dt: number, canStartReturnCycle: boolean) {
  if (rig.action === "approach") {
    ...
    if (rig.model) {
      ...
      rig.model.rotation.z = t < 0.45 ? lerp(0, -0.12, t / 0.45) : lerp(-0.12, 0.08, (t - 0.45) / 0.55);
    }
    if (rig.throwT >= 1) {
      rig.action = "recover";
      rig.recoverT = 0.001;
    ...
    if (rig.model) {
      ...
      rig.model.rotation.y = 0;
      rig.model.rotation.z = swing * 0.05;
    ...
    if (rig.model) {
      const swing = Math.sin(rig.walkCycle) * 0.12;
      rig.model.rotation.y = 0;
      rig.model.rotation.z = -swing * 0.04;
      rig.model.position.y = Math.abs(Math.sin(rig.walkCycle)) * 0.05;
    }
