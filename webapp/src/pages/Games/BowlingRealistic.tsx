...
function getHeldBallWorldPosition(rig: HumanRig) {
  let local = new THREE.Vector3(0.315, 0.91, 0.02);
  if (rig.action === "approach") {
    const s = Math.sin(rig.walkCycle);
    local = new THREE.Vector3(0.315, 0.84 + Math.abs(s) * 0.035, 0.02 + s * 0.035);
  } else if (rig.action === "throw") {
    const t = clamp01(rig.throwT);
    if (t < 0.38) {
      const k = easeInOut(t / 0.38);
      local = new THREE.Vector3(lerp(0.31, 0.34, k), lerp(0.84, 0.57, k), lerp(0.02, -0.52, k));
    } else if (t < CFG.releaseT) {
      const k = easeInOut((t - 0.38) / (CFG.releaseT - 0.38));
      local = new THREE.Vector3(lerp(0.34, 0.2, k), lerp(0.57, 0.36, k), lerp(-0.52, 1.12, k));
    } else {
...
function createEnvironment(scene: THREE.Scene, loader: THREE.TextureLoader) {
  ...
  const returnRailMat = new THREE.MeshPhysicalMaterial({ color: 0xf3f7ff, metalness: 1, roughness: 0.12, clearcoat: 1, clearcoatRoughness: 0.04 });
  const railL = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.05, 9.7), returnRailMat);
  railL.position.set(1.73, 0.27, 1.05);
  group.add(railL);
  const railR = railL.clone();
  railR.position.x = 1.99;
  group.add(railR);
  const hoodTrim = new THREE.Mesh(new THREE.TorusGeometry(0.33, 0.028, 18, 48, Math.PI), returnRailMat);
  hoodTrim.rotation.z = Math.PI / 2;
  hoodTrim.position.set(1.67, 0.53, 5.92);
  group.add(hoodTrim);
...
function updateHuman(rig: HumanRig, ball: BallState, dt: number, canStartReturnCycle: boolean) {
  if (rig.action === "approach") {
    ...
    if (rig.model) {
      ...
      rig.model.rotation.z = t < 0.45 ? lerp(0, -0.17, t / 0.45) : lerp(-0.17, -0.06, (t - 0.45) / 0.55);
      rig.model.rotation.y = t < 0.5 ? lerp(0, -0.08, t / 0.5) : lerp(-0.08, -0.02, (t - 0.5) / 0.5);
    }
    if (rig.throwT >= 1) {
      rig.action = "recover";
      rig.recoverT = 0.001;
    ...
    if (rig.model) {
      rig.model.rotation.x = lerp(-0.12, -0.02, clamp01(rig.recoverT));
