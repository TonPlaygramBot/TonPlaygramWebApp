...
type PlayerAction = "idle" | "seated" | "standingUp" | "approach" | "throw" | "recover" | "toSeat" | "toRack" | "pickBall" | "toApproach" | "replay";
...
const BOWLING_RULES = [
  "10 frames per player. Frames 1–9 end after a strike or after two rolls.",
  "A strike scores 10 plus the next two rolls; a spare scores 10 plus the next roll.",
  "In frame 10, a strike or spare earns bonus rolls with a fresh rack when rules require it.",
  "Only the active bowler stands, takes a ball, approaches, and releases; the waiting player remains seated.",
  "Swipe up for power. Start/drag left or right to choose board and hook direction.",
];
...
const PLAYER_SEATS = [
  { pos: new THREE.Vector3(-2.72, CFG.laneY, 6.7), yaw: Math.PI / 2, stand: new THREE.Vector3(-0.42, CFG.laneY, CFG.playerStartZ) },
  { pos: new THREE.Vector3(2.72, CFG.laneY, 6.7), yaw: -Math.PI / 2, stand: new THREE.Vector3(0.42, CFG.laneY, CFG.playerStartZ) },
];
...
function shouldResetRackForNextRoll(player: ScorePlayer, frameIndex: number, frameEnded: boolean) {
...
function makeFallbackHuman(color: number) {
  ...
  torso.name = "fallback_torso";
  torso.position.y = 1.05;
  g.add(torso);
  const leftLeg = new THREE.Mesh(new THREE.CapsuleGeometry(0.07, 0.52, 4, 10), pants);
  leftLeg.name = "fallback_leftLeg";
  leftLeg.position.set(-0.12, 0.35, 0);
  g.add(leftLeg);
  const rightLeg = leftLeg.clone();
  rightLeg.name = "fallback_rightLeg";
  rightLeg.position.x = 0.12;
  g.add(rightLeg);
  const leftArm = new THREE.Mesh(new THREE.CapsuleGeometry(0.055, 0.42, 4, 10), skin);
  leftArm.name = "fallback_leftArm";
  leftArm.position.set(-0.32, 1.16, 0);
  leftArm.rotation.z = 0.22;
  g.add(leftArm);
  const rightArm = leftArm.clone();
  rightArm.name = "fallback_rightArm";
  rightArm.position.set(0.32, 1.16, 0.06);
  rightArm.rotation.z = -0.18;
  g.add(rightArm);
  const shoeL = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.06, 0.28), shoes);
  shoeL.name = "fallback_leftShoe";
  shoeL.position.set(-0.12, 0.03, -0.02);
  g.add(shoeL);
  const shoeR = shoeL.clone();
  shoeR.name = "fallback_rightShoe";
  shoeR.position.x = 0.12;
  g.add(shoeR);
  g.userData.parts = { head, torso, leftLeg, rightLeg, leftArm, rightArm, shoeL, shoeR };
  enableShadow(g);
  return g;
}

function animateFallbackHuman(rig: HumanRig) {
  const parts = rig.fallback.userData.parts || {};
  const cycle = rig.walkCycle;
  const stride = Math.sin(cycle);
  const counter = Math.sin(cycle + Math.PI);
  const reset = (obj: THREE.Object3D | undefined) => {
    if (!obj) return;
    obj.rotation.set(0, 0, 0);
  };
  [parts.head, parts.torso, parts.leftLeg, parts.rightLeg, parts.leftArm, parts.rightArm, parts.shoeL, parts.shoeR].forEach(reset);
  if (rig.action === "seated") {
    parts.torso?.rotation?.set?.(-0.16, 0, 0);
    parts.leftLeg?.rotation?.set?.(-1.05, 0, 0.08);
    parts.rightLeg?.rotation?.set?.(-1.05, 0, -0.08);
    parts.leftArm?.rotation?.set?.(0.2, 0, 0.32);
    parts.rightArm?.rotation?.set?.(0.25, 0, -0.32);
    if (parts.head) parts.head.rotation.y = Math.sin(performance.now() * 0.0014) * 0.16;
    return;
  }
  if (rig.action === "approach" || rig.action === "toRack" || rig.action === "toApproach" || rig.action === "toSeat") {
    if (parts.leftLeg) parts.leftLeg.rotation.x = stride * 0.5;
    if (parts.rightLeg) parts.rightLeg.rotation.x = counter * 0.5;
    if (parts.leftArm) parts.leftArm.rotation.x = counter * 0.46;
    if (parts.rightArm) parts.rightArm.rotation.x = stride * 0.46;
    if (parts.torso) parts.torso.rotation.z = Math.sin(cycle * 2) * 0.025;
  }
  if (rig.action === "throw") {
    const t = clamp01(rig.throwT);
    if (parts.torso) parts.torso.rotation.x = lerp(0.02, 0.34, Math.min(t / CFG.releaseT, 1));
    if (parts.rightArm) parts.rightArm.rotation.x = t < CFG.releaseT ? lerp(-0.3, -1.75, t / CFG.releaseT) : lerp(-1.75, 0.78, (t - CFG.releaseT) / (1 - CFG.releaseT));
    if (parts.leftArm) parts.leftArm.rotation.x = lerp(0.1, 0.7, t);
    if (parts.leftLeg) parts.leftLeg.rotation.x = -0.42;
...
function createEnvironment(scene: THREE.Scene, loader: THREE.TextureLoader, tableFinishId: string, chromeColorId: string) {
  ...
  // Intentionally no dark grounding floor: the lane sits directly on the HDRI ground plane so the environment remains visible.
  ...
  group.add(foulLine);
  const boardMat = new THREE.MeshBasicMaterial({ color: 0x3f2b16, transparent: true, opacity: 0.18 });
  for (let i = -8; i <= 8; i++) {
    if (i === 0) continue;
    const boardLine = new THREE.Mesh(new THREE.PlaneGeometry(0.008, 18.2), boardMat);
    boardLine.rotation.x = -Math.PI / 2;
    boardLine.position.set(i * (CFG.laneHalfW / 10), CFG.laneY + 0.014, -4.15);
    group.add(boardLine);
  }
  const dotMat = new THREE.MeshBasicMaterial({ color: 0x1d3557, transparent: true, opacity: 0.58 });
  [-0.92, -0.46, 0, 0.46, 0.92].forEach((x) => {
    const dot = new THREE.Mesh(new THREE.CircleGeometry(0.035, 24), dotMat);
    dot.rotation.x = -Math.PI / 2;
    dot.position.set(x, CFG.laneY + 0.016, 3.22);
    group.add(dot);
  });
  ...
  for (let i = -3; i <= 3; i++) {
    ...
    mesh.position.set(i * 0.32, CFG.laneY + 0.017, CFG.arrowsZ);
    group.add(mesh);
  }
  const pinSpotMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.34 });
  [[0, 0], [-0.32, -0.56], [0.32, -0.56], [-0.64, -1.12], [0, -1.12], [0.64, -1.12], [-0.96, -1.68], [-0.32, -1.68], [0.32, -1.68], [0.96, -1.68]].forEach(([x, dz]) => {
    const spot = new THREE.Mesh(new THREE.RingGeometry(0.11, 0.125, 24), pinSpotMat);
  ...
  lounge.position.set(0, CFG.laneY, 6.7);
  try {
...
function updateCamera(camera: THREE.PerspectiveCamera, ball: BallState, player: HumanRig, dt: number) {
  ...
  if (ball.rolling) {
    ...
    const shoulderSide = player.pos.x <= 0 ? -0.34 : 0.34;
    desired = player.pos.clone().add(new THREE.Vector3(shoulderSide, 1.62, 1.18));
    look = new THREE.Vector3(clamp(player.pos.x * 0.28, -0.42, 0.42), CFG.laneY + 0.52, CFG.pinDeckZ - 0.42);
...
export default function MobileBowlingRealistic() {
  ...
  const [hud, setHud] = useState<HudState>({ power: 0, status: "Swipe up to bowl", compliment: "", activePlayer: 0, p1: 0, p2: 0, frame: 1, roll: 1, phase: "Ready", ruleHint: BOWLING_RULES[0] });
  ...
  useEffect(() => {
    ...
    const prepareNextTurnAfterReturn = () => {
      ...
      settleTimer = 0;
      pendingIntent = null;
      if (resetDeckOnReturn) resetPins(pins);
      resetDeckOnReturn = false;
      ...
      setHud((prev) => ({ ...prev, status: nextAction === "gameOver" ? "Game over" : activePlayer === 1 ? `${playerName} is choosing a line…` : `${playerName} swipe up to bowl`, phase: nextAction === "gameOver" ? "Final" : activePlayer === 1 ? "AI turn" : "Your turn" }));
    };
    ...
    const finalizeShot = () => {
      ...
      resetDeckOnReturn = shouldResetRackForNextRoll(playerBefore, result.frameIndex, result.frameEnded);
      ...
      syncReactScores();
      const ruleHint = resetDeckOnReturn && result.frameIndex === 9 ? "10th-frame bonus rack is being reset." : result.frameEnded ? "Frame complete — next bowler gets a full rack." : "Second ball: standing pins remain in play.";
      setHud((prev) => ({ ...prev, status, compliment, ruleHint, phase: result.frameEnded ? "Frame scored" : "Second ball" }));
      shotResolved = true;
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      setHud((prev) => ({ ...prev, power: 0, status: "Swipe up. Slide left/right to pick board + hook.", phase: "Aiming", ruleHint: BOWLING_RULES[4] }));
    };
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      pendingIntent = control.intent;
      updateAimVisual(aimLine, aimMarker, control.intent);
      setHud((prev) => ({ ...prev, power: control.intent!.power, phase: `Power ${Math.round(control.intent!.power * 100)}%` }));
    };
    ...
    const onPointerUp = (e: PointerEvent) => {
      ...
      if (intent.power < 0.05) {
        pendingIntent = null;
        setHud((prev) => ({ ...prev, power: 0, status: "Swipe higher for power", phase: "Try again" }));
        return;
      }
      pendingIntent = intent;
      startApproach(player, intent);
      setHud((prev) => ({ ...prev, power: 0, status: "Four-step approach: eyes on the arrows", phase: "Approach" }));
    };
    ...
    function animate() {
      ...
      if (activePlayer === 1 && !control.active && !ball.rolling && !waitingForBallReturn && !replayActive && player.action === "idle" && nextAction !== "gameOver") {
        ...
        if (aiTurnDelay <= 0) {
          ...
          setHud((prev) => ({ ...prev, status: "PLAYER 2 AI is bowling", phase: "AI release" }));
        }
      }
      ...
      if (player.action === "throw" && pendingIntent && player.throwT >= CFG.releaseT && ball.held) {
        ...
        pendingIntent = null;
        setHud((prev) => ({ ...prev, status: "Ball rolling through the oil pattern", phase: "Rolling" }));
      }
    ...
    <div style={{ position: "fixed", inset: 0, overflow: "hidden", background: "#090b11", touchAction: "none", userSelect: "none" }}>
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, sans-serif" }}>
        <div style={{ position: "absolute", left: 8, right: 8, top: 8, color: "white", background: "rgba(5,8,14,0.72)", border: "1px solid rgba(255,255,255,0.16)", borderRadius: 16, padding: "8px 8px 10px", boxShadow: "0 14px 30px rgba(0,0,0,0.28)", backdropFilter: "blur(10px)" }}>
          ...
          <div style={{ marginTop: 7, display: "flex", justifyContent: "center", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: 10, fontWeight: 900, color: "#001018", background: "#7fd6ff", borderRadius: 999, padding: "2px 8px" }}>{hud.phase}</span>
            <span style={{ fontSize: 11, fontWeight: 700, opacity: 0.94 }}>{hud.status}</span>
          </div>
          <div style={{ marginTop: 4, textAlign: "center", fontSize: 10, fontWeight: 700, color: "#c7d2fe" }}>{hud.ruleHint}</div>
        ...
        {menuOpen ? <div style={{ position:"absolute", top: 178, left: 8, right: 8, maxHeight:"48vh", overflow:"auto", borderRadius: 14, padding: 10, background:"rgba(5,8,14,0.88)", border:"1px solid rgba(255,255,255,0.18)", pointerEvents:"auto" }}>
          <div style={{fontSize:12,fontWeight:800,marginBottom:8}}>Rules + mobile controls</div>
          <div style={{display:"grid",gap:5,marginBottom:10}}>{BOWLING_RULES.map((rule, idx)=><div key={rule} style={{fontSize:11,lineHeight:1.25,color:"rgba(255,255,255,0.86)"}}><b style={{color:"#7fd6ff"}}>{idx + 1}.</b> {rule}</div>)}</div>
        ...
        <div style={{ position:"absolute", left: 10, right: 10, bottom: 16, padding: "10px 12px", borderRadius: 16, background:"linear-gradient(180deg, rgba(5,8,14,0.68), rgba(5,8,14,0.88))", border:"1px solid rgba(255,255,255,0.16)", color:"#fff", pointerEvents:"none", boxShadow:"0 18px 34px rgba(0,0,0,0.28)" }}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, marginBottom:6}}>
            <div style={{fontSize:11,fontWeight:900,letterSpacing:0.4}}>MOBILE RELEASE</div>
            <div style={{fontSize:10,fontWeight:800,color:"#7fd6ff"}}>Aim board + hook with one thumb</div>
          </div>
          <div style={{height:8,borderRadius:999,background:"rgba(255,255,255,0.14)",overflow:"hidden"}}>
            <div style={{height:"100%",width:`${Math.round(hud.power * 100)}%`,background:"linear-gradient(90deg,#86efac,#7fd6ff,#fbbf24)",transition:"width 80ms linear"}} />
          </div>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,fontWeight:700,opacity:0.82,marginTop:5}}>
            <span>low speed</span><span>strike speed</span>
          </div>
        </div>
