...
export default function SnookerRoyalLobby() {
  ...
  const navigateToSnookerRoyal = ({ tableId: startedId, roster = [], accountId, currentTurn }) => {
    ...
    navigate(`/games/snookerroyal?${params.toString()}`);
  };
  ...
  const startGame = async () => {
    ...
    if (playType === 'tournament') {
      window.location.href = `/snooker-royal-bracket.html?${params.toString()}`;
      return;
    }
    ...
    <div className="relative p-4 space-y-4 text-text min-h-screen tetris-grid-bg">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => setVariant('snooker')}
            className={`lobby-tile ${variant === 'snooker' ? 'lobby-selected' : ''}`}
          >
            Snooker
          </button>
        </div>
