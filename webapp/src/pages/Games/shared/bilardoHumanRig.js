...
function normalizeHuman(model, opts) {
  model.scale.setScalar(opts?.humanScale ?? 1.18);
  model.rotation.set(0, opts?.humanVisualYawFix ?? Math.PI, 0);
...
function poseFingers(fingers, mode, weight) {
  ...
  fingers.forEach((finger, i) => {
    ...
    if (mode === 'grip') {
      ...
      const curl = index
        ? base ? 0.52 : mid ? 0.82 : 0.6
        : middle
          ? base ? 0.72 : mid ? 1 : 0.74
          : ring
            ? base ? 0.66 : mid ? 0.88 : 0.66
            : pinky
              ? base ? 0.56 : mid ? 0.76 : 0.56
              : 0;
    ...
    if (thumb) {
      ...
      finger.rotation.x += (base ? (ring ? 0.1 : 0.06) : mid ? (ring ? 0.22 : 0.18) : (ring ? 0.14 : 0.12)) * w;
...
function driveHuman(human, frame) {
  ...
  setHandBasis(b.leftHand, bridgeSide, bridgeUp, cueDir, -0.82 * ik, 1 * ik);
  poseFingers(human.leftFingers, 'bridge', 1 * ik);
...
export function updateBilardoHumanPose(human, dt, frameData) {
  ...
  const bridgePalmTarget = frameData.bridgeTarget
    .clone()
  ...
  const rightHand = frameData.idleRight.clone().lerp(
