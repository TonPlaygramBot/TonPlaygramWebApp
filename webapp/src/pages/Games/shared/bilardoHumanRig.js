...
export function updateBilardoHumanPose(human, dt, frameData) {
  ...
  const halfX = Math.max(0.1, Number(frameData.walkHalfX ?? 1));
  const halfZ = Math.max(0.1, Number(frameData.walkHalfZ ?? 1));
  const perimeterGoalT = pointToPerimeterT(rootGoal, halfX, halfZ);
  if (!Number.isFinite(human.walkPerimeterT)) {
    human.walkPerimeterT = perimeterGoalT;
  }
  ...
  const perimeterPoint = perimeterTToPoint(human.walkPerimeterT, halfX, halfZ, rootGoal.y);
