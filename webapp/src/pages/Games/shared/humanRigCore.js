...
function createHumanFaceDetails(human) {
  ...
  const makeEye = (x) => {
    ...
    // The added eye details must sit on the avatar's face side.  The GLTF/RPM
    // head bones used here expose their forward facial side on local +Z after the
    // Pool Royale visual-yaw correction, so keep pupils/brows on +Z instead of
...
function resolveShootBendSign(human, frameData, cfg) {
  ...
  if (!human?.root) return fallbackSign;
  // Local -Z is the direction the rig is facing. During a shot the rig yaw is
  // locked toward the cue ball/table, so local -Z is always the table-facing side.
  // Keep the bend sign fixed to -Z to prevent any transient cue/reference flip
  // from making the body fold backward.
  return -1;
}
      ...
      (gltf) => {
        ...
        model.traverse((obj) => {
          ...
          mats.forEach((mat) => {
            ...
            if (cfg.refereeUniform) applyRefereeUniformMaterial(obj, mat);
            mat.depthWrite = true;
            mat.depthTest = true;
...
export function updateHumanPose(human, dt, frameData) {
  ...
  const torso = local(new THREE.Vector3(shotCounterLeanX(0.012 * t) * cfg.unit, lerp(1.3, 1.17, t) * cfg.unit + breath, shotBendZ(lerp(0.01, -0.14, t) - 0.01 * powerLean) * cfg.unit));
  const chest = local(new THREE.Vector3(shotCounterLeanX(0.038 * t + 0.006 * powerLean) * cfg.unit, lerp(1.52, 1.24, t) * cfg.unit + breath, shotBendZ(lerp(0.015, -0.44, t) - 0.018 * powerLean) * cfg.unit));
  const neck = local(new THREE.Vector3(shotCounterLeanX(0.055 * t + 0.008 * powerLean) * cfg.unit, lerp(1.68, 1.3, t) * cfg.unit + breath, shotBendZ(lerp(0.02, -0.62, t) - 0.02 * powerLean) * cfg.unit));
  const head = local(new THREE.Vector3(shotCounterLeanX(0.066 * t + 0.01 * powerLean) * cfg.unit, lerp(1.84, 1.38, t) * cfg.unit + breath - cfg.chinToCueHeight * 0.24 * t, shotBendZ(lerp(0.03, -0.74, t) - 0.022 * powerLean) * cfg.unit));
  const leftShoulder = local(new THREE.Vector3((-0.23 + shotCounterLeanX(0.048 * t)) * cfg.unit, lerp(1.58, 1.34, t) * cfg.unit + breath, shotBendZ(lerp(0, -0.46, t) - 0.014 * human.settleT) * cfg.unit));
  const rightShoulder = local(new THREE.Vector3((0.23 + shotCounterLeanX(0.034 * t)) * cfg.unit, lerp(1.58, 1.36, t) * cfg.unit + breath, shotBendZ(lerp(0, -0.34, t) - 0.014 * human.settleT) * cfg.unit));
