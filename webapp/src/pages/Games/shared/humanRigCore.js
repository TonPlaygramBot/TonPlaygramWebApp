...
function resolveShootBendSign(human, frameData, cfg) {
  ...
  const wantsBackwardBend = cfg.shootBendMode === 'backward';
  if (!cfg.shootBendTowardCueStick) return wantsBackwardBend ? 1 : fallbackSign;
  const cueReference = resolveTableReference(frameData);
  if (!cueReference || !human?.root) return wantsBackwardBend ? 1 : fallbackSign;
  const rootToTableLocal = cueReference
    .clone()
    .sub(human.root.position)
    .applyAxisAngle(Y_AXIS, -human.yaw);
  const tableIsForward = Math.abs(rootToTableLocal.z) > 1e-5 ? rootToTableLocal.z <= 0 : true;
  // Local -Z is the direction the rig is facing. Forward profiles fold into that
  // table-facing side; backward profiles deliberately fold to the opposite side.
  if (wantsBackwardBend) return tableIsForward ? 1 : -1;
  return tableIsForward ? -1 : 1;
}
  ...
  const loadModelAt = (urlIndex = 0) => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (!human.activeGlb && urlIndex + 1 < modelUrls.length) {
          onStatus?.('Human skeleton incomplete; trying fallback avatar…');
