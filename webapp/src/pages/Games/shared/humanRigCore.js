...
function updateOriginalSkeletonHumanPose(human, dt, frameData) {
  ...
  const originalShootingPoseActive = activeState === 'dragging' || activeState === 'striking';
  ...
  const visualYawFix = cfg.humanVisualYawFix || 0;
  const targetYaw = originalShootingPoseActive
    ? yawFromForward(rootToTableForward) - visualYawFix
    : yawFromForward(resolveTableFacingForward(frameData.aimForward, human.root.position, frameData, cfg)) - visualYawFix;
  human.yaw = originalShootingPoseActive
    ? targetYaw
  ...
  const strikeFollow = activeState === 'striking'
    ...
    : 0;
  // The original ReadyPlayer pose places chest/head reach offsets on local -Z.
  // Map local -Z directly toward the cue ball/table so the shooter never takes
  // the stance backwards; the visual model still receives humanVisualYawFix via
  // targetYaw/model normalization, while all IK targets lean toward the cloth.
  ...
  const rootWorld = human.root.position.clone().addScaledVector(rootToTableForward, (0.018 * powerLean + 0.026 * strikeFollow) * cfg.unit);
