...
function resolveWalkPerimeter(cfg) {
  const halfX = Math.max(cfg.unit, (cfg.tableW || 0) / 2 + cfg.edgeMargin);
  const halfZ = Math.max(cfg.unit, (cfg.tableL || 0) / 2 + cfg.edgeMargin);
  return { halfX, halfZ, total: 4 * (halfX + halfZ) };
}

function pointToPerimeterT(point, cfg) {
  const { halfX, halfZ, total } = resolveWalkPerimeter(cfg);
  const x = clamp(point.x, -halfX, halfX);
  const z = clamp(point.z, -halfZ, halfZ);
  const distances = [
    { side: 'bottom', d: Math.abs(z + halfZ), s: x + halfX },
    { side: 'right', d: Math.abs(x - halfX), s: 2 * halfX + z + halfZ },
    { side: 'top', d: Math.abs(z - halfZ), s: 2 * halfX + 2 * halfZ + (halfX - x) },
    { side: 'left', d: Math.abs(x + halfX), s: 4 * halfX + 2 * halfZ + (halfZ - z) }
  ];
  const best = distances.sort((a, b) => a.d - b.d)[0];
  return THREE.MathUtils.euclideanModulo(best.s / total, 1);
}

function perimeterTToPoint(t, cfg, y = 0) {
  const { halfX, halfZ, total } = resolveWalkPerimeter(cfg);
  let s = THREE.MathUtils.euclideanModulo(t, 1) * total;
  if (s <= 2 * halfX) return new THREE.Vector3(-halfX + s, y, -halfZ);
  s -= 2 * halfX;
  if (s <= 2 * halfZ) return new THREE.Vector3(halfX, y, -halfZ + s);
  s -= 2 * halfZ;
  if (s <= 2 * halfX) return new THREE.Vector3(halfX - s, y, halfZ);
  s -= 2 * halfX;
  return new THREE.Vector3(-halfX, y, halfZ - s);
}

function moveRootOnPerimeter(human, rootGoal, dt) {
  const cfg = human.cfg;
  if (!cfg.usePerimeterWalk || !Number.isFinite(cfg.tableW) || !Number.isFinite(cfg.tableL)) {
    return dampVector(human.root.position, rootGoal, cfg.moveLambda, dt);
  }
  const { total } = resolveWalkPerimeter(cfg);
  if (!Number.isFinite(human.walkPerimeterT)) {
    human.walkPerimeterT = pointToPerimeterT(human.root.position.lengthSq() > 1e-6 ? human.root.position : rootGoal, cfg);
    human.root.position.copy(perimeterTToPoint(human.walkPerimeterT, cfg, rootGoal.y));
  }
  const targetT = pointToPerimeterT(rootGoal, cfg);
  const loopDelta = THREE.MathUtils.euclideanModulo(targetT - human.walkPerimeterT + 0.5, 1) - 0.5;
  const speed = Math.max(cfg.unit * 0.8, cfg.perimeterWalkSpeed || cfg.unit * 2.8);
  const maxStepT = (speed * Math.max(dt, 1 / 240)) / Math.max(total, 1e-6);
  const stepT = clamp(loopDelta, -maxStepT, maxStepT);
  human.walkPerimeterT = THREE.MathUtils.euclideanModulo(human.walkPerimeterT + stepT, 1);
  const perimeterPoint = perimeterTToPoint(human.walkPerimeterT, cfg, rootGoal.y);
  if (Math.abs(loopDelta) <= maxStepT * 1.05) {
    perimeterPoint.lerp(rootGoal, clamp01(1 - Math.exp(-cfg.moveLambda * dt)));
    human.walkPerimeterT = pointToPerimeterT(perimeterPoint, cfg);
  }
  human.root.position.copy(perimeterPoint);
  return human.root.position;
}
...
function driveHuman(human, frame) {
  ...
  const liveHandSide = frame.side.clone().multiplyScalar(-1).addScaledVector(UP, -0.62).addScaledVector(frame.side, 0.5).addScaledVector(frame.forward, -0.08).normalize();
  const liveHandUp = UP.clone().multiplyScalar(0.12).addScaledVector(frame.side, -0.04).addScaledVector(frame.forward, -0.48).normalize();
  const rightHandSide = standingHandSide.clone().lerp(liveHandSide, ik).normalize();
  const rightHandUp = standingHandUp.clone().lerp(liveHandUp, ik).normalize();
  const handForwardForOrientation = standingCueDir.clone().lerp(cueDir, ik).normalize();
  setHandBasis(b.rightHand, rightHandSide, rightHandUp, handForwardForOrientation, lerp(cfg.rightHandRollIdle, cfg.rightHandRollShoot, ik), 1.0);
...
export function updateHumanPose(human, dt, frameData) {
  ...
  if (activeState === 'striking') {
    ...
    moveRootOnPerimeter(human, rootGoal, dt);
  }
