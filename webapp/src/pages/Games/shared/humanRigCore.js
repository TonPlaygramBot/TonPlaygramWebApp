...
export function updateHumanPose(human, dt, frameData) {
  ...
  if (![rootTarget, aimForward, bridgeTarget, idleRight, idleLeft, cueBack, cueTip].every(isFiniteVec3)) return;
  ...
  const idleWristTarget = idleRight.clone().sub(cueSocketOffsetWorld(idleGripSide, idleGripUp, cueDirForHand, cfg.rightHandRollIdle, socketLocal));
  const liveWristTarget = liveCueGripPoint.clone().sub(cueSocketOffsetWorld(liveGripSide, liveGripUp, cueDirForHand, lerp(cfg.rightHandRollIdle, cfg.rightHandRollShoot - cfg.rightHandDownPose, handIk), socketLocal));
