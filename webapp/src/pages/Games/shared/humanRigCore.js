...
function resolveShootBendSign(human, frameData, cfg) {
  ...
  // Local -Z is the direction the rig is facing. Forward bend folds toward the
  // table; backward bend deliberately leans away while the face remains table-facing.
  const tableSign = Math.abs(rootToTableLocal.z) > 1e-5
    ? (rootToTableLocal.z <= 0 ? -1 : 1)
    : -1;
  return cfg.shootBendMode === 'backward' ? -tableSign : tableSign;
}
