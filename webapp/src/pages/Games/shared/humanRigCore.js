...
const BASE_CFG = {
  ...
  shootBellyForward: 0.08,
  shootChestForward: 0.26,
  shootHeadForward: 0.38,
  shootHeadDrop: 0.28,
  shootHipInfluence: 0.06,
  shootLegIkStrength: 0.18,
...
function resolveShootBendSign(human, frameData, cfg) {
  if (cfg.shootBendMode === 'backward') return 1;
  if (cfg.shootBendMode === 'forward') return -1;
  ...
  // Local -Z is the rig's visual forward direction after resolveTableFacingForward().
  // Positive Z is backward, so force every pool-shooting profile to fold the belly,
  // chest, neck and head along -Z toward the table/cue ball. This removes the
  // previous sign ambiguity that made some avatars bend away from the cloth.
...
function driveHuman(human, frame) {
  ...
  if (ik >= 0.025) {
    const hipInfluence = Number.isFinite(cfg.shootHipInfluence) ? Math.max(0, Math.min(0.18, cfg.shootHipInfluence)) : 0.06;
    rotateBoneToward(b.hips, frame.torsoCenterWorld, hipInfluence * ik, frame.forward);
    twistBone(b.hips, frame.side, -0.012 * ik * hipInfluence);
    twistBone(b.hips, frame.forward, -0.006 * ik * hipInfluence);
    rotateBoneToward(b.spine, frame.chestCenterWorld, (0.38 + 0.34 * ik) * ik, frame.forward);
  ...
  const legIk = Number.isFinite(cfg.shootLegIkStrength) ? Math.max(0, Math.min(1, cfg.shootLegIkStrength)) : 0.18;
  aimTwoBone(b.leftUpperLeg, b.leftLowerLeg, frame.leftKnee, frame.leftFootWorld, frame.forward.clone().addScaledVector(UP, 0.18).normalize(), legIk * ik, legIk * ik);
  twistBone(b.leftUpperLeg, frame.forward, -0.012 * legIk * ik);
  setHandBasis(b.leftFoot, frame.side, frame.up, frame.forward, -0.012 * ik, cfg.footLockStrength * ik);
  aimTwoBone(b.rightUpperLeg, b.rightLowerLeg, frame.rightKnee, frame.rightFootWorld, frame.forward.clone().multiplyScalar(-1).addScaledVector(UP, 0.18).normalize(), legIk * ik, legIk * ik);
  twistBone(b.rightUpperLeg, frame.forward, 0.01 * legIk * ik);
  setHandBasis(b.rightFoot, frame.side, frame.up, frame.forward, 0.012 * ik, cfg.footLockStrength * ik);
}
...
export function updateHumanPose(human, dt, frameData) {
  ...
  const bellyForward = Math.max(0.02, cfg.shootBellyForward) * forwardBendScale;
  const chestForward = Math.max(bellyForward, cfg.shootChestForward) * forwardBendScale;
  const headForward = Math.max(chestForward, cfg.shootHeadForward) * forwardBendScale;
  const headDrop = Math.max(0, cfg.shootHeadDrop) * forwardBendScale;
  const torso = local(new THREE.Vector3(
    shotCounterLeanX(0.012 * t) * cfg.unit,
    lerp(1.3, 1.25, t) * cfg.unit + breath,
    shotBendZ(bellyForward * t + 0.006 * powerLean) * cfg.unit
  ));
  const chest = local(new THREE.Vector3(
    shotCounterLeanX(0.042 * t + 0.008 * powerLean) * cfg.unit,
    lerp(1.52, 1.42 - headDrop * 0.18, t) * cfg.unit + breath,
    shotBendZ(chestForward * t + 0.014 * powerLean) * cfg.unit
  ));
  const neck = local(new THREE.Vector3(
    shotCounterLeanX(0.07 * t + 0.01 * powerLean) * cfg.unit,
    lerp(1.68, 1.55 - headDrop * 0.28, t) * cfg.unit + breath,
    shotBendZ((headForward * 0.9) * t + 0.018 * powerLean) * cfg.unit
  ));
  const head = local(new THREE.Vector3(
    shotCounterLeanX(0.08 * t + 0.012 * powerLean) * cfg.unit,
    lerp(1.84, 1.66 - headDrop, t) * cfg.unit + breath - cfg.chinToCueHeight * 0.08 * t,
    shotBendZ(headForward * t + 0.018 * powerLean) * cfg.unit
  ));
  const leftShoulder = local(new THREE.Vector3((-0.23 + shotCounterLeanX(0.048 * t)) * cfg.unit, lerp(1.58, 1.45, t) * cfg.unit + breath, shotBendZ((chestForward * 0.82) * t + 0.012 * human.settleT) * cfg.unit));
  const rightShoulder = local(new THREE.Vector3((0.23 + shotCounterLeanX(0.036 * t)) * cfg.unit, lerp(1.58, 1.45, t) * cfg.unit + breath, shotBendZ((chestForward * 0.72) * t + 0.012 * human.settleT) * cfg.unit));
