...
function updateProvidedExactHumanPose (human, dt, frameData) {
  ...
  const exactFacing = resolveTableFacingForward(aimForward, rootGoal, frameData, cfg)
  const exactYaw = yawFromForward(exactFacing) - (cfg.humanVisualYawFix || 0)
  human.yaw = state === 'dragging' || state === 'striking'
    ? exactYaw
    : dampScalar(human.yaw, exactYaw, cfg.rotLambda, dt)
  ...
  const rootWorld = human.root.position.clone().addScaledVector(forward, (0.012 * powerLean + 0.018 * strikeFollow) * cfg.scale)
  rootWorld.y = cfg.groundY || 0
  const torso = local(new THREE.Vector3(0, lerp(1.3, 1.18, t) * cfg.scale + breath, (lerp(0.02, -0.2, t) - 0.01 * powerLean) * cfg.scale))
  const chest = local(new THREE.Vector3(0, lerp(1.52, 1.32, t) * cfg.scale + breath, (lerp(0.02, -0.48, t) - 0.018 * powerLean) * cfg.scale))
  const neck = local(new THREE.Vector3(0, lerp(1.68, 1.39, t) * cfg.scale + breath, (lerp(0.02, -0.66, t) - 0.02 * powerLean) * cfg.scale))
  const head = local(new THREE.Vector3(0, lerp(1.84, 1.48, t) * cfg.scale + breath - cfg.chinToCueHeight * 0.12 * t, (lerp(0.04, -0.76, t) - 0.02 * powerLean) * cfg.scale))
  const leftShoulder = local(new THREE.Vector3(-0.23 * cfg.scale, lerp(1.58, 1.43, t) * cfg.scale + breath, (lerp(0, -0.42, t) - 0.012 * human.settleT) * cfg.scale))
  const rightShoulder = local(new THREE.Vector3(0.23 * cfg.scale, lerp(1.58, 1.43, t) * cfg.scale + breath, (lerp(0, -0.3, t) - 0.012 * human.settleT) * cfg.scale))
  ...
  const footWalkBlend = cfg.plantFeetDuringShot !== false ? idle : 1
  const leftFoot = local(new THREE.Vector3(-0.13 * cfg.scale, cfg.footGroundY, 0.03 * cfg.scale + walk * 0.018 * cfg.scale * footWalkBlend).lerp(new THREE.Vector3(-cfg.stanceWidth * 0.42, cfg.footGroundY, -0.34 * cfg.scale), t))
  const rightFoot = local(new THREE.Vector3(0.13 * cfg.scale, cfg.footGroundY, -0.03 * cfg.scale - walk * 0.018 * cfg.scale * footWalkBlend).lerp(new THREE.Vector3(cfg.stanceWidth * 0.5, cfg.footGroundY, 0.34 * cfg.scale), t))
  ...
  if (frameData.gripTarget) liveCueGripPoint.lerp(frameData.gripTarget, 0.82 * t)
  ...
  const bridgeArmDownElbow = leftHand.clone()
    .addScaledVector(UP, 0.34 * cfg.scale)
    .addScaledVector(side, -0.01 * cfg.scale)
    .addScaledVector(forward, 0.012 * cfg.scale)
  const extendedBridgeElbow = leftShoulder.clone()
    .lerp(leftHand, 0.62)
    .addScaledVector(UP, -0.02 * cfg.scale * t)
    .addScaledVector(side, -0.044 * cfg.scale * t)
    .addScaledVector(forward, 0.065 * cfg.scale * t)
