...
function moveRootAroundPerimeter(human, rootGoal, cfg, dt) {
  ...
  if (!perimeter) {
    ...
    human.root.position.y = cfg.floorY || 0;
  ...
  else human.root.position.copy(clampPointToWalkPerimeter(human.root.position, perimeter));
  ...
  human.root.position.copy(perimeterTToPoint(human.perimeterT, perimeter));
  human.root.position.y = cfg.floorY || 0;
...
export function updateHumanPose(human, dt, frameData) {
  ...
  const rootGoal = (activeState === 'striking' ? human.strikeRoot : frameData.rootTarget).clone();
  rootGoal.y = cfg.floorY || 0;
