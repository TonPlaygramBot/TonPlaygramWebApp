...
function updateOriginalSkeletonHumanPose(human, dt, frameData) {
  ...
  const aimForward = frameData.aimForward.clone().setY(0);
  if (aimForward.lengthSq() < 1e-8) aimForward.set(0, 0, -1);
  aimForward.normalize();
  const tableForward = (frameData.bridgeTarget || frameData.cueTip || frameData.cueBack)
    ?.clone?.()
    ?.sub(human.root.position)
    ?.setY(0);
  const shootingPoseActive = activeState === 'dragging' || activeState === 'striking';
  const poseForward = shootingPoseActive && tableForward && tableForward.lengthSq() > 1e-8
    ? tableForward.normalize()
    : aimForward;
  const targetYaw = activeState === 'striking'
    ? human.strikeYaw
    : yawFromForward(poseForward) - (cfg.humanVisualYawFix || 0);
  ...
  const strikeFollow = activeState === 'striking'
    ...
    : 0;
  // The loaded ReadyPlayer model keeps its own visual yaw fix on the model child.
  // Drive the original skeleton bend/stance from that visible direction so the
  // avatar leans over the cue line instead of folding toward the opposite side.
  const visualYaw = human.yaw + (cfg.humanVisualYawFix || 0);
