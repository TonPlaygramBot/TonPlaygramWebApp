...
function resolvePerimeter(cfg) {
  return {
    halfX: Math.max(0.1, (cfg.tableW ?? 2.0) / 2 + cfg.edgeMargin),
    halfZ: Math.max(0.1, (cfg.tableL ?? 3.6) / 2 + cfg.edgeMargin)
  };
}

function projectToPerimeter(pos, cfg) {
  const { halfX, halfZ } = resolvePerimeter(cfg);
  const x = clamp(pos?.x ?? 0, -halfX, halfX);
  const z = clamp(pos?.z ?? 0, -halfZ, halfZ);
  const distances = [
    { side: 'left', d: Math.abs(x + halfX) },
    { side: 'right', d: Math.abs(halfX - x) },
    { side: 'bottom', d: Math.abs(z + halfZ) },
    { side: 'top', d: Math.abs(halfZ - z) }
  ].sort((a, b) => a.d - b.d);
  switch (distances[0].side) {
    case 'left':
      return new THREE.Vector3(-halfX, 0, z);
    case 'right':
      return new THREE.Vector3(halfX, 0, z);
    case 'bottom':
      return new THREE.Vector3(x, 0, -halfZ);
    default:
      return new THREE.Vector3(x, 0, halfZ);
  }
}

function perimeterDistance(point, cfg) {
  const { halfX, halfZ } = resolvePerimeter(cfg);
  const width = halfX * 2;
  const depth = halfZ * 2;
  const p = projectToPerimeter(point, cfg);
  if (Math.abs(p.z + halfZ) <= 1e-5) return p.x + halfX;
  if (Math.abs(p.x - halfX) <= 1e-5) return width + p.z + halfZ;
  if (Math.abs(p.z - halfZ) <= 1e-5) return width + depth + (halfX - p.x);
  return width + depth + width + (halfZ - p.z);
}

function pointAtPerimeterDistance(distance, cfg) {
  const { halfX, halfZ } = resolvePerimeter(cfg);
  const width = halfX * 2;
  const depth = halfZ * 2;
  const perimeter = Math.max(1e-6, 2 * (width + depth));
  let d = THREE.MathUtils.euclideanModulo(distance, perimeter);
  if (d <= width) return new THREE.Vector3(-halfX + d, 0, -halfZ);
  d -= width;
  if (d <= depth) return new THREE.Vector3(halfX, 0, -halfZ + d);
  d -= depth;
  if (d <= width) return new THREE.Vector3(halfX - d, 0, halfZ);
  d -= width;
...
function dampRootAroundPerimeter(root, target, cfg, lambda, dt) {
  const targetOnRail = projectToPerimeter(target, cfg);
  if (!root.userData?.perimeterInitialized || root.position.lengthSq() <= 1e-8) {
    root.position.copy(targetOnRail);
    root.userData = { ...(root.userData || {}), perimeterInitialized: true };
    return 0;
  }
  const currentOnRail = projectToPerimeter(root.position, cfg);
  const { halfX, halfZ } = resolvePerimeter(cfg);
  const perimeter = Math.max(1e-6, 4 * (halfX + halfZ));
  const currentD = perimeterDistance(currentOnRail, cfg);
  const targetD = perimeterDistance(targetOnRail, cfg);
  const delta = THREE.MathUtils.euclideanModulo(targetD - currentD + perimeter / 2, perimeter) - perimeter / 2;
  const alpha = 1 - Math.exp(-lambda * dt);
  const next = pointAtPerimeterDistance(currentD + delta * alpha, cfg);
  const moved = root.position.distanceTo(next);
...
export function updateHumanPose(human, dt, frameData) {
  ...
  const previousRoot = human.root.position.clone();
  const movedAlongPerimeter = dampRootAroundPerimeter(
    human.root,
    rootGoal,
    cfg,
    activeState === 'striking' ? 12 : cfg.moveLambda,
    dt
  );
  const moveAmountRaw = Math.max(movedAlongPerimeter, previousRoot.distanceTo(human.root.position));
