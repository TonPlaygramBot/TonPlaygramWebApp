...
function resolveShootBendSign(human, frameData, cfg) {
  if (!cfg.shootBendTowardCueStick) return cfg.shootBendDirection >= 0 ? 1 : -1;
  const cueReference = resolveTableReference(frameData);
  if (!cueReference || !human?.root) return cfg.shootBendDirection >= 0 ? 1 : -1;
  const rootToTableLocal = cueReference
    .clone()
    .sub(human.root.position)
    .applyAxisAngle(Y_AXIS, -human.yaw);
  // Local -Z is the direction the rig is facing. Keeping this sign negative
  // makes chest/head/shoulders fold toward the table instead of away from it.
  if (Math.abs(rootToTableLocal.z) > 1e-5) return rootToTableLocal.z <= 0 ? -1 : 1;
  return -1;
}
...
export function updateHumanPose(human, dt, frameData) {
  ...
  if (activeState !== 'idle') ensureHumanFacingTable(human, frameData, cfg);
