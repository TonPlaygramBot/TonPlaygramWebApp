...
const TEXAS_DOMINO_CHARACTER_PROPORTION_SCALE = 2.55;
const TEXAS_DOMINO_HUMAN_CHARACTER_SCALE_BOOST = 0.55;
const TEXAS_DOMINO_CHARACTER_CHAIR_COORDINATE_OUTWARD_RATIO = 0.08;
const TEXAS_CHARACTER_CARD_HAND_LIFT = 0.36 * MODEL_SCALE;
const TEXAS_CHARACTER_BET_REACH_MS = 700;
...
async function attachTexasDominoCharacterToSeat(seatGroup, seatIndex, renderer) {
  ...
  const mountInstance = (sourceInstance, { replace = false } = {}) => {
    ...
    const visibleSeatLift = Number.isFinite(seatGroup.stoolHeight) ? seatGroup.stoolHeight : STOOL_HEIGHT;
    seatRoot.position.set(
      0,
      visibleSeatLift + (theme.seatOffsetY ?? -0.56) - 0.22 - scaleDelta * 0.08 - 1.0 + (isHumanSeat ? 0.5 : 0),
      (theme.seatOffsetZ ?? 0.52) - 0.03 + SEAT_DEPTH * TEXAS_DOMINO_CHARACTER_CHAIR_COORDINATE_OUTWARD_RATIO + (isHumanSeat ? -0.72 : 0)
    );
