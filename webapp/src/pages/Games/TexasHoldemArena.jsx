...
function syncSeatGroupsToLayout(seatGroups = [], seatLayout = []) {
  ...
  for (let idx = 0; idx < count; idx += 1) {
    ...
    const seatFacingMultiplier = seatGroup.isHuman ? 1 : -1;
    if (seatGroup.tableLayout) {
      seatGroup.tableLayout.right = seat.right.clone().multiplyScalar(seatFacingMultiplier);
      seatGroup.tableLayout.forward = seat.forward.clone().multiplyScalar(seatFacingMultiplier);
    }
    if (seatGroup.railLayout) {
      seatGroup.railLayout.right = seat.right.clone().multiplyScalar(seatFacingMultiplier);
      seatGroup.railLayout.forward = seat.forward.clone().multiplyScalar(seatFacingMultiplier);
    }
  }
}
...
function TexasHoldemArena({ search }) {
  ...
  useEffect(() => {
    ...
    state.players.forEach((player, idx) => {
      ...
      const wideVs7VisualDrop = resolvePlayerSeatLayerExtraDrop(effectivePlayerCount, three.tableThemeId);
      ...
      seat.cardMeshes.forEach((mesh, cardIdx) => {
        ...
        if (state.showdown && Number.isInteger(winnerOrderIndex)) {
          ...
          position.y = (seat.isHuman ? clothY : clothY + CARD_H * 0.48) - wideVs7VisualDrop;
        }
