...
const VS_7_PLAYER_CARD_SURFACE_Y_OFFSET = -CARD_H * 0.2;
...
function resolvePlayerCardSurfaceYOffset(playerCount, tableThemeId) {
  return isVs7TableMode(playerCount) && VS_7_WIDE_TABLE_THEME_IDS.has(tableThemeId)
    ? VS_7_PLAYER_CARD_SURFACE_Y_OFFSET
    : 0;
}
  ...
  useEffect(() => {
    ...
    if (three.arenaGroup && (shapeChanged || rotationChanged || three.tableThemeId !== tableTheme.id)) {
      ...
      void (async () => {
        ...
        three.playerSurfaceYOffset = resolvePlayerCardSurfaceYOffset(effectivePlayerCount, tableTheme.id);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const orientHumanCards = () => {
        ...
        communityAxes,
        playerSurfaceYOffset: resolvePlayerCardSurfaceYOffset(initialPlayerCount, initialTheme?.id)
      };
    ...
    state.players.forEach((player, idx) => {
      ...
      seat.cardMeshes.forEach((mesh, cardIdx) => {
        ...
        if (state.showdown && Number.isInteger(winnerOrderIndex)) {
          ...
          const clothY = (three.tableInfo?.surfaceY ?? TABLE_HEIGHT) + CARD_D * 0.4 + (three.playerSurfaceYOffset ?? 0);
