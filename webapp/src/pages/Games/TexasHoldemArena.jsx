...
const LABEL_SIZE = Object.freeze({
  width: 1.24 * MODEL_SCALE,
  height: 0.58 * MODEL_SCALE
});
...
function computePotAnchor(options = {}) {
  ...
  return new THREE.Vector3(
    center.x,
    surfaceY + CARD_SURFACE_OFFSET,
    center.z + POT_BELOW_COMMUNITY_OFFSET
  );
}
...
function createRailTextSprite(initialLines = [], options = {}) {
  ...
  const draw = (lines) => {
    ...
    content.forEach((line, idx) => {
      ctx.font =
        idx === 0
          ? '700 92px "Inter", system-ui, sans-serif'
          : '600 76px "Inter", system-ui, sans-serif';
...
function createRaiseControls({ arena, seat, chipFactory, tableInfo }) {
  ...
  const chipCenter = seat.chipRailAnchor
    ...
    : fallbackAnchor
        ...
        .addScaledVector(
          axis,
          CARD_RAIL_LATERAL_SHIFT + CHIP_RAIL_LATERAL_SHIFT
        );
...
function TexasHoldemArena({ search }) {
  ...
  useEffect(() => {
    ...
    const communityRowRotation = Math.atan2(
      humanSeat.right.z,
      humanSeat.right.x
    );
    ...
    const applyOverheadCamera = (options = {}) => {
      ...
      const focus = cameraTarget.clone();
      const seatedDistance = Math.max(2.4, camera.position.distanceTo(focus));
      const overheadDistance = seatedDistance * zoom;
      const targetPosition = new THREE.Vector3(
        focus.x,
        focus.y + overheadDistance,
        focus.z + 0.001
      );
      const lookMatrix = new THREE.Matrix4();
      ...
      if (animate) {
        smoothCameraTransition(targetPosition, targetQuaternion, finalize, 220);
      } else {
        finalize();
      }
    };
    ...
    (async () => {
      ...
      potLabel.position.copy(
        potAnchor
          .clone()
          .add(new THREE.Vector3(0, CARD_SURFACE_OFFSET * 0.2, 0))
      );
  ...
  useEffect(() => {
    ...
    if (potLabel?.userData?.update) {
      potLabel.userData.update([
        'Total pot',
        `${Math.round(state.pot ?? 0)} ${state.token}`
      ]);
    ...
    state.players.forEach((player, idx) => {
      ...
      seat.cardMeshes.forEach((mesh, cardIdx) => {
        ...
        if (player.folded && !state.showdown) {
          ...
          const lateral = seat.right
            .clone()
            .multiplyScalar((cardIdx - 0.5) * HOLE_SPACING);
        ...
        const position = baseAnchor
          ...
          .addScaledVector(
            forward,
            seat.isHuman ? HUMAN_CARD_FORWARD_OFFSET : CARD_FORWARD_OFFSET
          )
          .add(
            right
              .clone()
              .multiplyScalar(
                (cardIdx - 0.5) *
                  (seat.isHuman ? HOLE_SPACING : HUMAN_CARD_SPREAD)
              )
          );
        position.y =
          (seat.isHuman ? baseAnchor.y : seat.cardRailAnchor.y) + CARD_D * 0.6;
        mesh.position.copy(position);
        ...
        const lookTarget = seat.isHuman
          ? three.camera.position
              ...
              .add(
                right
                  .clone()
                  .multiplyScalar((cardIdx - 0.5) * HUMAN_CARD_LOOK_SPLAY)
              )
              .add(new THREE.Vector3(0, CARD_LOOK_LIFT, 0))
          : lookOrigin
              .clone()
              .add(
                new THREE.Vector3(0, seat.stoolHeight * 0.5 + CARD_LOOK_LIFT, 0)
              )
              .add(
                right.clone().multiplyScalar((cardIdx - 0.5) * CARD_LOOK_SPLAY)
              )
              .addScaledVector(forward, 0);
        const face =
          overheadView || seat.isHuman || state.showdown ? 'front' : 'back';
  ...
  useEffect(() => {
    ...
    if (Array.isArray(three.seatGroups)) {
      three.seatGroups.forEach((seat, index) => {
        if (!seat?.group) return;
        seat.group.visible = !overheadView || index === humanPlayerIndex;
      });
    }
    if (overheadView) {
  ...
  }, [overheadView, overheadZoom, humanPlayerIndex]);
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {actor?.isHuman && gameState.stage !== 'showdown' && (
        <div className="absolute bottom-[calc(env(safe-area-inset-bottom,0px)+0.75rem)] left-1/2 z-20 flex -translate-x-1/2 flex-col items-center gap-2">
          <div className="flex max-w-[94vw] flex-wrap items-center justify-center gap-2">
            {CHIP_VALUES.map((value) => {
              const disabled = !sliderInteractive || value > sliderMax;
              return (
                <button
                  key={`chip-btn-${value}`}
                  type="button"
                  ...
                  disabled={disabled}
                  className={`rounded-full border px-3 py-1 text-xs font-extrabold tracking-wide shadow-lg transition ${
                    disabled
                      ? 'border-white/20 bg-black/35 text-white/40 shadow-none'
