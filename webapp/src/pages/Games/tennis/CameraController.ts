...
export class CameraController {
  update(
    ...
    if (preServe) {
      cameraPosTarget.copy(playerTarget).add(tempOffset.set(playerTarget.x * 0.08 + lateral, 10.85 * gameConfig.cameraViewScale, 18.25 * gameConfig.cameraViewScale));
      tempLookTarget.set(playerTarget.x * 0.2 + lateral, 2.02 * gameConfig.cameraViewScale, -gameConfig.courtL * 0.34);
      cameraTarget.lerp(tempLookTarget, targetDamping);
    } else {
      tempFollowAnchor.copy(playerTarget);
      tempBaseTarget.set(playerTarget.x + lateral, 1.9 * gameConfig.cameraViewScale, playerTarget.z - 11.6 * gameConfig.cameraViewScale);
      ...
      if (incomingToPlayer) {
        ...
        tempBallBias.y = Math.max(1.85 * gameConfig.cameraViewScale, ballPos.y * 0.3 + 1.65 * gameConfig.cameraViewScale);
