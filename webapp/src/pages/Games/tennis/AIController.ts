...
export class AIController {
  ...
  chooseReturnTarget(opponentPos: THREE.Vector3, ballPos: THREE.Vector3, ballVel: THREE.Vector3) {
    ...
    return { target: new THREE.Vector3(x, gameConfig.ballR, z), power: clamp(0.58 + pressure * 0.32 + gameConfig.aiDifficulty.power * 0.16, gameConfig.shotPower.min, gameConfig.shotPower.max), technique };
  }
  ...
  chooseServeTarget(side: ServeSide, server: PlayerSide = "far") {
    ...
    return {
      ...
      power: clamp(0.72 + gameConfig.aiDifficulty.serveQuality * 0.18 + Math.random() * 0.05, gameConfig.servePower.min, gameConfig.servePower.max),
