...
export class AIController {
  ...
  chooseReturnTarget(opponentPos: THREE.Vector3, ballPos: THREE.Vector3, ballVel: THREE.Vector3) {
    ...
    return { target: new THREE.Vector3(x, gameConfig.ballR, z), power: clamp(0.58 + pressure * 0.34 + gameConfig.aiDifficulty.power * 0.17, 0.56, 0.96), technique };
  }
  ...
  chooseServeTarget(side: ServeSide, server: PlayerSide = "far") {
    ...
    return {
      ...
      power: clamp(0.74 + gameConfig.aiDifficulty.serveQuality * 0.2 + Math.random() * 0.05, 0.72, 0.97),
