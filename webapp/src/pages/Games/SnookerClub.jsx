...
// Dimensions tuned for a championship snooker footprint while keeping the arena readable on mobile.
const TABLE_SIZE_SHRINK = 1.08; // allow a larger silhouette so the 12 ft proportions remain obvious
const TABLE_REDUCTION = 1 * TABLE_SIZE_SHRINK; // no aggressive trim; keep the snooker field generous
const SIZE_REDUCTION = 0.7;
const GLOBAL_SIZE_FACTOR = 0.9 * SIZE_REDUCTION;
const TABLE_DISPLAY_SCALE = 0.94; // pull the entire table set closer without compressing the rails visually
const WORLD_SCALE = 0.9 * GLOBAL_SIZE_FACTOR * 0.7 * TABLE_DISPLAY_SCALE;
...
const TABLE_BASE_SCALE = 1.32;
const TABLE_SCALE = TABLE_BASE_SCALE * TABLE_REDUCTION; // push the snooker build to a larger footprint without altering proportions
const TABLE = {
  W: 66 * TABLE_SCALE,
  H: 132 * TABLE_SCALE,
...
const LIGHTING_STORAGE_KEY = 'snookerLightingPreset';
...
function Table3D(
  ...
  table.userData.componentPreset = tableSizeMeta?.componentPreset || 'snooker';
...
function SnookerClubGame({
  ...
  const [railMarkerColorId, setRailMarkerColorId] = useState(() => {
    if (typeof window !== 'undefined') {
      const stored = window.localStorage.getItem('snookerRailMarkerColor');
