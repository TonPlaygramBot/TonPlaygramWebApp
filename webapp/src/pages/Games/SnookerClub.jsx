...
import { SnookerClubPowerSlider } from '../../../../snooker-club-power-slider.js';
import '../../../../snooker-club-power-slider.css';
...
// Dimensions tuned for a full-length snooker cloth so the arena matches the Pool Royale room
// while giving the table extra breathing space and preserving the 2:1 aspect ratio.
const TABLE_SIZE_SHRINK = 1.05; // widen the cloth silhouette for a true snooker footprint
const TABLE_REDUCTION = 1.02 * TABLE_SIZE_SHRINK; // keep the arena readable while expanding the playfield over Pool Royale
const SIZE_REDUCTION = 0.7;
const GLOBAL_SIZE_FACTOR = 0.9 * SIZE_REDUCTION;
const TABLE_DISPLAY_SCALE = 0.92; // keep the enlarged table visible inside the same arena trims
const WORLD_SCALE = 0.92 * GLOBAL_SIZE_FACTOR * 0.7 * TABLE_DISPLAY_SCALE;
const TOUCH_UI_SCALE = SIZE_REDUCTION;
const POINTER_UI_SCALE = 1;
const CUE_STYLE_STORAGE_KEY = 'snookerClubCueStyleIndex';
const TABLE_FINISH_STORAGE_KEY = 'snookerClubTableFinish';
const CLOTH_COLOR_STORAGE_KEY = 'snookerClubClothColor';
const POCKET_LINER_STORAGE_KEY = 'snookerClubPocketLiner';
const ENABLE_CUE_GALLERY = false;
const ENABLE_TRIPOD_CAMERAS = false;
const TABLE_BASE_SCALE = 1.22;
const TABLE_SCALE = TABLE_BASE_SCALE * TABLE_REDUCTION; // enlarge the snooker build for the dedicated Snooker Club footprint
const TABLE = {
  W: 66 * TABLE_SCALE,
  H: 132 * TABLE_SCALE,
...
const BALL_SIZE_SCALE = 0.94248; // 5% larger than the last Snooker Club build (15.8% over the original baseline)
...
// Snooker Club feedback: increase standard shots by 30% and amplify the break by 50% to open racks faster.
...
// Snooker Club previously lifted the table surface dramatically; trim the legs so the playfield sits lower
const LEG_SCALE = 6.2;
...
// angle for cushion cuts guiding balls into corner pockets (Snooker Club spec now requires 35°)
...
// Snooker Club no longer exposes wood grain customization and should render the
...
const LIGHTING_STORAGE_KEY = 'snookerClubLightingPreset';
...
function SnookerClubGame({
  ...
  const rules = useMemo(() => new SnookerClubRules(variantKey), [variantKey]);
  ...
  const [railMarkerShapeId, setRailMarkerShapeId] = useState(() => {
    if (typeof window !== 'undefined') {
      const stored = window.localStorage.getItem('snookerClubRailMarkerShape');
  ...
  const [railMarkerColorId, setRailMarkerColorId] = useState(() => {
    if (typeof window !== 'undefined') {
      const stored = window.localStorage.getItem('snookerClubRailMarkerColor');
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('snookerClubRailMarkerShape', railMarkerShapeId);
  }, [railMarkerShapeId]);
  useEffect(() => {
    if (typeof window === 'undefined') return;
    window.localStorage.setItem('snookerClubRailMarkerColor', railMarkerColorId);
  }, [railMarkerColorId]);
  useEffect(() => {
    if (typeof window === 'undefined') return;
  ...
  useEffect(() => {
    document.title = 'Snooker Club 3D';
  }, []);
  useEffect(() => {
    setPlayer({
  ...
  useEffect(() => {
    ...
    const lobbyUrl = `/games/snookerclub/lobby?winner=${winnerParam}`;
    ...
    try {
      ...
      const createMatchTvEntry = () => {
        ...
        return {
          ...
          update(delta) {
            ...
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText('Snooker Club Match of the Day', width / 2, 60);
      ...
      const fire = () => {
        ...
        aiShoot.current = () => {
          ...
          try {
            ...
            console.error('Snooker Club AI shot failed:', err);
            stopAiThinking();
            setAiPlanning(null);
            aiPlanRef.current = null;
      ...
      function resolve() {
        ...
        try {
          ...
          console.error('Snooker Club shot resolution failed:', err);
        }
        if (isTraining) {
        ...
        try {
          ...
          console.error('Snooker Club post-resolution update failed:', err);
        } finally {
  ...
  useEffect(() => {
    ...
    const slider = new SnookerClubPowerSlider({
      mount,
...
export default function SnookerClub() {
  ...
  useTelegramBackButton(() => {
    confirmExit().then((confirmed) => {
      if (confirmed) {
        navigate('/games/snookerclub/lobby');
      }
    });
  });
  useEffect(() => {
