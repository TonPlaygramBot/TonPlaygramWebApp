  ...
  return (
    <div className="p-4 pb-32 space-y-4 text-text flex flex-col justify-end items-center relative w-full flex-grow">
      <SnakeBoard
        ...
        tokenType="normal"
      />
      <ul className="absolute top-2 left-2 bg-black/50 text-white text-xs p-2 rounded space-y-1">
        {players.map((p) => (
          <li key={p.id}>{p.name}</li>
        ))}
      </ul>
      {waiting && (
        <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/70 space-y-2">
