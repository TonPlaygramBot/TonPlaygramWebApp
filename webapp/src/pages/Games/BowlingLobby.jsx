...
export default function BowlingLobby() {
  ...
  const launchGame = ({ accountId = '', tgId = '', tableId = '' } = {}) => {
    ...
    params.set('players', String(mode === 'online' ? 2 : playerCount));
  ...
  return (
    <div className="relative min-h-screen bg-[#070b16] text-text">
      ...
      <div className="relative z-10 space-y-4 p-4 pb-8">
        ...
        {mode === 'local' ? (
          ...
          <div className="space-y-2 rounded-2xl border border-white/10 bg-white/5 p-4 shadow">
            ...
            <p className="text-center text-xs text-white/60">
              Start bet: {startBet.toLocaleString('en-US')} TPG • Table stake:{' '}
          ...
          <div className="grid grid-cols-5 gap-2">
            {PLAYER_OPTIONS.map((count) => (
              <button
                key={count}
                type="button"
                disabled={mode === 'online' && count !== 2}
                onClick={() => {
                  setPlayerCount(count);
                  if (flags.length > count) saveFlags(flags.slice(0, count));
                }}
                className={`rounded-2xl border px-3 py-3 text-center font-black transition disabled:opacity-25 ${
                  playerCount === count
        ...
        >
          {matching ? 'Finding Rival…' : 'Start Bowling'}
        </button>
        {(matchStatus || matchError) && (
          <p className={`text-center text-xs ${matchError ? 'text-rose-300' : 'text-cyan-200'}`}>
