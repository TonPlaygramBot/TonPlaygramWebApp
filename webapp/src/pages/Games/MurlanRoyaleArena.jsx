...
function getSharedKTX2Loader(renderer = null) {
  ...
  if (!ktx2SupportDetected && renderer) {
    try {
      sharedKTX2Loader.detectSupport(renderer);
      ktx2SupportDetected = true;
    } catch (error) {
      console.warn('MurlanRoyale: KTX2 detect support failed', error);
    }
  }
...
function createConfiguredGLTFLoader(renderer = null) {
  ...
  try {
    ...
    console.warn('MurlanRoyale: failed to attach KTX2 loader', error);
  }
  return loader;
}
...
async function loadPolyhavenModel(assetId, renderer = null) {
  ...
  const promise = (async () => {
    ...
    const loader = createConfiguredGLTFLoader(renderer);
    loader.manager = manager || loader.manager;
