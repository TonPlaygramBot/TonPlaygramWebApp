...
const HUMAN_CHARACTER_EXTRA_OUTWARD_OFFSET = 0.79; // bring seated humans closer to the table on portrait mobile framing.
const HUMAN_CHARACTER_EXTRA_LOWER_OFFSET = 0.13; // seat humans lower so hips/legs rest properly on the chair cushion.
const TOP_SEAT_AVATAR_UP_LIFT = 5.65;
const NON_HUMAN_SEAT_AVATAR_UP_LIFT = 1.34;
...
function createCharacterRig(instance, seatRoot, seatConfig, characterTheme, player, playerIndex, cardTheme, cardTextureSize = null) {
  ...
  applyRotationOffset(leftUpperArm, THREE.MathUtils.degToRad(-38), THREE.MathUtils.degToRad(6), THREE.MathUtils.degToRad(-2));
  applyRotationOffset(leftForeArm, THREE.MathUtils.degToRad(22), THREE.MathUtils.degToRad(3), THREE.MathUtils.degToRad(0));
  applyRotationOffset(leftHand, THREE.MathUtils.degToRad(2), THREE.MathUtils.degToRad(2), 0);
  applyRotationOffset(rightUpperArm, THREE.MathUtils.degToRad(-41), THREE.MathUtils.degToRad(-8), THREE.MathUtils.degToRad(2));
  applyRotationOffset(rightForeArm, THREE.MathUtils.degToRad(26), THREE.MathUtils.degToRad(-3), THREE.MathUtils.degToRad(0));
...
const TOP_AI_HAND_CARD_SPACING_MULTIPLIER = 0.74;
const TOP_AI_HAND_CARD_MAX_SPREAD_MULTIPLIER = 0.68;
const SIDE_AI_HAND_CARD_SPACING_MULTIPLIER = 0.74;
const SIDE_AI_HAND_CARD_MAX_SPREAD_MULTIPLIER = 0.64;
const AI_TOP_SIDE_HAND_UP_SHIFT_Y = 0.095 * MODEL_SCALE;
const AI_TOP_SIDE_HAND_OUTWARD_PUSH = 0.105 * MODEL_SCALE;
...
const AI_HAND_TABLE_EDGE_MARGIN = CARD_H * 0.14;
    ...
    state.players.forEach((player, idx) => {
      ...
      cards.forEach((card, cardIdx) => {
        ...
        if (isHumanCard && selectionSet.has(card.id)) {
          ...
          target.addScaledVector(forward, -0.012 * MODEL_SCALE);
        }
