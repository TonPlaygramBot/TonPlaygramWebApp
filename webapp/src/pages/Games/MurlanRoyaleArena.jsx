...
function getSharedKTX2Loader(renderer = null) {
  if (sharedKTX2Loader) {
    if (renderer) sharedKTX2Loader.detectSupport(renderer);
    return sharedKTX2Loader;
  }
  try {
    const loader = new KTX2Loader();
    loader.setTranscoderPath(BASIS_TRANSCODER_PATH);
    let supportRenderer = renderer;
    if (!supportRenderer && typeof document !== 'undefined') {
      supportRenderer = new THREE.WebGLRenderer({ antialias: false, alpha: true });
    }
    if (supportRenderer) {
      loader.detectSupport(supportRenderer);
...
function createConfiguredGLTFLoader({ manager, renderer } = {}) {
  ...
  const ktx2Loader = getSharedKTX2Loader(renderer);
  if (ktx2Loader) {
