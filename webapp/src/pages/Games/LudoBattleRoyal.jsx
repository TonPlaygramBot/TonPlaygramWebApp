...
import {
  ...
  FIREARM_SCATTER_PROJECTILE_IDS,
  FIREARM_BALLISTICS_PROFILE_BY_ID,
  FIREARM_RECOIL_ROTATION_RAD,
  FIREARM_RECOIL_RECOVER_MS,
  FIREARM_FINAL_BULLET_SLOWMO_FACTOR,
  FIREARM_FINAL_BULLET_SPIN_RATE,
  FIREARM_NON_FINAL_BULLET_CINEMATIC_FACTOR,
  addFxSphere,
  createCaptureBulletTracerFx,
  createCaliberProjectileFx,
  createCaliberShellCasingFx,
  createBulletAerodynamicRingsFx,
  createCaptureMuzzleFx,
  createCaptureTargetReticleFx,
  getLudoFirearmTiming,
  getLudoFirearmBallistics,
  sampleLudoFirearmVolley
} from '../../utils/ludoFirearmPresentation';
...
import { playLudoDiceRollSfx, playLudoTokenStepSfx, ludoWeaponSoundUrl, playLudoPresentationSfx } from '../../utils/ludoSfx.js';
...
const FIREARM_ANIMATION_ORIENTATION_ROTATION =
  FIREARM_HAND_ATTACH_TUNING.smgBurstAttack?.rotation ||
  FIREARM_UNIFIED_DIRECTION_ROTATION;
...
function orientFirearmRackTowardBoardCenter(root, target) {
  if (!root?.isObject3D || !target?.isVector3) {
...
function playCaptureWeaponSourceSound(captureAnimationId, options = {}) {
  return playLudoPresentationSfx(ludoWeaponSoundUrl(captureAnimationId), options);
}
...
async function loadCaptureWeaponModel(captureAnimationId) {
  ...
  const promise = (async () => {
    ...
    for (let i = 0; i < candidateUrls.length; i += 1) {
      ...
      try {
        // Match the May 9 Gunify GLB/GLTF path: let GLTFLoader consume the
...
function alignFirearmRackFlatByBounds(root, baseRotation = [0, 0, 0]) {
  ...
  const zOffsets = [0, Math.PI * 0.5, -Math.PI * 0.5, Math.PI];
  ...
  xOffsets.forEach((xOffset) => {
    zOffsets.forEach((zOffset) => {
      root.rotation.set(baseRotation[0] + xOffset, baseRotation[1], baseRotation[2] + zOffset);
      root.updateMatrixWorld?.(true);
      ...
      if (height < bestHeight) {
        bestHeight = height;
        bestRotation = [root.rotation.x, root.rotation.y, root.rotation.z];
      }
    });
  });
...
async function attachFirearmToRightHand(attackerEntry, captureAnimationId) {
  ...
  // Use the SMG burst hand pose as the shared visual orientation baseline so
  // every firearm points/twists the same way during capture animations.
  weapon.rotation.set(...FIREARM_ANIMATION_ORIENTATION_ROTATION);
  ...
  const sourceLeftGrip = findObjectByNeedles(weapon, ['l_wrist', 'left_wrist', 'handguard', 'foregrip', 'support']);
  ...
  rightGrip.name = 'rightGripContact';
  if (sourceRightGrip) rightGrip.position.copy(weapon.worldToLocal(sourceRightGrip.getWorldPosition(new THREE.Vector3())));
  else if (attackerEntry.rightPalm) rightGrip.position.copy(weapon.worldToLocal(boneWorld(attackerEntry.rightPalm)));
  weapon.add(rightGrip);
  ...
  return {
    ...
    // Preserve the roll authored for each grip profile.
...
async function applyCaptureWeaponDisplay(entry, captureAnimationId) {
  ...
  entry.weaponHolder.clear();
  const displayTuning = MAY_9_TABLE_FIREARM_DISPLAY_TUNING_BY_ID[captureAnimationId] ||
    (LARGE_RACK_FIREARM_IDS.has(captureAnimationId) ? FIREARM_RACK_DISPLAY_TUNING.large : FIREARM_RACK_DISPLAY_TUNING.default);
  const length = CAPTURE_PARK_BOX_TARGET_SIZE * displayTuning.targetSizeMultiplier *
    (FIREARM_RACK_SIZE_MULTIPLIER_BY_ID[captureAnimationId] ?? 1) * CAPTURE_PARKED_WEAPON_VISUAL_SCALE;
  const clone = prepareLudoParkedWeapon(weaponModel.clone(true), captureAnimationId, length,
    !!FIREARM_RACK_MUZZLE_YAW_CORRECTION_BY_ID[captureAnimationId]);
...
async function createCaptureMissileTruckFx() {
  const root = new THREE.Group();
  root.userData.lockCaptureTexture = true;
  root.add(createLudoBlenderModel('truck'));
...
async function createCaptureDroneFx() {
  const root = new THREE.Group();
  root.userData.lockCaptureTexture = true;
  root.add(createLudoBlenderModel('drone'));
  const propeller = createLudoBlenderModel('drone-propeller');
  propeller.position.set(-1.57, 0, 0);
  root.add(propeller);
  fitObjectToTargetSize(root, 3.85 * CAPTURE_DRONE_SIZE_MULTIPLIER);
  root.visible = false;
...
const SEATED_FACE_CAMERA_GAMEPLAY_FORWARD = 0.10;
...
const SEATED_FACE_CAMERA_GAMEPLAY_UP = 0.075;
const SEATED_FACE_CAMERA_GAMEPLAY_LOOK_DOWN = 0;
...
const CAMERA_FREE_LOOK_POLAR_DELTA = THREE.MathUtils.degToRad(10);
const CAMERA_ZOOM_MIN_FACTOR = 1;
const CAMERA_ZOOM_MAX_FACTOR = 1;
const LUDO_CAMERA_PHI_MIN = 0.92;
const LUDO_CAMERA_PHI_MAX = 1.14;
...
const CAMERA_LOOK_PITCH_LIMIT = THREE.MathUtils.degToRad(12);
...
const CAMERA_2D_TOPDOWN_MIN_POLAR = THREE.MathUtils.degToRad(2);
const CAMERA_2D_TOPDOWN_MAX_POLAR = THREE.MathUtils.degToRad(18);
const CAMERA_2D_DISTANCE_FACTOR = 2.56;
const CAMERA_2D_MIN_DISTANCE_FACTOR = 1.2;
...
const LUDO_CAPTURE_FIREARM_SHELL_SOUND_URL = '/assets/ludo/audio/shell.wav';
...
function spinDice(
  ...
  return new Promise((resolve) => {
    ...
    if (dice.userData?.tabletopDiceLane) wobble.multiplyScalar(0.2);
    // Online rolls are generated by the server. Supplying that value makes
    // every phone animate the same throw instead of choosing a local face.
    const targetValue = Number.isInteger(value) && value >= 1 && value <= 6
      ? value
...
function Ludo3D({ avatar, username, aiFlagOverrides, playerCount, aiCount, onlineContext = null, onlinePlayers = [] }) {
  ...
  const applyCameraViewMode = useCallback((nextIs2d) => {
    ...
    if (nextIs2d) {
      ...
      const min2dDistance = CAMERA_BASE_RADIUS * CAMERA_2D_MIN_DISTANCE_FACTOR;
      const max2dDistance = CAMERA_BASE_RADIUS * CAMERA_2D_MAX_DISTANCE_FACTOR;
      const topDownDistance = clamp(CAMERA_BASE_RADIUS * CAMERA_2D_DISTANCE_FACTOR, min2dDistance, max2dDistance);
      const topDownOffset = new THREE.Vector3(0, topDownDistance, 0.0001);
      const spherical = new THREE.Spherical().setFromVector3(topDownOffset);
      spherical.radius = topDownDistance;
      spherical.phi = clamp(spherical.phi, CAMERA_2D_TOPDOWN_MIN_POLAR, CAMERA_2D_TOPDOWN_MAX_POLAR);
      ...
      controls.minPolarAngle = CAMERA_2D_TOPDOWN_MIN_POLAR;
      controls.maxPolarAngle = CAMERA_2D_TOPDOWN_MAX_POLAR;
  ...
  const updateParkedCaptureVehicleVisibility = useCallback((humanCaptureAnimationIndex = null) => {
    ...
    const refreshWeaponRackPose = (entry, captureAnimationId) => {
      ...
      if (FIREARM_CAPTURE_ANIMATION_IDS.has(captureAnimationId) && arena.boardGroup && entry.weaponHolder.children.length) {
        const frame = ludoTableFrame(arena.boardGroup, anchor, RAW_BOARD_SIZE / 2);
        const dice = diceRef.current;
        const dicePositions = (dice?.userData?.railPositions ?? []).map(point => dice.parent.localToWorld(point.clone()));
        const obstacles = [...(stateRef.current?.tokens ?? []).flat(),
          ...Array.from(parkedCaptureVehiclesRef.current.values())
            .filter(other => other !== entry && other.weaponRack?.visible && other.weaponHolder?.children.length)
            .map(other => other.weaponHolder)];
        const placed = parkLudoWeapon({ holder: entry.weaponHolder, frame, table: arena.tableInfo, obstacles, dicePositions });
        entry.weaponHolder.visible = placed;
        entry.weaponRackHit.visible = placed;
        if (placed) {
          const grip = entry.weaponHolder.userData.ludoParkedGrip;
          entry.weaponRackHit.position.copy(entry.weaponRack.worldToLocal(grip?.clone() ?? entry.weaponHolder.getWorldPosition(new THREE.Vector3())));
        }
        return;
      }
      // Air-attack controls retain their existing parking slot.
      const position = resolveCaptureParkingAnchors(playerIndex, 'firearmRack');
      if (position) entry.weaponRack.position.copy(position);
  ...
  }, [aiLoadoutByPlayer, getKingTokenPositionForPlayer, resolveCaptureParkingAnchors]);
  ...
  const moveDiceToRail = (player, immediate = false) => {
    ...
    // Rest on the table until pickup; repeated rolls use this same right-hand lane.
    if (immediate) {
      stopDiceTransition();
      dice.position.copy(target);
  ...
  const configureDiceAnchors = useCallback(
    ({ dice, boardGroup, chairs, tableInfo } = {}) => {
      ...
      const rightHandDice = rails.map((fallback, i) => {
        const anchor = chairList?.[i]?.group;
        if (!anchor || !table) return fallback;
        const world = ludoRightDicePosition(ludoTableFrame(group, anchor, RAW_BOARD_SIZE / 2), table, DICE_SIZE * scaleWorld.x);
        return world ? group.worldToLocal(world) : fallback;
      });
      diceObj.userData.railPositions = rightHandDice;
      diceObj.userData.rollTargets = rightHandDice.map(point => point.clone());
      diceObj.userData.tabletopDiceLane = true;
  ...
  const setupScene = async () => {
    ...
    if (camera && controls) {
      ...
      cameraSeatLockPositionRef.current = initialBottomCameraViewRef.current.position.clone();
      cameraLookStateRef.current.lockedPosition = initialBottomCameraViewRef.current.position.clone();
    }
    ...
    const step = () => {
      ...
      if (
        ...
        if (liftedTarget) {
          ...
          if (cameraTurnStateRef.current.followOffset?.isVector3) {
            ...
            camera.position.lerp(followCameraTarget, 1 - Math.exp(-7.7 * delta));
          }
      ...
      if (!isCamera2d && camera && controls && dynamicFirearmCameraRef.current && firearmCinematicCameraPoseRef.current) {
        ...
        const lerpFactor = 1 - Math.pow(1 - clamp(pose.lerp ?? 0.28, 0.02, 1), delta * 60);
        if (pose.position?.isVector3) camera.position.lerp(pose.position, lerpFactor);
      ...
      if (!isCamera2d && camera && LUDO_CAMERA_SEAT_LOCK_ENABLED && !dynamicFirearmCameraRef.current) {
        ...
        if (hardLockedPosition) {
          // Keep the portrait start-seat camera position steady, but let the
          // current turn view decide the target so top/bottom turns can keep
          // the exact start-game direction and side dice results do not tilt upward.
      ...
      // Keep OrbitControls from applying any orbit/pan deltas while custom look is active.
      // Hide only the local head in eye view; hands/body and all opponents stay visible.
      const selfActor = seatedHumanActorsRef.current?.find((entry) => entry.playerIndex === 0);
      if (selfActor?.rig?.head && selfActor.headScale) {
        selfActor.rig.head.scale.copy(selfActor.headScale).multiplyScalar(!isCamera2d && !dynamicFirearmCameraRef.current ? .001 : 1);
      }
  ...
  const playFirearmShellSound = () => {
    ...
    playLudoPresentationSfx('/assets/ludo/audio/shell.wav', { volume: .45 * getGameVolume(), muted: !settingsRef.current.soundEnabled });
  };
    ...
    new Promise((resolve) => {
      void (async () => {
        ...
        if (isFirearmAttack) {
          ...
          const firearmOwner = {};
          const rightPalmStart = attackerEntry?.rightPalm ? boneWorld(attackerEntry.rightPalm) : null;
          const leftPalmStart = attackerEntry?.leftPalm ? boneWorld(attackerEntry.leftPalm) : null;
          const aimMode = FIREARM_TWO_HANDED_IDS.has(resolvedCaptureAnimationId) ? 'firearmAimRifle' : 'firearmAimPistol';
          if (attackerEntry?.rig) {
            applySeatedHumanPose(attackerEntry.rig, aimMode, 1, .85, {}, { idleBreathAmp: 0 }, attackerEntry.chairSupportsArmrest !== false);
            attackerEntry.actor.updateMatrixWorld(true);
          }
          const handWeaponAttachment = await attachFirearmToRightHand(attackerEntry, resolvedCaptureAnimationId);
          if (handWeaponAttachment) {
            scene.attach(handWeaponAttachment.weapon);
            attackerEntry.propMotion = firearmOwner;
          }
          const firearmTiming = getLudoFirearmTiming(resolvedCaptureAnimationId);
          const { pickupLeadMs, reloadLeadMs, aimLeadMs } = firearmTiming;
          const rackWorld = new THREE.Vector3();
          parkedEntry?.weaponHolder?.getWorldPosition?.(rackWorld);
          if (parkedEntry?.weaponHolder?.userData?.ludoParkedGrip) rackWorld.copy(parkedEntry.weaponHolder.userData.ludoParkedGrip);
          // Reach the actual trigger grip before replacing the parked model with the carried one.
          const pickupContactMs = pickupLeadMs * 0.65;
          const drawEndMs = pickupLeadMs + reloadLeadMs;
          if (parkedEntry?.weaponHolder && handWeaponAttachment) handWeaponAttachment.weapon.visible = false;
          ...
          const { shots, cadenceMs, preFireLeadMs, durationMs } = firearmTiming;
          const mergedBallistics = getLudoFirearmBallistics(resolvedCaptureAnimationId);
          ...
          const shells = Array.from({ length: shots }, () => createCaliberShellCasingFx(mergedBallistics));
          const { pelletsPerShot } = firearmTiming;
          ...
          if (handWeaponAttachment?.weapon?.parent?.isObject3D && rackWorld.isVector3) {
            ...
            const gripOffset = handWeaponAttachment.rightGrip.getWorldPosition(new THREE.Vector3())
              .sub(handWeaponAttachment.weapon.getWorldPosition(new THREE.Vector3()));
            rackLocalStart.sub(gripOffset);
            handWeaponAttachment.weapon.parent.worldToLocal(rackLocalStart);
          }
          const tickFirearm = () => {
            if (attackerEntry?.propMotion === firearmOwner) {
              applySeatedHumanPose(attackerEntry.rig, aimMode, 1, .85, {}, { idleBreathAmp: 0 }, attackerEntry.chairSupportsArmrest !== false);
              attackerEntry.actor.updateMatrixWorld(true);
            }
            const elapsed = performance.now() - volleyStart;
            const { elapsedShooting, shotIndex: shotIdx, firing: weaponAnimationActive } = sampleLudoFirearmVolley(elapsed, firearmTiming);
            ...
            if (handWeaponAttachment?.weapon?.isObject3D && handWeaponAttachment.weapon.parent?.isObject3D) {
              const drawPhase = clamp((elapsed - pickupContactMs) / Math.max(1, drawEndMs - pickupContactMs), 0, 1);
              if (parkedEntry?.weaponHolder) {
                parkedEntry.weaponHolder.visible = elapsed < pickupContactMs;
                handWeaponAttachment.weapon.visible = elapsed >= pickupContactMs;
              }
              const shoulderPhase = clamp((elapsed - pickupLeadMs) / Math.max(1, reloadLeadMs + aimLeadMs), 0, 1);
              if (elapsed < drawEndMs) {
            ...
            if (handWeaponAttachment?.rightGrip && attackerEntry?.rightPalm) {
              const reach = smoother01(clamp(elapsed / pickupContactMs, 0, 1));
              const forward = muzzleTarget.clone().sub(muzzleOrigin).normalize();
              const up = new THREE.Vector3(0, 1, 0);
              const rightTarget = elapsed < pickupContactMs ? rackWorld : handWeaponAttachment.rightGrip.getWorldPosition(new THREE.Vector3());
              solveArm(attackerEntry.rig, 'right', attackerEntry.rightPalm,
                rightPalmStart.clone().lerp(rightTarget, reach),
                palmOrientation(attackerEntry.rig, 'right', forward, up), true);
              if (handWeaponAttachment.twoHanded && attackerEntry.leftPalm) {
                const support = handWeaponAttachment.offhandTarget.getWorldPosition(new THREE.Vector3());
                solveArm(attackerEntry.rig, 'left', attackerEntry.leftPalm,
                  leftPalmStart.clone().lerp(support, reach),
                  palmOrientation(attackerEntry.rig, 'left', forward.clone().negate(), up));
              }
            }
            ...
            if (elapsed >= preFireLeadMs && elapsedShooting >= 0 && elapsedShooting < shots * cadenceMs) {
              if (shotIdx !== lastAudibleShot) {
                lastAudibleShot = shotIdx;
            ...
            bullets.forEach((bulletMesh, idx) => {
              ...
              const travelMs = isFinalProjectile
                ? clamp(760 * (.26 / mergedBallistics.bulletSpeed), 680, 1080)
                : clamp(115 * (.26 / mergedBallistics.bulletSpeed), 85, 220);
              const rawShotProgress = clamp(life / travelMs, 0, 1);
              const shotProgress = isFinalProjectile ? easeInOutSine01(rawShotProgress) : rawShotProgress;
              if (!bulletMesh.userData.launchOrigin) {
                bulletMesh.userData.launchOrigin = muzzleOrigin.clone();
                bulletMesh.userData.launchTarget = muzzleTarget.clone();
              }
              const start = bulletMesh.userData.launchOrigin.clone();
              ...
              const pelletOffset = FIREARM_SCATTER_PROJECTILE_IDS.has(resolvedCaptureAnimationId)
                ...
                : new THREE.Vector3();
              const end = bulletMesh.userData.launchTarget.clone().add(pelletOffset);
              const bulletPos = start.lerp(end, shotProgress);
              const projectileDir = end.clone().sub(bulletMesh.userData.launchOrigin);
            ...
            shells.forEach((shell, idx) => {
              ...
              const shellLife = elapsedShooting - idx * cadenceMs;
              const retainsCase = /Revolver|smithSidearm/.test(resolvedCaptureAnimationId);
              const explosive = FIREARM_BALLISTICS_PROFILE_BY_ID[resolvedCaptureAnimationId] === 'explosive';
              if (shellLife < 0 || retainsCase || explosive) { shell.visible = false; return; }
              if (!shellState.origin) {
                const hand = attackerEntry?.rig?.rightHand;
                shellState.origin = hand?.getWorldPosition(new THREE.Vector3()) ?? muzzleOrigin.clone();
                const aim = muzzleTarget.clone().sub(muzzleOrigin).normalize();
                const side = new THREE.Vector3().crossVectors(aim, cameraWorldUp).normalize();
                shellState.velocity = side.multiplyScalar(.20 + (idx % 3) * .025).add(new THREE.Vector3(0, .30, 0));
              }
              shell.visible = true;
              if (shellState.landed) return;
              const seconds = shellLife / 1000;
              shell.position.copy(shellState.origin).addScaledVector(shellState.velocity, seconds);
              shell.position.y -= .9 * seconds * seconds;
              const floor = tableSurfaceY + HDRI_GROUND_ALIGNMENT_OFFSET + mergedBallistics.shellRadius;
              if (shell.position.y <= floor) {
                shell.position.y = floor;
                shellState.landed = true;
                shellState.settledAt = elapsed;
                shell.rotation.set(Math.PI / 2, 0, idx * 2.399);
                playFirearmShellSound();
              } else shell.rotation.set(seconds * 12, seconds * 8, seconds * 10);
            });
            const impactPhase = clamp((elapsedShooting - shots * cadenceMs * 0.72) / Math.max(140, shots * cadenceMs * 0.28), 0, 1);
            if (!shatterDone && (finalImpactDone || elapsed >= durationMs - 60)) {
              shatterDone = true;
              ...
              setFirearmCinematicPose(cinematicPosition, cinematicTarget, 0.62);
            }
            ...
            [...bullets, ...shells].forEach((object) => object?.userData?.dispose?.());
            handWeaponAttachment?.release?.();
            if (attackerEntry?.propMotion === firearmOwner) attackerEntry.propMotion = null;
  ...
  const resolveTurnCameraState = useCallback((player, offset = CAMERA_TARGET_LIFT) => {
    ...
    const isTopOrBottomSeat = seatIndex === 0 || seatIndex === 2 || Math.abs(direction.z) >= Math.abs(direction.x);
  ...
  const setCameraViewForTurn = useCallback((player, duration = CAMERA_TURN_VIEW_DURATION_MS, { force = false } = {}) => {
    ...
    if (player === 0) {
      ...
      if (facePose?.position?.isVector3 && facePose?.target?.isVector3) {
        ...
        cameraLookStateRef.current.lockedPosition = facePose.position.clone();
      ...
      if (initialBottomView?.position?.isVector3 && initialBottomView?.target?.isVector3) {
        ...
        cameraSeatLockPositionRef.current = alignedPosition.clone();
        cameraLookStateRef.current.lockedPosition = alignedPosition.clone();
        animateCameraPose(
  ...
  const setCameraFocus = useCallback(
    (focus = {}) => {
      ...
      const stableTurnView = resolveTurnCameraState(state.turn, offset);
      const isTopOrBottomTurn = state.turn === 0 || state.turn === 2;
      const diceWorldPosition = diceRef.current?.isObject3D
        ? diceRef.current.getWorldPosition(new THREE.Vector3())
        : null;
      const isDiceFocus =
        object === diceRef.current ||
        (target?.isVector3 && diceWorldPosition?.isVector3 && target.distanceTo(diceWorldPosition) < DICE_SIZE * 3);
      const useStableSeatDirection =
        stableTurnView?.target?.isVector3 &&
        (isTopOrBottomTurn || ((state.turn === 1 || state.turn === 3) && isDiceFocus));
      if (useStableSeatDirection) {
        nextTarget = stableTurnView.target.clone();
      }
      ...
      const nextFocusState = useStableSeatDirection
        ? {
          target: nextTarget.clone(),
          position: stableTurnView.position?.isVector3
            ? stableTurnView.position.clone()
            : cameraRef.current?.position?.clone?.()
        }
        : resolveFocusCameraState(nextTarget, offset);
      if (nextFocusState) {
  ...
  const rollDice = async () => {
    ...
    if (!dice.userData.tabletopDiceLane) {
  ...
  applyOnlineStateRef.current = (payload = {}) => {
    ...
    const previousMoveProgress = movingPlayer >= 0 && Number.isInteger(movingToken)
      ? state.progress?.[movingPlayer]?.[movingToken]
      : null;
    ...
    state.onlineRevision = remoteRevision;
    ...
    if (
      action.type === 'move' &&
      movingPlayer >= 0 && movingPlayer < count &&
      Number.isInteger(movingToken) && movingToken >= 0 && movingToken < 4 &&
      Number.isFinite(previousMoveProgress) &&
      Number.isFinite(Number(action.to)) &&
      previousMoveProgress !== Number(action.to)
    ) {
      const authoritativeProgress = visualProgress.map((tokens) => [...tokens]);
      state.progress[movingPlayer][movingToken] = previousMoveProgress;
      ...
      scheduleMove(movingPlayer, movingToken, Number(action.to), () => {
        ...
        state.progress.forEach((tokens, player) => tokens.forEach((progress, token) => {
          const node = state.tokens?.[player]?.[token];
          if (node) node.position.copy(getWorldForProgress(player, progress, token));
        }));
        updateTokenStacks();
