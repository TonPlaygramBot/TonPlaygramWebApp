...
const SNAKE_GUNIFY_CAPTURE_WEAPON_ID_BY_LUDO_ID = Object.freeze({
...
const snakeMatchedGunifyCaptureConfig = (ludoId, fallbackModelName, fallbackScale) => {
  const snakeId = SNAKE_GUNIFY_CAPTURE_WEAPON_ID_BY_LUDO_ID[ludoId];
  const sourceOption = SNAKE_CAPTURE_WEAPON_OPTION_BY_ID[snakeId] || {};
  return {
    label: sourceOption.label || `Gunify ${fallbackModelName}`,
    urls: Array.isArray(sourceOption.urls) && sourceOption.urls.length
      ? [...sourceOption.urls]
      : gunifyModelUrls(fallbackModelName),
    modelName: sourceOption.modelName || fallbackModelName,
    source: sourceOption.source || 'Gunify',
