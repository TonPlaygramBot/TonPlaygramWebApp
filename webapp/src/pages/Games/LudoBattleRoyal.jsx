...
const PLAYER_VIEW_CAMERA_FORWARD_OFFSET_LANDSCAPE = 1.02;
...
const CAMERA_2D_TOPDOWN_POLAR = THREE.MathUtils.degToRad(2);
...
function Ludo3D({ avatar, username, aiFlagOverrides, playerCount, aiCount }) {
  ...
  const applyCameraViewMode = useCallback((nextIs2d) => {
    ...
    const topDownPolar = CAMERA_2D_TOPDOWN_POLAR;
    if (nextIs2d) {
      ...
      const topDownOffset = new THREE.Vector3().setFromSpherical(new THREE.Spherical(topDownDistance, topDownPolar, 0));
      camera.position.copy(boardLookTarget).add(topDownOffset);
