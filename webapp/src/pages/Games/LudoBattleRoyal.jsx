...
const FIREARM_CAPTURE_WEAPON_URLS = Object.freeze({
  glockSidearmAttack: [
...
function normalizeCaptureWeaponModel(model, targetSize = 0.065) {
...
async function loadCaptureWeaponModel(captureAnimationId) {
  if (!captureAnimationId) return null;
  const cached = CAPTURE_WEAPON_MODEL_CACHE.get(captureAnimationId);
  if (cached) return cached.clone(true);
  const urls = FIREARM_CAPTURE_WEAPON_URLS[captureAnimationId];
  ...
  for (const url of urls) {
    try {
      ...
      if (!scene) continue;
      const weapon = normalizeCaptureWeaponModel(scene.clone(true), 0.064);
      ...
      CAPTURE_WEAPON_MODEL_CACHE.set(captureAnimationId, weapon);
      return weapon.clone(true);
    } catch (error) {
      console.warn('Capture weapon model load failed', captureAnimationId, url, error);
    }
  }
  return null;
}
...
async function createCaptureWeaponRackFx() {
  ...
  for (let idx = 0; idx < firearmOptions.length; idx += 1) {
    const option = firearmOptions[idx];
    // eslint-disable-next-line no-await-in-loop
    let weaponProxy = await loadCaptureWeaponModel(option.id);
    if (!weaponProxy) {
      weaponProxy =
...
function Ludo3D({ avatar, username, aiFlagOverrides, playerCount, aiCount }) {
  ...
  const updateParkedCaptureVehicleVisibility = useCallback((humanCaptureAnimationIndex = null) => {
    ...
    parkedCaptureVehiclesRef.current.forEach((entry, playerIndex) => {
      ...
      if (entry?.weaponRack) entry.weaponRack.visible = true;
    });
  }, [aiLoadoutByPlayer]);
  ...
  const setupScene = async () => {
    ...
    const attemptHumanSelection = (clientX, clientY) => {
      ...
      if (!hits.length) {
        ...
        const pointerPx = new THREE.Vector2(clientX, clientY);
        const projected = new THREE.Vector3();
        selection.options.forEach((opt) => {
          const token = state.tokens?.[0]?.[opt.token];
          if (!token?.isObject3D) return;
          token.getWorldPosition(projected);
          projected.project(camera);
          const sx = ((projected.x + 1) * 0.5) * rect.width + rect.left;
          const sy = ((-projected.y + 1) * 0.5) * rect.height + rect.top;
          const dist = pointerPx.distanceTo(new THREE.Vector2(sx, sy));
          if (dist < bestDist) {
            bestDist = dist;
            best = opt;
          }
        });
        if (!best || bestDist > 96) return false;
        option = best;
      }
    ...
    const attemptWeaponQuickSwap = (clientX, clientY) => {
      ...
      const unlockedOptions = CAPTURE_ANIMATION_OPTIONS.filter((option) =>
        isLudoOptionUnlocked('captureAnimation', option.id, ludoInventory)
      );
      setWeaponSwapPopup({
        x: clientX,
        y: clientY,
        options: unlockedOptions.length ? unlockedOptions : CAPTURE_ANIMATION_OPTIONS
      });
      return true;
    };
  ...
  const scheduleMove = (player, tokenIndex, targetProgress, onComplete, options = {}) => {
    ...
    const token = state.tokens[player][tokenIndex];
    const isEnteringMove = fromProgress < 0;
    const pickupHelper = new THREE.Vector3();
    const placeHelper = new THREE.Vector3();
    const hasPickupHelper = false;
    const hasPlaceHelper = isEnteringMove && sampleHumanActionHelperPosition(player, 'tokenPlace', placeHelper);
