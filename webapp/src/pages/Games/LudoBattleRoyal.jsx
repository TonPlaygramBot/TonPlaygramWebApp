...
const FIREARM_RACK_DISPLAY_TUNING = Object.freeze({
  default: Object.freeze({
    ...
    position: [0, 0, -0.006],
    rotation: [-Math.PI * 0.5, Math.PI * 0.98, 0]
  }),
  large: Object.freeze({
    targetSizeMultiplier: 2.02,
    position: [0.02, 0, -0.016],
    rotation: [-Math.PI * 0.5, Math.PI * 0.98, 0]
  })
});
...
async function createCaptureWeaponRackFx() {
  ...
  weaponHolder.position.set(0.054, 0.018, -0.028);
  ...
  actionButton.position.set(0.068, 0.016, -0.052);
  ...
  weaponRackHit.position.set(0.054, 0.018, -0.028);
  ...
  const updateParkedCaptureVehicleVisibility = useCallback((humanCaptureAnimationIndex = null) => {
    ...
    const refreshWeaponRackPose = (entry, captureAnimationId) => {
      ...
      const basePosition = isLargeFirearm
        ? seatPos
            .clone()
            .addScaledVector(rightSide, CAPTURE_PARK_SIDE_OFFSET * 0.84)
            .addScaledVector(inward, -0.012)
        : oppositeToPlayer
            .clone()
            .addScaledVector(rightSide, CAPTURE_PARK_SIDE_OFFSET * 0.62)
            .addScaledVector(inward, -0.012);
    ...
    parkedCaptureVehiclesRef.current.forEach((entry, playerIndex) => {
      ...
      if (entry?.weaponRack) {
        ...
        if (entry.actionButton?.isObject3D) {
          ...
          if (showActionButton) {
            entry.actionButton.position.set(0.068, 0.016, -0.052);
          } else {
            entry.actionButton.position.set(0.068, 0.016, -0.052);
          }
        }
  ...
  const playCaptureMissileSequence = ({
    ...
    new Promise((resolve) => {
      void (async () => {
        ...
        if (isDroneAttack) parkedDronePayload = null;
  ...
  return (
    ...
    >
      <div className="absolute inset-0 pointer-events-none">
        {weaponSwapPopup && (
          <div
            className="pointer-events-auto absolute z-30 w-[min(92vw,19rem)] max-h-[54vh] overflow-hidden rounded-2xl border border-white/20 bg-black/88 p-2 shadow-2xl backdrop-blur"
            style={{
              left: clamp(weaponSwapPopup.x - 112, 8, (typeof window !== 'undefined' ? window.innerWidth : 360) - 292),
              top: clamp(weaponSwapPopup.y - 10, 88, (typeof window !== 'undefined' ? window.innerHeight : 640) - 360)
            }}
          >
            ...
            <div className="grid max-h-[42vh] grid-cols-3 gap-1 overflow-y-auto pr-1 touch-pan-y overscroll-contain">
              {weaponSwapPopup.options.map((option) => {
                ...
                return (
                  <button
                    key={option.id}
                    type="button"
                    className={`overflow-hidden rounded-xl border p-1 text-[7px] font-semibold ${
                  ...
                  >
                    <div className="mx-auto mb-1 flex h-5 w-5 items-center justify-center rounded border border-white/20 bg-slate-900/70 text-[10px] leading-none text-slate-100">
                      {shape}
                    </div>
        ...
        <div className="absolute bottom-[8.7rem] left-3 z-20 flex flex-col items-start gap-2">
          <div className="pointer-events-auto">
            <button
              type="button"
              onClick={(event) => {
                const rect = event.currentTarget.getBoundingClientRect();
                openWeaponSwapPopup(rect.right + 2, rect.top - 8);
              }}
              aria-label="Open quick weapon swap"
              className="flex h-7 w-7 items-center justify-center rounded-full border border-white/20 bg-black/60 text-[11px] text-white shadow-[0_6px_18px_rgba(2,6,23,0.45)] transition hover:border-white/35 hover:bg-black/75"
            >
              ⇄
            </button>
          </div>
