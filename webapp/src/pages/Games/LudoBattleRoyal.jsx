...
const CAPTURE_WEAPON_MODEL_CONFIG = Object.freeze({
  ...
  marksmanDmrAttack: {
    label: 'Marksman DMR',
    urls: [
      'https://raw.githubusercontent.com/pmndrs/drei-assets/master/sniper.glb'
    ],
    scale: 0.228
  }
});
...
const FIREARM_ATTACK_PROFILE = Object.freeze({
  glockSidearmAttack: { shots: 11, intervalMs: 58, bulletScale: 0.52 },
  pistolSidearmAttack: { shots: 12, intervalMs: 55, bulletScale: 0.56 },
  assaultRifleAttack: { shots: 22, intervalMs: 34, bulletScale: 0.66 },
  uziSprayAttack: { shots: 26, intervalMs: 26, bulletScale: 0.5 },
  ak47VolleyAttack: { shots: 20, intervalMs: 32, bulletScale: 0.72 },
  grenadeBlastAttack: { shots: 6, intervalMs: 92, bulletScale: 0.9 },
  shotgunBlastAttack: { shots: 9, intervalMs: 76, bulletScale: 0.8 },
  sniperShotAttack: { shots: 3, intervalMs: 170, bulletScale: 1.05 },
  smgBurstAttack: { shots: 18, intervalMs: 30, bulletScale: 0.58 },
  compactCarbineAttack: { shots: 16, intervalMs: 34, bulletScale: 0.64 },
  marksmanDmrAttack: { shots: 8, intervalMs: 78, bulletScale: 0.84 }
});
...
async function loadCaptureWeaponModel(captureAnimationId) {
  ...
  const promise = (async () => {
    ...
    for (let i = 0; i < candidateUrls.length; i += 1) {
      try {
        ...
        if (i === candidateUrls.length - 1) {
          console.warn(
            'Capture weapon model load failed',
            captureAnimationId,
            candidateUrls[i],
            error
          );
    ...
    try {
      ...
      console.warn(
        'Capture weapon model setup failed',
        captureAnimationId,
        error
      );
      return null;
...
async function createCaptureWeaponRackFx() {
  ...
  const buttonBase = new THREE.Mesh(
    ...
    createCaptureVehicleMaterial('truck', {
      color: '#111827',
      roughness: 0.55,
      metalness: 0.2
    })
...
async function applyCaptureWeaponDisplay(entry, captureAnimationId) {
  ...
  if (
    ...
    entry.weaponHolder.children.length > 0
  )
    return;
  entry.weaponHolder.clear();
...
function createAiUniqueLoadout(activePlayerCount) {
  ...
  const humanPool = shuffle(
    Array.from({ length: HUMAN_CHARACTER_OPTIONS.length }, (_, idx) => idx)
  );
...
const CUSTOMIZATION_SECTIONS = [
  ...
  },
  {
    ...
    options: HUMAN_CHARACTER_OPTIONS
  }
];
...
const LUDO_CAPTURE_FIREARM_SHOT_SOUND_URL =
  '/assets/sounds/080998_bullet-hit-39870.mp3';
const LUDO_CAPTURE_FIREARM_SHELL_SOUND_URL =
  '/assets/sounds/metal-whistle-6121.mp3';
const LUDO_CAPTURE_FIREARM_GLASS_BREAK_SOUND_URL =
...
function resolveSeatedHumanActionPose(
  ...
  if (
    ...
    return {
      ...
      mode: 'holdDice',
      intensity: 1,
      handGrip: holdPlayer === 0 ? 0.14 : 0.92,
  ...
  if (
    ...
    const phase = normalizedPhase(
      nowMs,
      actorState.captureStartMs,
      actorState.captureEndMs - actorState.captureStartMs
    );
    const attackId = actorState.captureAnimationId;
    const isAirAttack =
      CAPTURE_AIR_ATTACK_ID_SET.has(attackId) || attackId === 'missileJavelin';
    if (isAirAttack) {
      return {
        ...basePose,
        mode: 'gripToken',
    ...
    return {
      ...
      intensity:
        phase < 0.45
          ? easeInOutSine01(phase / 0.45)
          : 0.7 + smoother01((phase - 0.45) / 0.55) * 0.3,
...
async function loadSeatedHumanTemplate(
  renderer = null,
  humanOption = HUMAN_CHARACTER_OPTIONS[0]
) {
  const optionId =
    humanOption?.id || HUMAN_CHARACTER_OPTIONS[0]?.id || 'rpm-current';
  if (!seatedHumanTemplatePromiseById.has(optionId)) {
    seatedHumanTemplatePromiseById.set(
      ...
      (async () => {
        ...
        loader.setCrossOrigin('anonymous');
        const fallbackUrls =
          Array.isArray(humanOption?.modelUrls) && humanOption.modelUrls.length
        ...
        if (!gltf)
          throw (
            lastError ||
            new Error(`Unable to load seated human model for ${optionId}`)
          );
        const root = gltf?.scene || gltf?.scenes?.[0];
        if (!root) throw new Error('Missing seated human scene');
        const preserveOriginalCharacterTextures = optionId !== 'rpm-current';
        ...
        root.traverse((obj) => {
          if (obj?.isMesh) {
            ...
            const useHair = /hair|beard|mustache|moustache|eyebrow/.test(
              meshName
            );
            const fallbackTex = useHair
              ? hairTex
              : useSkin
                ? skinTex
                : clothTex;
            const materials = Array.isArray(obj.material)
              ...
              : obj.material
                ...
                : [];
            materials.forEach((mat) => {
              if (!preserveOriginalCharacterTextures && !mat?.map)
                mat.map = fallbackTex;
...
function Ludo3D({ avatar, username, aiFlagOverrides, playerCount, aiCount }) {
  ...
  const firearmGlassBreakSoundRef = useRef(null);
    ...
    (humanCaptureAnimationIndex = null) => {
      ...
      parkedCaptureVehiclesRef.current.forEach((entry, playerIndex) => {
        ...
        const selectedCaptureAnimationId =
          ...
          'missileJavelin';
        if (entry?.jet)
          entry.jet.visible = selectedCaptureAnimationId === 'fighterJetAttack';
        if (entry?.helicopter)
        ...
        if (entry?.weaponRack) {
          const showFirearm = FIREARM_CAPTURE_ANIMATION_IDS.has(
  ...
  const rebuildParkedCaptureVehicles = useCallback(async () => {
    ...
    for (
      ...
      const weaponRackPark = resolveCaptureParkingAnchors(
        playerIndex,
        'firearmRack'
      );
      if (
      ...
      weaponRackFx.root.position.copy(
        weaponRackPark.clone().add(new THREE.Vector3(0.024, 0, -0.015))
      );
  ...
  useEffect(() => {
    ...
    firearmGlassBreakSoundRef.current = new Audio(
      LUDO_CAPTURE_FIREARM_GLASS_BREAK_SOUND_URL
    );
    ...
    const setupScene = async () => {
      ...
      try {
        ...
        for (
          ...
          const humanOption =
            HUMAN_CHARACTER_OPTIONS[humanIndex] ?? HUMAN_CHARACTER_OPTIONS[0];
          let humanTemplate = null;
          try {
            // eslint-disable-next-line no-await-in-loop
            humanTemplate = await loadSeatedHumanTemplate(
              renderer,
              humanOption
            );
          } catch (error) {
            console.warn(
              'AI/human seat model failed, falling back to default seated avatar',
              playerIndex,
              humanOption?.id,
              error
            );
      ...
      const attemptWeaponQuickSwap = (clientX, clientY) => {
        const state = stateRef.current;
        if (!state || state.turn !== 0 || state.animation || state.winner)
          return false;
      ...
      const step = () => {
        ...
        if (
          ...
          now > actorState.captureEndMs + 60
        ) {
            ...
            if (!hasContactTarget) {
              const captureAttackId =
                actorState?.capturePlayer === playerIndex
                  ? actorState?.captureAnimationId
                  : null;
              const parkedEntry = captureAttackId
                ? parkedCaptureVehiclesRef.current.get(playerIndex)
                : null;
              const isCaptureAirAttack =
                captureAttackId &&
                (CAPTURE_AIR_ATTACK_ID_SET.has(captureAttackId) ||
                  captureAttackId === 'missileJavelin');
              const captureTargetObject = isCaptureAirAttack
                ? parkedEntry?.actionButton
                : parkedEntry?.weaponHolder;
  ...
  const playFirearmShotSound = () => {
    if (!settingsRef.current.soundEnabled || !firearmShotSoundRef.current)
      return;
  ...
  const playFirearmShellSound = () => {
    if (!settingsRef.current.soundEnabled || !firearmShellSoundRef.current)
      return;
    firearmShellSoundRef.current.currentTime = 0;
    firearmShellSoundRef.current.play().catch(() => {});
  };

  const playFirearmGlassBreakSound = () => {
    if (!settingsRef.current.soundEnabled || !firearmGlassBreakSoundRef.current)
      return;
    firearmGlassBreakSoundRef.current.currentTime = 0;
    firearmGlassBreakSoundRef.current.play().catch(() => {});
  };
      ...
      void (async () => {
        ...
        try {
          ...
          const resolvedCaptureAnimationId =
            ...
            CAPTURE_ANIMATION_OPTIONS[0]?.id ??
            'missileJavelin';
          const isFirearmAttack = FIREARM_CAPTURE_ANIMATION_IDS.has(
            resolvedCaptureAnimationId
          );
          const firearmProfile =
            FIREARM_ATTACK_PROFILE[resolvedCaptureAnimationId] ??
            FIREARM_ATTACK_PROFILE.pistolSidearmAttack;
          seatedHumanActionRef.current = {
            ...seatedHumanActionRef.current,
          ...
          const primaryFx = isFirearmAttack
          ...
          const jetMissiles = isFirearmAttack
            ? []
            : resolvedCaptureAnimationId === 'fighterJetAttack' ||
                resolvedCaptureAnimationId === 'helicopterAttack'
              ? [
          ...
          if (!isFirearmAttack) playMissileLaunchSound();
          ...
          const tunedTravelTime = isFirearmAttack
            ? 80
            : travelTime * captureTuning.speed;
          ...
          const tick = () => {
            ...
            if (isFirearmAttack) {
              ...
              const hasHand = sampleSeatedContactEffectorPosition(
                attackerPlayer,
                handWorld
              );
              const muzzleOrigin = hasHand
                ? handWorld.clone().add(new THREE.Vector3(0, 0.015, 0.02))
                : dynamicFrom.clone();
              const liveImpact = liveTarget
                .clone()
                .add(new THREE.Vector3(0, 0.02, 0));
              const shouldFire =
                firedShots < firearmProfile.shots &&
                elapsed - lastFireAt >= firearmProfile.intervalMs;
              if (shouldFire) {
                firedShots += 1;
                lastFireAt = elapsed;
                const bullet = new THREE.Mesh(
                  new THREE.SphereGeometry(
                    0.005 * firearmProfile.bulletScale,
                    8,
                    6
                  ),
                ...
                const shell = new THREE.Mesh(
                  new THREE.CylinderGeometry(0.0025, 0.0025, 0.01, 8),
                  new THREE.MeshStandardMaterial({
                    color: '#d9b453',
                    roughness: 0.28,
                    metalness: 0.7
                ...
                shell.position
                  .copy(muzzleOrigin)
                  .add(new THREE.Vector3(-0.01, 0.008, -0.008));
                shell.userData = {
                  bornAt: elapsed,
              ...
              activeBullets.forEach((bullet, index) => {
                const age = elapsed - (bullet.userData?.bornAt ?? elapsed);
                const life = clamp(
                  age / Math.max(40, bullet.userData?.travelMs ?? 120),
                  0,
                  1
                );
                const fromPos = bullet.userData?.from ?? muzzleOrigin;
                const toPos = bullet.userData?.to ?? liveImpact;
                bullet.position.copy(fromPos).lerp(toPos, life);
                if (life >= 1) {
                  scene.remove(bullet);
              ...
              activeShells.forEach((shell, index) => {
                const age =
                  (elapsed - (shell.userData?.bornAt ?? elapsed)) / 1000;
                shell.position.addScaledVector(
                  shell.userData?.velocity ?? new THREE.Vector3(),
                  dt
                );
                shell.position.y +=
                  (shell.userData?.velocity?.y ?? 0) * dt - 0.22 * age * dt;
                shell.rotation.y += dt * 10;
                if (age > 0.65) {
                  scene.remove(shell);
                  activeShells.splice(index, 1);
                }
              });
              const shotsProgress = clamp(
                firedShots / firearmProfile.shots,
                0,
                1
              );
              if (
                ...
                explosion.root.visible = true;
                explosion.root.position.copy(muzzleOrigin);
                updateExplosionRig(0.05);
  ...
  const scheduleMove = (
    ...
    const hasPickupHelper =
      enteringMove &&
      sampleHumanActionHelperPosition(player, 'tokenPickup', pickupHelper);
    const hasPlaceHelper =
      enteringMove &&
      sampleHumanActionHelperPosition(player, 'tokenPlace', placeHelper);
    const pushPathNode = (
      collection,
      position,
      progress = null,
  ...
  const rollDice = async () => {
    ...
    await syncDiceToThrowHand(player, dice, { duration: 180 });
    ...
    if (player === 0) {
      ...
      if (enteringOptions.length) {
        beginHumanSelection(value, enteringOptions, {
      ...
      <div className="absolute inset-0 pointer-events-none">
        {weaponSwapPopup && (
          <div
            ...
            style={{
              left: clamp(
                weaponSwapPopup.x - 80,
                8,
                (typeof window !== 'undefined' ? window.innerWidth : 360) - 200
              ),
              top: clamp(
                weaponSwapPopup.y - 12,
                96,
                (typeof window !== 'undefined' ? window.innerHeight : 640) - 170
              )
            }}
          >
            <p className="px-1 pb-1 text-[10px] uppercase tracking-[0.28em] text-sky-200/80">
              Quick Weapon Swap
            </p>
            <div className="grid grid-cols-2 gap-1">
              {weaponSwapPopup.options.map((option) => {
                const optionIndex = CAPTURE_ANIMATION_OPTIONS.findIndex(
