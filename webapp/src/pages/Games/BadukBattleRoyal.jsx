...
export default function BadukBattleRoyal() {
  ...
  const slotRadius = Math.min(boardWidth / cols, boardHeight / rows) * 0.275;
  ...
  useEffect(() => {
    ...
    for (let r = 0; r < rows; r += 1) {
      for (let c = 0; c < cols; c += 1) {
        ...
        holePath.absellipse(x, y, slotRadius * 0.94, slotRadius * 0.94, 0, Math.PI * 2, true);
    ...
    const holeRimGeo = new THREE.TorusGeometry(slotRadius * 0.95, slotRadius * 0.028, 16, 42);
    ...
    const createChipStack = (tokenColor, side = 'front', xOffset = 0, count = 6) => {
      ...
      for (let i = 0; i < count; i += 1) {
        const chip = new THREE.Mesh(new THREE.CylinderGeometry(slotRadius * 1.04, slotRadius * 1.04, chipDepth, 36), chipMat);
        chip.position.y = i * chipDepth * 0.85;
        stack.add(chip);
      }
      const topChip = new THREE.Mesh(new THREE.CylinderGeometry(slotRadius * 1.02, slotRadius * 1.02, chipDepth * 0.5, 36), topMat);
