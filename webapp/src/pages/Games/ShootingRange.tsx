...
function materialFrom(object: THREE.Object3D | null, fallback = 0xc48a35) {
...
function splitMesh(mesh: THREE.Mesh | null, keep: 'front' | 'back', size: number) {
  ...
  group.add(new THREE.Mesh(geo, materialFrom(mesh, keep === 'front' ? 0xb66b35 : 0xc48a35)));
...
function makeFallbackBulletHead() {
  const mat = materialFrom(null, 0xb66b35);
  const group = new THREE.Group();
...
function makeBulletHead(source: THREE.Object3D | null) {
  if (!source) return makeFallbackBulletHead();
  const group = new THREE.Group();
...
function makeShell(source: THREE.Mesh | null) {
  const split = splitMesh(source, 'back', 0.22);
  if (split) return split;
  const group = new THREE.Group();
  const mat = materialFrom(source, 0xc9953d);
...
function extractServicePistolAmmo(root: THREE.Object3D): AmmoTemplates {
  ...
  if (explicitBullet && explicitShell) {
    return {
      bullet: makeBulletHead(explicitBullet),
      shell: makeShell(explicitShell),
  ...
  if (carrier) {
    const splitBullet = splitMesh(carrier, 'front', 0.135);
    return {
      bullet: makeBulletHead(splitBullet || carrier),
      shell: makeShell(carrier),
      source: 'service pistol GLB split cartridge'
    };
  }

  return {
    bullet: makeFallbackBulletHead(),
    shell: makeShell(null),
    source: 'fallback 9mm cartridge'
  };
}

function createFallbackAmmoTemplates(): AmmoTemplates {
  return {
    bullet: makeFallbackBulletHead(),
    shell: makeShell(null),
    source: 'fallback 9mm cartridge'
        ...
        void loadGLTF(entry.name, entry.urls)
          .then((gltf) => {
            ...
            if (entry.id === 'service-pistol') {
              ammoTemplatesRef.current = extractServicePistolAmmo(gltf.scene);
            }
