...
const servicePistolTexture = (res: '1k' | '2k' | '4k', name: string) =>
  `https://dl.polyhaven.org/file/ph-assets/Models/jpg/${res}/service_pistol/service_pistol_${name}_${res}.jpg`;

const SERVICE_PISTOL_TEXTURE_URLS = {
  diff: [servicePistolTexture('2k', 'diff'), servicePistolTexture('1k', 'diff'), servicePistolTexture('4k', 'diff')],
  normal: [servicePistolTexture('2k', 'nor_gl'), servicePistolTexture('1k', 'nor_gl'), servicePistolTexture('4k', 'nor_gl')],
  rough: [servicePistolTexture('2k', 'rough'), servicePistolTexture('1k', 'rough'), servicePistolTexture('4k', 'rough')],
  metal: [servicePistolTexture('2k', 'metal'), servicePistolTexture('1k', 'metal'), servicePistolTexture('4k', 'metal')],
  ao: [servicePistolTexture('2k', 'ao'), servicePistolTexture('1k', 'ao'), servicePistolTexture('4k', 'ao')]
};
...
async function loadServicePistolTextureMaps() {
  const loader = new THREE.TextureLoader();
  loader.setCrossOrigin('anonymous');
  const [diff, normal, rough, metal, ao] = await Promise.all([
    loadTextureAny(loader, SERVICE_PISTOL_TEXTURE_URLS.diff, THREE.SRGBColorSpace),
    loadTextureAny(loader, SERVICE_PISTOL_TEXTURE_URLS.normal),
    loadTextureAny(loader, SERVICE_PISTOL_TEXTURE_URLS.rough),
    loadTextureAny(loader, SERVICE_PISTOL_TEXTURE_URLS.metal),
    loadTextureAny(loader, SERVICE_PISTOL_TEXTURE_URLS.ao)
  ]);
  return { diff, normal, rough, metal, ao } as ServicePistolTextureMaps;
}

function applyServicePistolMaps(
  material: THREE.MeshStandardMaterial,
  maps: ServicePistolTextureMaps | null
) {
  if (!maps) return;
  if (maps.diff) material.map = maps.diff;
  if (maps.normal) material.normalMap = maps.normal;
  if (maps.rough) material.roughnessMap = maps.rough;
  if (maps.metal) material.metalnessMap = maps.metal;
...
function applyServicePistolMapsToObject(
  root: THREE.Object3D,
  maps: ServicePistolTextureMaps | null
) {
  if (!maps) return;
  root.traverse((child) => {
    ...
    const materials = Array.isArray((mesh as any).material)
      ...
      : (mesh as any).material
        ? [(mesh as any).material]
        : [];
    const next = materials.map((material: THREE.Material) => {
      const cloned = material.clone() as THREE.MeshStandardMaterial;
      applyServicePistolMaps(cloned, maps);
      return cloned;
    });
    if (next.length) (mesh as any).material = Array.isArray((mesh as any).material) ? next : next[0];
  });
}
...
function materialFrom(
  ...
  applyServicePistolMaps(mat, maps);
...
function makeBulletHead(
  ...
  applyServicePistolMapsToObject(group, maps);
  centerRoot(group);
  ...
  // Keep the extracted GLB projectile nose on local -Z so alignToDirection()
  // launches it tip-first instead of flipping it backwards.
  centerRoot(group);
...
function createBullet(
  ...
  if (nineMm) {
    ...
    const bulletGeometry = shotgun
      ...
      : new THREE.CapsuleGeometry(longGun ? 0.015 : 0.016, longGun ? 0.13 : 0.1, 4, 10);
...
function createShell(
  ...
  if (nineMm) {
    ...
    const shotgun = weapon.weaponClass === 'shotgun';
    const shellRadius = shotgun ? 0.04 : 0.026;
    const shellLength = shotgun ? 0.22 : 0.145;
    root = new THREE.Mesh(
      ...
      WEAPONS.forEach((entry, index) => {
        ...
        void loadGLTF(entry.name, entry.urls)
          .then(async (gltf) => {
            ...
            let servicePistolMaps: ServicePistolTextureMaps | null = null;
            if (entry.id === 'service-pistol') {
              servicePistolMaps = await loadServicePistolTextureMaps().catch((error) => {
                console.warn('Service pistol texture set failed, using model materials:', error);
                return null;
              });
              if (disposed) return;
              ammoTemplatesRef.current = extractServicePistolAmmo(gltf.scene, servicePistolMaps);
              applyServicePistolMapsToObject(gltf.scene, servicePistolMaps);
            }
