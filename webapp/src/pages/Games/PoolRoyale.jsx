...
const CHROME_CORNER_SIDE_EXPANSION_SCALE = 1.016; // let the corner chrome fascia stretch a touch further toward the side pockets
...
const CHROME_SIDE_NOTCH_DEPTH_SCALE = 1.013; // ease the middle chrome arch farther from centre while keeping the pocket origin fixed
...
const CHROME_SIDE_PLATE_POCKET_SPAN_SCALE = 1.02; // stretch the chrome arch slightly wider so the middle pockets sit farther from centre
...
const SIDE_POCKET_JAW_LATERAL_EXPANSION = 1.14; // give the widened middle pocket arc a touch more jaw coverage
const SIDE_POCKET_JAW_RADIUS_EXPANSION = 1.012; // keep the side jaw radius identical to the chrome/rail pocket arc
...
const SIDE_POCKET_EXTRA_SHIFT = BALL_R * 0.215; // push middle pockets farther outward so the chrome, rails, and jaws track the widened side span
...
function Table3D(
  ...
  const CUSHION_CORNER_CLEARANCE_REDUCTION = TABLE.THICK * 0.098; // stretch the green cushions further into the corner pocket throats per new spec
  const SIDE_CUSHION_POCKET_REACH_REDUCTION = TABLE.THICK * 0.004; // fine-tune side cushion span so all six rest cleanly between chrome cuts
  const SIDE_CUSHION_RAIL_REACH = TABLE.THICK * 0.034; // press the side cushions firmly into the rails without creating overlap
  const SIDE_CUSHION_CORNER_SHIFT = BALL_R * 0.37; // nudge the side cushions closer to the middle pockets
