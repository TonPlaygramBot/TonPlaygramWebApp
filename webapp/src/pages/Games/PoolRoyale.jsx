...
const powerRef = useRef(hud.power);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const ensureCueButtClearance = (group) => {
        const cushionTop =
          table?.userData?.cushionTopLocal ?? TABLE_RAIL_TOP_Y ?? TABLE.THICK;
        const minButtY = cushionTop + BALL_R * 0.12;
        if (TMP_VEC3_BUTT.y < minButtY) {
          const liftDelta = minButtY - TMP_VEC3_BUTT.y;
          group.position.y += liftDelta;
          TMP_VEC3_BUTT.y = minButtY;
        }
      };
      const applyCueStickTransform = (tipTarget) => {
        ...
        ensureCueButtClearance(cueStick);
      };
