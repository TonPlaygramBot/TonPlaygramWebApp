...
function Table3D(
  ...
  const CUSHION_RAIL_FLUSH = -TABLE.THICK * 0.012; // push cushions farther outward so they meet the wooden rails without gaps
  const CUSHION_SHORT_RAIL_CENTER_NUDGE = 0; // pull the short rail cushions tight so they meet the wood with no visible gap
  const CUSHION_LONG_RAIL_CENTER_NUDGE = 0; // remove the long-rail setback so cushions sit flush with the rails
  ...
  const SIDE_CUSHION_POCKET_REACH_REDUCTION = TABLE.THICK * 0.1; // trim middle cushion tips a bit more so they clear the pocket openings
