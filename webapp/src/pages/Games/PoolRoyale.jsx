...
const AI_MIN_SHOT_TIME_MS = 6500;
const AI_MAX_SHOT_TIME_MS = 9000;
const AI_MIN_AIM_PREVIEW_MS = 1200;
...
const AI_CAMERA_DROP_LEAD_MS = 620; // start lowering into cue view shortly before the AI pulls the trigger
const AI_CAMERA_SETTLE_MS = 520; // allow time for the cue view to settle before firing
const AI_CAMERA_DROP_BLEND = 0.1;
...
const powerRef = useRef(hud.power);
  ...
  useEffect(() => {
    const hudState = hudRef.current;
    if (!hudState || hudState.over || hudState.turn !== 0) return;
    if (shootingRef.current) return;
    const cue = cueRef.current;
    const cueLen = cueLengthRef.current;
    if (!cue || !cue.active || !Number.isFinite(cueLen) || cueLen <= 0) return;
    const aimDir = (aimDirRef.current ?? new THREE.Vector2(0, 1)).clone();
    if (aimDir.lengthSq() < 1e-6) aimDir.set(0, 1);
    const backInfo = calcTarget(
      cue,
      aimDir.clone().multiplyScalar(-1),
      Array.isArray(ballsRef.current) ? ballsRef.current : []
    );
    const rawMaxPull = Math.max(0, backInfo.tHit - cueLen - CUE_TIP_GAP);
    const maxPull = Number.isFinite(rawMaxPull) ? rawMaxPull : CUE_PULL_BASE;
    const desiredPull = Math.min(
      Math.max(powerRef.current ?? 0, 0) * CUE_PULL_BASE,
      maxPull
    );
