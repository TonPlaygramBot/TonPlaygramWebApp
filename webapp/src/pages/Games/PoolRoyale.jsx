...
const DEFAULT_HDRI_CONFIG =
  HDRI_OPTION_MAP[DEFAULT_POOL_HDRI_ID] ??
  POOL_ROYALE_HDRI_OPTIONS[0] ?? {
...
function normalizeHdriConfig(config = DEFAULT_HDRI_CONFIG) {
  const base = config ?? DEFAULT_HDRI_CONFIG;
  const fallback = DEFAULT_HDRI_CONFIG;
  return {
    assetId: base.assetId ?? fallback.assetId ?? 'neon_photostudio',
    preferredResolutions:
      Array.isArray(base.preferredResolutions) && base.preferredResolutions.length
        ? base.preferredResolutions
        : fallback.preferredResolutions ?? ['4k', '2k'],
    fallbackResolution: base.fallbackResolution || fallback.fallbackResolution || '4k',
    exposure: base.exposure ?? fallback.exposure ?? 1.2,
    environmentIntensity: base.environmentIntensity ?? fallback.environmentIntensity ?? 1.2,
    backgroundIntensity:
      base.backgroundIntensity ??
      base.environmentIntensity ??
      fallback.backgroundIntensity ??
      1.1,
    backgroundBlurriness: base.backgroundBlurriness ?? fallback.backgroundBlurriness ?? 0.12
  };
}
...
async function resolvePolyHavenHdriUrl(config = DEFAULT_HDRI_CONFIG) {
  const normalized = normalizeHdriConfig(config);
  const fallbackRes = normalized?.fallbackResolution || '4k';
  const fallbackUrl = `https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/${fallbackRes}/${normalized?.assetId ?? 'neon_photostudio'}_${fallbackRes}.hdr`;
  if (!normalized?.assetId || typeof fetch !== 'function') return fallbackUrl;
  try {
    const response = await fetch(`https://api.polyhaven.com/files/${encodeURIComponent(normalized.assetId)}`);
    if (!response?.ok) return fallbackUrl;
    const json = await response.json();
    const picked = pickPolyHavenHdriUrl(json, normalized.preferredResolutions);
...
async function loadPolyHavenHdriEnvironment(renderer, config = DEFAULT_HDRI_CONFIG) {
  if (!renderer) return null;
  const normalized = normalizeHdriConfig(config);
  const url = await resolvePolyHavenHdriUrl(normalized);
  ...
  return new Promise((resolve) => {
    loader.load(
      ...
      (texture) => {
        ...
        envMap.name = `${normalized.assetId}-env`;
        texture.dispose();
        pmrem.dispose();
        resolve({ envMap, settings: normalized });
      },
      undefined,
      (error) => {
        console.warn('Failed to load Poly Haven HDRI', error);
        resolve({ envMap: null, settings: normalized });
      }
    );
  });
}
...
function PoolRoyaleGame({
  ...
  const availableHdriOptions = useMemo(
    () =>
      POOL_ROYALE_HDRI_OPTIONS.filter((option) =>
  ...
  const activeHdriOption = useMemo(
    () =>
      availableHdriOptions.find((opt) => opt.id === environmentHdriId) ??
      availableHdriOptions[0] ??
      HDRI_OPTION_MAP[DEFAULT_POOL_HDRI_ID] ??
      POOL_ROYALE_HDRI_OPTIONS[0],
    [availableHdriOptions, environmentHdriId]
  );
  useEffect(() => {
    ...
    if (!isPoolOptionUnlocked('environmentHdri', environmentHdriId, poolInventory)) {
      setEnvironmentHdriId(DEFAULT_POOL_HDRI_ID);
    }
  }, [
    chromeColorId,
    clothColorId,
  ...
  useEffect(() => {
    hdriSelectionRef.current = activeHdriOption;
    if (typeof window !== 'undefined' && activeHdriOption?.id) {
      window.localStorage.setItem(HDRI_STORAGE_KEY, activeHdriOption.id);
    }
    applyHdriEnvironment(activeHdriOption);
  }, [activeHdriOption, applyHdriEnvironment]);
  useEffect(() => {
    if (typeof window !== 'undefined') {
...
const powerRef = useRef(hud.power);
  ...
  const environmentCleanupRef = useRef(() => {});
  const hdriSelectionRef = useRef(activeHdriOption);
  ...
  const applyHdriEnvironment = useCallback(
    async (option) => {
      ...
      const selection = option ?? hdriSelectionRef.current ?? DEFAULT_HDRI_CONFIG;
      const normalized = normalizeHdriConfig(selection);
      const result = await loadPolyHavenHdriEnvironment(renderer, normalized);
      if (!result || sceneRef.current !== scene) {
        result?.envMap?.dispose?.();
        return;
      }
      const { envMap, settings } = result;
      const finalSettings = normalizeHdriConfig(settings ?? normalized);
      environmentCleanupRef.current?.();
      if (envMap) {
        ...
        if ('backgroundIntensity' in scene) {
          scene.backgroundIntensity =
            finalSettings.backgroundIntensity ?? scene.backgroundIntensity ?? 1.1;
        }
        if ('backgroundBlurriness' in scene) {
          scene.backgroundBlurriness =
            finalSettings.backgroundBlurriness ?? scene.backgroundBlurriness ?? 0.12;
        }
        renderer.toneMappingExposure =
          finalSettings.exposure ?? renderer.toneMappingExposure ?? 1.2;
        environmentCleanupRef.current = () => {
      ...
      hdriSelectionRef.current = selection;
    },
    []
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      hdriSelectionRef.current = activeHdriOption;
      void applyHdriEnvironment(activeHdriOption);
      ...
      const addMobileLighting = () => {
        ...
        lightingRig.name = 'environmentLightingRig';
            ...
            <div className="mt-4 max-h-72 space-y-4 overflow-y-auto pr-1">
              <div>
                ...
                <p className="mt-1 text-[0.7rem] text-white/70">
                  4K reflections replace the arena lights so the table scale and chrome read correctly.
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {availableHdriOptions.map((option) => {
                    const active = option.id === environmentHdriId;
                    const [primarySwatch, secondarySwatch] = option.swatches || [];
                    return (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => setEnvironmentHdriId(option.id)}
                        aria-pressed={active}
                      ...
                      >
                        <span className="flex items-center justify-center gap-2">
                          <span className="flex h-3.5 w-6 items-center gap-0.5 rounded-full border border-white/30 bg-white/10 px-0.5">
                            <span
                              className="h-2.5 w-2.5 rounded-full"
                              style={{ backgroundColor: toHexColor(primarySwatch || '#0f172a') }}
                              aria-hidden="true"
                            />
                            <span
                              className="h-2.5 w-2.5 rounded-full"
                              style={{ backgroundColor: toHexColor(secondarySwatch || '#22c55e') }}
                              aria-hidden="true"
                            />
                          </span>
                          {option.name}
