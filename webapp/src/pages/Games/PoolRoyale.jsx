...
const POCKET_JAW_DEPTH_SCALE = 0.82; // deepen the jaws further so the underside fills out the pocket throat further downward
...
const POCKET_JAW_BOTTOM_CLEARANCE = TABLE.THICK * 0.032; // keep a slimmer gap beneath the jaws so the extended depth still clears the cloth
...
const SPIN_STRENGTH = BALL_R * 0.0295 * 1.25;
const SPIN_DECAY = 0.9;
const SPIN_ROLL_STRENGTH = BALL_R * 0.0175 * 1.25;
...
const SWERVE_TRAVEL_MULTIPLIER = 0.55 * 1.25; // dampen sideways drift while swerve is active so it stays believable
...
const AIM_LINE_WIDTH = Math.max(1, BALL_R * 0.09); // narrow the guide so the aiming line reads sharper on mobile
...
const CUE_VIEW_AIM_LINE_LERP = 0.22; // reduce smoothing so the aim line tracks the cue direction more precisely
const STANDING_VIEW_AIM_LINE_LERP = 0.32; // reduce smoothing so the aim line tracks the cue direction more precisely
