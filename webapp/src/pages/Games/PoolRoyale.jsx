...
const POCKET_SIZE_REDUCTION = 0.82; // tighten pocket mouths further while keeping chrome arcs proportional
const BASE_CORNER_MOUTH = CORNER_MOUTH_REF * MM_TO_UNITS;
const BASE_SIDE_MOUTH = SIDE_MOUTH_REF * MM_TO_UNITS;
const POCKET_CORNER_MOUTH = BASE_CORNER_MOUTH * POCKET_SIZE_REDUCTION;
const POCKET_SIDE_MOUTH = BASE_SIDE_MOUTH * POCKET_SIZE_REDUCTION;
const POCKET_VIS_R = POCKET_CORNER_MOUTH / 2;
const POCKET_R = POCKET_VIS_R * 0.985;
const SIDE_POCKET_RADIUS = POCKET_SIDE_MOUTH / 2;
const POCKET_RAIL_ANCHOR_R = (BASE_CORNER_MOUTH * PREVIOUS_POCKET_SIZE_REDUCTION) / 2;
const SIDE_POCKET_RAIL_ANCHOR_R =
  (BASE_SIDE_MOUTH * PREVIOUS_POCKET_SIZE_REDUCTION) / 2;
...
const BREAK_VIEW_PHI = CUE_SHOT_PHI - 0.25; // baseline orientation used when the rack spawns
const STANDING_VIEW_PHI = BREAK_VIEW_PHI + 0.008; // match the break view angle while sitting fractionally lower
const STANDING_VIEW_MARGIN = 1.06; // pull the standing camera slightly back so it mirrors the intro framing
...
// Keep the standing/broadcast framing synced with the break camera while leaning back slightly for extra clearance
const BROADCAST_DISTANCE_MULTIPLIER = 0.18;
// Allow portrait/landscape standing camera framing to pull in closer without clipping the table
const STANDING_VIEW_MARGIN_LANDSCAPE = 1.02;
const STANDING_VIEW_MARGIN_PORTRAIT = 1.08;
const BROADCAST_RADIUS_PADDING = TABLE.THICK * 0.012;
const CAMERA = {
...
function Table3D(
  ...
  const POCKET_GAP =
    POCKET_RAIL_ANCHOR_R * 0.88 * POCKET_VISUAL_EXPANSION; // keep cushion alignment locked to the previous green rail position
  const SHORT_CUSHION_EXTENSION =
    POCKET_RAIL_ANCHOR_R * -0.02 * POCKET_VISUAL_EXPANSION; // preserve the short-rail setback while the pocket arcs shrink
  const LONG_CUSHION_TRIM =
    POCKET_RAIL_ANCHOR_R * 0.32 * POCKET_VISUAL_EXPANSION; // keep the long cushions tidy while preserving pocket clearance
  const LONG_CUSHION_CORNER_EXTENSION =
    POCKET_RAIL_ANCHOR_R * 0.018 * POCKET_VISUAL_EXPANSION; // stop the long cushions right at the chrome arcs without overlap
  const SIDE_CUSHION_POCKET_CLEARANCE =
    SIDE_POCKET_RAIL_ANCHOR_R * 0.1 * POCKET_VISUAL_EXPANSION; // keep clearance around the pockets while allowing longer cushions
  const SIDE_CUSHION_CENTER_PULL =
    SIDE_POCKET_RAIL_ANCHOR_R * 0.14 * POCKET_VISUAL_EXPANSION; // keep cushions aligned without crowding the pocket mouths
  const SIDE_CUSHION_CORNER_TRIM =
    SIDE_POCKET_RAIL_ANCHOR_R * 0.07 * POCKET_VISUAL_EXPANSION; // tighten the side cushion length so it ends precisely at the chrome arcs
  const horizLen =
    PLAY_W -
  ...
  const cornerChamfer = POCKET_VIS_R * 0.36 * POCKET_VISUAL_EXPANSION;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const camera = new THREE.PerspectiveCamera(
        ...
        const getStandingOrbit = () => {
          const bounds = cameraBoundsRef.current ?? {};
          const standingPhi = THREE.MathUtils.clamp(
            bounds.standing?.phi ?? STANDING_VIEW.phi,
            CAMERA.minPhi,
            CAMERA.maxPhi
          );
          const standingRadius = clampOrbitRadius(
            bounds.standing?.radius ?? BREAK_VIEW.radius
          );
          return { phi: standingPhi, radius: standingRadius };
        };

        const computeStandingOffsets = (extraRadius = 0) => {
          const orbit = getStandingOrbit();
          const effectiveRadius = clampOrbitRadius(orbit.radius + extraRadius);
          const phi = THREE.MathUtils.clamp(
            orbit.phi,
            CAMERA.minPhi,
            CAMERA.maxPhi
          );
          const horizontal = Math.max(0, Math.sin(phi) * effectiveRadius);
          const vertical = Math.cos(phi) * effectiveRadius;
          return { phi, radius: effectiveRadius, horizontal, vertical };
        };

        const computeActionRailDir = (
          axis,
          cueBall,
          previousDir = 1,
          targetBall = null,
          ...
          const fallback = signed(previousDir, 1);
          if (axis === 'side') {
            const reference =
              cueBall?.launchDir?.x ??
              cueBall?.vel?.x ??
              ...
              cueBall?.pos?.x ??
              fallback;
            return signed(reference, fallback);
          }
          const forwardSources = [
            cueBall?.launchDir?.y,
            cueBall?.vel?.y,
            targetBall ? targetBall.pos.y - cueBall.pos.y : null,
            view?.railNormal?.y
          ];
          let forward = 0;
          for (const src of forwardSources) {
            if (Number.isFinite(src) && Math.abs(src) > 1e-4) {
              forward = src;
              break;
            }
          }
          if (!Number.isFinite(forward) || Math.abs(forward) < 1e-4) {
            forward = cueBall?.pos?.y ?? fallback;
          }
          const travelSign = signed(forward, fallback);
          return travelSign === 0 ? -fallback : -travelSign;
        };
        ...
        const updateCamera = () => {
          ...
          if (topViewRef.current) {
            ...
            if (!cueBall?.active) {
              ...
              let targetBall = null;
              if (activeShotView.targetId != null) {
                targetBall = ballsList.find((b) => b.id === activeShotView.targetId) || null;
              }
              const railDir = computeActionRailDir(
                axis,
                cueBall,
                activeShotView.railDir ?? 1,
                targetBall,
                activeShotView
              );
              ...
              if (activeShotView.stage === 'pair') {
                ...
                if (axis === 'short') {
                  ...
                  const standing = computeStandingOffsets(longShotPullback);
                  const desired = new THREE.Vector3(
                    0,
                    Math.max(
                      heightBase,
                      lookAnchor.y + standing.vertical + ACTION_CAM.heightOffset + heightLift
                    ),
                    lookAnchor.z - railDir * standing.horizontal
                  );
                ...
                if (axis === 'short') {
                  ...
                  const desired = new THREE.Vector3(
                    ...
                    Math.max(
                      heightBase,
                      lookAnchor.y + standing.vertical + ACTION_CAM.followHeightOffset + heightLift
                    ),
                    lookAnchor.z - railDir * standing.horizontal
                  );
                  applyStandingViewElevation(desired, lookAnchor, heightBase);
        ...
        const makeActionCameraView = (
          ...
          const initialRailDir = computeActionRailDir(
            axis,
            cueBall,
            1,
            targetBall,
      ...
      function resolve() {
        ...
        if (!nextState.frameOver) {
          const nextTurn = nextState.activePlayer === 'A' ? 0 : 1;
          if (nextTurn === 1) {
            stopAiThinkingRef.current?.();
            startAiThinkingRef.current?.();
          } else {
      ...
      const step = (now) => {
        ...
        if (
          ...
          for (const ball of ballsList) {
            ...
            const intentForced = matchesIntent && (pocketIntent?.forced ?? false);
            const keepCurrentView =
              lastPocketBall && lastPocketBall === ball.id && activeShotView?.mode === 'pocket';
            if (!qualifiesAsGuaranteed && !intentForced && !keepCurrentView) {
              continue;
            }
            ...
            if (intentForced) priority += 1.2;
            else if (matchesIntent) priority += 0.6;
            if (qualifiesAsGuaranteed) priority += 0.5;
