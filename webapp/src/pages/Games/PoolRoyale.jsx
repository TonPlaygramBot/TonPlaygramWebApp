...
const CAMERA_CUE_SURFACE_MARGIN = BALL_R * 0.42; // keep orbit height aligned with the cue while leaving a safe buffer above and stop dips below the cue line
...
const MAX_CROWDING_TILT = THREE.MathUtils.degToRad(11);
const CUE_CROWDING_CLEARANCE = BALL_R * 1.65;
const CUE_CROWDING_DEPTH = BALL_R * 6;
...
const STANDING_VIEW_MARGIN = 0.0011; // pull the standing frame closer so the table and balls fill more of the view
const STANDING_VIEW_FOV = 64;
const CAMERA_ABS_MIN_PHI = 0.1;
const CAMERA_MIN_PHI = Math.max(CAMERA_ABS_MIN_PHI, STANDING_VIEW_PHI - 0.48);
const CAMERA_MAX_PHI = CUE_SHOT_PHI - 0.4; // halt the downward sweep sooner so the lowest angle stays slightly higher and stops above the cue
...
const AI_CAMERA_SETTLE_MS = 260; // allow time for the cue view to settle before firing
...
const TMP_VEC3_CROWD_DIR = new THREE.Vector3();
const TMP_VEC3_CROWD_PERP = new THREE.Vector3();
const TMP_VEC3_CROWD_DELTA = new THREE.Vector3();
const CORNER_SIGNS = [
  { sx: -1, sy: -1 },
  { sx: 1, sy: -1 },
  { sx: -1, sy: 1 },
...
const powerRef = useRef(hud.power);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const computeCueCrowdingTilt = (dir, cueBall, ballList) => {
        if (!dir || !cueBall?.pos || !Array.isArray(ballList)) return 0;
        TMP_VEC3_CROWD_DIR.copy(dir);
        if (TMP_VEC3_CROWD_DIR.lengthSq() < 1e-6) return 0;
        TMP_VEC3_CROWD_DIR.normalize();
        TMP_VEC3_CROWD_PERP.set(-TMP_VEC3_CROWD_DIR.z, 0, TMP_VEC3_CROWD_DIR.x);
        if (TMP_VEC3_CROWD_PERP.lengthSq() > 1e-8) {
          TMP_VEC3_CROWD_PERP.normalize();
        } else {
          TMP_VEC3_CROWD_PERP.set(0, 0, 0);
        }
        let pressure = 0;
        ballList.forEach((ball) => {
          if (!ball || ball.id === 'cue' || !ball.active) return;
          TMP_VEC3_CROWD_DELTA.set(
            ball.pos.x - cueBall.pos.x,
            0,
            ball.pos.y - cueBall.pos.y
          );
          const backAlong = -TMP_VEC3_CROWD_DELTA.dot(TMP_VEC3_CROWD_DIR);
          if (backAlong <= 0 || backAlong > cueLen) return;
          const lateral = Math.abs(TMP_VEC3_CROWD_DELTA.dot(TMP_VEC3_CROWD_PERP));
          if (lateral >= CUE_CROWDING_CLEARANCE) return;
          const lateralPressure = 1 - lateral / CUE_CROWDING_CLEARANCE;
          const depthFactor = Math.min(1, backAlong / CUE_CROWDING_DEPTH);
          pressure = Math.max(pressure, lateralPressure * depthFactor);
        });
        return pressure * MAX_CROWDING_TILT;
      };
      ...
      // Fire (slider triggers on release)
        ...
        const aimDir = aimDirRef.current.clone();
          ...
          const shotPull = Math.min(CUE_PULL_BASE * clampedPower, maxPull);
          const pull = THREE.MathUtils.clamp(
            Math.max(cuePullCurrentRef.current ?? 0, shotPull),
            0,
            maxPull
          );
          ...
          const forwardDuration = Math.max(70, THREE.MathUtils.lerp(220, 90, clampedPower));
          const backDuration = forwardDuration * 0.9;
          const animateBack = (backStart) => {
            const tickBack = (timestamp) => {
              const backElapsed = timestamp - backStart;
              const tBack = Math.min(1, backElapsed / backDuration);
              cueStick.position.lerpVectors(endPos, startPos, tBack);
              if (tBack < 1) {
          ...
          const animateCue = (startTs) => {
            const tickForward = (timestamp) => {
              const elapsed = timestamp - startTs;
              const tForward = Math.min(1, elapsed / forwardDuration);
              cueStick.position.lerpVectors(startPos, endPos, tForward);
              if (tForward < 1) {
                requestAnimationFrame(tickForward);
              } else {
  ...
  const step = (now) => {
    ...
    const playback = replayPlaybackRef.current;
        ...
        if (canShowCue && (isPlayerTurn || previewingAiShot)) {
          ...
          const crowdingTilt = computeCueCrowdingTilt(dir, cue, balls);
          applyCueButtTilt(cueStick, spinTilt + crowdingTilt);
          ...
          const tiltAmount = Math.abs(spinY);
          const spinTilt = MAX_BACKSPIN_TILT * Math.min(tiltAmount, 1);
          const crowdingTilt = computeCueCrowdingTilt(baseDir, cue, balls);
          applyCueButtTilt(cueStick, spinTilt + crowdingTilt);
          cueStick.rotation.y = Math.atan2(dir.x, dir.z) + Math.PI;
          if (tipGroupRef.current) {
            tipGroupRef.current.position.set(0, 0, -cueLen / 2);
