...
const CLOTH_TEXTURE_INTENSITY = 2.05;
const CLOTH_HAIR_INTENSITY = 1.45;
const CLOTH_BUMP_INTENSITY = 2.2;
const CLOTH_SOFT_BLEND = 0.32;
...
const CLOTH_NORMAL_SCALE = new THREE.Vector2(2.25, 1.05);
...
const powerRef = useRef(hud.power);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const resolveObstructionTilt = (liftAmount, length = cueLen) => {
        if (!liftAmount || liftAmount <= 0) return 0;
        const safeLength = Math.max(length, 1e-4);
        const ratio = THREE.MathUtils.clamp(liftAmount / safeLength, 0, 1);
      ...
      const fire = () => {
        ...
        if (shotPrediction.railNormal) replayTags.add('bank');
          ...
          const obstructionTiltFromLift = resolveObstructionTilt(obstructionLift, cueLen);
