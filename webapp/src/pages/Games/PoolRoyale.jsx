  ...
  useEffect(() => {
    ...
    try {
      ...
      const addMobileLighting = () => {
        ...
        const shadowHalfSpan = Math.max(PLAY_W, PLAY_H) * 0.72;
        const targetY = BALL_CENTER_Y;

        const ambient = new THREE.AmbientLight(0xffffff, 0.22);
        lightingRig.add(ambient);

        const key = new THREE.DirectionalLight(0xffffff, 1.6);
        key.position.set(lightOffsetX * 0.42, lightRigHeight * 1.05, lightOffsetZ * 0.38);
        ...
        fill.position.set(-lightOffsetX * 0.48, lightRigHeight * 0.96, lightOffsetZ * 0.4);
        fill.target.position.set(0, targetY, 0);
        lightingRig.add(fill);
        lightingRig.add(fill.target);

        const rim = new THREE.DirectionalLight(0xffffff, 0.55);
        rim.position.set(0, lightRigHeight * 1.02, -lightOffsetZ * 0.85);
