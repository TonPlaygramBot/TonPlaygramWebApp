...
const POCKET_VISUAL_EXPANSION = 1.022;
const CORNER_POCKET_INWARD_SCALE = 1.021; // push the rounded corner cuts deeper without moving the pocket centers
const CORNER_POCKET_SCALE_BOOST = 0.992; // ease the restriction so the corner mouth opens slightly wider than before
const CORNER_POCKET_EXTRA_SCALE = 1.026; // further relax the corner mouth while leaving side pockets unchanged
const CHROME_CORNER_POCKET_RADIUS_SCALE = 1.018;
...
const CHROME_CORNER_EXPANSION_SCALE = 1.006; // trim back the fascia so it now finishes flush with the pocket jaw edge along the long rail
const CHROME_CORNER_SIDE_EXPANSION_SCALE = 1.006; // mirror the lighter reach so the chrome stops exactly where the jaw shoulder begins
...
const CHROME_CORNER_FIELD_CLIP_WIDTH_SCALE = 0.046; // extend the fascia slightly farther along the arc so the chrome starts flush with the pocket curve
const CHROME_CORNER_FIELD_CLIP_DEPTH_SCALE = 0.046; // mirror the extension along the second axis so the rounded arc ends stay covered
...
const CHROME_CORNER_WIDTH_SCALE = 0.988; // shave the chrome plate slightly so it ends at the jaw line on the long rail
const CHROME_CORNER_HEIGHT_SCALE = 0.978; // mirror the trim on the short rail so the fascia meets the jaw corner without overlap
...
const CHROME_SIDE_POCKET_RADIUS_SCALE =
  ...
  0.988; // tighten the middle chrome arches so their rounded cuts sit smaller around the pocket centres
...
const CHROME_PLATE_THICKNESS_SCALE = 0.0334; // match diamond thickness on the wooden rails for fascia depth
const CHROME_SIDE_PLATE_THICKNESS_BOOST = 1.24; // thicken the middle fascia so its depth now matches the corner plates
...
const CHROME_SIDE_PLATE_POCKET_SPAN_SCALE = 2.36; // push the side fascia farther along the arch so it blankets the larger chrome reveal
const CHROME_SIDE_PLATE_HEIGHT_SCALE = 2.78; // extend fascia reach so the middle pocket cut gains a broader surround on the remaining three sides (~30% boost)
const CHROME_SIDE_PLATE_CENTER_TRIM_SCALE = 0; // keep the middle fascia centred on the pocket without carving extra relief
const CHROME_SIDE_PLATE_WIDTH_EXPANSION_SCALE = 1.7; // trim fascia span further so the middle plates finish before intruding into the pocket zone while keeping the rounded edge intact
const CHROME_SIDE_PLATE_OUTER_EXTENSION_SCALE = 1.4; // widen the middle fascia outward so it blankets the exposed wood like the corner plates without altering the rounded cut
...
const CHROME_SIDE_PLATE_OUTWARD_SHIFT_SCALE = 0.082; // pull the side fascias inward so their outer edge trims back while keeping the reveal tidy
...
const CHROME_SIDE_POCKET_CUT_SCALE = 1.014; // pull back the middle chrome arch so the rounded cut radius reads smaller at the side pockets while leaving a touch more breathing room
const CHROME_SIDE_POCKET_CUT_CENTER_PULL_SCALE = 0.18; // pull the middle chrome arch toward centre a little less so the fascia stops before the side pockets
const WOOD_RAIL_POCKET_RELIEF_SCALE = 0.94; // ease the wooden rail pocket relief so the rounded corner cuts expand a hair and keep pace with the broader chrome reveal
const WOOD_CORNER_RELIEF_INWARD_SCALE = 0.984; // ease the wooden corner relief fractionally less so chrome widening does not alter the wood cut
const WOOD_CORNER_RAIL_POCKET_RELIEF_SCALE =
  (1 / WOOD_RAIL_POCKET_RELIEF_SCALE) * WOOD_CORNER_RELIEF_INWARD_SCALE; // corner wood arches now sit a hair inside the chrome radius so the rounded cut creeps inward
const WOOD_SIDE_RAIL_POCKET_RELIEF_SCALE = 0.978; // tighten the middle rail arches so the rounded cuts stay slimmer and avoid the pocket area
const WOOD_SIDE_POCKET_CUT_CENTER_OUTSET_SCALE = 0.18; // ease the centre pull so the wood cut stops short of the side-pocket throat
...
const POCKET_JAW_VERTICAL_LIFT = TABLE.THICK * 0.098; // lower the visible rim slightly more so the pocket lips sit nearer the cloth plane
...
const SIDE_POCKET_JAW_RADIUS_EXPANSION = 0.84; // pull the middle jaw arc tighter so the side pocket radius reads smaller
...
const SIDE_POCKET_JAW_OUTWARD_SHIFT = 0; // pull the middle pocket jaws inward so the lips sit closer to centre
...
const SIDE_POCKET_JAW_EDGE_TRIM_SCALE = 0.88; // taper the outer jaw radius near the ends to keep a slightly wider gap before the cushions
const SIDE_POCKET_JAW_EDGE_TRIM_CURVE = 1.28; // ease the taper into the trimmed ends for a smooth falloff
...
const SIDE_POCKET_MOUTH_REDUCTION_SCALE = 0.958; // tighten the middle pocket mouth further so the radius and cloth cutouts shrink a little more
...
const SIDE_POCKET_CUT_SCALE = 0.964; // shrink the middle cloth/rail cutouts slightly so the softened mouth keeps a smaller radius and tighter fit
