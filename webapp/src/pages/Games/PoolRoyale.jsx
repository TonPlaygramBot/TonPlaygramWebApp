...
const POCKET_DROP_DEPTH = POCKET_NET_DEPTH * 0.98; // drop nearly the full net depth so potted balls clear the rim
...
const POCKET_NET_VERTICAL_LIFT = BALL_R * 0.22; // lift the net so the weave sits closer to the chrome ring
...
const POCKET_HOLDER_REST_DROP = BALL_R * 2.45; // drop the resting spot so potted balls settle lower on the chrome rails
...
const CUE_WOOD_REPEAT_SCALE = 1.15 / 3; // enlarge cue grain 3× without affecting table rail tiling
...
function Table3D(
  ...
  const pocketNetProfile = [
    ...
    new THREE.Vector2(pocketGuideRingRadius * 0.92, -POCKET_NET_DEPTH * 0.74),
  ...
  pocketCenters().forEach((p, index) => {
    ...
    net.position.set(
      ...
      pocketTopY -
        POCKET_WALL_HEIGHT -
        POCKET_WALL_OPEN_TRIM +
        pocketLift +
        POCKET_NET_VERTICAL_LIFT,
      p.y
    );
  ...
  const brandPlateDepth = Math.min(endRailW * 0.46, TABLE.THICK * 0.68);
  ...
  const brandPlateOutwardShift = endRailW * 0.3;
  ...
  chalkSlots.forEach((slot, index) => {
    ...
    mesh.rotation.set(slot.rotationX ?? 0, slot.rotationY ?? 0, slot.rotationZ ?? 0);
