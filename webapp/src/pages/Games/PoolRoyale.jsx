...
const CUE_BUTT_CUSHION_CLEARANCE = BALL_R * 0.02;
...
const powerRef = useRef(hud.power);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const applyCueButtClearance = () => {
        const cushionTop = table?.userData?.cushionTopLocal;
        if (!Number.isFinite(cushionTop)) return;
        const minButtY = cushionTop + CUE_BUTT_CUSHION_CLEARANCE;
        if (TMP_VEC3_BUTT.y >= minButtY) return;
        const lift = minButtY - TMP_VEC3_BUTT.y;
