...
const CHROME_SIDE_PLATE_HEIGHT_SCALE = 3.12; // extend fascia reach so the middle pocket cut gains a much broader surround on the remaining three sides (~60% boost)
const CHROME_SIDE_PLATE_CENTER_TRIM_SCALE = 0; // keep the middle fascia centred on the pocket without carving extra relief
const CHROME_SIDE_PLATE_WIDTH_EXPANSION_SCALE = 2.68; // push fascia span farther along the side rails so the middle plates run deeper toward the corners without widening the rounded pocket edge
const CHROME_SIDE_PLATE_WIDTH_REDUCTION_SCALE = 0.92; // lightly trim the straight fascia span beside middle pockets while keeping the rounded cut and outer edge fully covered
...
const CHROME_SIDE_PLATE_OUTWARD_SHIFT_SCALE = 0.14; // push the side fascias farther from centre so their outer edge stays flush while widening the reveal (nearly double the baseline offset)
