...
function PoolRoyaleGame({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const carpetShadowCatch = new THREE.Mesh(
        new THREE.PlaneGeometry(carpetWidth * 1.05, carpetDepth * 1.05),
        new THREE.ShadowMaterial({ color: 0x000000, opacity: 0.42 })
      );
      carpetShadowCatch.rotation.x = -Math.PI / 2;
      carpetShadowCatch.position.set(0, floorY + 0.01, 0);
      carpetShadowCatch.receiveShadow = true;
      carpetShadowCatch.renderOrder = carpet.renderOrder + 0.01;
      world.add(carpetShadowCatch);
      ...
      const addMobileLighting = () => {
        ...
        const lightOffsetX = Math.max(PLAY_W * 0.12, TABLE.THICK * 2.2);
        const lightOffsetZ = Math.max(PLAY_H * 0.14, TABLE.THICK * 2.4);
        const shadowHalfSpan = Math.max(PLAY_W, PLAY_H) * 0.92;
        ...
        key.position.set(lightOffsetX * 0.4, lightRigHeight * 0.98, lightOffsetZ * 0.35);
        key.target.position.set(0, targetY, 0);
        key.castShadow = true;
        key.shadow.mapSize.set(2048, 2048);
        key.shadow.camera.near = 0.1;
        key.shadow.camera.far = shadowHalfSpan * 2.8;
        ...
        fill.position.set(-lightOffsetX * 0.65, lightRigHeight * 0.9, lightOffsetZ * 0.58);
        fill.target.position.set(0, targetY, 0);
        lightingRig.add(fill);
        lightingRig.add(fill.target);

        const rim = new THREE.DirectionalLight(0xffffff, 0.55);
        rim.position.set(lightOffsetX * 0.12, lightRigHeight * 0.94, -lightOffsetZ * 1.1);
