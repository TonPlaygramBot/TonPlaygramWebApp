...
const CHROME_CORNER_WIDTH_SCALE = 0.974; // shave the chrome plate a touch more so both fascia sides stop before the jaw line
const CHROME_CORNER_HEIGHT_SCALE = 0.954; // mirror the extra trim on the short rail so the fascia clears both jaw corners
...
const CHROME_SIDE_PLATE_WIDTH_REDUCTION_SCALE = 0.982; // trim middle fascias slightly on both sides so they stop shy of the jaws
...
const CHROME_SIDE_PLATE_OUTWARD_SHIFT_SCALE = 0.128; // push the side fascias farther outward so their outer edge follows the relocated middle pocket cuts
...
const SIDE_POCKET_OUTWARD_BIAS = TABLE.THICK * 0.04; // push the middle pocket centres and cloth cutouts slightly outward away from the table midpoint
const SIDE_POCKET_FIELD_PULL = TABLE.THICK * 0.04; // gently bias the middle pocket centres and cuts back toward the playfield
...
const CLOTH_UNDERLAY_THICKNESS = TABLE.THICK * 0.16; // add a thin plywood board beneath the cloth
const CLOTH_UNDERLAY_GAP = TABLE.THICK * 0.012; // leave a narrow void so the cloth floats above the board without z-fighting
const CLOTH_UNDERLAY_EXTRA_DROP = TABLE.THICK * 0.02; // lower the board slightly to keep the pocket throats clear
const CLOTH_UNDERLAY_POCKET_SCALE = 1.1; // cut the board holes 10% larger than the pockets to avoid interference
const CLOTH_UNDERLAY_COLOR = 0x1f6b3a; // matte painted green to boost visible ball shadows
const CLOTH_EXTENDED_DEPTH = CLOTH_THICKNESS; // wrap only the cloth depth; the underlay sits on its own board
...
const SPIN_DOT_DIAMETER_PX = 10;
  // angle for cushion cuts guiding balls into corner pockets (slightly steeper for even mouth entry)
...
function Table3D(
  ...
  if (CLOTH_UNDERLAY_THICKNESS > MICRO_EPS) {
    const underlayShape = buildSurfaceShape(
      POCKET_HOLE_R * CLOTH_UNDERLAY_POCKET_SCALE
    );
    ...
    const underlayMat = new THREE.MeshStandardMaterial({
      color: CLOTH_UNDERLAY_COLOR,
      metalness: 0,
      roughness: 1,
      envMapIntensity: 0,
      emissive: new THREE.Color(CLOTH_UNDERLAY_COLOR).multiplyScalar(0.05)
    });
  ...
  const CUSHION_CORNER_CLEARANCE_REDUCTION = TABLE.THICK * 0.22; // shorten the corner cushions slightly more so the noses stay clear of the pocket openings
