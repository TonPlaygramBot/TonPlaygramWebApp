...
const POCKET_JAW_DEPTH_SCALE = 1.07; // extend the jaw bodies so the underside reaches deeper below the cloth
...
const SIDE_POCKET_JAW_VERTICAL_TWEAK = -TABLE.THICK * 0.008; // drop the middle jaws slightly so they sit level with the other pockets
...
const POCKET_LEATHER_TEXTURE_REPEAT = Object.freeze({ x: 0.6, y: 0.6 });
...
const POCKET_NET_RING_VERTICAL_OFFSET = BALL_R * 0.02; // lift the ring so the holders sit slightly higher against the net
...
const POCKET_GUIDE_DROP = BALL_R * 0.18;
...
const POCKET_GUIDE_VERTICAL_DROP = BALL_R * 0.12; // lift the chrome holder rails so the short L segments meet the ring
...
const POCKET_HOLDER_REST_DROP = BALL_R * 2.35; // drop the resting spot so potted balls settle lower on the chrome rails
...
const CUE_TIP_GAP = BALL_R * 1.2 + CUE_TIP_CLEARANCE; // pull the blue tip farther back so it reads clearly against the cue ball
...
const CUE_Y = BALL_CENTER_Y - BALL_R * 0.095; // lower the cue so the tip sits centered on the cue ball at rest
...
function Table3D(
  ...
  const pocketStrapWidth = BALL_R * 1.6;
  ...
  pocketCenters().forEach((p, index) => {
    ...
    if (strapOrigin && strapEnd) {
      ...
      const strapTopLimit = pocketTopY - TABLE.THICK * 0.04;
