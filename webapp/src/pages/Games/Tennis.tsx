...
export default function MobileThreeTennisPrototype() {
  ...
  useEffect(() => {
    ...
    getPoolRoyalInventory().then((inventory) => {
      ...
      const options = POOL_ROYALE_HDRI_VARIANTS.filter((v) => (owned.has(v.id) || v.id === POOL_ROYALE_DEFAULT_HDRI_ID) && v.id !== "colorfulStudio");
      setHdriChoices(options);
      if (!options.some((v) => v.id === selectedHdriId) || selectedHdriId === "colorfulStudio") setSelectedHdriId(options[0]?.id || POOL_ROYALE_DEFAULT_HDRI_ID);
    }).catch(() => {});
