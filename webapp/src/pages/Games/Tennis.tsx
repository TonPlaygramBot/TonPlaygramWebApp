...
const CFG = {
  ...
  serveOuterBaselineZ: 10.2 * WORLD_SCALE,
...
function serveXForSide(player: PlayerSide, side: "deuce" | "ad") {
  const outsideSingles = CFG.courtW / 2 + 0.62;
...
export default function MobileThreeTennisPrototype() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (camera.aspect < 0.72) cameraOffset.set(0, 6.2 * CFG.worldScale, 10.4 * CFG.worldScale);
      else cameraOffset.set(0, 5.2 * CFG.worldScale, 8.8 * CFG.worldScale);
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const servingNow = ball.lastHitBy === null;
      nearPlayer.target.x = servingNow
        ? clamp(control.startPlayer.x + dx * 0.012, -CFG.doublesW / 2 + 0.18, CFG.doublesW / 2 - 0.18)
        : clamp(control.startPlayer.x + dx * 0.012, -CFG.courtW / 2 + 0.35, CFG.courtW / 2 - 0.35);
      const minZ = servingNow ? CFG.serveOuterBaselineZ : 0.76;
      if (ball.lastHitBy === null) {
        nearPlayer.target.z = clamp(control.startPlayer.z + dy * 0.012, minZ, CFG.courtL / 2 + 1.8);
      } else {
    ...
    const onPointerUp = (e: PointerEvent) => {
      ...
      if (isServe) currentServeSide = nearPlayer.target.x >= 0 ? "deuce" : "ad";
    ...
    function updateAi() {
      ...
      if (ballComingToAi) {
        ...
        farPlayer.target.z = clamp(Math.min(-0.76, landing.z + 0.32), -CFG.courtL / 2 + 0.38, -0.6);
      } else {
        farPlayer.target.lerp(home, 0.035);
      }
      if (ball.lastHitBy === "near" && ball.pos.z < 2.1) {
        farPlayer.target.x = clamp(ball.pos.x * 0.92, -CFG.courtW / 2 + 0.35, CFG.courtW / 2 - 0.35);
      }
    ...
    function animate() {
      ...
      const servingPhase = ball.lastHitBy === null;
      if (servingPhase) {
        cameraPosTarget.copy(nearPlayer.target).add(new THREE.Vector3(0, 6.5 * CFG.worldScale, 11.2 * CFG.worldScale));
      } else {
        cameraPosTarget.copy(nearPlayer.target).add(cameraOffset);
      }
      ...
      const cameraTargetZ = servingPhase ? nearPlayer.target.z - 12.2 * CFG.worldScale : nearPlayer.target.z - 6.9 * CFG.worldScale;
      cameraTarget.z += (cameraTargetZ - cameraTarget.z) * (1 - Math.exp(-4.3 * dt));
