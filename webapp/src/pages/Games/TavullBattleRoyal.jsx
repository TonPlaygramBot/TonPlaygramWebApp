...
export default function TavullBattleRoyal() {
  ...
  useEffect(() => {
    ...
    sceneBundleRef.current = {
      ...
      animateDiceThrow: (values = [], seat = 0) => {
        ...
        const targetLeftX = -BOARD_HALF_X * 0.33
        const targetRightX = BOARD_HALF_X * 0.33
        diceMeshes.forEach((mesh, index) => {
          ...
          const laneX = index === 0 ? targetLeftX : targetRightX
          const startPos = new THREE.Vector3(laneX * 0.45, BOARD_Y + 0.24, startZ)
          const endPos = new THREE.Vector3(laneX, BOARD_Y + 0.17, 0)
  ...
  const roll = () => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      setMessage('Tap a checker point, then tap destination triangle/point to move.')
    }, 820)
    pendingTimeoutsRef.current.push(timeoutId)
  }
  ...
  useEffect(() => {
    ...
    const onPointTap = (event) => {
      ...
      const sourceCandidates = available.filter((seq) => seq?.line?.some((move) => move.from === point))
      const touchCandidates = available.filter((seq) => seq?.line?.some((move) => move.from === point || move.to === point))
      if (!touchCandidates.length) return

      if (selectedPoint == null) {
        if (sourceCandidates.length === 1) {
          handleSequence(sourceCandidates[0])
          return
        }
        setSelectedPoint(point)
        setMessage('Selected checker. Tap destination triangle/point to move.')
        return
      }

      const narrowed = available.filter((seq) =>
        seq?.line?.some((move) => move.from === selectedPoint && move.to === point)
      )
      ...
      setMessage('No valid destination there. Tap a highlighted destination point.')
    }
