...
export default function TavullBattleRoyal() {
  ...
  const ownedBoardTextureOptions = useMemo(
    () =>
      BACKGAMMON_BOARD_TEXTURE_OPTIONS.filter(
        (option) =>
          isChessOptionUnlocked('boardFinish', option.id, chessInventory) ||
          BACKGAMMON_ROYAL_DEFAULT_UNLOCKS.boardFinish.includes(option.id)
      ),
    [chessInventory]
  )
  const ownedFrameFinishOptions = useMemo(
    () =>
      BACKGAMMON_FRAME_FINISH_OPTIONS.filter(
        (option) =>
          isChessOptionUnlocked('boardFrameFinish', option.id, chessInventory) ||
          BACKGAMMON_ROYAL_DEFAULT_UNLOCKS.boardFrameFinish.includes(option.id)
      ),
    [chessInventory]
  )
  const ownedP1PieceOptions = useMemo(
    () =>
      BACKGAMMON_PIECE_STYLE_OPTIONS.filter(
        (option) =>
          isChessOptionUnlocked('playerOnePieceStyle', option.id, chessInventory) ||
          BACKGAMMON_ROYAL_DEFAULT_UNLOCKS.playerOnePieceStyle.includes(option.id)
      ),
    [chessInventory]
  )
  const ownedP2PieceOptions = useMemo(
    () =>
      BACKGAMMON_PIECE_STYLE_OPTIONS.filter(
        (option) =>
          isChessOptionUnlocked('playerTwoPieceStyle', option.id, chessInventory) ||
          BACKGAMMON_ROYAL_DEFAULT_UNLOCKS.playerTwoPieceStyle.includes(option.id)
      ),
    [chessInventory]
  )
