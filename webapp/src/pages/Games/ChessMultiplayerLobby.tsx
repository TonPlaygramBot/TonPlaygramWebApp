...
const EMPTY: LobbySnapshot = { players: [], phase: 'idle', invitationCode: '', countdownEndsAt: 0, minPlayers: 4, maxPlayers: 8 };
...
function snapshot(state: any): LobbySnapshot {
  ...
  return { players, phase: state?.phase || 'waiting', invitationCode: state?.invitationCode || '', countdownEndsAt: state?.countdownEndsAt || 0, minPlayers: state?.minPlayers || 4, maxPlayers: state?.maxPlayers || 8 };
}
...
export default function ChessMultiplayerLobby() {
  ...
  const bindRoom = (room: Room) => {
    ...
    setStatus('joined');
    setMessage('Connected. Ready up when your squad arrives.');
  ...
  const join = async (createPrivate = false) => {
    ...
    try {
      ...
      const room = await client.joinOrCreate('chess_lobby', { ...identity, visibility, invitationCode: code, maxPlayers });
  ...
  return (
    <section className="mx-auto w-full max-w-md space-y-4 rounded-[28px] border border-cyan-300/20 bg-[#07111f]/95 p-4 shadow-2xl" aria-label="Chess multiplayer lobby">
      ...
      {status === 'idle' || status === 'error' ? <>
        ...
        <label className="block text-xs text-white/60">Match size <select value={maxPlayers} onChange={(e) => setMaxPlayers(Number(e.target.value))} className="mt-1 w-full rounded-xl border border-white/10 bg-[#050b14] p-3 text-white">{[4,5,6,7,8].map((n) => <option key={n} value={n}>{n} players</option>)}</select></label>
        ...
        <div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-white/5 p-2"><b className="block text-lg text-white">{lobby.players.length}/{lobby.maxPlayers}</b><span className="text-[10px] text-white/45">FOUND</span></div><div className="rounded-xl bg-white/5 p-2"><b className="block text-lg text-white">{Math.floor(elapsed/60)}:{String(elapsed%60).padStart(2,'0')}</b><span className="text-[10px] text-white/45">WAITING</span></div><div className="rounded-xl bg-white/5 p-2"><b className="block text-lg text-amber-300">{countdown || '—'}</b><span className="text-[10px] text-white/45">START</span></div></div>
