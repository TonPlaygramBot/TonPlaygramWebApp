...
function buildRealisticTableTennisTable() {
  ...
  addBox(group, [0.035, CFG.netH, CFG.tableL + 0.12], [0, netY, 0], netMat);
  addCylinder(group, 0.025, 0.025, 0.42, [0, CFG.tableY + 0.22, -(TABLE_HALF_L + 0.11)], material(0x111827, 0.65, 0.05), 12);
  addCylinder(group, 0.025, 0.025, 0.42, [0, CFG.tableY + 0.22, TABLE_HALF_L + 0.11], material(0x111827, 0.65, 0.05), 12);
  addBox(group, [0.18, 0.16, 0.22], [0, CFG.tableY - 0.02, -(TABLE_HALF_L + 0.11)], material(0x111827, 0.5, 0.12));
  addBox(group, [0.18, 0.16, 0.22], [0, CFG.tableY - 0.02, TABLE_HALF_L + 0.11], material(0x111827, 0.5, 0.12));
...
function createTableTennisPaddle(colorA: number, colorB = 0x090909) {
  ...
  const face = new THREE.Mesh(new THREE.CylinderGeometry(0.36 * s, 0.36 * s, 0.06 * s, 56), frontMat);
  face.rotation.x = Math.PI / 2;
  g.add(face);

  const back = new THREE.Mesh(new THREE.CylinderGeometry(0.34 * s, 0.34 * s, 0.064 * s, 56), backMat);
  back.rotation.x = Math.PI / 2;
  back.position.z = -0.04 * s;
  g.add(back);

  const rim = new THREE.Mesh(new THREE.TorusGeometry(0.365 * s, 0.025 * s, 14, 80), rimMat);
  rim.position.z = 0.008 * s;
  g.add(rim);

  const handle = new THREE.Mesh(new THREE.CapsuleGeometry(0.08 * s, 0.55 * s, 10, 20), wood);
