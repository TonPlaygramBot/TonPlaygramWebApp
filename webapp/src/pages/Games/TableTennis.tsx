...
const DEFAULT_HUMAN_URL = "https://threejs.org/examples/models/gltf/readyplayer.me.glb";
...
function createTableTennisPaddle(colorA: number, colorB = 0x090909) {
  ...
  const handle = new THREE.Mesh(new THREE.BoxGeometry(0.048, 0.14, 0.034), wood);
  handle.position.y = 0.045;
  handle.rotation.z = 0.02;
  enableShadow(handle);
  g.add(handle);

  const throat = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.05, 0.014), wood);
  throat.position.y = 0.125;
  enableShadow(throat);
  g.add(throat);

  const bladeEdge = new THREE.Mesh(new THREE.CylinderGeometry(0.092, 0.092, 0.012, 56), edge);
  bladeEdge.rotation.x = Math.PI / 2;
  bladeEdge.scale.x = 0.92;
  bladeEdge.scale.y = 1.08;
  bladeEdge.position.y = 0.19;
  enableShadow(bladeEdge);
  g.add(bladeEdge);

  const front = new THREE.Mesh(new THREE.CircleGeometry(0.089, 56), redRubber);
  front.scale.y = 1.08;
  front.position.set(0, 0.19, 0.0078);
  g.add(front);

  const back = new THREE.Mesh(new THREE.CircleGeometry(0.089, 56), blackRubber);
  back.scale.y = 1.08;
  back.rotation.y = Math.PI;
  back.position.set(0, 0.19, -0.0078);
  g.add(back);

  for (let i = 0; i < 18; i++) {
    const dot = new THREE.Mesh(new THREE.BoxGeometry(0.005, 0.005, 0.018), edge);
    const a = (i / 18) * Math.PI * 2;
    dot.position.set(Math.cos(a) * 0.084 * 0.92, 0.19 + Math.sin(a) * 0.09 * 1.08, 0);
    dot.rotation.z = a;
    g.add(dot);
  }
...
function addHuman(scene: THREE.Scene, side: PlayerSide, start: THREE.Vector3, accent: number, modelUrls: string[]): HumanRig {
  ...
  const urls = Array.isArray(modelUrls) && modelUrls.length ? modelUrls : [DEFAULT_HUMAN_URL];
...
function updatePlayerMotion(player: HumanRig, ball: BallState, dt: number) {
  ...
  player.pos.x = clamp(player.pos.x, -0.95, 0.95);
...
export default function MobileRealisticTableTennisGame() {
  ...
  const selectedHumanOption = useMemo(
    () =>
      unlockedHumanCharacters.find((entry) => entry.id === selectedHumanCharacterId) ||
      unlockedHumanCharacters[0],
    [selectedHumanCharacterId, unlockedHumanCharacters]
  );
  const selectedHdriOption = useMemo(
    () => unlockedHdris.find((entry) => entry.id === selectedHdriId) || unlockedHdris[0],
  ...
  useEffect(() => {
    ...
    const loadHdri = (idx = 0) => {
      const dynamicHdriUrls = [
        selectedHdriOption?.manualUrl,
        selectedHdriOption?.assetUrl,
        selectedHdriOption?.url,
        ...HDRI_URLS
      ].filter(Boolean) as string[];
      const url = dynamicHdriUrls[idx];
      if (!url) return;
    ...
    const modelUrls = selectedHumanOption?.modelUrls || [DEFAULT_HUMAN_URL];
    const nearPlayer = addHuman(scene, "near", new THREE.Vector3(0, 0, TABLE_HALF_L + 0.48), 0xff6b2e, modelUrls);
    const farPlayer = addHuman(scene, "far", new THREE.Vector3(0, 0, -TABLE_HALF_L - 0.48), 0x4ab7ff, modelUrls);
    ...
    const resize = () => {
      ...
      camera.position.set(0, camera.aspect < 0.72 ? 2.85 : 2.45, camera.aspect < 0.72 ? 3.25 : 2.8);
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      nearPlayer.target.x = clamp(control.startPlayer.x + dx * 0.0047, -0.95, 0.95);
    ...
    function updateBall(dt: number) {
      ...
      if ((Math.abs(ball.pos.x) > 2.7 || Math.abs(ball.pos.z) > 3.7 || ball.pos.y < -0.7) && ball.lastHitBy) {
        const hitter = ball.lastHitBy;
        const hadLegalBounce = ball.bounceSide === opposite(hitter) && ball.bounceCount >= 1;
        awardPoint(hadLegalBounce ? hitter : opposite(hitter), "out");
      }
    ...
    function animate() {
      ...
      if (pointLock) {
        ...
        if (pointLockT <= 0) {
          ...
          nearPlayer.swingT = 0;
          farPlayer.swingT = 0;
          nearPlayer.target.set(0, 0, TABLE_HALF_L + 0.48);
          farPlayer.target.set(0, 0, -TABLE_HALF_L - 0.48);
  ...
  }, [graphicsId, selectedHdriOption, selectedHumanOption]);
