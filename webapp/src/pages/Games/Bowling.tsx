...
const BOWLING_HDRI_STORAGE_KEY = 'bowlingHdriEnvironment';
const BOWLING_HDRI_OPTIONS = [
  { id: 'dancingHall', name: 'Dancing Hall HDRI', url: 'https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/dancing_hall_1k.hdr' },
  { id: 'sepulchralChapelRotunda', name: 'Sepulchral Chapel Rotunda HDRI', url: 'https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/sepulchral_chapel_rotunda_1k.hdr' },
  { id: 'vestibule', name: 'Vestibule HDRI', url: 'https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/vestibule_1k.hdr' }
] as const;
...
export default function BowlingGame() {
  ...
  const [selectedHdri, setSelectedHdri] = useState(() => localStorage.getItem(BOWLING_HDRI_STORAGE_KEY) || BOWLING_HDRI_OPTIONS[0].id);
  ...
  useEffect(() => {
    ...
    const hdri = BOWLING_HDRI_OPTIONS.find((entry) => entry.id === selectedHdri) || BOWLING_HDRI_OPTIONS[0];
    const envLoader = new THREE.TextureLoader();
    envLoader.load(hdri.url, (texture) => {
  ...
  return <div style={{position:"fixed",inset:0,overflow:"hidden",background:"#07090d",touchAction:"none",userSelect:"none"}}>
    ...
    <div style={{position:"fixed",inset:0,pointerEvents:"none",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      <button onClick={() => setMenuOpen((v) => !v)} style={{position:'absolute',top:26,left:10,pointerEvents:'auto',width:42,height:42,borderRadius:10,border:'1px solid rgba(255,255,255,.25)',background:'rgba(6,10,18,.8)',color:'#fff'}}>☰</button>
      {menuOpen && <div style={{position:'absolute',top:74,left:10,width:280,padding:12,borderRadius:12,pointerEvents:'auto',background:'rgba(6,10,18,.92)',border:'1px solid rgba(255,255,255,.2)',color:'#fff'}}>
        ...
        <div style={{fontWeight:800,margin:'10px 0 6px'}}>HDRI</div>
        <div style={{display:'grid',gap:6}}>
          {BOWLING_HDRI_OPTIONS.map((entry) => (
            <button key={entry.id} onClick={() => { setSelectedHdri(entry.id); localStorage.setItem(BOWLING_HDRI_STORAGE_KEY, entry.id); }} style={{pointerEvents:'auto',textAlign:'left',padding:'6px 8px',borderRadius:8,border:'1px solid rgba(255,255,255,.22)',background:selectedHdri===entry.id?'rgba(123,226,255,.22)':'transparent',color:'#fff'}}>{entry.name}</button>
          ))}
        </div>
      </div>}
