    ...
    if (mode === 'online') {
      await runSimpleOnlineFlow({
        ...
        onMatched: ({ accountId, tableId }) => {
          ...
          navigate(`/games/table-tennis?${params.toString()}`);
        }
      });
      return;
    }
  ...
  return (
    <div className="relative min-h-screen bg-[#070b16] text-text">
      ...
      <div className="relative z-10 space-y-4 p-4 pb-8">
        <GameLobbyHeader slug="table-tennis" title="Table Tennis Lobby" badge="1v1 only" />
        <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#101828]/80 to-[#0b1324]/90 p-4 text-white/80">
          <p className="text-xs">Mode rules: 1v1 only. Vs AI is free, Online uses TPG stake.</p>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            ...
            <button type="button" onClick={() => setMode('online')} className={`lobby-option-card ${mode === 'online' ? 'lobby-option-card-active' : 'lobby-option-card-inactive'}`}>
              <p className="lobby-option-label">Online 1v1</p><p className="lobby-option-subtitle">TPG stake match</p>
            </button>
          </div>
          {mode === 'online' && (
            <div className="mt-3">
              <label className="text-xs uppercase text-white/60">TPG Stake</label>
