...
import ThreeDice from '../../components/ThreeDice.jsx';
...
function LudoBoard({ game, state, rolling, displayRoll, handleRoll }) {
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <div className="flex items-center gap-4">
        <ThreeDice value={displayRoll} rolling={rolling} />
        <button
          className="px-3 py-1 bg-primary text-white rounded disabled:opacity-50"
          onClick={handleRoll}
          disabled={state.gameState !== 'playerHasToRollADice' || rolling}
        >
          Roll Dice
        </button>
        <button
...
export default function LudoGame() {
  ...
  useEffect(() => {
    if (!rolling && gameState.lastDiceRoll !== null) {
      setDisplayRoll(gameState.lastDiceRoll);
    }
  }, [rolling, gameState.lastDiceRoll]);
  ...
  const handleRoll = () => {
    if (rolling || gameState.gameState !== 'playerHasToRollADice') return;
    setRolling(true);
    const rollValue = ludo.rollDiceForCurrentPiece();
    setTimeout(() => {
      setRolling(false);
      setDisplayRoll(rollValue);
    }, 800);
  };
