...
export default function Exchange() {
  ...
  const loadCoinGeckoDirectly = async () => {
    ...
    if (!res.ok) throw new Error(`CoinGecko unavailable (${res.status})`);
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (!incoming.length) {
        const fallback = await loadCoinGeckoDirectly();
        setMarkets(fallback);
      ...
      try {
        ...
        setMarkets(fallback);
        setUpdatedAt(new Date().toISOString());
        setError('Using direct CoinGecko feed. Backend exchange feed is temporarily unavailable.');
      } catch {
          ...
          {sorted.map((coin) => (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                <img src={coin.image || '/assets/icons/profile.svg'} alt={coin.symbol} className="w-6 h-6 rounded-full" />
