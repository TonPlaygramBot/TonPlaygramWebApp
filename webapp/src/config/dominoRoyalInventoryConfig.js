...
const DOMINO_POOL_CLOTH_OPTIONS = POOL_ROYALE_CLOTH_VARIANTS.map((cloth) => ({
  id: cloth.id,
  label: cloth.name,
...
export const DOMINO_ROYAL_OPTION_SETS = Object.freeze({
  ...
  tableCloth: DOMINO_POOL_CLOTH_OPTIONS,
...
export const DOMINO_ROYAL_STORE_ITEMS = [
  ...
  ...DOMINO_ROYAL_OPTION_SETS.tableCloth.slice(1).map((option, idx) => ({
    ...
    description:
      option.description ||
      'Unlock a new Pool Royale cloth texture for the Domino Royal table surface.',
    thumbnail:
      option.thumbnail ||
