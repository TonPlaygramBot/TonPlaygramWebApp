import { khronosThumb } from './storeThumbnails.js';

const THREEJS_READYPLAYER_ME_MODEL_URL = 'https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/models/gltf/readyplayer.me.glb';
const THREEJS_XBOT_MODEL_URL = 'https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/models/gltf/Xbot.glb';
const MURLAN_CHARACTER_SAFE_FALLBACK_MODEL_URLS = Object.freeze([
  THREEJS_READYPLAYER_ME_MODEL_URL,
  THREEJS_XBOT_MODEL_URL
]);

const murlanCharacterRuntimeFallbacks = (...urls) =>
  Object.freeze([
    ...urls,
    ...MURLAN_CHARACTER_SAFE_FALLBACK_MODEL_URLS
  ].filter(Boolean));

export const MURLAN_CHARACTER_THEMES = Object.freeze([
  {
    id: 'rpm-current',
    label: 'Current Avatar',
    source: 'three.js examples',
    license: 'MIT examples bundle',
    price: 0,
    description: 'Ready Player Me seated avatar with full PBR texture maps.',
    url: 'https://threejs.org/examples/models/gltf/readyplayer.me.glb',
    modelUrls: ['https://threejs.org/examples/models/gltf/readyplayer.me.glb', THREEJS_READYPLAYER_ME_MODEL_URL],
    thumbnail: khronosThumb('ReadyPlayerMe'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'royalDenim',
    hairColor: 0x24150f,
    eyeColor: 0x2f5d7c,
    skinTone: 0xd9a27d
  },
  {
    id: 'rpm-67d411',
    label: 'RPM 67d411',
    source: 'Ready Player Me public GLB',
    license: 'Check Ready Player Me terms',
    price: 390,
    description: 'Ready Player Me public avatar seated with the same Ludo rig logic and motion set.',
    url: 'https://models.readyplayer.me/67d411b30787acbf58ce58ac.glb',
    modelUrls: [
      'https://models.readyplayer.me/67d411b30787acbf58ce58ac.glb',
      'https://api.readyplayer.me/v1/avatars/67d411b30787acbf58ce58ac.glb',
      'https://avatars.readyplayer.me/67d411b30787acbf58ce58ac.glb'
    ],
    thumbnail: khronosThumb('ReadyPlayerMe67d411'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'casinoCheck',
    hairColor: 0x14100c,
    eyeColor: 0x5a3d2b,
    skinTone: 0xc78f68
  },
  {
    id: 'rpm-67f433',
    label: 'RPM 67f433',
    source: 'Ready Player Me public GLB',
    license: 'Check Ready Player Me terms',
    price: 420,
    description: 'Ready Player Me public avatar aligned to default seated scale/orientation for Ludo.',
    url: 'https://models.readyplayer.me/67f433b69dc08cf26d2cf585.glb',
    modelUrls: [
      'https://models.readyplayer.me/67f433b69dc08cf26d2cf585.glb',
      'https://api.readyplayer.me/v1/avatars/67f433b69dc08cf26d2cf585.glb',
      'https://avatars.readyplayer.me/67f433b69dc08cf26d2cf585.glb'
    ],
    thumbnail: khronosThumb('ReadyPlayerMe67f433'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'linenStreet',
    hairColor: 0x2c1b12,
    eyeColor: 0x406a45,
    skinTone: 0xe0b18d
  },
  {
    id: 'rpm-67e1b5',
    label: 'RPM 67e1b5',
    source: 'Ready Player Me public GLB',
    license: 'Check Ready Player Me terms',
    price: 450,
    description: 'Ready Player Me public avatar using the same seated helper offsets and animation logic.',
    url: 'https://models.readyplayer.me/67e1b51ae11c93725e4395c9.glb',
    modelUrls: [
      'https://models.readyplayer.me/67e1b51ae11c93725e4395c9.glb',
      'https://api.readyplayer.me/v1/avatars/67e1b51ae11c93725e4395c9.glb',
      'https://avatars.readyplayer.me/67e1b51ae11c93725e4395c9.glb'
    ],
    thumbnail: khronosThumb('ReadyPlayerMe67e1b5'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'jacquardNight',
    hairColor: 0x3a2418,
    eyeColor: 0x364f7d,
    skinTone: 0xb87957
  },
  {
    id: 'webgl-vietnam-human',
    label: 'Vietnam Human',
    source: 'hmthanh/3d-human-model GitHub',
    license: 'Check repository license',
    price: 480,
    description: 'Open WebGL GLB humanoid seated with the same Ludo scale/orientation/motion rig.',
    url: 'https://raw.githubusercontent.com/hmthanh/3d-human-model/main/TranThiNgocTham.glb',
    thumbnail: khronosThumb('VietnamHuman'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'softFleece',
    hairColor: 0x120d0a,
    eyeColor: 0x33271e,
    skinTone: 0xd39a72
  },
  {
    id: 'webgl-ai-teacher',
    label: 'AI Teacher',
    source: 'Surbh77/AI-teacher GitHub',
    license: 'Check repository license',
    price: 510,
    description: 'Open-source AI Teacher avatar adapted to the default seated pose pipeline.',
    url: 'https://raw.githubusercontent.com/Surbh77/AI-teacher/main/avatar.glb',
    thumbnail: khronosThumb('AiTeacher'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'patternedRed',
    hairColor: 0x231915,
    eyeColor: 0x3d5f73,
    skinTone: 0xc88b64
  },
  {
    id: 'webgl-ai-teacher-1',
    label: 'AI Teacher 1',
    source: 'Surbh77/AI-teacher GitHub',
    license: 'Check repository license',
    price: 540,
    description: 'Second AI Teacher avatar using the same seated helpers and interaction logic.',
    url: 'https://raw.githubusercontent.com/Surbh77/AI-teacher/main/avatar1.glb',
    thumbnail: khronosThumb('AiTeacher1'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'mixedDenim',
    hairColor: 0x0f0b08,
    eyeColor: 0x4c3425,
    skinTone: 0xe3b08b
  },
  {
    id: 'webgl-thanh-human',
    label: 'Thanh Human',
    source: 'hmthanh/3d-human-model GitHub',
    license: 'Check repository license',
    price: 560,
    description: 'Distinct full-body WebGL humanoid model used as a non-Soldier Pool Royale shooter option.',
    url: 'https://raw.githubusercontent.com/hmthanh/3d-human-model/main/Thanh.glb',
    thumbnail: khronosThumb('ThanhHuman'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'linenStreet',
    hairColor: 0x160f0a,
    eyeColor: 0x334d5f,
    skinTone: 0xd09a73
  },
  {
    id: 'sketchfab-agent-47',
    label: 'Agent 47',
    source: 'Sketchfab: Agent 47 (rigged+face morphs) by Veterock (@windofglass)',
    license: 'CC BY-NC 4.0; attribution required; non-commercial use only',
    price: 760,
    description: 'Sketchfab rigged Mixamo-style character loaded from the converted glTF package and prepared for Murlan Royale seating. Converted glTF folder is installed locally outside git with npm run fetch:murlan-characters.',
    url: '/models/murlan/agent-47-rigged-face-morphs/scene.gltf',
    modelUrls: ['/models/murlan/agent-47-rigged-face-morphs/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/agent-47-riggedface-morphs-1680cad927304bb687d6a9ad5b9dd98a',
    sketchfabUid: '1680cad927304bb687d6a9ad5b9dd98a',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/1680cad927304bb687d6a9ad5b9dd98a/thumbnails/546b0676b5b24eaaa2732e1ee886a938/3ed8cfb016f441be89ee95908ba61de8.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: true,
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'formalHitman',
    hairColor: 0x1a1714,
    eyeColor: 0x6d7f8f,
    skinTone: 0xc89a7b
  },
  {
    id: 'sketchfab-leather-jacket-portrait',
    label: 'Leather Jacket Portrait',
    source: 'Sketchfab: Leather Jacket Portrait by restore50',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 780,
    description: 'Sketchfab human portrait loaded from the converted glTF package and prepared for Murlan Royale seating. Converted glTF folder is installed locally outside git with npm run fetch:murlan-characters.',
    url: '/models/murlan/leather-jacket-portrait/scene.gltf',
    modelUrls: ['/models/murlan/leather-jacket-portrait/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/leather-jacket-portrait-e4b6a08211c746fe932e0d5041d28812',
    sketchfabUid: 'e4b6a08211c746fe932e0d5041d28812',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/e4b6a08211c746fe932e0d5041d28812/thumbnails/15d23a9764644acc84ffff749e907fd0/c916e94be42d4f348f02074e2f1c5a03.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'leatherPortrait',
    hairColor: 0x1d1714,
    eyeColor: 0x4b5563,
    skinTone: 0xc48f69
  },
  {
    id: 'sketchfab-suede-gentleman',
    label: 'Suede Gentleman',
    source: 'Sketchfab: Seated Gentleman in Suede Jacket by restore50',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 820,
    description: 'Sketchfab seated gentleman character loaded from the converted glTF package and prepared for Murlan Royale seating. Converted glTF folder is installed locally outside git with npm run fetch:murlan-characters.',
    url: '/models/murlan/seated-gentleman-suede-jacket/scene.gltf',
    modelUrls: ['/models/murlan/seated-gentleman-suede-jacket/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/seated-gentleman-in-suede-jacket-8b1101c090d4454caf9f311b3c008946',
    sketchfabUid: '8b1101c090d4454caf9f311b3c008946',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/8b1101c090d4454caf9f311b3c008946/thumbnails/96241a6fb9f546e78c8a55f24c07902d/e9241bd5d7f944d4b3023353c43698e5.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'suedeGentleman',
    hairColor: 0x2b1d16,
    eyeColor: 0x5f4b32,
    skinTone: 0xcf9b73
  },
  {
    id: 'sketchfab-red-hibiscus-hair',
    label: 'Red Hibiscus Hair',
    source: 'Sketchfab: Red Hibiscus in the Hair by restore50',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 840,
    description: 'Sketchfab hibiscus portrait character loaded from the converted glTF package and prepared for Murlan Royale seating. Converted glTF folder is installed locally outside git with npm run fetch:murlan-characters.',
    url: '/models/murlan/red-hibiscus-in-the-hair/scene.gltf',
    modelUrls: ['/models/murlan/red-hibiscus-in-the-hair/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/red-hibiscus-in-the-hair-dc65f86920814a4296f930e7d85ab314',
    sketchfabUid: 'dc65f86920814a4296f930e7d85ab314',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/dc65f86920814a4296f930e7d85ab314/thumbnails/bb410be41f4144e79bfadf92bb5208cf/16fb3067511248108ce4cd89ffc22fab.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'hibiscusPortrait',
    hairColor: 0x140f0b,
    eyeColor: 0x3f5f4f,
    skinTone: 0xd7a17d
  },
  {
    id: 'sketchfab-casual-confidence',
    label: 'Casual Confidence',
    source: 'Sketchfab: Casual Confidence by restore50',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 860,
    description: 'Sketchfab casual human character loaded from the converted glTF package and prepared for Murlan Royale seating. Converted glTF folder is installed locally outside git with npm run fetch:murlan-characters.',
    url: '/models/murlan/casual-confidence/scene.gltf',
    modelUrls: ['/models/murlan/casual-confidence/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/casual-confidence-bff76010d9534241ae6c96a4a46a7959',
    sketchfabUid: 'bff76010d9534241ae6c96a4a46a7959',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/bff76010d9534241ae6c96a4a46a7959/thumbnails/dd7d6f15c0ee42d6a634c4131c35e353/f13d7ed0732d41308e4ddc4967063027.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'casualConfidence',
    hairColor: 0x21150f,
    eyeColor: 0x4a5d6a,
    skinTone: 0xc98f68
  },
  {
    id: 'sketchfab-generic-man-scan',
    label: 'Generic Man Scan',
    source: 'Sketchfab: Generic Man by Scanlab Photogrammetry Inc.',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 880,
    description: 'Runtime-installable photogrammetry/3D-scan human compiled from about 50 scans, added as a realistic Murlan Royale character. Install the converted glTF locally with npm run fetch:murlan-characters.',
    url: '/models/murlan/generic-man-scan/scene.gltf',
    modelUrls: ['/models/murlan/generic-man-scan/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/generic-man-9b4527d4bc524c31bed48054f0c04b71',
    sketchfabUid: '9b4527d4bc524c31bed48054f0c04b71',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/9b4527d4bc524c31bed48054f0c04b71/thumbnails/3f95b2f9f67144fab13a93e55fb3b7ef/5b218d2935384f1186c457f478aab732.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'softFleece',
    hairColor: 0x241a13,
    eyeColor: 0x4a4035,
    skinTone: 0xc6906e
  },
  {
    id: 'sketchfab-thunk3d-human-scan',
    label: 'Thunk3D Human Scan',
    source: 'Sketchfab: Human by Diana Liu (@Diana123456)',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 900,
    description: 'Runtime-installable handheld Thunk3D scanner human scan for a more realistic Murlan Royale character. Install the converted glTF locally with npm run fetch:murlan-characters.',
    url: '/models/murlan/thunk3d-human-scan/scene.gltf',
    modelUrls: ['/models/murlan/thunk3d-human-scan/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/human-a75f10c50fb44a70a1b04bcb49f3d71f',
    sketchfabUid: 'a75f10c50fb44a70a1b04bcb49f3d71f',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/a75f10c50fb44a70a1b04bcb49f3d71f/thumbnails/5f5f61f24f6c445d9a8bd1a830e66a44/873352fa8ad54a24a92802c376bf3efa.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 0.78,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'linenStreet',
    hairColor: 0x1d1712,
    eyeColor: 0x4c3a2e,
    skinTone: 0xd2a07c
  },
  {
    id: 'sketchfab-hoodie-apose-scan',
    label: 'Hoodie A-Pose Scan',
    source: 'Sketchfab: A-Pose - Male Full Body 3D Scan by OverseerThuror',
    license: 'CC BY 4.0; attribution required; commercial use allowed',
    price: 920,
    description: 'Runtime-installable full-body photogrammetry scan of a casual clothed person in A-pose, prepared for Murlan Royale seating. Install the converted glTF locally with npm run fetch:murlan-characters.',
    url: '/models/murlan/hoodie-apose-scan/scene.gltf',
    modelUrls: ['/models/murlan/hoodie-apose-scan/scene.gltf'],
    sourceUrl: 'https://sketchfab.com/3d-models/a-pose-male-full-body-3d-scan-78c2d136f91c46a8a696d5bacfde2daf',
    sketchfabUid: '78c2d136f91c46a8a696d5bacfde2daf',
    sourceFormat: 'sketchfab-converted-gltf',
    installCheck: 'gltf-json',
    thumbnail: 'https://media.sketchfab.com/models/78c2d136f91c46a8a696d5bacfde2daf/thumbnails/637400a332684bc7a2f39c2ea8e8b875/b7f426f091d74bd993ad77f0e70a36c5.jpeg',
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    requiresAttribution: true,
    nonCommercialOnly: false,
    scale: 0.74,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'mixedDenim',
    hairColor: 0x18140f,
    eyeColor: 0x3d352d,
    skinTone: 0xcb9670
  },
  {
    id: 'threejs-xbot-human',
    label: 'Xbot Human',
    source: 'three.js examples',
    license: 'MIT',
    price: 580,
    description: 'Full-body Xbot humanoid from the three.js examples, used as a second new non-Soldier Pool Royale character.',
    url: 'https://threejs.org/examples/models/gltf/Xbot.glb',
    modelUrls: ['https://threejs.org/examples/models/gltf/Xbot.glb', THREEJS_XBOT_MODEL_URL],
    thumbnail: khronosThumb('XbotHuman'),
    runtimeFallbackModelUrls: murlanCharacterRuntimeFallbacks(),
    scale: 1.0,
    seatOffsetY: -0.84,
    seatOffsetZ: -0.22,
    normalizedSeatOffsetY: -0.4,
    normalizedSeatOffsetZ: 0.52,
    seatPitch: 0,
    seatYaw: 0,
    handLift: 1.04,
    clothCombo: 'casinoCheck',
    hairColor: 0x111111,
    eyeColor: 0x2f4b5f,
    skinTone: 0xc9906e
  }
]);
