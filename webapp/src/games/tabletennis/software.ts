  ...
  render(scene: T.Scene, camera: T.Camera) {
    ...
    c.fillStyle =
      scene.background instanceof T.Color
        ...
        : '#0c2833';
    c.fillRect(0, 0, w, h);
    if (scene.background instanceof T.DataTexture) {
      const t = scene.background;
      let bg = this.backgrounds.get(t);
      if (!bg) {
        const im = t.image;
        ...
        const pixels = ctx.createImageData(im.width, im.height);
        for (let n = 0; n < im.width * im.height; n++) {
          for (let k = 0; k < 3; k++) {
            const raw = im.data![n * 4 + k],
              value =
                t.type === T.HalfFloatType
                  ? T.DataUtils.fromHalfFloat(raw)
                  : raw;
            pixels.data[n * 4 + k] =
          ...
          for (
            ...
            if (mat.map && geo.attributes.uv) {
              ...
              if (pixels) {
                ...
                const texAlpha = pixels.data[offset + 3] / 255;
                if (texAlpha < mat.alphaTest) continue;
                if (mat.transparent) alpha *= texAlpha;
                color.setRGB(
