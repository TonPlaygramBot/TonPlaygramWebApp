...
import {prepareBusWheels,collectRollingWheels,rollWheels,type RollingWheel} from '../rollingWheels';
type BusActor={root:T.Group;rear?:T.Object3D;passengers:T.Group;doors:T.Object3D[];wheels:RollingWheel[];driver:T.Group};
...
export class BusVisuals {
 ...
 update(state:State,p:Point,dt:number,battery=false,firstPersonId?:string|null){
  ...
  for(const car of selected){let a=this.actors.get(car.id);if(!a){const root=clone(this.source) as T.Group;root.name=car.id;const passengers=new T.Group(),driver=this.person((car.livery||0)*2,4);driver.position.set(-.64,1,-7.7);root.add(driver,passengers);
    ...
    prepareBusWheels(root);const wheels=collectRollingWheels(root),doors:T.Object3D[]=[];root.traverse(o=>{if(o.name.startsWith('Door_')){o.userData.rest=o.position.clone();doors.push(o);}});
