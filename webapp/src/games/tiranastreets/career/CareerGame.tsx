...
export function CareerGame({onExit}:{onExit:()=>void}){
 ...
 const drag=(e:PE<HTMLDivElement>)=>{if(!runtime.current||view?.paused||view?.ride||view?.overlook)return;const r=e.currentTarget.getBoundingClientRect();let x=(e.clientX-r.left-r.width/2)/35,y=(e.clientY-r.top-r.height/2)/35;const n=Math.max(1,Math.hypot(x,y));x/=n;y/=n;runtime.current.input.move={x,y:-y};setStick({x:x*30,y:y*30});};
