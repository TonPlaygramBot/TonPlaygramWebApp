...
export function poseForce(root:T.Object3D, anim:string, alive=true) {
  if(!alive)return;
  ...
  const aim=anim==='aim', crouch=anim==='cover', ride=anim==='ride';
  ...
  if(aim || ride || crouch) {
    const y=ride?1.08:aim?1.3:1.02, z=ride?.46:aim?.63:.38;
  ...
  if(crouch||ride) {
    turn('upperleg01.L',-1.05);turn('upperleg01.R',-1.05);
    turn('lowerleg01.L',1.3);turn('lowerleg01.R',1.3);turn('spine02',.15);
  }
  rest.set(root,bases);
}
