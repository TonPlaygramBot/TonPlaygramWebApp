...
export function StreetArsenal({player:p,state,onAction}:{player:Player;state:State;onAction:(a:string)=>void}){
 ...
 const near=!p.carId&&Math.hypot(p.x-state.shop.x,p.z-state.shop.z)<=9;
 const canBuy=near&&p.health>0&&!p.finished&&!p.failed&&wantedStars(p.wanted)===0;
 return <section><p>{near?'Arben · Choose equipment':'Walk to Arben’s shop marker near your starting car to buy equipment.'}</p><p><strong>${p.cash}</strong> street cash · Local to this career, not TPG.</p>
 ...
 <p>Free-roam purchases save on this device. During a job, retry restores the starting loadout; successful completion saves the resulting loadout.</p></section>;
}
