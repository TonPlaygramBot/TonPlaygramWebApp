...
export class ExploreRuntime {
  ...
  readonly audio=new CityAudio();readonly renderer:T.WebGLRenderer;readonly city=new FpsCity();readonly humans=new SharedHumans(undefined,{bikes:false});readonly faces=new FacePanels();
  readonly details:ReturnType<typeof attachEnhancements>;readonly input:CityInput;
  yaw=0; pitch=.3; paused=true;
  private userPaused=false;private overlay=false;private blurred=false;private connected=false;private measuredFPS=0;
  ...
  constructor(private root:HTMLDivElement,private connection:ExploreConnection,private fail:(s:string)=>void,private publish:(s:{fps:number;ready:boolean;paused:boolean;avatarReady:boolean})=>void){
    ...
    this.scene.add(this.city.group,this.humans.group);this.details=attachEnhancements(this.scene,{x:0,z:0},{profile:'fps'});this.details.bindBuildings(this.city.group,[this.city.landmarks.group]);
    ...
    this.renderer.domElement.addEventListener('webglcontextlost',this.lost);window.addEventListener('blur',this.blur);window.addEventListener('focus',this.focus);document.addEventListener('visibilitychange',this.visibility);
    // Base mapped geometry is already built synchronously. Optional furniture or
    // an avatar CDN failure must not indefinitely lock the movement controls.
    this.assetsReady=true;
    this.city.ready.then(()=>{if(!this.dead)this.assetsReady=true;}).catch(e=>fail(String(e)));this.frame=requestAnimationFrame(this.loop);
  }
  private lost=(e:Event)=>{e.preventDefault();this.pause(true);this.fail('Graphics context lost. Leave and reopen Explore.');};
  private blur=()=>{this.blurred=true;this.refreshPause();};
  private focus=()=>{this.blurred=false;this.refreshPause();};
  private visibility=()=>this.refreshPause();
  private refreshPause(){
    this.paused=this.userPaused||this.overlay||this.blurred||!this.connected||document.hidden;
    this.input.setEnabled(!this.paused&&this.visibleReady);this.input.clear();
    if(this.paused)this.connection.controls({...this.input.read(this.yaw,false),x:0,y:0,fast:false,fire:false,brake:true});
    this.publish({fps:this.measuredFPS,ready:this.visibleReady&&this.connected,paused:this.paused,avatarReady:!!this.state&&this.humans.has(`explorer-${this.state.playerId}`)});
  }
  setConnectionReady(value:boolean){this.connected=value;this.refreshPause();}
  setOverlay(value:boolean){this.overlay=value;this.refreshPause();}
  retryCharacters(){this.humans.retryFailed();}

  pause(value:boolean){this.userPaused=value;this.refreshPause();}
  accept(snapshot:ExploreSnapshot){if(!this.state||this.state.id!==snapshot.id){this.shown.clear();const p=snapshot.state.players[snapshot.playerId];if(p){this.yaw=p.heading;this.target.set(p.x,1,p.z);}}this.state=snapshot;}
  ...
  private draw=(now:number)=>{
    ...
    if(s&&p){
      const controls=this.input.read(this.yaw,!!p.carId);if(this.paused||!this.visibleReady)Object.assign(controls,{x:0,y:0,fast:false,brake:true});this.connection.controls(controls);
      ...
      this.humans.update([...players,...ambient],p,s.state.elapsed,dt,false,players.map(n=>n.id));
      ...
      this.visibleReady=this.assetsReady&&this.connected;this.input.setEnabled(this.visibleReady&&!this.paused);
    ...
    this.renderer.render(this.scene,this.camera);this.frames++;this.seconds+=real;if(this.seconds>=1){if(this.humans.errors.length>this.assetErrorCount){this.assetErrorCount=this.humans.errors.length;this.fail(this.humans.errors.at(-1)||'A shared character asset failed');}this.measuredFPS=Math.round(this.frames/this.seconds);this.publish({fps:this.measuredFPS,ready:this.visibleReady,paused:this.paused,avatarReady:this.state?this.humans.has(`explorer-${this.state.playerId}`):false});this.frames=0;this.seconds=0;}
    this.frame=requestAnimationFrame(this.loop);
  };
  dispose(){if(this.dead)return;this.dead=true;cancelAnimationFrame(this.frame);this.observer.disconnect();window.removeEventListener('blur',this.blur);window.removeEventListener('focus',this.focus);document.removeEventListener('visibilitychange',this.visibility);this.renderer.domElement.removeEventListener('webglcontextlost',this.lost);this.input.destroy();this.audio.destroy();this.faces.dispose();this.humans.dispose();this.details.dispose();this.city.dispose();for(const g of this.templates.values())disposeObject(g);this.cars.clear();this.shown.clear();this.renderer.dispose();this.renderer.domElement.remove();}
}
