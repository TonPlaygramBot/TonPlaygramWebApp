...
export class ExploreRuntime {
  ...
  private draw=(now:number)=>{
    ...
    if(s&&p){
      ...
      this.visibleReady=this.networkAvailable&&this.assetsReady&&(p.carId?this.cars.has(p.carId):this.humans.has(`explorer-${s.playerId}`));this.syncInput();
    ...
    this.renderer.render(this.scene,this.camera);this.frames++;this.seconds+=real;if(this.seconds>=1){if(this.humans.errors.length>this.assetErrorCount){this.assetErrorCount=this.humans.errors.length;this.fail(this.humans.errors.at(-1)||'A shared character asset failed');}this.fps=Math.round(this.frames/this.seconds);this.publishStatus();this.frames=0;this.seconds=0;}
