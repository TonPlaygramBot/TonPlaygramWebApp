...
export class ExploreRuntime {
  ...
  constructor(private root:HTMLDivElement,private connection:ExploreConnection,private fail:(s:string)=>void,private publish:(s:{fps:number;ready:boolean})=>void){
    ...
    this.renderer.domElement.addEventListener('webglcontextlost',this.lost);window.addEventListener('blur',this.blur);
  ...
  pause(value:boolean){this.paused=value;this.input.setEnabled(!value&&this.visibleReady);this.input.clear();if(value)this.connection.controls({x:0,y:0,yaw:this.yaw,seq:Date.now()*1000,fast:false,fire:false,brake:true});}
  accept(snapshot:ExploreSnapshot){if(!this.state){const p=snapshot.state.players[snapshot.playerId];if(p){this.yaw=p.heading;this.target.set(p.x,1,p.z);}}this.state=snapshot;}
  ...
  private draw=(now:number)=>{
    ...
    if(s&&p){
      ...
      this.visibleReady=this.assetsReady&&(p.carId?this.cars.has(p.carId):this.humans.has(`explorer-${s.playerId}`));this.input.setEnabled(this.visibleReady&&!this.paused);
    ...
    this.renderer.render(this.scene,this.camera);this.frames++;this.seconds+=real;if(this.seconds>=1){if(this.humans.errors.length>this.assetErrorCount){this.assetErrorCount=this.humans.errors.length;this.fail(this.humans.errors.at(-1)||'A shared character asset failed');}this.publish({fps:Math.round(this.frames/this.seconds),ready:this.visibleReady});this.frames=0;this.seconds=0;}
    this.frame=requestAnimationFrame(this.loop);
  };
  dispose(){if(this.dead)return;this.dead=true;cancelAnimationFrame(this.frame);this.observer.disconnect();window.removeEventListener('blur',this.blur);this.renderer.domElement.removeEventListener('webglcontextlost',this.lost);this.input.destroy();this.audio.destroy();this.faces.dispose();this.humans.dispose();this.details.dispose();this.city.dispose();for(const g of this.templates.values())disposeObject(g);this.cars.clear();this.shown.clear();this.renderer.dispose();this.renderer.domElement.remove();}
}
