...
export function ExploreGame({onExit,sourceGame='streets'}:{onExit:()=>void;sourceGame?:'streets'|'racing'}){
  ...
  const [connectionStatus,setConnectionStatus]=useState<ExploreStatus>('idle'),[attempt,setAttempt]=useState(0),[avatarReady,setAvatarReady]=useState(false);
  ...
  useEffect(()=>{
    setError('');setReady(false);setSnapshot(null);
    ...
    let alive=true;const c=new ExploreConnection(accountId,s=>{if(alive){setSnapshot(s);runtime.current?.accept(s);}},e=>{if(alive)setError(e);},status=>{if(alive){setConnectionStatus(status);runtime.current?.setConnectionReady(status==='ready');if(status!=='ready')setReady(false);}});connection.current=c;
    let r:ExploreRuntime|undefined;try{if(root.current){r=new ExploreRuntime(root.current,c,e=>{if(alive)setError(e);},s=>{if(alive){setFps(s.fps);setReady(s.ready);setPaused(s.paused);setAvatarReady(s.avatarReady);}});runtime.current=r;}}catch(e){setError(e instanceof Error?e.message:'Unable to open 3D view');return()=>{alive=false;c.dispose();};}
  ...
  return <main className="te-game" aria-label="Tirana shared exploration">
    ...
    <header className="te-header"><div><b>{sourceGame==='racing'?'RACING ROYAL':'TIRANA STREETS'}</b><small>EXPLORE · {connectionStatus==='ready'&&snapshot?`${snapshot.members.length}/4 people`:connectionStatus.toUpperCase()} · {fps} FPS</small></div><button onClick={togglePanel}>PEOPLE / CHAT</button><button onClick={()=>{void media.stop();onExit();}}>EXIT</button></header>
    {(error||media.error)&&<p className="te-error" role="alert">{error||media.error}</p>}
    {error&&<div className="te-recovery"><button onClick={()=>void run(()=>connection.current?.retry()||Promise.reject(Error('Sign in first')))}>RECONNECT</button><button onClick={()=>{void media.stop();setAttempt(n=>n+1);}}>RELOAD 3D VIEW</button></div>}
    {ready&&!avatarReady&&<p className="te-avatar-loading" role="status">City connected · loading your existing human model. Movement is available.<button onClick={()=>runtime.current?.retryCharacters()}>RETRY AVATAR</button></p>}
    {!ready&&!error&&<p className="te-loading" role="status">{connectionStatus==='ready'?'Preparing the city view…':'Connecting to the shared game server…'}</p>}
