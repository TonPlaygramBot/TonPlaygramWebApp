...
import {ExploreConnection,type ExploreStatus} from './ExploreConnection';
...
export function ExploreGame({onExit,sourceGame='streets'}:{onExit:()=>void;sourceGame?:'streets'|'racing'}){
  ...
  const [sessionVersion,setSessionVersion]=useState(0),[status,setStatus]=useState<ExploreStatus>('idle');
  ...
  useEffect(()=>{
    ...
    setSnapshot(null);setReady(false);setError('');setPaused(false);setPanel(false);
    let alive=true;const c=new ExploreConnection(accountId,s=>{if(alive){setSnapshot(s);runtime.current?.accept(s);}},e=>{if(alive)setError(e);},stage=>{if(alive){setStatus(stage);runtime.current?.setNetworkAvailable(stage==='connected');}});connection.current=c;
    let r:ExploreRuntime|undefined;try{if(root.current){r=new ExploreRuntime(root.current,c,e=>{if(alive)setError(e);},s=>{if(alive){setFps(s.fps);setReady(s.ready);setPaused(s.paused);}});runtime.current=r;}}catch(e){setError(e instanceof Error?e.message:'Unable to open 3D view');return()=>{alive=false;c.dispose();};}
  ...
  function dragLook(e:PointerEvent<HTMLDivElement>){const p=look.current;if(!p||e.pointerId!==p.id||!runtime.current||paused||panel)return;runtime.current.yaw-=(e.clientX-p.x)*.005;runtime.current.pitch=Math.max(.05,Math.min(.8,runtime.current.pitch+(e.clientY-p.y)*.004));look.current={id:e.pointerId,x:e.clientX,y:e.clientY};}
  function move(e:PointerEvent<HTMLDivElement>){if(!runtime.current||!ready||status!=='connected'||panel||paused)return;const box=e.currentTarget.getBoundingClientRect();let x=(e.clientX-box.left-box.width/2)/38,y=-(e.clientY-box.top-box.height/2)/38;const m=Math.max(1,Math.hypot(x,y));x/=m;y/=m;runtime.current.input.touch.x=x;runtime.current.input.touch.y=y;runtime.current.input.touch.gas=y;}
  function togglePanel(){const open=!panel;setPanel(open);runtime.current?.setOverlay(open);stopMove();}
  ...
  return <main className="te-game" aria-label="Tirana shared exploration">
    ...
    <header className="te-header"><div><b>{sourceGame==='racing'?'RACING ROYAL':'TIRANA STREETS'}</b><small>EXPLORE · {status==='connected'&&snapshot?`${snapshot.members.length}/4 people`:status.toUpperCase()} · {fps} FPS</small></div><button onClick={togglePanel}>PEOPLE / CHAT</button><button onClick={()=>{void media.stop();onExit();}}>EXIT</button></header>
    {(error||media.error)&&<p className="te-error" role="alert">{error||media.error} <button onClick={()=>{void media.stop();setSessionVersion(v=>v+1);}}>RECONNECT</button></p>}
    {status==='reconnecting'&&<p className="te-loading" role="status">Connection interrupted. Rejoining the city…</p>}
    {!ready&&!error&&status!=='reconnecting'&&<p className="te-loading" role="status">Loading the mapped city and your shared human character…</p>}
