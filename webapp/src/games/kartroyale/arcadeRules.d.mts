...
export function boostPads(track: Track): { x: number; z: number; yaw: number; index: number; id: number; width: number; length: number }[];
export function stepBoostPads(r: Racer, track: Track, time: number, previousX?: number, previousZ?: number): void;
