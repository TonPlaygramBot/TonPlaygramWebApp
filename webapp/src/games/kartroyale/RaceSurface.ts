...
export function createRaceSurfaceGeometry(track:Track) {
  ...
  for(let i=0;i<=count;i++)for(const side of ['left','right'] as const){
    const p=sides[side][i%count];positions.push(p.x,ROAD_SURFACE_Y+surfaceHeight(track,p.x,p.z),p.z);normals.push(0,1,0);uvs.push(p.x/3,p.z/3);
  }
  ...
  if(track.terrainMode)geometry.computeVertexNormals();
  return geometry;
}
