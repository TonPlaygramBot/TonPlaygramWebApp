...
export class TiranaScenery extends BaseTiranaScenery {
  ...
  constructor(track: Track) {
    ...
    const lifetime = new THREE.BufferGeometry();
    lifetime.addEventListener('dispose', () => {
      this.enhancements.retire();
      clearAtlas();
    });
    const sentinel = new THREE.Mesh(lifetime, new THREE.MeshBasicMaterial());
    sentinel.visible = false;
    sentinel.name = 'Tirana:async-lifetime';
    ...
    const replacement = replaceLegacyCityLandmarks(
      this.group,
      local,
      'kartroyale'
    );
    this.nativeLandmarks = new NativeLandmarkLayer(local);
    const buildings = raceCityBuildings(track, WORLD.buildings).filter((item) =>
      item.p.some((p) => near(p[0], p[1]))
    );
    this.urbanDetails = new UrbanDetailLayer(
      { ...WORLD, buildings },
      new Set(landmarks.flatMap((l) => (l.buildingId ? [l.buildingId] : [])))
    );
    this.group.add(this.nativeLandmarks.group, this.urbanDetails.group);
    this.enhancements.bindBuildings(this.group, [
      this.nativeLandmarks.group,
      this.urbanDetails.group
    ]);
  ...
  override update(x: number, z: number, performance: boolean) {
    ...
    const camera = root.children.find(
      (o) => o instanceof THREE.PerspectiveCamera
    ) as THREE.PerspectiveCamera | undefined;
    this.enhancements.update(
