...
export function buildRaceCatalog(legacy, routes) {
  const diagnostics = [],
    ...
    cache = new Map();
  try {
    const route = combineMappedLoops(
      routes.find((r) => r.id === 'lana'),
      routes.find((r) => r.id === 'pyramid')
    );
    ...
    const xs = samples.points.map((p) => p.x),
      zs = samples.points.map((p) => p.z);
    const bounds = [
      Math.min(...xs),
      Math.min(...zs),
      Math.max(...xs),
    ...
    const config = {
      ...
      ...samples,
      width: old.width,
      name: 'Lana–Pyramid · Grand',
      bounds,
      center: {
        x: (bounds[0] + bounds[2]) / 2,
        z: (bounds[1] + bounds[3]) / 2
      },
    ...
    diagnostics.push(
      `Grand unavailable: ${e instanceof Error ? e.message : String(e)}`
    );
  }
  return {
    tracks: [...legacy.TRACKS, ...extra],
    cups: [
      ...legacy.CUPS,
      ...extra.map((t) => ({
        name: `${t.name} Cup`,
    ...
    makeTrack(id = 'skanderbeg') {
      ...
      if (id.endsWith('-grand'))
        throw Error(
          'This unvalidated Grand variant is unavailable. Choose a listed circuit.'
        );
