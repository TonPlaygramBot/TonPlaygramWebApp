...
export class UrbanLighting {
 ...
 private shops=nearbyIndex([...STREET_LIFE.storefronts,...NEIGHBOURHOOD.storefronts,...BUSINESS_SIGNS]);
 ...
 private poles=new T.InstancedMesh(this.unit,this.steel,840);
 private bulbs=new T.InstancedMesh(this.unit,this.bulb,280);
 ...
 update(viewer:{x:number;z:number},seconds:number,battery=false){
  ...
  const selected=this.near(viewer,battery?200:480,battery?90:280);this.poles.count=0;this.bulbs.count=0;
  ...
  this.lights.forEach((light,i)=>{const p=selected[i];light.userData.lightAvailable=!!p&&i<(battery?2:8);if(p)light.position.set(p.x+Math.cos(p.yaw)*1.35,6.05,p.z-Math.sin(p.yaw)*1.35);});
