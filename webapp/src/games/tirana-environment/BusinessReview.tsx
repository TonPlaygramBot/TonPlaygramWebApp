...
const views=Object.entries(BUSINESS_BUILDING_PROFILES).filter(([,p])=>p.category!=='hotel-part').map(([id,p])=>{
 const b=WORLD.buildings.find(b=>b.id===id)!;
 const xs=b.p.map(v=>v[0]),zs=b.p.map(v=>v[1]);
 return {name:p.name,x:(Math.min(...xs)+Math.max(...xs))/2,z:(Math.min(...zs)+Math.max(...zs))/2,height:id==='400647876'?44.8:b.h,span:Math.max(Math.max(...xs)-Math.min(...xs),Math.max(...zs)-Math.min(...zs)),yaw:Math.atan2(p.front![0],p.front![1])};
});
export function BusinessReview(){
 ...
 useEffect(()=>{
  ...
  const buildings=new MappedBuildingCells(WORLD.buildings.filter(b=>!NEIGHBOURHOOD_REFERENCE_PROFILES[b.id]),wall,glass,false,false),roads=new UrbanRoadCells(false),trees=new MatureTreeLayer(CANOPY_TREES);
  const facades=new ReferenceFacades(WORLD,new Set(Object.keys(BUSINESS_BUILDING_PROFILES))),signs=new StreetLifeLayer({storefronts:[...BUSINESS_SIGNS,...STREET_LIFE.storefronts],stops:[],fuel:[],advertising:[]} as any,{},true);
