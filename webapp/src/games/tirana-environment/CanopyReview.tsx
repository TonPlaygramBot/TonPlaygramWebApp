...
const VIEWS=[{name:'Lana',x:22,z:429,height:105,distance:180,yaw:.55},{name:'Bulevardi',x:28,z:760,height:85,distance:155,yaw:-.45},{name:'Panorama',x:0,z:330,height:780,distance:900,yaw:.15}];
export function CanopyReview(){
 ...
 useEffect(()=>{
  ...
  const camera=new T.PerspectiveCamera(53,1,.15,3500),controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.dampingFactor=.08;controls.minDistance=25;controls.maxDistance=1600;controls.maxPolarAngle=Math.PI*.47;controls.screenSpacePanning=true;
  ...
  const buildings=new MappedBuildingCells(WORLD.buildings,wall,glass,false,false),trees=new MatureTreeLayer([...CANOPY_TREES,...CITY_COMPLETION.trees]);
  scene.add(landscape.group,roads.group,buildings.group,trees.group);
  ...
  const draw=(now:number)=>{
   try{
    ...
    if(now-labelAt>750){labelAt=now;const loading=roads.group.userData.pendingJobs+buildings.group.userData.pendingJobs;setStatus(`${trees.group.userData.visibleTrunks||0} pemë · ${loading?'duke ngarkuar zonën':quality.current?'850 m gjelbërim':'1.4 km gjelbërim'}`);}
    frame=requestAnimationFrame(draw);
   }catch(e){setError('Pamja nuk u ngarkua: '+String(e));}
  };frame=requestAnimationFrame(draw);
  return()=>{cancelAnimationFrame(frame);observer.disconnect();controls.dispose();roads.dispose();buildings.dispose();trees.dispose();landscape.dispose();scene.traverse(o=>{if(o instanceof T.Mesh)o.geometry.dispose();});for(const m of [wall,glass,grass,stone])m.dispose();renderer.dispose();renderer.domElement.remove();};
 },[]);
...
const root=document.getElementById('tirana-canopy-review-root');if(root)createRoot(root).render(<CanopyReview/>);
