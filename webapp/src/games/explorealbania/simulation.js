...
export const neutralControls = () => ({ steer: 0, throttle: 0, brake: 0 });
...
export function jobsFrom(cityId, day = 0) {
...
export function acceptJob(state, job) {
    if (state.activeJob || job.from !== state.currentCity || Math.abs(state.speed) > 1)
        return false;
    state.activeJob = job;
...
function nearestOnSegment(p, a, b) {
    ...
    return { x, z, distance: Math.hypot(p.x - x, p.z - z) };
}
...
export function remainingDistance(state) {
    if (!state.activeJob)
        return 0;
    ...
    for (let i = 1; i < route.length; i++) {
        ...
        if (q.distance < best) {
            best = q.distance;
            nearest = travelled + Math.hypot(q.x - route[i - 1].x, q.z - route[i - 1].z);
        }
        travelled += len;
        total += len;
    }
...
export function advanceTruck(state, input, dt) {
    ...
    if (state.engine && state.fuel > 0) {
        ...
        state.speed -= Math.sign(state.speed) * Math.min(Math.abs(state.speed), 1.5 * dt);
    if (brake > .5 && Math.abs(state.speed) < .5 && throttle > .15)
        state.speed = Math.max(-4, state.speed - 2.2 * dt);
    ...
    const km = Math.abs(state.speed) * dt * .055;
    state.odometer += km;
    ...
    if (offroad && impactSpeed > 14)
        state.damage = Math.min(100, state.damage + (impactSpeed - 14) * .008 * dt);
    ...
    if (cityDistance < 18)
        state.currentCity = nearestCity.id;
    if (state.activeJob && state.currentCity === state.activeJob.to && cityDistance < 16 && Math.abs(state.speed) < 1.2) {
        ...
        state.money += state.activeJob.payout - damagePenalty;
        state.delivered++;
        state.level = 1 + Math.floor(state.delivered / 3);
        state.notice = `Delivered · +Lek ${(state.activeJob.payout - damagePenalty).toLocaleString()}`;
        state.noticeUntil = state.time + .1;
        state.activeJob = null;
    }
}
export function serviceTruck(state, kind) {
    if (Math.abs(state.speed) > 1)
        return false;
    if (kind === 'fuel') {
        const amount = 100 - state.fuel, cost = Math.ceil(amount * 18);
        if (cost > state.money)
            return false;
        state.money -= cost;
        state.fuel = 100;
        state.notice = `Fueled · Lek ${cost.toLocaleString()}`;
    }
    else {
        const cost = Math.ceil(state.damage * 95);
        if (cost > state.money)
            return false;
        state.money -= cost;
        state.damage = 0;
        state.notice = `Truck repaired · Lek ${cost.toLocaleString()}`;
    }
    state.noticeUntil = state.time + .06;
    return true;
}
