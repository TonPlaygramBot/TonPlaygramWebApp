...
const city = (
  ...
  lon: number,
  industry: string
): City => ({ id, name, lat, lon, industry, ...project(lon, lat) });
...
type RoadInput = [string, string, string, [number, number][]?];
const ROAD_INPUTS: RoadInput[] = [
...
export const ROADS: Road[] = ROAD_INPUTS.map(([a, b, name, mid = []]) => ({
  id: `${a}-${b}`,
  a,
  b,
  name,
...
const segmentLength = (a: {x:number;z:number}, b:{x:number;z:number}) =>
  Math.hypot(b.x-a.x,b.z-a.z);

export const roadLength = (road: Road) => road.points.slice(1).reduce(
  (sum, p, i) => sum + segmentLength(road.points[i], p), 0
);

export function pathBetween(from: string, to: string) {
  const distance = new Map(CITIES.map((c) => [c.id, Infinity]));
  const previous = new Map<string, { city: string; road: Road }>();
  ...
  const legs: { road: Road; reverse: boolean }[] = [];
  let current = to;
  while (current !== from) {
