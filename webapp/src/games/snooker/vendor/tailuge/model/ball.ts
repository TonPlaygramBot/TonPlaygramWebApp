...
type BallAppearance = string
type BallMesh = { trace: { forceTrace(pos: Vector3): void } }
...
export class Ball {
  ...
  constructor(pos, color?, label?: number, appearance?: BallAppearance, id = Ball.id++) {
    this.id = id
    this.pos = pos.clone()
    this.label = label
  ...
  updateMesh(t) {
    // Rendering belongs to Snooker Royal.
  }
