...
import {
  OPEN_WORLD_ASSET_MANIFEST,
  OPEN_WORLD_SYSTEM_NAMES
} from './systems/openWorldAssets';
...
function FirstPersonWeaponAsset({
  ...
  useFrame(() => {
    ...
    groupRef.current.position.z = -0.86 + recoil * 1.55;
    groupRef.current.position.y = -0.42 + recoil * 0.62;
    groupRef.current.rotation.x = 0.03 - recoil * 0.55;
  });
...
function CameraController({ enemies }: { enemies: EnemyRuntime[] }) {
  ...
  const smoothedMove = useRef(new THREE.Vector2(0, 0));
  ...
  useFrame((_, dt) => {
    ...
    const lookSensitivity =
      state.aimSensitivity * (state.vehicleMode === 'helicopter' ? 0.62 : 1);
    yaw.current -= state.input.lookX * lookSensitivity;
    pitch.current = THREE.MathUtils.clamp(
      pitch.current - state.input.lookY * lookSensitivity,
      -1.34,
      1.22
    );
    ...
    while (fixedAccumulator.current >= stableStep) {
      ...
      smoothedMove.current.lerp(
        new THREE.Vector2(state.input.moveX, state.input.moveY),
        flying ? 0.18 : 0.42
      );
      const altitudeInput = state.input.ascend - state.input.descend;
      const move = forward
        .multiplyScalar(smoothedMove.current.y)
        .add(right.multiplyScalar(smoothedMove.current.x));
      if (move.lengthSq() > 1) move.normalize();
      player.current.addScaledVector(move, (flying ? 18 : 8.6) * stableStep);
      if (flying)
        player.current.y +=
          (altitudeInput * 11 +
            smoothedMove.current.y * Math.sin(-pitch.current) * 5.5) *
          stableStep;
  ...
  return (
    <div className="mobile-fps-hud">
      ...
      <label className="mobile-fps-sensitivity">
        <span>AIM {aimSensitivity.toFixed(2)}x</span>
        <input
          type="range"
          min="0.55"
          max="1.8"
      ...
      <div
        ...
        onPointerMove={(event) => {
          ...
          lookLast.current = { x: event.clientX, y: event.clientY };
          setInput({
            lookX: THREE.MathUtils.clamp(dx / 42, -1.2, 1.2),
            lookY: THREE.MathUtils.clamp(dy / 42, -1.2, 1.2)
          });
        }}
        onPointerUp={() => {
          lookId.current = null;
      ...
      {vehicleMode === 'helicopter' && (
        <div className="mobile-fps-heli-controls">
          <button
            type="button"
            onPointerDown={() => setInput({ ascend: 1 })}
            onPointerUp={() => setInput({ ascend: 0 })}
            onPointerCancel={() => setInput({ ascend: 0 })}
          >
            UP
          </button>
          <button
            type="button"
            onPointerDown={() => setInput({ descend: 1 })}
            onPointerUp={() => setInput({ descend: 0 })}
            onPointerCancel={() => setInput({ descend: 0 })}
          >
            DOWN
          </button>
        </div>
      ...
      <div className="mobile-fps-notes">
        {OPEN_WORLD_SYSTEM_NAMES.length} modular systems ·{' '}
        {OPEN_WORLD_ASSET_MANIFEST.length} GLB/GLTF replacement slots · mobile
        LOD ready
      </div>
    </div>
  );
