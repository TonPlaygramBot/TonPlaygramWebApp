...
export function Game(props:ComponentProps<typeof OperationGame>){
 ...
 if(activity==='explore')return <GameModeBoundary onBack={()=>setActivity('operation')}><Suspense fallback={<p>Loading shared exploration…</p>}><Explore onExit={()=>setActivity('operation')}/></Suspense></GameModeBoundary>;
 if(activity==='stories')return <GameModeBoundary onBack={()=>setActivity('operation')}><CareerGame onExit={()=>setActivity('operation')}/></GameModeBoundary>;
 if(activity==='street-career')return <GameModeBoundary onBack={()=>setActivity('operation')}><Suspense fallback={<div className="tc-game"><p className="tc-status">Loading street career…</p></div>}><StreetCareer onExit={()=>setActivity('operation')}/></Suspense></GameModeBoundary>;
