...
import {StreetDetailLayer,type StreetDetailOptions} from '../tirana-street-detail/StreetDetailLayer';
...
export class WorldEnhancements extends ExistingEnhancements{
 ...
 constructor(options:StreetDetailOptions={}){super();this.ground=new GroundDetailLayer(WORLD,options,this.civic.errors);this.group.add(this.ground.group);this.streets=new StreetDetailLayer(WORLD,STREET_DETAILS,this.civic.errors,options);this.group.add(this.shopfronts.group,this.streets.group);}
 bindBuildings(root:T.Object3D,excluded:readonly T.Object3D[]=[]){this.streets.bindBuildings(root,[this.group,...excluded]);}
 override update(seconds:number,camera?:T.PerspectiveCamera,worldViewer?:{x:number;z:number},battery=false){
  ...
  this.shopfronts.update(seconds,viewer,battery);this.streets.update(seconds,viewer,battery);this.ground.update(viewer,battery);
 }
 override retire(){this.ground.retire();this.streets.retire();this.shopfronts.retire();super.retire();}
 override dispose(){this.ground.dispose();this.streets.dispose();this.shopfronts.dispose();super.dispose();}
}
const layers=new WeakMap<T.Scene,WorldEnhancements>();
export function attachEnhancements(scene:T.Scene,origin={x:0,z:0},options:StreetDetailOptions={}){
 const previous=layers.get(scene);if(previous)return previous;
 const layer=new WorldEnhancements({...options,profile:options.profile||(origin.x||origin.z?'fps':'street')});layer.group.position.set(-origin.x,0,-origin.z);scene.add(layer.group);layers.set(scene,layer);return layer;
}
