...
export class WorldEnhancements extends ExistingEnhancements{
 ...
 constructor(options:StreetDetailOptions={}){super();this.streets=new StreetDetailLayer(WORLD,STREET_DETAILS,this.civic.errors,options);this.group.add(this.shopfronts.group,this.streets.group);}
 ...
 override update(seconds:number,camera?:T.PerspectiveCamera,worldViewer?:{x:number;z:number},battery=false){
  ...
  this.shopfronts.update(seconds,viewer,battery);this.streets.update(seconds,viewer,battery);
 }
 override retire(){this.streets.retire();this.shopfronts.retire();super.retire();}
 override dispose(){this.streets.dispose();this.shopfronts.dispose();super.dispose();}
}
