...
export function createSnakeInteractionScene(assets: Record<string, object>, seat = 0, weaponId = REVIEW_WEAPONS[0]) {
  ...
  camera.position.copy(point(4.8, 3.7, -1.5)); camera.lookAt(point(0.2, 1.35, 2.7));
  ...
  const layout = computeDiceThrowLayout({ ...board, seatAnchors: anchors }, seat, 1);
  ...
  let kind = 'dice';
  const duration = () => kind === 'dice' ? SNAKE_DICE_PRESENTATION_MS : motion?.duration ?? 0;
  const start = (next: string) => {
    motion?.dispose(); human.reset(); kind = next; die.position.copy(dieRest); die.quaternion.copy(getDiceOrientationQuaternion(1));
    if (kind === 'dice') {
      motion = createSnakeDiceInteraction(die, dieRest.clone(), { human, startedAt: 0, target: getDiceOrientationQuaternion(6), height: 0.13 });
    } else {
  ...
  return { scene, camera, human, die, weapon, token, start, duration,
    update(time: number) { return motion?.update(time) ?? true; },
    dispose() {
