...
import { CFG, createCue, setCuePose, type ShotState } from '../pages/Games/shared/poolRoyalReferenceHuman.ts';
...
function CharacterPreview() {
  ...
  const live = useRef({ state: 'dragging' as ShotState, view: 'player' as View, paused: false, power: 0.25, yaw: 0, seat: 'A' as PlayerSeat, strikeAt: 0 });
  ...
  useEffect(() => {
    ...
    scene.add(aimLine);
    const liveCue = createCue(); scene.add(liveCue.group);
    // Subdivision preserves the shaft in the software renderer when it crosses
    // the close camera's near plane. The production cue is unchanged.
    liveCue.shaft.geometry.dispose();
    liveCue.shaft.geometry = new THREE.CylinderGeometry(1, 1, 1, 12, 40);
    ...
    const draw = (now: number) => {
      ...
      if (current.paused && !needsRender && current.view === lastView) { raf = requestAnimationFrame(draw); return; }
      ...
      const strokeT = current.state === 'striking' ? THREE.MathUtils.clamp((now - current.strikeAt) / (CFG.strikeTime * 1000), 0, 1) : 0;
      const pull = METRICS.cuePull * current.power * Math.pow(1 - strokeT, 3);
      const cueTip = ballPosition.clone().addScaledVector(forward, -(METRICS.cueGap + pull));
      const cueBack = cueTip.clone().addScaledVector(forward, -METRICS.cueLength).addScaledVector(THREE.Object3D.DEFAULT_UP, METRICS.cueButtLift);
      setCuePose(liveCue, cueBack, cueTip);
      for (const part of [liveCue.shaft, liveCue.ferrule, liveCue.tip]) { part.scale.x = METRICS.cueRadius; part.scale.z = METRICS.cueRadius; }
      liveCue.group.visible = current.state !== 'idle';
      if (!current.paused) players?.update(dt, { activeSeat: current.seat, state: current.state, power: current.power,
        cueBall: ballPosition, aimForward: forward, nowMs: now, cueBack, cueTip });
  ...
  return <div className="pool-preview">
    ...
    <div className="viz-controls" aria-label="Camera view">
      {(['player', 'table', 'bridge'] as View[]).map(value => <button key={value} className="btn" aria-pressed={view === value}
        onClick={() => { setView(value); live.current.view = value; if (value !== 'table') changeState('dragging'); }}>
    ...
    <div className="viz-controls">
      ...
      <label className="form-label">Direction · {direction}°<input className="form-range" type="range" min="-180" max="180" value={direction}
        onChange={event => { const value = +event.target.value; live.current.paused = false; setPaused(false); setDirection(value); live.current.yaw = -value * Math.PI / 180; }} /></label>
    </div>
