...
export default function SuperTuxKartPlayablePreview() {
  ...
  const [hud, setHud] = useState({
    ...
    checkpoint: 0,
    status: "Loading...",
    mode: "playable-preview",
    crash: "",
    weapon: "FIREARM",
    ammo: 999,
    collectedWeapons: ["FIREARM"],
    autoDrive: false,
    boost: 0,
    incomingAlert: ""
  });
  ...
  useEffect(() => {
    ...
    const gameState = { autoDrive: false, boostEnergy: 0, boostActive: false, incomingAlertT: 0, cinematicT: 0 };
    ...
    const onKeyDown = (e) => {
      ...
      if (e.code === "KeyQ") {
        gameState.autoDrive = !gameState.autoDrive;
        hudRef.current.status = gameState.autoDrive ? "Auto Drive engaged" : "Auto Drive off";
      }
      if (e.code === "ShiftLeft" || e.code === "ShiftRight") gameState.boostActive = true;
    };
    ...
    async function start() {
      ...
      const playerCombat = { activeWeapon: "FIREARM", inventory: new Set(["FIREARM"]), defenses: new Set(), ammo: { FIREARM: 999, RIFLE: 45, MISSILE: 2, DRONE: 1, HELICOPTER: 1, JET: 1 }, radar: false };
      ...
      const utilityPickups = ["BOOST_CELL", "AUTO_DRIVE_CHIP", "DEFENSE_RADAR"].map((util, i) => {
        const p = pointOnTrack((i / 3) * TAU + 2.5, selectedTrack, i % 2 === 0 ? 1.2 : -1.2);
        const color = util === "BOOST_CELL" ? 0xff8a4c : util === "AUTO_DRIVE_CHIP" ? 0x58d8ff : 0x85ff9f;
        const node = new THREE.Mesh(new THREE.DodecahedronGeometry(0.3, 0), makeMat(color, 0.4, 0.22));
        node.position.copy(p).add(new THREE.Vector3(0, 0.74, 0));
        node.userData = { utility: util, taken: false };
        scene.add(node);
        return node;
      });
      ...
      const animate = () => {
        ...
        if (!(input.keys.ShiftLeft || input.keys.ShiftRight)) gameState.boostActive = false;
        if (gameState.autoDrive) {
          const target = pointOnTrack(angleOnTrack(player.pos, selectedTrack) + 0.42, selectedTrack, 0);
          const dir = target.sub(player.pos).normalize();
          const desiredYaw = yawFromForward(dir);
          const delta = Math.atan2(Math.sin(desiredYaw - player.yaw), Math.cos(desiredYaw - player.yaw));
          input.steer = clamp(-delta * 1.3, -0.9, 0.9);
          input.accel = 0.75;
        }
        if (gameState.boostActive && gameState.boostEnergy > 0) {
          input.accel = Math.max(input.accel, 1);
          gameState.boostEnergy = Math.max(0, gameState.boostEnergy - dt * 22);
          player.speed = Math.min(player.speed + dt * 8.5, 23);
        }
        ...
        for (const ai of [ai1, ai2, ai3, ai4]) {
          ...
          if (d < 18 && Math.random() < dt * 0.7) {
            ...
            if (playerCombat.radar) {
              hudRef.current.incomingAlert = "Incoming missile threat!";
              gameState.incomingAlertT = 1.1;
            }
          }
        }
        ...
        defensePickups.forEach((pickup) => {
          ...
          if (pickup.position.distanceTo(player.pos) < 1.5) {
            ...
            if (pickup.userData.defense === "MISSILE_RADAR" || pickup.userData.defense === "DRONE_RADAR") playerCombat.radar = true;
            hudRef.current.status = `Defense online: ${pickup.userData.defense}`;
          }
        });
        utilityPickups.forEach((pickup) => {
          if (pickup.userData.taken) return;
          pickup.rotation.y += dt * 1.4;
          pickup.position.y += Math.sin(now * 0.003 + pickup.position.x) * 0.0009;
          if (pickup.position.distanceTo(player.pos) < 1.4) {
            pickup.userData.taken = true;
            pickup.visible = false;
            if (pickup.userData.utility === "BOOST_CELL") {
              gameState.boostEnergy = Math.min(100, gameState.boostEnergy + 55);
              hudRef.current.status = "Boost cell collected";
            } else if (pickup.userData.utility === "AUTO_DRIVE_CHIP") {
              gameState.autoDrive = true;
              hudRef.current.status = "Auto Drive module installed";
            } else {
              playerCombat.radar = true;
              hudRef.current.status = "Defense radar activated";
            }
          }
        });
        ...
        if (gameState.incomingAlertT > 0) gameState.incomingAlertT -= dt;
        if (gameState.incomingAlertT <= 0) hudRef.current.incomingAlert = "";
        ...
        setHud({
          ...
          status: baseStatus,
          crash: crashTextT > 0 ? hudRef.current.crash : "",
          weapon,
          ammo,
          collectedWeapons: Array.from(playerCombat.inventory),
          autoDrive: gameState.autoDrive,
          boost: Math.round(gameState.boostEnergy),
          incomingAlert: hudRef.current.incomingAlert || ""
        });
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, sans-serif", color: "white" }}>
        <div style={{ position: "absolute", top: 10, left: 10, right: 10, display: "flex", justifyContent: "space-between", gap: 8 }}>
          <div style={{ background: "rgba(0,0,0,0.56)", border: "1px solid rgba(255,255,255,0.16)", borderRadius: 16, padding: "10px 12px", boxShadow: "0 12px 30px rgba(0,0,0,0.22)", maxWidth: 540 }}>
            ...
            <div style={{ fontSize: 11, opacity: 0.9 }}>Auto: {hud.autoDrive ? "ON" : "OFF"} · Boost: {hud.boost}%</div>
            {hud.incomingAlert && <div style={{ marginTop: 2, fontSize: 12, fontWeight: 900, color: "#ff8b8b" }}>{hud.incomingAlert}</div>}
