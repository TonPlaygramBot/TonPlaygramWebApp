...
const WEAPON_MODELS = [
  { id: "FIREARM", models: ["https://static.poly.pizza/3b53f0fe-f86e-451c-816d-6ab9bd265cdc.glb", "https://static.poly.pizza/9e728565-67a3-44db-9567-982320abff09.glb"] },
  { id: "RIFLE", models: ["https://static.poly.pizza/b3e6be61-0299-4866-a227-58f5f3fe610b.glb", "https://static.poly.pizza/fb8ae707-d5b9-4eb8-ab8c-1c78d3c1f710.glb"] },
  { id: "MISSILE", models: ["https://cdn.jsdelivr.net/gh/srcejon/sdrangel-3d-models@main/atlas_v.glb"] },
  { id: "DRONE", models: ["https://cdn.jsdelivr.net/gh/srcejon/sdrangel-3d-models@main/drone.glb"] },
  { id: "HELICOPTER", models: ["https://cdn.jsdelivr.net/gh/srcejon/sdrangel-3d-models@main/helicopter.glb"] },
  { id: "JET", models: ["https://cdn.jsdelivr.net/gh/srcejon/sdrangel-3d-models@main/f15.glb"] }
];
const WEAPON_PICKUP_VISUALS = {
  FIREARM: { size: 1.35, lift: 0.86 },
  RIFLE: { size: 1.42, lift: 0.9 },
  MISSILE: { size: 1.18, lift: 0.88 },
  DRONE: { size: 1.28, lift: 0.9 },
  HELICOPTER: { size: 1.3, lift: 0.94 },
  JET: { size: 1.35, lift: 0.95 }
};
...
export default function SuperTuxKartPlayablePreview() {
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      const WEAPON_MODEL_BY_ID = Object.fromEntries(WEAPON_MODELS.map((item) => [item.id, item.models]));
      const weaponTemplates = {};
      await Promise.all(WEAPON_PICKUPS.map(async (weapon) => {
        const modelUrls = WEAPON_MODEL_BY_ID[weapon] || [];
      ...
      const makeWeaponPickupMesh = (weapon) => {
        ...
        if (model) {
          ...
          const visual = WEAPON_PICKUP_VISUALS[weapon] || { size: 1.2, lift: 0.88 };
          fitVehicleModel(pickup, { targetLength: visual.size, lift: 0.03, yaw: 0 });
          return pickup;
        }
      ...
      const pickups = WEAPON_PICKUPS.map((weapon, i) => {
        ...
        weaponPickup.position.copy(p).add(new THREE.Vector3(0, (WEAPON_PICKUP_VISUALS[weapon]?.lift ?? 0.88), 0));
        weaponPickup.userData = { weapon, taken: false, baseY: weaponPickup.position.y };
      ...
      const animate = () => {
        ...
        pickups.forEach((pickup) => {
          ...
          pickup.position.y = (pickup.userData.baseY ?? 0.88) + Math.sin(now * 0.004 + pickup.position.x) * 0.05;
