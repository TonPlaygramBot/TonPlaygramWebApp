...
async function safeLoadGltf(dracoDecoderPath, url, label) {
  try {
    return await loadGltf(dracoDecoderPath, url);
  } catch (err) {
    console.warn(`GLTF load failed (${label})`, url, err);
    return null;
  }
}
...
async function tryLoadVehicleModel(dracoDecoderPath, urls) {
  for (const url of urls) {
    const gltf = await safeLoadGltf(dracoDecoderPath, url, "vehicle");
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      const animate = () => {
        ...
        console.error("GoCrazy startup fatal error:", fatal);
        createProceduralTrack(scene, selectedTrack);
        setHud((h) => ({ ...h, status: "Fallback mode: assets failed to load", mode: "playable-preview" }));
      }
    }
    start();
