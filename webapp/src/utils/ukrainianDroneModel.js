...
export function normalizeExactUkrainianDroneObject (model, targetLength = 4.2) {
  ...
  if (Number.isFinite(maxLength) && maxLength > 0) {
    ...
    model.position.y += -fittedBox.min.y + targetLength / 3
    model.updateMatrixWorld?.(true)
  }
  return model
}
      ...
      try {
        for (const source of UKRAINIAN_DRONE_SOURCES) {
          try {
            ...
            return normalizeExactUkrainianDroneObject(configureUkrainianDroneModel(gltf.scene, renderer))
          } catch (error) {
            lastError = error
