...
export const TABLE_SHAPE_OPTIONS = Object.freeze([
  ...
  },
  {
    ...
    createShapes: ({ radius, scaleFactor }) => {
      ...
      const outerHalfX = radius * 1.06;
      const outerHalfY = radius;
      const clothHalfX = outerHalfX * 0.8;
      const clothHalfY = outerHalfY * 0.8;
      const innerHalfX = outerHalfX * 0.86;
      const innerHalfY = outerHalfY * 0.86;
      const topShape = createRoundedRectangleShape(outerHalfX * 2, outerHalfY * 2, 0.12 * factor);
      const feltShape = createRoundedRectangleShape(clothHalfX * 2, clothHalfY * 2, 0.08 * factor);
      const rimInnerShape = createRoundedRectangleShape(innerHalfX * 2, innerHalfY * 2, 0.1 * factor);
      return { topShape, feltShape, rimInnerShape, feltRadius: Math.min(clothHalfX, clothHalfY) };
    }
  }
]);
