...
export type Rig = Record<string, any>;
...
export function palmMarker(rig: Rig, side: 'right' | 'left') {
  ...
  if (mid) marker.position.copy(hand.worldToLocal(world(mid))).multiplyScalar(.76);
  marker.name = side + 'PalmContact';
  hand.add(marker);
  return marker;
}
...
export function solveArm(rig: Rig, side: 'right' | 'left', palm: THREE.Object3D, target: THREE.Vector3, handQ?: THREE.Quaternion, lean = false) {
  ...
  const hand = rig[side + 'Hand'] as THREE.Bone;
  if (!upper || !lower || !hand || !palm) return;
  ...
  aim.normalize();
  const bend = V(side === 'right' ? -.16 : .16, -1, -.1).applyQuaternion(rig.hips.parent.getWorldQuaternion(Q()));
