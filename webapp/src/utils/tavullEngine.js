...
const canLand = (point, color) =>
  !point.color || point.color === color || point.count === 1;
...
const destinationForBar = (color, die) =>
  color === WHITE ? 24 - die : die - 1;
...
export const getSingleDieMoves = (state, color, die) => {
  ...
  for (let i = 0; i < 24; i += 1) {
    ...
    if (dest >= 0 && dest < 24) {
      if (canLand(state.points[dest], color))
    ...
    if (color === WHITE && dest < 0) {
      const exact = i - die === -1;
      if (exact || !hasHigherHomeChecker(state, color, i))
...
const applyUncheckedMove = (state, color, move) => {
...
export const applyMove = (state, color, move) => {
  if (
    !move ||
    !getSingleDieMoves(state, color, move.die).some(
      (legal) => legal.from === move.from && legal.to === move.to
    )
  ) {
    throw new Error('Illegal Backgammon move');
  }
  return applyUncheckedMove(state, color, move);
};

export const expandDice = (dice) => {
  if (
    !Array.isArray(dice) ||
    ![0, 1, 2, 3, 4].includes(dice.length) ||
    dice.some((die) => !Number.isInteger(die) || die < 1 || die > 6)
  )
    throw new Error('Invalid dice');
  if (dice.length > 2 && !dice.every((die) => die === dice[0]))
    throw new Error('Invalid doubles');
...
export const collectTurnSequences = (state, color, dice) => {
  ...
  const recurse = (currentState, diceLeft, line = [], used = []) => {
    ...
    options.forEach((mv) =>
      recurse(
        applyUncheckedMove(currentState, color, mv),
        rest,
        [...line, mv],
        [...used, die]
      )
  ...
  const maxUsed = sequences.reduce(
    (max, s) => Math.max(max, s.usedDice.length),
    0
  );
...
export const pickAiSequence = (state, dice, color = BLACK) => {
  const aiSequences = collectTurnSequences(state, color, dice);
  if (!aiSequences.length) return null;
  // Score each legal outcome once. The previous unbounded reply search evaluated
  // every doubles permutation for all 36 replies on the phone's render thread.
  let best = aiSequences[0];
  let bestScore = -Infinity;
  const seen = new Set();
  for (const sequence of aiSequences) {
    const key = JSON.stringify(sequence.resultingState);
    if (seen.has(key)) continue;
    seen.add(key);
    const score =
      sequence.resultingState.off[color] === 15
        ? Infinity
        : scorePosition(sequence.resultingState, color);
    if (score > bestScore) {
      best = sequence;
      bestScore = score;
    }
