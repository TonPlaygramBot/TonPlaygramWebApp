    ...
    <main>
      {error ? <div className="wall-profile-error"><strong>Profile not found</strong><p>{error}</p><Link to="/wall">Return to the wall</Link></div> : !profile ? <div className="wall-profile-loading">Loading profile…</div> : <>
        <section className="wall-profile-card">
          ...
          <div className="wall-profile-identity"><WallAvatar name={profile.name} src={profile.avatar} /><div><h1>{profile.name} <ShieldCheck aria-label="Community member" /></h1><p>{profile.bio || 'Sharing moments with the TonPlayGram community.'}</p></div></div>
