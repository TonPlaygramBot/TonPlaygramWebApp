...
const getStaffRole = () => localStorage.getItem('flamingoRole') || 'USER';
...
function BottomNavigation() {
  const items = [['/flamingo', Home, 'Kreu'], ['/flamingo/live', Radio, 'Live'], ['/flamingo/groups', Users, 'Grupet'], ['/flamingo/map', Map, 'Harta'], ['/flamingo/profile', CircleUserRound, 'Profili']] as const;
...
function Shell({ children, title, back = false, noNav = false }: { children: ReactNode; title?: string; back?: boolean; noNav?: boolean }) {
  ...
  return <div className="flamingo"><TopBar title={title} back={back} />{!online && <div className="fr-offline" role="status"><Smartphone /> Pa internet • po shfaqen të dhënat e fundit të ruajtura</div>}<main className="fr-main">{children}</main>{!noNav && <BottomNavigation />}<ReportButton /><Toast /></div>;
}
...
function UpdateCard({ item }: { item: LiveUpdate }) { return <article className={`fr-update ${item.type === 'URGJENTE' ? 'urgent' : ''}`}><time>{item.time}</time><div><div className="fr-update-meta"><span className={badgeClass(item.type)}>{item.type}</span><span className={item.verified ? 'fr-source verified' : 'fr-source'}>{item.verified && <ShieldCheck />} {item.authorRole} • {item.author}</span></div><p>{item.text}</p></div></article>; }
...
function ProtestDetail() {
  const { id } = useParams(); const event = protests.find(x => x.id === id); const [tab, setTab] = useState('Harta'); const { joined, joinProtest } = useFlamingoStore();
...
function CallsPage() { const notify = useFlamingoStore(s => s.notify); const shareRoom = async () => { const data = { title: 'Flamingo Video Wall', text: 'Ndiq videokonferencën live të Flamingo Revolution', url: location.href }; if (navigator.share) await navigator.share(data); else { await navigator.clipboard.writeText(location.href); notify('Lidhja publike e videokonferencës u kopjua.'); } }; return <Shell title="Video Wall • Live" back><div className="fr-cert-call"><ShieldCheck /><div><span>DHOMË PUBLIKE • DERI NË 20 PJESËMARRËS</span><h1>Qendra video e organizatorëve.</h1><p>Pjesëmarrësit lidhen me kamerë e mikrofon; spektatorët mund të hapin dhe shpërndajnë këtë faqe të veçantë.</p><button className="fr-secondary" onClick={shareRoom}><Share2 /> Ndaj lidhjen live</button></div></div><MediaWall compact /><HomeSocialHub /></Shell>; }
...
function NotificationPreferences() { const { notificationLevel, setNotificationLevel } = useFlamingoStore(); const options = [['urgent','Vetëm urgjente'],['official','Urgjente + zyrtare'],['all','Të gjitha'],['none','Asnjë']] as const; return <div className="fr-notification-options">{options.map(([value,label]) => <label key={value}><input type="radio" name="notification-level" value={value} checked={notificationLevel === value} onChange={() => setNotificationLevel(value)} /><span>{label}</span></label>)}</div>; }
function NotFound() { return <Shell title="Nuk u gjet" back><section className="fr-empty"><AlertTriangle /><h1>Faqja nuk u gjet</h1><p>Lidhja mund të jetë e vjetër ose përmbajtja mund të jetë hequr.</p><Link className="fr-primary" to="/flamingo">Kthehu te Kreu</Link></section></Shell>; }
function ProfilePage() { return <Shell title="Profili"><div className="fr-profile"><div className="fr-avatar">AM</div><h1>Arta M.</h1><span className="fr-badge">✓ ANËTAR I CERTIFIKUAR</span><p>Të dhënat e kontaktit nuk shfaqen publikisht.</p></div><div className="fr-menu">{[[Globe2, 'Kontributet online', '/flamingo/digital'], [Video, 'Media Wall', '/flamingo/media'], [Send, 'Njoftimet Telegram', '/flamingo/telegram'], [Users, 'Grupet e mia', '/flamingo/groups'], [Video, 'Krijo thirrje video', '/flamingo/calls'], [ClipboardCheck, 'Detyrat e mia', '/flamingo/tasks'], [FileText, 'Dokumentet', '/flamingo/documents'], [LayoutDashboard, 'Paneli i organizatorit', '/flamingo/admin']].map(([Icon, label, to]) => <Link key={String(label)} to={String(to)}><Icon /><span>{String(label)}</span><ChevronRight /></Link>)}</div><section className="fr-panel fr-notification-settings"><h2>Preferencat e njoftimeve</h2><p>Zgjidh vetëm nivelin që të duhet.</p><NotificationPreferences /></section><section className="fr-safety"><ShieldCheck /><div><strong>Privacy-first</strong><p>Vendndodhja, telefoni dhe email-i yt nuk publikohen.</p></div></section></Shell>; }

function ReportPage() {
  const nav = useNavigate(); const notify = useFlamingoStore(s => s.notify); const [anonymous, setAnonymous] = useState(true); const [step, setStep] = useState(1); const [category, setCategory] = useState('Problem organizativ');
  ...
  return <Shell title="Denonco në siguri" back noNav><div className="fr-report-hero"><LockKeyhole /><span className="fr-kicker">KANAL I MBROJTUR</span><h1>Fjala jote ka peshë.<br /><em>Identiteti yt, jo.</em></h1><p>Zgjidh çfarë të ndash. Mos dërgo të dhëna që mund të rrezikojnë ty apo persona të tjerë.</p></div><div className="fr-steps"><span className={step >= 1 ? 'active' : ''}>1</span><i /><span className={step >= 2 ? 'active' : ''}>2</span><i /><span className={step >= 3 ? 'active' : ''}>3</span></div><form className="fr-form fr-secure-form" onSubmit={submit}>
    ...
    <label>Kategoria<select value={category} onChange={e => setCategory(e.target.value)}>{['Nevojë mjekësore', 'Person i humbur', 'Problem organizativ', 'Objekt i humbur', 'Informacion i gabuar', 'Tjetër'].map(x => <option key={x}>{x}</option>)}</select></label>
