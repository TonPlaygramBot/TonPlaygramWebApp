...
import {
  ...
  ClipboardCheck, Clock3, Download, ExternalLink, EyeOff, FileText, Globe2,
  Home, Info, LayoutDashboard, LockKeyhole, Map, MapPin, Megaphone,
  MessageCircle, Plus, Radio, Search, Send, Share2, ShieldCheck, Siren, Smartphone,
  Users, X,
} from 'lucide-react';
...
function QuickGrid() {
  return <div className="fr-home-grid">
    ...
    <Link to="/flamingo/groups"><span><Users /></span><strong>Grupet</strong><small>Gjej rolin tënd</small><ChevronRight /></Link>
  </div>;
}
...
function DigitalPage() { const notify = useFlamingoStore(s => s.notify); const [country, setCountry] = useState(''); return <Shell title="Kontribuo online" back><div className="fr-page-intro digital"><span className="fr-kicker"><Globe2 /> PËR SHQIPTARËT KUDO</span><h1>Nuk je në shesh?<br /><em>Je ende pjesë e zërit.</em></h1><p>Veprime të vogla, të koordinuara dhe të verifikuara. Pa spam, pa gjuhë urrejtjeje, pa rrezikuar askënd.</p></div><div className="fr-digital-steps">{digitalActions.map(({ icon: Icon, title, text, action }, i) => <article key={title}><span>0{i + 1}</span><div className="fr-digital-icon"><Icon /></div><h2>{title}</h2><p>{text}</p><button onClick={() => notify(`${action}: u shtua në detyrat e tua.`)}>{action}<ChevronRight /></button></article>)}</div><section className="fr-diaspora"><Globe2 /><div><span className="fr-kicker">RRJETI I DIASPORËS</span><h2>Lidhu me qytetin tënd</h2><p>Merr njoftime në gjuhën dhe zonën tënde.</p></div><label><span>Shteti ku jeton</span><select value={country} onChange={e => setCountry(e.target.value)}><option value="">Zgjidh shtetin</option><option>Itali</option><option>Greqi</option><option>Gjermani</option><option>Mbretëri e Bashkuar</option><option>SHBA</option><option>Tjetër</option></select></label><button disabled={!country} onClick={() => notify(`U regjistrove në rrjetin e diasporës: ${country}.`)}>Bashkohu me rrjetin</button></section><section className="fr-safety"><ShieldCheck /><div><strong>Kodi digjital</strong><p>Ndaj burime, jo thashetheme. Fshih fytyrat pa pëlqim. Mos publiko vendndodhje private.</p></div></section></Shell>; }
...
function MapPage() { const [selected, setSelected] = useState<(typeof locations)[number] | null>(null); return <Shell title="Harta"><p className="fr-map-note"><ShieldCheck />Harta shfaq vetëm pika publike organizative.</p><RoutePanel /><div className="fr-map" role="img" aria-label="Hartë e pikave publike"><span className="road r1" /><span className="road r2" /><span className="plaza">SHESHI<br />SKËNDERBEJ</span>{locations.map((x, i) => <button aria-label={x.name} className={`fr-pin pin-${i}`} style={{ left: `${x.x}%`, top: `${x.y}%` }} onClick={() => setSelected(x)} key={x.id}>{x.type === 'Ndihmë' ? <Siren /> : <MapPin />}</button>)}</div>{selected && <div className="fr-sheet"><div className="fr-sheet-handle" /><button className="fr-sheet-close" onClick={() => setSelected(null)}><X /></button><span className="fr-badge">{selected.type}</span><h2>{selected.name}</h2><p>{selected.description}</p><div><strong>Statusi</strong><span>{selected.status}</span></div><div><strong>Orari</strong><span>{selected.hours}</span></div></div>}</Shell>; }
