...
function openMediaDb() { return new Promise<IDBDatabase>((resolve, reject) => { if (!('indexedDB' in window)) return reject(new Error('IndexedDB unavailable')); const request = indexedDB.open(DB_NAME, 2); request.onupgradeneeded = () => { const store = request.result.objectStoreNames.contains(STORE_NAME) ? request.transaction?.objectStore(STORE_NAME) : request.result.createObjectStore(STORE_NAME, { keyPath: 'id' }); store?.clear(); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); }
...
export default function MediaWall({ compact = false, profileAccountId = '', showComposer = true, showHeading = true }: { compact?: boolean; profileAccountId?: string; showComposer?: boolean; showHeading?: boolean }) {
  ...
  const visiblePosts = profileAccountId ? posts.filter(post => post.authorAccountId === profileAccountId) : posts;
  ...
  return <section className={compact ? 'fr-wall compact' : 'fr-wall'}>
    ...
    {showHeading && <div className="fr-wall-heading"><div><span className="fr-kicker">TONPLAYGRAM COMMUNITY</span><h1>TonPlayGram Wall</h1><p>Share updates, photos, videos and documents with the whole community.</p></div><span className="fr-wall-live"><i /> LIVE</span></div>}
    <div className="fr-discover"><Search /><span>Kërko në komunitet</span><button>Më të rejat <ChevronDown /></button></div>
    {showComposer && <form className="fr-wall-compose" id="wall-composer" onSubmit={publish}>
