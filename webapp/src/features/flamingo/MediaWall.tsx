...
import {
  CheckCircle2, ChevronDown, Download, FileText, Image, Maximize2, MessageCircle,
...
type Attachment = { name: string; size: number; type: string; src: string; blob?: Blob; duration?: number };
...
async function downloadAttachment(file: Attachment, postIdValue?: string) {
  ...
  if (postIdValue && /^[a-f\d]{24}$/i.test(postIdValue) && file.type.startsWith('video/')) {
...
function AttachmentPreview({ file, postIdValue, onError }: { file: Attachment; postIdValue: string; onError: (message: string) => void }) {
  ...
  if (file.type.startsWith('video/')) return <><div className="fr-video-frame"><video key={file.src} src={file.src} controls playsInline preload="metadata" /><span><Play /> VIDEO</span><button type="button" className="fr-expand-video" onClick={() => setExpanded(true)} aria-label="Hap videon në ekran të plotë"><Maximize2 /> Ekran i plotë</button></div>{expanded && createPortal(<div className="fr-video-fullscreen" role="dialog" aria-modal="true" aria-label={`Video: ${file.name}`}><header><strong>{file.name}</strong><button type="button" onClick={() => setExpanded(false)} aria-label="Mbyll videon"><X /></button></header><video src={file.src} controls autoPlay playsInline preload="auto" /><button type="button" className="fr-fullscreen-download" onClick={() => downloadAttachment(file, postIdValue).catch(error => onError(error.message))}><Download /> Shkarko videon · {file.duration && file.duration >= 20 ? (file.duration > 40 ? 300 : 200) : 0} TPG</button></div>, document.body)}</>;
...
export default function MediaWall({ compact = false }: { compact?: boolean }) {
  ...
  async function selectFile(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; event.target.value = ''; if (!file) return; if (file.size > MAX_UPLOAD_BYTES) return setNotice('Skedari tejkalon kufirin maksimal prej 5 GB.'); setLoadingFile(true); const src = URL.createObjectURL(file); urls.current.push(src); const duration = await readVideoDuration(file); setSelected({ name: file.name, size: file.size, type: attachmentType(file), src, blob: file, duration }); setLoadingFile(false); setNotice(`${file.type.startsWith('video/') ? 'Videoja' : 'Skedari'} është gati në cilësinë origjinale. Prek “Publiko”.`); }
  async function publish(event: FormEvent) { event.preventDefault(); if (loadingFile) return; const validOptions = pollOptions.map(x => x.trim()).filter(Boolean); if (postKind === 'poll' && (!pollQuestion.trim() || validOptions.length < 2)) return setNotice('Shto një pyetje dhe të paktën dy alternativa.'); if (postKind !== 'poll' && !text.trim() && !selected) return setNotice('Shkruaj diçka ose zgjidh një skedar.'); if (postKind === 'article' && !title.trim()) return setNotice('Shto titullin e artikullit.'); setLoadingFile(true); try { let payload; if (postKind === 'poll' || postKind === 'article') { const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts/content`, { method: 'POST', headers: identityHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ text: text.trim(), title: postKind === 'article' ? title.trim() : undefined, poll: postKind === 'poll' ? { question: pollQuestion.trim(), options: validOptions } : undefined }) }); payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Publikimi dështoi.'); } else if (selected?.blob) payload = await uploadLargePost(selected.blob, selected.name, text.trim(), selected.duration || 0, percent => setNotice(`Duke ngarkuar videon… ${percent}%`)); else { const form = new FormData(); form.set('text', text.trim()); const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts`, { method: 'POST', headers: identityHeaders(), body: form }); payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Ngarkimi dështoi.'); } const remote = payload.post; setPosts(items => [{ ...remote, id: remote._id, createdAt: 'Tani', attachment: remote.attachment ? { ...remote.attachment, src: `${API_BASE_URL}${remote.attachment.url}` } : undefined, canManage: true }, ...items]); resetComposer(); setNotice('Postimi u publikua për të gjithë komunitetin.'); } catch (error) { setNotice(error instanceof Error ? error.message : 'Ngarkimi dështoi.'); } finally { setLoadingFile(false); } }
