...
type WallIdentity = { author: string; authorAvatar: string; accountId?: string };
...
export default function MediaWall({ compact = false, profileAccountId = '', hideComposer = false }: { compact?: boolean; profileAccountId?: string; hideComposer?: boolean }) {
  ...
  useEffect(() => {
    ...
    const loadPosts = async (showFallback = false) => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts?paginated=1&limit=20${profileAccountId ? `&profile=${encodeURIComponent(profileAccountId)}` : ''}`, { headers: identityHeaders(), cache: 'no-store' });
  ...
  async function loadOlderPosts() {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts?paginated=1&limit=20&cursor=${encodeURIComponent(nextCursor)}${profileAccountId ? `&profile=${encodeURIComponent(profileAccountId)}` : ''}`, { headers: identityHeaders(), cache: 'no-store' });
      const payload = await response.json();
  ...
  return <section className={compact ? 'fr-wall compact' : 'fr-wall'}>
    ...
    {!profileAccountId && <div className="fr-wall-heading"><div><span className="fr-kicker"><span>🇦🇱</span> TONPLAYGRAM SOCIAL WALL</span><h1>Your voice. No barriers.</h1><p>An open space where every member of the community can speak.</p></div><span className="fr-wall-live"><i /> LIVE</span></div>}
    {!profileAccountId && <div className="fr-discover"><Search /><span>Search the community</span><button>Newest <ChevronDown /></button></div>}
    {!hideComposer && <form className="fr-wall-compose" id="wall-composer" onSubmit={publish}>
