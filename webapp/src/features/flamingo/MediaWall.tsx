...
type Post = { id: string; text: string; title?: string; author: string; createdAt: string; source?: 'community' | 'telegram'; attachment?: Attachment; poll?: Poll; canManage?: boolean };
...
async function uploadLargePost(file: Blob, name: string, text: string, onProgress: (percent: number) => void) {
  ...
  const started = await fetch(`${API_BASE_URL}/api/flamingo-wall/uploads`, { method: 'POST', headers: { 'X-Wall-Owner-Token': OWNER_TOKEN, 'X-Upload-Size': String(file.size), 'X-Upload-Name': encoded(name), 'X-Upload-Type': encoded(file.type || 'application/octet-stream'), 'X-Upload-Text': encoded(text), 'X-Upload-Author': encoded('Anëtar i komunitetit') } });
...
export default function MediaWall({ compact = false }: { compact?: boolean }) {
  ...
  async function updatePost(post: Post) { const value = editText.trim(); const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts/${post.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'X-Wall-Owner-Token': OWNER_TOKEN }, body: JSON.stringify({ text: value }) }); const payload = await response.json(); if (!response.ok) return setNotice(payload.error || 'Ndryshimi dështoi.'); setPosts(items => items.map(item => item.id === post.id ? { ...item, text: value } : item)); setEditingPost(undefined); setNotice('Përshkrimi u ruajt.'); }
  async function deletePost(post: Post) { if (!window.confirm('Ta fshijmë këtë postim përgjithmonë?')) return; const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts/${post.id}`, { method: 'DELETE', headers: { 'X-Wall-Owner-Token': OWNER_TOKEN } }); if (!response.ok) { const payload = await response.json().catch(() => ({})); return setNotice(payload.error || 'Fshirja dështoi.'); } setPosts(items => items.filter(item => item.id !== post.id)); setNotice('Postimi u fshi.'); }
