...
function FullscreenVideoFeed({ posts, initialPostId, engagement, commentDrafts, identity, onClose, onReact, onComment, onDraft, onShare }: { posts: Post[]; initialPostId: string; engagement: Record<string, Engagement>; commentDrafts: Record<string, string>; identity: WallIdentity; onClose: () => void; onReact: (postId: string, reaction: Reaction) => void; onComment: (event: FormEvent, postId: string) => void; onDraft: (postId: string, value: string) => void; onShare: (post: Post) => void }) {
...
export default function MediaWall({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={compact ? 'fr-wall compact' : 'fr-wall'}>
    {fullscreenPost && <FullscreenVideoFeed posts={videoPosts} initialPostId={fullscreenPost} engagement={engagement} commentDrafts={commentDrafts} identity={identity} onClose={() => setFullscreenPost(undefined)} onReact={react} onComment={addComment} onDraft={(postIdValue, value) => setCommentDrafts(drafts => ({ ...drafts, [postIdValue]: value }))} onShare={post => { share(post).catch(() => setNotice('Ndarja dështoi.')); }} />}
