...
export default function MediaWall({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    const loadPosts = async (showFallback = false) => {
      ...
      try {
        ...
        const normalized = payload.posts.map((post: any) => ({ ...post, id: post._id || post.id, createdAt: new Date(post.createdAt).toLocaleString('sq-AL'), attachment: post.attachment ? { ...post.attachment, src: resolveWallMediaUrl(API_BASE_URL, post.attachment.url, post.attachment.size) } : undefined })).filter((post: Post) => post.id);
        loadedOnce = true;
        setFeedState('live');
  ...
  async function selectFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (selectedBytes + incomingBytes > MAX_UPLOAD_BYTES) {
      return setNotice(`Skedarët e zgjedhur janë ${formatBytes(selectedBytes + incomingBytes)}. Mund të ngarkosh deri në 5 GB gjithsej.`);
    }
    setLoadingFile(true);
  ...
  async function publish(event: FormEvent) { event.preventDefault(); if (loadingFile) return; if (premium && (!selected.length || Number(priceTpg) < 1)) return setNotice('Vendos shumën e kërkuar TPG për përmbajtjen premium.'); const validOptions = pollOptions.map(x => x.trim()).filter(Boolean); if (postKind === 'poll' && (!pollQuestion.trim() || validOptions.length < 2)) return setNotice('Shto një pyetje dhe të paktën dy alternativa.'); if (postKind !== 'poll' && !text.trim() && !selected.length) return setNotice('Shkruaj diçka ose zgjidh një skedar.'); if (postKind === 'article' && !title.trim()) return setNotice('Shto titullin e artikullit.'); setLoadingFile(true); const completedFiles: Attachment[] = []; const uploadedPosts: any[] = []; try { let payload; if (postKind === 'poll' || postKind === 'article') { const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts/content`, { method: 'POST', headers: identityHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ text: text.trim(), title: postKind === 'article' ? title.trim() : undefined, poll: postKind === 'poll' ? { question: pollQuestion.trim(), options: validOptions } : undefined }) }); payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Publikimi dështoi.'); } else if (selected.length) {
      ...
      for (let index = 0; index < selected.length; index += 1) {
        ...
        const result = await uploadLargePost(file.blob, file.name, file.type, text.trim(), file.duration || 0, premium, Number(priceTpg) || 0, percent => {
          ...
          setNotice(`Duke ngarkuar ${index + 1}/${selected.length}… ${percent}% · Gjithsej ${totalPercent}%`);
        });
    ...
    } else { const form = new FormData(); form.set('text', text.trim()); const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts`, { method: 'POST', headers: identityHeaders(), body: form }); payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Ngarkimi dështoi.'); } const remotePosts = payload.posts || [payload.post]; const normalizedPosts = remotePosts.filter(Boolean).map((remote: any) => ({ ...remote, id: remote._id, createdAt: 'Tani', attachment: remote.attachment ? { ...remote.attachment, src: resolveWallMediaUrl(API_BASE_URL, remote.attachment.url, remote.attachment.size) } : undefined, canManage: true })); normalizedPosts.forEach((post: Post) => pendingPostIds.current.add(post.id)); setPosts(items => [...normalizedPosts.reverse(), ...items]); resetComposer(); setNotice('Postimi u publikua për të gjithë komunitetin.'); } catch (error) { if (uploadedPosts.length) { const normalizedPosts = uploadedPosts.map(remote => ({ ...remote, id: remote._id, createdAt: 'Tani', attachment: remote.attachment ? { ...remote.attachment, src: resolveWallMediaUrl(API_BASE_URL, remote.attachment.url, remote.attachment.size) } : undefined, canManage: true })); normalizedPosts.forEach((post: Post) => pendingPostIds.current.add(post.id)); setPosts(items => [...normalizedPosts.reverse(), ...items]); setSelected(items => items.filter(file => !completedFiles.includes(file))); } const message = error instanceof Error ? error.message : 'Ngarkimi dështoi.'; setNotice(uploadedPosts.length ? `${uploadedPosts.length} skedarë u publikuan. Skedarët e mbetur nuk u ngarkuan: ${message}` : message); } finally { setLoadingFile(false); } }
