...
type Post = { id: string; text: string; title?: string; author: string; authorAvatar?: string; createdAt: string; source?: 'community' | 'telegram'; attachment?: Attachment; attachments?: Attachment[]; poll?: Poll; canManage?: boolean };
...
function normalizeRemotePost(post: any, createdAt?: string): Post {
  const media = (attachment: any) => attachment ? { ...attachment, src: resolveWallMediaUrl(API_BASE_URL, attachment.url, attachment.size) } : undefined;
  return { ...post, id: post._id || post.id, createdAt: createdAt || new Date(post.createdAt).toLocaleString('sq-AL'), attachment: media(post.attachment), attachments: post.attachments?.map(media) };
}
...
function PostMedia({ post, onExpand }: { post: Post; onExpand: (file: Attachment) => void }) {
  const files = post.attachments?.length ? post.attachments : post.attachment ? [post.attachment] : [];
  if (files.length === 1) return <AttachmentPreview file={files[0]} onExpand={() => onExpand(files[0])} />;
  if (!files.length) return null;
  return <div className={`fr-album fr-album-${Math.min(files.length, 4)}`} aria-label={`Album me ${files.length} foto`}>
    {files.map((file, index) => <button type="button" key={`${file.name}-${index}`} onClick={() => onExpand(file)} aria-label={`Hap ${file.name}`}>
      {file.type.startsWith('image/') ? <img src={file.src} alt={file.name} loading="lazy" /> : <video src={file.src} playsInline preload="metadata" />}
      {index === 3 && files.length > 4 && <strong>+{files.length - 4}</strong>}
    </button>)}
  </div>;
}
...
export default function MediaWall({ compact = false, postId: requestedPostId }: { compact?: boolean; postId?: string }) {
  ...
  useEffect(() => {
    ...
    const loadPosts = async (showFallback = false) => {
      ...
      try {
        ...
        const normalized = payload.posts.map((post: any) => normalizeRemotePost(post)).filter((post: Post) => post.id);
        loadedOnce = true;
        setFeedState('live');
  ...
  async function publish(event: FormEvent) { event.preventDefault(); if (loadingFile) return; if (premium && (!selected.length || Number(priceTpg) < 1)) return setNotice('Vendos shumën e kërkuar TPG për përmbajtjen premium.'); const validOptions = pollOptions.map(x => x.trim()).filter(Boolean); if (postKind === 'poll' && (!pollQuestion.trim() || validOptions.length < 2)) return setNotice('Shto një pyetje dhe të paktën dy alternativa.'); if (postKind !== 'poll' && !text.trim() && !selected.length) return setNotice('Shkruaj diçka ose zgjidh një skedar.'); if (postKind === 'article' && !title.trim()) return setNotice('Shto titullin e artikullit.'); setLoadingFile(true); try { let payload; if (postKind === 'poll' || postKind === 'article') { const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts/content`, { method: 'POST', headers: identityHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ text: text.trim(), title: postKind === 'article' ? title.trim() : undefined, poll: postKind === 'poll' ? { question: pollQuestion.trim(), options: validOptions } : undefined }) }); payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Publikimi dështoi.'); } else if (selected.length) {
      const albumId = selected.length > 1 ? postId() : '';
      let uploaded;
      for (let index = 0; index < selected.length; index += 1) {
        const file = selected[index];
        if (!file.blob) continue;
        const result = await uploadLargePost(file.blob, file.name, file.type, text.trim(), file.duration || 0, premium, Number(priceTpg) || 0, albumId, percent => setNotice(`Duke ngarkuar ${index + 1}/${selected.length}… ${percent}%`));
        uploaded = result.post;
      }
      payload = { post: uploaded };
    } else { const form = new FormData(); form.set('text', text.trim()); const response = await fetch(`${API_BASE_URL}/api/flamingo-wall/posts`, { method: 'POST', headers: identityHeaders(), body: form }); payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Ngarkimi dështoi.'); } const remotePosts = payload.posts || [payload.post]; const normalizedPosts = remotePosts.filter(Boolean).map((remote: any) => ({ ...normalizeRemotePost(remote, 'Tani'), canManage: true })); normalizedPosts.forEach((post: Post) => pendingPostIds.current.add(post.id)); setPosts(items => [...normalizedPosts.reverse(), ...items.filter(item => !normalizedPosts.some((post: Post) => post.id === item.id))]); resetComposer(); setNotice('Postimi u publikua për të gjithë komunitetin.'); } catch (error) { setNotice(error instanceof Error ? error.message : 'Ngarkimi dështoi.'); } finally { setLoadingFile(false); } }
  ...
  const visiblePosts = requestedPostId ? posts.filter(post => post.id === requestedPostId) : posts;
