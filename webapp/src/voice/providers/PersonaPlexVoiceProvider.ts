...
export class PersonaPlexUnavailableError extends Error {
...
export class PersonaPlexVoiceProvider implements VoiceProvider {
  ...
  private async ensureAudioUnlocked() {
    ...
    try {
      const unlock = new Audio(
        'data:audio/mpeg;base64,SUQzAwAAAAAAI1RTU0UAAAAPAAADTGF2ZjU2LjE1LjEwNAAAAAAAAAAAAAAA//sQxAADAgAASAAAABgAAAACAAACcQCAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAg'
      );
      unlock.muted = true;
      unlock.volume = 0;
      ...
      // We retry actual audio play below.
    }
  }
  ...
  private async fetchAudioSource(text: string, opts: VoiceSpeakOptions): Promise<string> {
    ...
    throw new PersonaPlexUnavailableError(
      payload?.warning || payload?.error || 'PersonaPlex response missing audio payload'
    );
  }
