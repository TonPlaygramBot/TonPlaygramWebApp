...
import { PERSONAPLEX_UNAVAILABLE, PersonaPlexVoiceProvider } from './providers/PersonaPlexVoiceProvider';
...
const shouldFallbackToCurrent = (error: unknown) => {
  if (VOICE_PROVIDER !== 'personaplex') return false;
  const message = String((error as Error)?.message || '');
  return message.includes(PERSONAPLEX_UNAVAILABLE);
};
...
const next = async () => {
  ...
  try {
    ...
    if (shouldFallbackToCurrent(error)) {
      try {
