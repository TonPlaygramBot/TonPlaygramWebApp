...
Pool Royale uses the authentic Sketchfab GLB for **Pool Table Traditional** by
**fizyman**. The binary GLB is intentionally not committed; install it locally or
in deployment with:
...
A manually downloaded authentic Sketchfab GLB can also be copied and validated
with:

```bash
npm run fetch:pool-royale-traditional-table -- --from /path/to/pool-table-traditional.glb
```
...
The downloaded GLB keeps the original table shape, UV mapping, material slots,
