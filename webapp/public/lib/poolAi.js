...
function clearShotCandidates (req) {
  ...
  for (const target of targets) {
    for (const pocket of pockets) {
      ...
      const cueLaneClearance = clearanceMargin(cue, ghost, req.state.balls, [cueBallId, target.id], r, 1.22)
      const targetLaneClearance = clearanceMargin(target, entry, req.state.balls, [cueBallId, target.id], r, 1.18)
      const clearanceScore = Math.max(0, Math.min((Math.min(cueLaneClearance, targetLaneClearance) - 0.5) / 0.8, 1))
      const rank =
        pocketView * 0.52 +
      ...
      if (
        cut <= maxCut &&
        pocketView >= minView &&
        cornerLaneSafety >= 0.3 &&
        entranceOpenness >= 0.28 &&
        cueLaneClearance >= 0.56 &&
        targetLaneClearance >= 0.62
      ) {
...
function evaluate (req, cue, target, pocket, power, spin, ballsOverride, strict = false, options = {}) {
  ...
  const cueGhostClearance = clearanceMargin(cue, ghost, balls, [cueBallId, target.id], r, 1.2)
  const targetLaneClearance = clearanceMargin(target, entry, balls, [cueBallId, target.id], r, 1.16)
  if (laneClearance < 0.5) {
    return null
  }
  if (cueGhostClearance < 0.55 || targetLaneClearance < 0.6) {
    return null
  }
  ...
  const entranceOpenness = pocketEntranceOpenness(target, pocket, req)
  const effectivePocketOpen = pocketOpen * (0.72 + 0.28 * entranceOpenness)
  const cornerLaneSafety = cornerPocketLaneSafety(target, entry, pocket, req)
  if (cornerLaneSafety <= 0.1) {
    return null
  }
  if (entranceOpenness < 0.24) {
    return null
  }
  const cueToTarget = dist(cue, target)
...
function safetyShot (req) {
  ...
  const legalContact = legalContactSafetyShot(req)
  if (legalContact) return legalContact
  const corners = [
    { x: 0, y: 0 },
...
function legalContactSafetyShot (req) {
  const cueBallId = resolveCueBallId(req.state)
  const cue = req.state.balls.find(b => b.id === cueBallId)
  if (!cue) return null
  const legalTargets = chooseTargets(req)
  ...
  let best = null
  for (const target of legalTargets) {
    if (pathBlocked(cue, target, req.state.balls, [cueBallId, target.id], req.state.ballRadius, 1.03)) continue
    const angle = Math.atan2(target.y - cue.y, target.x - cue.x)
    const d = dist(cue, target)
    const quality = Math.max(0.05, 0.26 - Math.min(d / Math.max(req.state.ballRadius * 45, 1), 0.22))
    const candidate = {
      angleRad: angle,
