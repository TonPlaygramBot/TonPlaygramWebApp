 ...
 * @property {{balls:Ball[],pockets:Pocket[],width:number,height:number,ballRadius:number,friction:number,myGroup?:'SOLIDS'|'STRIPES'|'UNASSIGNED',ballOn?:'blue'|'red'|null|string|string[],legalBallIds?:number[],ballInHand?:boolean,mustPlayFromBaulk?:boolean,baulkLineY?:number,breakInProgress?:boolean,breakPlacementRestricted?:boolean}} state
...
function parseBallIdToken (token) {
  if (Number.isFinite(token)) return Number(token)
  if (typeof token !== 'string') return null
  const upper = token.trim().toUpperCase()
  if (!upper) return null
  if (upper === 'BLACK' || upper === 'EIGHT' || upper === 'BALL_8') return 8
  const match = upper.match(/(?:BALL[_\s-]*)?(\d{1,2})$/)
...
function resolveLegalBallIds (state) {
  ...
  if (Array.isArray(state?.legalBallIds)) {
    for (const entry of state.legalBallIds) {
      const id = parseBallIdToken(entry)
      if (Number.isFinite(id)) ids.add(id)
    }
  }
  const ballOn = state?.ballOn
  const entries = Array.isArray(ballOn) ? ballOn : [ballOn]
...
function chooseTargets (req) {
  ...
  const legalIds = resolveLegalBallIds(req.state)
  if (legalIds.size > 0) {
    const legalTargets = balls.filter(b => legalIds.has(b.id))
