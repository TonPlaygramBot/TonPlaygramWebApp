 ...
 * @property {{balls:Ball[],pockets:Pocket[],width:number,height:number,ballRadius:number,friction:number,myGroup?:'SOLIDS'|'STRIPES'|'UNASSIGNED',ballOn?:'blue'|'red'|null,ballInHand?:boolean,mustPlayFromBaulk?:boolean,baulkLineY?:number,breakInProgress?:boolean,breakPlacementRestricted?:boolean,preferredTargetBallId?:number,legalBallIds?:number[]}} state
...
function chooseTargets (req) {
  ...
  const legalIds = Array.isArray(req.state.legalBallIds)
    ? req.state.legalBallIds.filter(id => Number.isFinite(id))
    : []
  const legalSet = legalIds.length > 0 ? new Set(legalIds) : null
  const preferredTargetBallId = Number.isFinite(req.state.preferredTargetBallId)
    ? Number(req.state.preferredTargetBallId)
    : null
  const restrictToLegal = candidateBalls => {
    if (!legalSet) return candidateBalls
    return candidateBalls.filter(ball => legalSet.has(ball.id))
  }
  if (req.game === 'NINE_BALL') {
    const playable = restrictToLegal(balls)
    if (preferredTargetBallId != null) {
      const preferred = playable.find(ball => ball.id === preferredTargetBallId)
      if (preferred) return [preferred]
    }
    const lowest = playable.reduce((m, b) => (b.id < m.id ? b : m), playable[0])
  ...
  if (req.game === 'AMERICAN_BILLIARDS') {
    ...
    if (group === 'SOLIDS') {
      if (solids.length > 0) return restrictToLegal(solids)
      if (eight) return restrictToLegal([eight])
      return []
    }
    if (group === 'STRIPES') {
      if (stripes.length > 0) return restrictToLegal(stripes)
      if (eight) return restrictToLegal([eight])
      return []
    }
    return restrictToLegal(balls)
  }
  if (req.game === 'EIGHT_POOL_UK') {
    ...
    return restrictToLegal(balls.filter(b => b.id !== 8))
  }
  return restrictToLegal(balls)
}
