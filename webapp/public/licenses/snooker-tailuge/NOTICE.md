...
provided by Snooker Royal. No upstream table, ball, texture, audio, network or
account assets were copied. The upstream ClawBreak constant-power/nearest-ball
policy is replaced with the local `shotPlanner.ts` adapter's route checks and
Snooker Royal's existing power mapping.
...
the real shot still goes through SnookerRoyalRules, including foul penalties.

Corresponding integration source: https://github.com/TonPlaygramBot/TonPlaygramWebApp/tree/codex/snooker-tailuge-logic
