...
const tmpSeatTargetLocal = new THREE.Vector3();
const tmpSeatRootWorld = new THREE.Vector3();
...
function applyLoadedHumanPose(seatHuman, timeSeconds, actionStrength = 0, knockStrength = 0) {
  ...
  if (actionTarget && seatHuman?.root?.parent?.worldToLocal) {
    const targetLocal = tmpSeatTargetLocal.copy(actionTarget);
    seatHuman.root.parent.worldToLocal(targetLocal);
    ...
    const seatLocal = tmpSeatRootWorld;
    seatHuman.root.getWorldPosition(seatLocal);
    seatHuman.root.parent.worldToLocal(seatLocal);
  ...
  seatedHumanTemplatePromise = (async () => {
    ...
    root.traverse((obj) => {
      ...
      mats.forEach((mat) => {
        ...
        if (typeof normalizeMaterialTextures === 'function') {
