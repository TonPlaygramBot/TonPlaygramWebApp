...
const HDRI_RESOLUTION_OPTIONS_BY_ID = Object.freeze(
...
function normalizeHdriResolutionId(rawId) {
  if (!rawId) return null;
  return HDRI_RESOLUTION_OPTIONS_BY_ID[rawId] ? rawId : null;
}

function resolveInitialHdriResolutionId() {
  const urlPreset =
    urlParams.get('hdriResolutionId') ||
    urlParams.get('hdriResolution') ||
    urlParams.get('hdri');
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      console.warn('Failed to read Domino HDRI resolution selection', error);
    }
  }
...
function resolveHdriResolutionOrder() {
  if (hdriResolutionId && hdriResolutionId !== 'auto') {
    const preferredIndex = HDRI_RESOLUTION_ORDER.indexOf(hdriResolutionId);
    if (preferredIndex >= 0) {
      return HDRI_RESOLUTION_ORDER.slice(0, preferredIndex + 1).reverse();
    }
  }
  switch (frameRateId) {
    case 'ultra144':
...
function applyHdriResolutionSelection(
  optionId,
  { refreshUi = true } = {}
) {
  const resolvedId = normalizeHdriResolutionId(optionId) || DEFAULT_HDRI_RESOLUTION_ID;
...
function refreshConfigUI() {
  ...
  const hdriResolutionWrapper = document.createElement('div');
  hdriResolutionWrapper.style.display = 'flex';
  hdriResolutionWrapper.style.flexDirection = 'column';
  hdriResolutionWrapper.style.gap = '0.35rem';
  hdriResolutionWrapper.style.marginBottom = '0.65rem';
  const hdriResolutionLabel = document.createElement('p');
  hdriResolutionLabel.textContent = 'HDRI Resolution';
  hdriResolutionLabel.style.margin = '0';
  hdriResolutionLabel.style.fontSize = '0.65rem';
  hdriResolutionLabel.style.letterSpacing = '0.14em';
  hdriResolutionLabel.style.textTransform = 'uppercase';
  hdriResolutionLabel.style.fontWeight = '700';
  hdriResolutionLabel.style.color = 'rgba(226,232,240,0.8)';
  hdriResolutionWrapper.appendChild(hdriResolutionLabel);
  const hdriResolutionGrid = document.createElement('div');
  hdriResolutionGrid.className = 'config-options';
  hdriResolutionGrid.style.gridTemplateColumns =
  ...
  HDRI_RESOLUTION_OPTIONS.forEach((option) => {
    ...
    hdriResolutionGrid.appendChild(button);
  });
  hdriResolutionWrapper.appendChild(hdriResolutionGrid);
  graphicsWrapper.appendChild(hdriResolutionWrapper);
