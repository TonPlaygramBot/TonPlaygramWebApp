...
const rigSource = `// Generated from LudoBattleRoyal.jsx by build-ludo-tabletop-preview.mjs.\n// @ts-nocheck\nimport * as THREE from 'three';\nconst clamp = THREE.MathUtils.clamp;\nconst SEATED_HUMAN_MOTION_TUNING = { idleBreathAmp: 0.012 };\n` +
...
const files = {
  avatar: 'table-tennis/chess-human.glb',
...
const result = await build({ entryPoints: [resolve(root, 'webapp/src/previews/ludo/LudoTabletopPreview.tsx')],
  bundle: true, format: 'esm', platform: 'browser', jsx: 'automatic', minify: true, target: 'es2022', write: false,
  define: { LUDO_TABLETOP_ASSETS: JSON.stringify(packedAssets) },
...
const html = (await readFile(resolve(root, 'scripts/ludo-motion-preview.fragment.html'), 'utf8')).replace('/* LUDO_PREVIEW */', () => result.outputFiles[0].text);
if (Buffer.byteLength(html) > 1000000) throw new Error('Preview exceeds inline size limit: ' + Buffer.byteLength(html));
const output = process.argv[2] || '/workspace/ludo-table-parking.html';
