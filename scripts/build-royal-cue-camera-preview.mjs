...
const output = process.argv[2] || '/workspace/royal-cue-camera-spin.html';
...
const result = await build({
  entryPoints: [resolve(root, 'webapp/src/previews/RoyalCueCameraPreview.tsx')],
...
const fragment = (await readFile(resolve(root, 'scripts/royal-cue-camera-preview.fragment.html'), 'utf8'))
  .replace('/* ROYAL_CUE_CAMERA_PREVIEW */', () => result.outputFiles[0].text);
if (Buffer.byteLength(fragment) > 1_000_000) throw new Error('Inline preview exceeds 1 MB.');
await writeFile(output, fragment);
console.log(`Camera and spin preview written: ${output} (${Buffer.byteLength(fragment)} bytes)`);
