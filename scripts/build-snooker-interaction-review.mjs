...
const output = process.argv[2] || '/workspace/snooker-shot-placement-equipment.html';
...
const result = await build({
  entryPoints: [resolve(root, 'webapp/src/previews/SnookerInteractionReview.tsx')],
  ...
  plugins: [{ name: 'inline-cdn-imports', setup(api) {
    api.onLoad({ filter: /cueReachMeshes\.ts$/ }, async args => {
      const source = await readFile(args.path, 'utf8');
      const data = source.slice(source.indexOf('export default ') + 15).trim().replace(/;$/, '');
      const packed = gzipSync(Buffer.from(data)).toString('base64');
      return { contents: `const b=Uint8Array.from(atob('${packed}'),c=>c.charCodeAt(0));
        export default JSON.parse(await new Response(new Blob([b]).stream().pipeThrough(new DecompressionStream('gzip'))).text());`, loader: 'js' };
    });
...
const fragment = (await readFile(resolve(root, 'scripts/snooker-interaction-review.fragment.html'), 'utf8'))
  .replace('/* SNOOKER_INTERACTION_REVIEW */', () => result.outputFiles[0].text);
...
console.log(`Snooker interaction review written: ${output} (${Buffer.byteLength(fragment)} bytes)`);
