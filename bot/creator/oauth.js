...
import { available, credentials } from './catalog.js';
...
const configs = {
  ...
  youtube: ['https://accounts.google.com/o/oauth2/v2/auth', 'https://oauth2.googleapis.com/token', 'https://www.googleapis.com/auth/youtube.force-ssl https://www.googleapis.com/auth/youtube.upload'],
  ...
  threads: ['https://threads.net/oauth/authorize', 'https://graph.threads.net/oauth/access_token', 'threads_basic threads_content_publish'],
  tiktok: ['https://www.tiktok.com/v2/auth/authorize/', 'https://open.tiktokapis.com/v2/oauth/token/', 'user.info.basic video.publish'],
  x: ['https://x.com/i/oauth2/authorize', 'https://api.x.com/2/oauth2/token', 'tweet.read tweet.write users.read offline.access'],
  twitch: ['https://id.twitch.tv/oauth2/authorize', 'https://id.twitch.tv/oauth2/token', 'channel:read:stream_key channel:manage:broadcast']
};
export const callback = p => `${publicOrigin()}/api/creator/oauth/${p}/callback`;
export async function beginOAuth(req, res, platform) {
  if (!available(platform) || !configs[platform]) throw problem(503, 'This platform connection is being prepared. Please try again later.');
  if (platform !== 'google' && !req.creator) throw problem(401, 'Sign in to Creator Studio first.');
  ...
  if (['facebook', 'instagram', 'threads', 'tiktok'].includes(platform)) args.scope = args.scope.replaceAll(' ', ',');
  if (['google', 'youtube', 'x'].includes(platform)) Object.assign(args, { code_challenge: (await import('node:crypto')).createHash('sha256').update(verifier).digest('base64url'), code_challenge_method: 'S256' });
...
async function exchange(platform, code, verifier) {
  ...
  if (['google', 'youtube', 'x'].includes(platform)) args.code_verifier = verifier;
  const options = form(args);
  if (platform === 'x') { delete args.client_secret; Object.assign(options, form(args)); options.headers.Authorization = `Basic ${Buffer.from(`${c.id}:${c.secret}`).toString('base64')}`; }
  ...
  if (platform === 'threads') {
    ...
    u.search = new URLSearchParams({ grant_type: 'th_exchange_token', client_secret: c.secret, access_token: token.access_token });
    token = { ...token, ...await request(u) };
  }
...
export async function finishOAuth(req, res, platform) {
  ...
  if (platform !== 'google' && pending.owner !== req.creator?.owner) throw problem(401, 'Sign in with the same Studio account and try again.');
  ...
  if (platform === 'youtube') {
    ...
    const data = await get('https://graph.threads.net/v1.0/me?fields=id,username'); accounts = [{ id: data.id, name: data.username }];
    ...
    const data = await get('https://api.x.com/2/users/me'); accounts = [{ id: data.data?.id, name: data.data?.username }];
  } else if (platform === 'twitch') {
    const data = await request('https://api.twitch.tv/helix/users', json('GET', token.access_token, undefined, { 'Client-Id': credentials(platform).id }));
    accounts = (data.data || []).map(x => ({ id: x.id, name: x.display_name }));
  }
  ...
  for (const account of accounts) {
    ...
    const existing = await Connection.findOne({ owner: pending.owner, platform, providerId: String(account.id) }).select('+credentials');
    if (!ownToken.refresh_token && existing?.credentials) {
      try { ownToken.refresh_token = unseal(existing.credentials, pending.owner).refresh_token; } catch { /* new grant can replace an invalid token */ }
    }
    await Connection.findOneAndUpdate({ owner: pending.owner, platform, providerId: String(account.id) }, { $set: { name: account.name || platform, credentials: seal(ownToken, pending.owner), status: 'connected', expiresAt: ownToken.expires_in ? new Date(Date.now() + ownToken.expires_in * 1000) : null } }, { upsert: true, new: true });
  }
}
...
export async function accessToken(connection) {
  ...
  const action = (async () => {
    ...
    try {
      ...
      let fresh;
      if (['instagram', 'threads'].includes(p)) {
        const u = new URL(p === 'instagram' ? 'https://graph.instagram.com/refresh_access_token' : 'https://graph.threads.net/refresh_access_token');
        u.search = new URLSearchParams({ grant_type: p === 'instagram' ? 'ig_refresh_token' : 'th_refresh_token', access_token: token.access_token });
        ...
        if (p === 'x') { delete args.client_secret; Object.assign(opts, form(args)); opts.headers.Authorization = `Basic ${Buffer.from(`${c.id}:${c.secret}`).toString('base64')}`; }
