...
const absoluteUrl = (req, value) => {
  if (!value) return '';
  try { return new URL(value, `${req.protocol}://${req.get('host')}`).href; } catch { return ''; }
};
...
router.get('/share/:id', async (req, res) => {
  ...
  const frontend = String(process.env.WEBAPP_URL || process.env.FRONTEND_URL || `${req.protocol}://${req.get('host')}`).replace(/\/$/, '');
