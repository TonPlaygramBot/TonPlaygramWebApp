...
function resolveAuth(req) {
  ...
  if (initData) {
    ...
    if (data?.user) {
      try {
        return { telegramId: Number(JSON.parse(data.user).id) };
      } catch {
        return null;
      }
    }
...
router.post('/get', async (req, res) => {
  ...
  const isOwner = auth?.telegramId && user?.telegramId === auth.telegramId;
  res.json({
    ...(isOwner ? sanitizeUser(user) : publicProfile(user)),
    filledFromTelegram
  });
});
...
router.post('/addTransaction', authenticate, async (req, res) => {
  ...
  if (!req.auth?.telegramId || (telegramId != null && req.auth.telegramId !== telegramId)) {
    return res.status(403).json({ error: 'forbidden' });
  }
  let user = null;
  if (telegramId != null) user = await User.findOne({ telegramId });
  if (!user && accountId) user = await User.findOne({ accountId });
  if (accountId && user?.telegramId && user.telegramId !== req.auth.telegramId) {
    return res.status(403).json({ error: 'forbidden' });
  }
  if (!user) {
    const data = {
