...
async function synthesizeVoice(args) {
  ...
  if (provider === 'personaplex') {
    const mode = String(process.env.PERSONAPLEX_MODE || 'service').toLowerCase();
    return mode === 'remote-api' ? synthesizeWithPersonaplex(args) : synthesizeWithPersonaPlexService(args);
  }
...
router.get('/health', async (_req, res) => {
  ...
  try {
    ...
    const payload = await response.json();
    return res.json({ ok: true, provider: 'personaplex', ...payload });
  } catch (error) {
    return res.status(503).json({ ok: false, provider: 'personaplex', error: error.message });
  }
});
