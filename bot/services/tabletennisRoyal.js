...
import {
  ...
  refundExpiredTableTennisStakes
} from './tabletennisStake.js';
import { updateTableTennisCareer } from './tabletennisCareer.js';

/** Uses the existing seat/ready/gameStart queue. Clients submit controls only. */
export function createTableTennisRoyal({
  io,
  tableMap,
  now = Date.now,
  reserve = reserveTableTennisStake,
  settle = settleTableTennisStake,
  career = updateTableTennisCareer,
  autoTick = true
}) {
  const rooms = new Map();
  ...
  async function finish(r, winner, reason) {
    ...
    try {
      ...
      console.error('TableTennis settlement', e.message);
    } finally {
      r.settling = false;
      r.revision++;
    }
  ...
  function attach(socket) {
    ...
    const handle = (name, fn) =>
      socket.on(name, async (payload = {}, ack) => {
        try {
          ...
          ack?.({
            success: false,
            error: e.message || 'tabletennis_unavailable'
          });
        }
      });
    handle('tabletennisJoin', (payload) => {
      ...
      const choices = [
        ...
        'athlete-female',
        'athlete-male-gold',
        'athlete-female-violet'
      ];
      if (choices.includes(payload.character))
        r.players[seat].character = payload.character;
      return snapshot(r, seat);
    });
    handle('tabletennisInput', (payload) => {
    ...
    handle('tabletennisLeave', (payload) => {
      const { r, seat } = withRoom(payload);
      if (!r.result) end(r, 1 - seat, 'Opponent retired');
      socket.leave(r.id);
      return { ok: true };
    });
    handle('tabletennisCareer', async (payload) => {
      const id = identify(payload);
      const time = now();
      if (
        payload.action !== 'get' &&
        time - (socket.data.tabletennisCareerAt || 0) < 150
      )
        throw Error('Please wait a moment before saving again.');
      if (payload.action !== 'get') socket.data.tabletennisCareerAt = time;
  ...
  const refunds = autoTick
    ? setInterval(
        () =>
          refundExpiredTableTennisStakes().catch((e) =>
            console.error('TableTennis expired stake recovery', e.message)
          ),
        60000
      )
    : null;
