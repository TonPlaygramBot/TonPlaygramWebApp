...
import TennisCareer from '../models/TennisCareer.js';
import { shouldUseMemoryUserStore } from '../utils/memoryUserStore.js';
import { freshCareer, careerResult } from '../../shared/tennis/career.js';
const memory = new Map();
export async function updateTennisCareer(accountId, action, body = {}) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    const p = inMemory
      ...
      : await TennisCareer.findOne({ accountId }).lean();
    const c = p.career;
    ...
    const updated = await TennisCareer.updateOne(
