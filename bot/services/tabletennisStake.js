...
import TableTennisMatch from '../models/TableTennisMatch.js';
import {
  findMemoryUser,
  saveMemoryUser,
...
const transaction = (tableId, id, amount, type) => ({
  transactionId: `tabletennis:${tableId}:${type}:${id}`,
  amount,
  type,
  token: 'TPG',
...
export async function reserveTableTennisStake(table) {
  ...
  if (
    ...
    throw Error('invalid_tabletennis_stake');
  const record = {
    tableId,
    accounts,
    stake,
  ...
  await TableTennisMatch.init();
  const session = await mongoose.startSession();
  try {
    let result;
    await session.withTransaction(async () => {
      const existing = await TableTennisMatch.findOne({ tableId }).session(
        session
      );
      if (existing) {
      ...
      [result] = await TableTennisMatch.create([record], { session });
    });
    return result;
  } finally {
...
export async function settleTableTennisStake(
  tableId,
  winner = null,
  reason = 'completed'
) {
  ...
  try {
    let result;
    await session.withTransaction(async () => {
      const m = await TableTennisMatch.findOne({ tableId }).session(session);
      if (!m) throw Error('match_not_found');
      if (terminal(m)) {
        result = m;
        return;
...
export async function refundExpiredTableTennisStakes() {
  ...
  for (const m of matches)
    await settleTableTennisStake(m.tableId, null, 'match_expired');
}
