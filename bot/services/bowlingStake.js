...
const transaction = (tableId, id, amount, type) => ({
  transactionId: `bowling:${tableId}:${type}:${id}`,
  amount,
  type,
  token: 'TPG',
...
export async function reserveBowlingStake(table) {
  ...
  if (
    ...
    stake <= 0 ||
    !Number.isSafeInteger(stake * 2) ||
    table.gameType !== 'bowlingroyal' ||
    String(table.meta?.token).toUpperCase() !== 'TPG' ||
    table.meta?.mode !== 'online' ||
    table.meta?.format !== 'tenpin' ||
    accounts.some((id) => !id || id === 'undefined')
  )
    throw Error('invalid_bowling_stake');
  const record = {
    tableId,
    accounts,
    stake,
  ...
  if (shouldUseMemoryUserStore()) {
    ...
    if (previous) {
      ...
      if (
        previous.stake !== stake ||
        previous.accounts.join('|') !== accounts.join('|')
      )
        throw Error('match_contract_mismatch');
      return previous;
    }
    if (
    ...
    if (
      users.some(
        (u) =>
          !u ||
          u.isBanned ||
          !Number.isFinite(Number(u.balance)) ||
          Number(u.balance) < stake
      )
    )
      throw Error('insufficient_balance');
    if (users.some((u) => u.currentTableId && u.currentTableId !== tableId))
      throw Error('account_already_in_active_match');
    users.forEach((u, i) =>
      saveMemoryUser({
        ...u,
  ...
  await BowlingMatch.init();
  const session = await mongoose.startSession();
  try {
    let result;
    await session.withTransaction(async () => {
      const existing = await BowlingMatch.findOne({ tableId }).session(session);
      if (existing) {
        ...
        const changed = await User.updateOne(
          {
            $and: [
              identity(id),
              { $or: [{ currentTableId: null }, { currentTableId: tableId }] }
            ],
            balance: { $gte: stake },
            isBanned: { $ne: true }
          },
      ...
      [result] = await BowlingMatch.create([record], { session });
    });
    return result;
  } finally {
...
export async function settleBowlingStake(
  tableId,
  winner = null,
  reason = 'completed'
) {
  ...
  try {
    let result;
    await session.withTransaction(async () => {
      const m = await BowlingMatch.findOne({ tableId }).session(session);
      if (!m) throw Error('match_not_found');
      if (terminal(m)) {
        result = m;
        return;
...
export async function refundExpiredBowlingStakes() {
  const now = new Date();
  const matches = shouldUseMemoryUserStore()
    ? [...memoryMatches.values()].filter(
        (m) => !terminal(m) && m.expiresAt <= now
      )
    : await BowlingMatch.find({ status: 'playing', expiresAt: { $lte: now } })
        .limit(30)
        .lean();
  for (const m of matches)
    await settleBowlingStake(m.tableId, null, 'match_expired');
}
