...
test('Tirana store resolves price from authoritative catalog', () => {
  const result = validatePurchase({ itemId: 'tirana-ar15Attack', idempotencyKey: 'purchase_key_12345' });
  assert.equal(result.item.priceTPG, 3000);
  assert.equal(result.item.weaponId, 'ar15Attack');
...
test('Tirana store rejects client-defined and invalid items', () => {
  assert.equal(validatePurchase({ itemId: 'tirana-ar15Attack', priceTPG: 1 }).code, 'INVALID_REQUEST');
...
function account(balance=5000) {
  ...
  async function buy(key='purchase_key_12345',itemId='tirana-ar15Attack',extras={}) {
...
test('simultaneous duplicate purchases debit TPG and deliver exactly once',async()=>{
  ...
  assert.equal(user.balance,2000);assert.equal(user.transactions.length,1);assert.deepEqual(user.tiranaStreetsInventory.weapons,['ar15Attack']);
  for(const r of results){assert.equal(r.body.balanceTPG,2000);assert.ok(r.body.ownedWeaponIds.includes('ak47VolleyAttack'));assert.ok(r.body.ownedWeaponIds.includes('combatKnife'));}
  assert.equal(user.transactions[0].token,'TPG');assert.equal(user.transactions[0].amount,-3000);
});
test('lost response can replay the same receipt, but a key cannot purchase another item',async()=>{
  ...
  assert.equal(user.balance,2000);assert.equal(user.transactions.length,1);
});
test('concurrent distinct references cannot buy the same unlock twice',async()=>{
  ...
  assert.deepEqual(results.map(r=>r.statusCode).sort(),[201,409]);assert.equal(user.balance,2000);assert.equal(user.transactions.length,1);
});
test('funds cannot go negative when different weapons race',async()=>{
  ...
  assert.deepEqual(results.map(r=>r.statusCode).sort(),[201,402]);assert.equal(user.balance,0);assert.equal(user.transactions.length,1);
});
test('client prices are ignored and starter equipment is never charged',async()=>{
  ...
  assert.equal(result.statusCode,201);assert.equal(user.balance,2000);
  const starter=await buy('purchase_starter_key','tirana-ak47VolleyAttack');assert.equal(starter.statusCode,400);assert.equal(user.balance,2000);
});
...
test('all five uploaded weapons can be purchased at the catalog price', async () => {
  for (const weapon of UPLOADED_WEAPONS) {
    const { user, buy } = account(10000);
    const result = await buy('uploaded_weapon_purchase', `tirana-${weapon.id}`, { priceTPG: 1 });
    assert.equal(result.statusCode, 201, weapon.id);
    assert.deepEqual(user.tiranaStreetsInventory.weapons, [weapon.id]);
    assert.equal(user.balance, 10000 - weapon.priceTPG);
    assert.equal(user.transactions[0].amount, -weapon.priceTPG);
  }
});
