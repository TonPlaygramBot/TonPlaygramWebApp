...
async function verifyByChain(input: {
  ...
  action: 'LINK_WALLET' | 'SET_PRIMARY' | 'UNLINK_WALLET';
  message?: string;
  ...
  if (input.chain === 'solana') {
    verifySolanaSignature({
      ...
      signature: (input.signature as number[]) ?? [],
...
export function walletsRouter(prisma: PrismaClient, nonceStore: NonceStore) {
  ...
  router.get('/link/nonce', async (req, res) => {
    ...
    if (chain === 'ton') return res.json({ nonce, tonProofPayload: nonce });
  ...
  router.post('/set-primary', async (req, res) => {
    try {
      ...
      await verifyByChain({ chain: wallet.chain as Chain, address: wallet.address, nonce, accountId: session.accountId, telegramUserId: session.telegramUserId, action: 'SET_PRIMARY', message, signature, tonProof });
  ...
  router.post('/remove', async (req, res) => {
    try {
      ...
      await verifyByChain({ chain: wallet.chain as Chain, address: wallet.address, nonce, accountId: session.accountId, telegramUserId: session.telegramUserId, action: 'UNLINK_WALLET', message, signature, tonProof });
