...
- Complete mapped footprint walls and roofs, roads and public footways are resident in spatial batches before gameplay. The minimum visibility radius is 2.2 km, with complete boundary blocks retained. Courtyard holes remain open. Nearby ornament is still queued independently. This trades additional resident geometry/startup work for predictable visibility; physical-phone memory and frame-rate measurements remain necessary.
...
Namazgah's uploaded scan and its runtime loader are removed. The replacement is an original, editable Blender exterior with four minarets, three balcony rings per minaret, a central dome, cascading domes, courtyard wings, arcades, window reveals and entrance steps. It has a 102,488-triangle near model and a 17,480-triangle synchronous fallback (six material draws each). Low graphics uses the fallback throughout; a failed near download leaves it visible and retries after 30 seconds. Source `.blend` files, generators and exported GLBs are included.
