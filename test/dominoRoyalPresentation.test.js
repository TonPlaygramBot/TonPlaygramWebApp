...
describe('Domino Royal lightweight presentation', () => {
  ...
  test('restores the shared seated human characters at every occupied chair', () => {
    ...
    expect(gameSource).toContain('applySeatedHumanHandTargets');
    expect(gameSource).toContain('applySeatedHumanReachPose');
    expect(gameSource).toContain('mesh.localToWorld');
