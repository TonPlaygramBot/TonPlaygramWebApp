...
describe('Pool Royale live cue contact stroke wiring', () => {
  ...
  it('keeps the shot unresolved until the animated cue delivers power', () => {
    expect(source).toContain('const cueImpactPending = Boolean(');
    expect(source).toContain('!cueStrokeStateRef.current.shotApplied');
    expect(source).toContain(`} else if (!any) {
              resolve();`);
  });
