...
describe('Pool Royale live cue contact stroke wiring', () => {
  ...
  it('falls back to the committed power scale if the impact payload has no launch velocity', () => {
    expect(source).toContain('resolvePoolRoyaleShotPowerScale(clampedPower)');
    expect(source).toContain('cue.vel.copy(shotDir3).multiplyScalar(SHOT_BASE_SPEED * fallbackPowerScale)');
  });
