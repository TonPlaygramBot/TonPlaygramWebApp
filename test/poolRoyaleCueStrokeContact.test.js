...
describe('Pool Royale live cue contact stroke wiring', () => {
  ...
  it('guarantees a non-zero cue-ball velocity fallback at impact', () => {
    expect(source).toContain('const shotDir2 = new THREE.Vector2(shotDir3.x, shotDir3.z)');
    expect(source).toContain('cue.vel.copy(shotDir2).multiplyScalar(speedBase * powerScale)');
    expect(source).toContain('cue.vel.lengthSq() <= 1e-10 && clampedPower >= MIN_SHOT_POWER_TO_FIRE');
  });
