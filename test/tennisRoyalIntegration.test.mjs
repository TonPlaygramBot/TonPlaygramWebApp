...
test('both online seats see fault, point and match-winning rebounds with one score and settlement', async () => {
  const ids = ['bounce-online-a', 'bounce-online-b'];
  ids.forEach((id) => user(id));
  const t = table('bounce-online', ids);
  let clock = 900000;
  const service = createTennisRoyal({
    io: {},
  ...
  try {
    await service.prepare(t);
    const clients = ids.map((id, i) => fakeSocket(`bounce-socket-${i}`, id));
    clients.forEach(service.attach);
    for (const client of clients)
      await client.request('tennisJoin', { tableId: t.id });
    const s = service.rooms.get(t.id).state;
    for (const serve of [true, false]) {
      ...
      clock += 33;
      service.tick();
      const first = await clients[0].request('tennisInput', {
        tableId: t.id,
        input: neutralInput()
      });
      assert.equal(first.data.state.phase, serve ? 'fault' : 'point');
      assert.ok(first.data.state.ball.vy > 0);
      const y = first.data.state.ball.y,
        count = s.pointCount;
      for (let i = 0; i < 4; i++) {
        clock += 33;
        service.tick();
      ...
      for (const client of clients)
        snapshots.push(
          (
            await client.request('tennisInput', {
              tableId: t.id,
              input: { ...neutralInput(), swing: 50 }
            })
          ).data.state
        );
      assert.deepEqual(snapshots[0], snapshots[1]);
      assert.ok(snapshots[0].ball.y > y + 0.2);
    ...
    assert.equal(s.phase, 'over');
    const y = s.ball.y,
      finalScore = structuredClone(s.score);
    for (let i = 0; i < 4; i++) {
      clock += 33;
      service.tick();
    }
    ...
    assert.deepEqual(s.score, finalScore);
    assert.equal(funds(ids[0]), 1100);
