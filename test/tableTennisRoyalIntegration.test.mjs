...
import { createTableTennisRoyal } from '../bot/services/tabletennisRoyal.js';
import {
  reserveTableTennisStake,
  settleTableTennisStake
} from '../bot/services/tabletennisStake.js';
import {
  createMemoryUser,
  findMemoryUser
} from '../bot/utils/memoryUserStore.js';
import { validateSeatTableRequest } from '../bot/config/onlineGamePolicy.js';
import { updateTableTennisCareer } from '../bot/services/tabletennisCareer.js';
import {
  createMatch,
  advance,
  setInput,
...
const table = (id, accounts = ['a', 'b']) => ({
  ...
  stake: 100,
  meta: { arena: 'dancingHall', format: 'set' },
...
test('TableTennis queue validates court, format, player count and TPG token', () => {
  const base = {
  ...
  for (const patch of [
    { stake: 0 },
    { maxPlayers: 3 },
    { matchMeta: { ...base.matchMeta, arena: 'ice' } },
...
test('Stake reservation, insufficiency, payout replay and refunds conserve balances', async () => {
  ...
  await reserveTableTennisStake(t);
  await reserveTableTennisStake(t);
  assert.equal(funds('ledger-a'), 900);
  assert.equal(funds('ledger-b'), 900);
  await assert.rejects(
    reserveTableTennisStake(table('duplicate', ['ledger-a', 'ledger-b'])),
    /active_match/
  );
  await assert.rejects(
    settleTableTennisStake('ledger', 'outsider'),
    /invalid_winner/
  );
  await settleTableTennisStake('ledger', 'ledger-a');
  await settleTableTennisStake('ledger', 'ledger-a');
  ...
  await assert.rejects(
    reserveTableTennisStake(table('poor-match', ['ledger-a', 'poor'])),
    /insufficient/
  );
  assert.equal(funds('ledger-a'), before);
  assert.equal(funds('poor'), 50);
  const refund = table('refund', ['ledger-a', 'ledger-b']);
  await reserveTableTennisStake(refund);
  await settleTableTennisStake('refund');
  await settleTableTennisStake('refund');
  assert.equal(funds('ledger-a'), 1100);
  assert.equal(funds('ledger-b'), 900);
});
test('Two clients share authoritative tabletennis; outsiders, spoofed identities and replaced sockets cannot move', async () => {
  user('a');
  user('b');
  const t = table('room'),
  ...
  assert.equal(
    (await outsider.request('tabletennisJoin', { tableId: t.id })).success,
    false
  );
  assert.equal(
    (await a.request('tabletennisJoin', { tableId: t.id, accountId: 'b' }))
      .success,
    false
  );
  assert.equal(
    (await a.request('tabletennisJoin', { tableId: t.id })).data.seat,
    0
  );
  assert.equal(
    (await b.request('tabletennisJoin', { tableId: t.id })).data.seat,
    1
  );
  await a.request('tabletennisInput', {
    tableId: t.id,
  ...
  const sa = await a.request('tabletennisInput', {
      tableId: t.id,
      input: { ...neutralInput(), swing: 1 }
    }),
    sb = await b.request('tabletennisInput', {
      tableId: t.id,
      input: neutralInput()
    });
  ...
  assert.equal(sa.data.state.config.gamesToWin, 2);
  assert.deepEqual(sa.data.state.score.games, [0, 0]);
  const replacement = fakeSocket('s-a-new', 'a');
  service.attach(replacement);
  await replacement.request('tabletennisJoin', { tableId: t.id });
  a.emit('disconnect');
  assert.equal(
    (
      await a.request('tabletennisInput', {
  ...
  assert.equal(
    (
      await replacement.request('tabletennisInput', {
        tableId: t.id,
        input: neutralInput()
      })
    ).success,
  ...
  await replacement.request('tabletennisLeave', { tableId: t.id });
...
test('Both absent clients get one refund and a lone connected player wins after grace', async () => {
  ...
  service.attach(stay);
  await stay.request('tabletennisJoin', { tableId: t2.id });
  clock += 61000;
  await stay.request('tabletennisInput', {
    tableId: t2.id,
    input: neutralInput()
  });
  service.tick();
...
test('Cancellation while the stake transaction runs refunds both accounts', async () => {
  ...
  const service = createTableTennisRoyal({
    ...
    autoTick: false,
    reserve: async (table) => {
      const r = await reserveTableTennisStake(table);
      table.players.pop();
      return r;
    }
  });
...
test('Career persists by account, rejects replay and awards only skill points', async () => {
  user('career');
  const start = await updateTableTennisCareer('career', 'start');
  const result = await updateTableTennisCareer('career', 'finish', {
    id: start.id,
    won: true,
    rally: 8
  });
  ...
  assert.equal(
    (
      await updateTableTennisCareer('career', 'finish', {
        id: start.id,
        won: true,
        rally: 8
      })
  ...
  assert.equal((await updateTableTennisCareer('career', 'get')).wins, 1);
  await assert.rejects(
    updateTableTennisCareer('other', 'finish', {
      id: start.id,
      won: true,
      rally: 8
    }),
  ...
  assert.equal(
    (await updateTableTennisCareer('career', 'upgrade', { stat: 0 }))
      .upgrades[0],
    1
  );
  await assert.rejects(
    updateTableTennisCareer('career', 'upgrade', { stat: 1 }),
    /unavailable/
  );
