...
test('eight-pool targets black when only 8 remains and group metadata is missing', () => {
  const decision = planShot({
    ...
    state: {
      balls: [
        { id: 0, x: 140, y: 240, vx: 0, vy: 0, pocketed: false },
        { id: 8, x: 520, y: 240, vx: 0, vy: 0, pocketed: false }
      ],
      pockets: [
        { x: 0, y: 0 }, { x: 500, y: 0 }, { x: 1000, y: 0 },
        { x: 0, y: 500 }, { x: 500, y: 500 }, { x: 1000, y: 500 }
...
test('fallback keeps targetPocket on mouth entry instead of pocket center', () => {
  const req = {
    game: 'AMERICAN_BILLIARDS',
    state: {
      balls: [
        { id: 0, x: 50, y: 50, vx: 0, vy: 0, pocketed: false },
        { id: 2, x: 220, y: 80, vx: 0, vy: 0, pocketed: false }
      ],
      pockets: [{ x: 300, y: 0 }],
      width: 300,
      height: 180,
  ...
  assert.equal(decision.targetBallId, 2)
  assert(decision.targetPocket)
  assert.notEqual(decision.targetPocket.y, 0)
})

test('safety falls back to legal contact and still targets an object ball', () => {
  const req = {
    game: 'AMERICAN_BILLIARDS',
    state: {
      balls: [
        { id: 0, x: 80, y: 240, vx: 0, vy: 0, pocketed: false },
        { id: 2, x: 260, y: 240, vx: 0, vy: 0, pocketed: false },
        { id: 12, x: 170, y: 240, vx: 0, vy: 0, pocketed: false },
        { id: 10, x: 260, y: 140, vx: 0, vy: 0, pocketed: false },
        { id: 11, x: 260, y: 340, vx: 0, vy: 0, pocketed: false }
      ],
      pockets: [{ x: 0, y: 0 }, { x: 520, y: 0 }, { x: 0, y: 480 }, { x: 520, y: 480 }],
      width: 520,
      height: 480,
      ballRadius: 10,
      friction: 0.01,
  ...
  const decision = planShot(req)
  assert.equal(decision.targetBallId, 2)
  assert.match(decision.rationale, /safety-legal-contact|one-cushion-kick/)
})
