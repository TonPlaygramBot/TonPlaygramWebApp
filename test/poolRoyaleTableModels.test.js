...
describe('Pool Royale table models', () => {
  ...
  test('Traditional Sketchfab table is selectable as an authentic local GLB option', () => {
    ...
    assert.equal(traditional.sourceFormat, 'sketchfab-original-glb');
  ...
  test('Traditional table installer fetches and validates the authentic Sketchfab GLB', async () => {
    ...
    assert.ok(source.includes('data?.glb?.url'));
    assert.ok(source.includes('validateAuthenticTraditionalGlb'));
    ...
    assert.ok(
      help.includes('--from /path/to/authentic-sketchfab-download.glb')
    );
  });

  test('Traditional table keeps installer and attribution files in git instead of binary GLB', async () => {
