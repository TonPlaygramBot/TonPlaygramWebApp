...
describe('Pool Royale table models', () => {
  ...
  test('Showood uses original GLB surface layout with slightly larger rail sights and apron', () => {
    ...
    assert.equal(showood.railSightApronVisualScale, 1.108);
    assert.equal(showood.railSightOutwardOffset, 0.064);
    assert.equal(showood.railSightVisualHeightScale, 1.132);
    assert.equal(showood.sideApronVisualHeightScale, 1.156);
    assert.equal(showood.sideApronOutwardOffset, 0.09);
    assert.equal(showood.shortRailMarkerOutwardOffset, 0.082);
    assert.equal(showood.brandPlateOutwardOffset, 0.106);
