...
describe('TPG game smart contract API descriptors', () => {
  test('every online lobby game uses private TPG identity and public profile fields', () => {
    expect(Object.keys(TPG_GAME_CONTRACTS)).toHaveLength(20);
  ...
  test('keeps each game option logic separate', () => {
    expect(getTpgGameContract('archeryroyal').matchmaking).toEqual([
      'stake',
      'arena',
      'format'
    ]);
    expect(getTpgGameContract('tennisroyal').matchmaking).toEqual(['stake', 'surface', 'format']);
