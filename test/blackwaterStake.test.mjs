...
import { createBlackwaterStakeService } from '../bot/services/blackwaterStake.js';
...
function fixture() {
  ...
  return {
    service: createBlackwaterStakeService({ database, UserModel, MatchModel }),
    users: () => users,
    matches: () => matches,
    fault: (id) => {
      fault = id;
...
test('TPG stakes reserve only on a complete contract and settle once from server outcome', async () => {
  ...
  assert.equal(f.users()[1].transactions[1].game, 'blackwater');
  assert.equal(
    f.users()[1].transactions[1].transactionId,
    'blackwater:grid-1:paid:tpg-2'
  );
});
