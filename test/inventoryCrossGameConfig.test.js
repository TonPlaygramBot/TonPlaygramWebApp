...
describe('cross-game inventory alignment', () => {
  ...
  test('murlan includes the runtime-only Sketchfab character roster', () => {
    const expectedCharacters = [
      ['sketchfab-agent-47', '/models/murlan/agent-47-rigged-face-morphs.glb'],
      ['sketchfab-leather-jacket-portrait', '/models/murlan/leather-jacket-portrait.glb'],
      ['sketchfab-seated-gentleman-suede', '/models/murlan/seated-gentleman-suede-jacket.glb'],
      ['sketchfab-red-hibiscus-hair', '/models/murlan/red-hibiscus-in-the-hair.glb'],
      ['sketchfab-casual-confidence', '/models/murlan/casual-confidence.glb']
    ];

    expectedCharacters.forEach(([themeId, modelUrl]) => {
      const theme = MURLAN_CHARACTER_THEMES.find((entry) => entry.id === themeId);
      expect(theme).toBeTruthy();
      expect(theme.modelUrls).toEqual([modelUrl]);
      expect(theme.sourceUrl).toContain('sketchfab.com/3d-models/');
      expect(theme.requiresAttribution).toBe(true);
