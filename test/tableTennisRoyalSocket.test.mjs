...
import { neutralInput } from '../shared/tabletennis/engine.js';
...
test(
  'two Socket.IO clients join one tabletennis match, exchange state and settle retirement once',
  { timeout: 12000 },
  async () => {
    ...
    const table = {
      ...
      stake: 100,
      meta: { arena: 'dancingHall', format: 'quick' },
      ready: new Set(ids)
    };
    ...
    try {
      ...
      for (let i = 0; i < 2; i++) {
        const joined = await ack(clients[i], 'tabletennisJoin', {
          tableId: table.id,
          accountId: ids[i]
        });
        assert.equal(joined.data.seat, i);
      }
      await ack(clients[0], 'tabletennisInput', {
        tableId: table.id,
        accountId: ids[0],
      ...
      assert.ok(
        states.every(
          (s) =>
            ...
            s.data.state.config.gamesToWin === 1
        )
      );
      assert.deepEqual(states[0].data.state.score, states[1].data.state.score);
      const retired = await ack(clients[0], 'tabletennisLeave', {
        tableId: table.id,
        accountId: ids[0]
      });
      assert.equal(retired.success, true);
      const result = await ack(clients[1], 'tabletennisInput', {
        tableId: table.id,
        accountId: ids[1],
        input: neutralInput()
      });
