...
const blockedWorkers=process.env.RACING_TEST_BLOCK_WORKERS==='1';
const directives={...contentSecurityPolicy.directives};
if(blockedWorkers)delete directives.workerSrc;
const policy=Object.entries(directives).map(([k,v])=>k.replace(/[A-Z]/g,c=>'-'+c.toLowerCase())+' '+v.join(' ')).join(';');
...
try{
 ...
 await page.addInitScript(()=>{window.policyViolations=[];document.addEventListener('securitypolicyviolation',e=>window.policyViolations.push({directive:e.effectiveDirective,blocked:e.blockedURI}));});
 await page.goto(`http://127.0.0.1:${server.address().port}`);
 const next=page.getByRole('button',{name:'Next vehicle',exact:true});
 await next.waitFor();
 if(blockedWorkers){
  await page.waitForFunction(()=>window.policyViolations.some(v=>v.directive==='worker-src'),{},{timeout:30000});
  const broken=await page.evaluate(()=>({selectionDisabled:document.querySelector('[aria-label="Next vehicle"]').disabled,modelCount:window.game.showroom.children.length,violations:window.policyViolations}));
  assert.equal(broken.selectionDisabled,true);assert.equal(broken.modelCount,0);
  console.log('Reproduced production policy bug:',JSON.stringify(broken));
  await writeFile(join(out,'blocked-results.json'),JSON.stringify(broken,null,2));
  await page.screenshot({path:join(out,'blocked-workers.png')});
  process.exitCode=0;
 }else{
 await page.waitForFunction(()=>!document.querySelector('[aria-label="Next vehicle"]').disabled,{},{timeout:60000});
 ...
 assert.equal(await page.getByRole('button',{name:'FIND TPG RACE',exact:true}).isEnabled(),true);
 ...
 await writeFile(join(out,'lobby-results.json'),JSON.stringify({viewport:{width:390,height:844},productionCsp:policy,wrapper:'Actual pages/Games/KartRoyale route with TPG matchmaking',vehicles:seen,swipeLeft:'apex → oobi',swipeRight:'oobi → apex',start,errors},null,2));
