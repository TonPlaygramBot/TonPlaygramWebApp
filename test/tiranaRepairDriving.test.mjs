...
test('five original tracks, cup indices and their physics exports are preserved', () => {
  ...
  for (const c of legacy.TRACKS)
    assert.equal(makeTrack(c.id), legacy.makeTrack(c.id));
  for (let i = 0; i < legacy.CUPS.length; i++)
    assert.equal(CUPS[i], legacy.CUPS[i]);
  assert.equal(createRacer, legacy.createRacer);
  assert.equal(stepRace, legacy.stepRace);
});
for (const difficulty of ['rookie', 'street', 'pro'])
  for (const config of TRACKS) {
    test(`${config.id}: six ${difficulty} AI karts finish the real race within the unchanged limit`, () => {
      const t = makeTrack(config.id),
        racers = Array.from({ length: 6 }, (_, i) =>
          createRacer(t, `test-${i}`, `AI ${i}`, i, true)
        );
      for (
        let tick = 1;
        tick <= RACE_LIMIT / STEP &&
        !racers.every((r) => r.finished || r.retired);
        tick++
      )
        stepRace(racers, t, STEP, tick * STEP, difficulty);
      for (const r of racers) {
        assert.ok(
          [r.x, r.z, r.yaw, r.health, r.finishTime].every(Number.isFinite)
        );
        assert.ok(
          r.finished && !r.retired,
          `${config.id}/${difficulty}/${r.slot}: lap ${r.lap}, health ${r.health}, index ${r.index}`
        );
        assert.ok(r.finishTime > 0 && r.finishTime <= RACE_LIMIT);
      }
    });
  }
test('verified Tirana street coordinates remain byte-identical', () => {
  const expected = {
    'tirana-routes.mjs': '69529cb43abe8754fcf347d6293bbc07911db94c'
  };
  for (const [name, sha] of Object.entries(expected)) {
    const b = readFileSync(
      new URL(`../webapp/src/games/kartroyale/${name}`, import.meta.url)
    );
    assert.equal(
