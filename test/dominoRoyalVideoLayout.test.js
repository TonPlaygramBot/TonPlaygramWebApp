...
test('Domino Royal bottom video frame is slightly larger than the Murlan Royale avatar', () => {
  ...
  const arena = fs.readFileSync(path.resolve(__dirname, '../webapp/src/pages/Games/dominoRoyalTemplate.js'), 'utf8');
  ...
  expect(arena).toContain('width: 3.5rem;');
  expect(arena).toContain('height: 3.5rem;');
});
...
test('Domino Royal builds only occupied Murlan-style chair and character seats', () => {
  ...
  expect(script).toContain('chairs.forEach((chair, visualSeatIndex) => {');
});
