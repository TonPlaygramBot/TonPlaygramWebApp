...
test('population persists, uses a varied fleet, and every bus has passengers',()=>{
 const s=state();assert.equal(s.traffic.filter(c=>c.model!=='tirana-bus').length,CITY_POPULATION.vehicles);assert.equal(s.traffic.filter(c=>c.model==='tirana-bus').length,30);assert.equal(s.shops.length,15);assert.equal(s.pickups.length,300);assert.ok(s.npcs.length>=600);
