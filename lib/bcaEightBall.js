...
export class BcaEightBall {
  ...
  shotTaken(shot = {}) {
    ...
    if (!foul && wasBreak && objectPotted.length === 0 &&
        (Array.isArray(shot.objectBallsToRailAfterContact)
          ? new Set(shot.objectBallsToRailAfterContact.map(Number).filter(id => Number.isInteger(id) && id >= 1 && id <= 15)).size
          : shot.railContactsAfterFirstHit ?? 0) < 4) {
      foul = true;
      reason = 'illegal break';
    }
    if (!foul && pottedEight && !eightLegallyOn && !wasBreak) {
      foul = true;
    ...
    s.ballInHandRegion = ballInHandNext ? (wasBreak ? 'headstring' : 'table') : null;
