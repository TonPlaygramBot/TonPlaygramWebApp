...
export class BcaEightBall {
  ...
  shotTaken(shot = {}) {
    ...
    const offTableIds = normalizePoolPots(shot.offTable, s.ballsOnTable, 15);
    const offTable = offTableIds.filter(id => id !== 0);
    const cueScratched = pottedList.includes(0) || offTableIds.includes(0) || Boolean(shot.cueOffTable);
    const shooterGroup = s.assignments[shooter];
    const shooterGroupRemaining = remainingGroupBalls(s.ballsOnTable, shooterGroup);
    // WPA 4.4 permits claiming an already-cleared group on an open table.
    const eightLegallyOn = Boolean(shooterGroup && shooterGroupRemaining === 0) ||
      (!isReference && !shooterGroup && !wasBreak &&
        ['SOLID', 'STRIPE'].some(group => remainingGroupBalls(s.ballsOnTable, group) === 0));
    const firstContactGroup = Number.isFinite(first) ? groupForBall(first) : null;
    const hasCall = !isReference && !wasBreak && shot.calledBallId != null;
    const calledBall = normalizePoolBallId(shot.calledBallId, 15);
    const calledGroup = groupForBall(calledBall);
    const legalCalledBall = calledBall === 8 ? eightLegallyOn
      : calledGroup && (!shooterGroup || calledGroup === shooterGroup);
    const calledPocket = shot.calledPocket;
    const calledPot = !hasCall || Boolean(legalCalledBall && objectPotted.includes(calledBall) &&
      (calledPocket == null || shot.pottedPockets?.[calledBall] === calledPocket));
    const safety = !isReference && !wasBreak && Boolean(shot.safety);
    ...
    } else if (first === 8 && (isReference || (!wasBreak && !eightLegallyOn))) {
      foul = true;
      reason = 'wrong first contact';
    }
    ...
    if (!foul && !isReference && pottedEight && !wasBreak &&
        (safety || (hasCall && (calledBall !== 8 || !calledPot)))) {
      foul = true;
      reason = 'black in uncalled pocket';
    }
    ...
    if (isReference && pottedEight) {
      ...
      if ((pottedEight || offTable.includes(8)) && !wasBreak) {
        frameOver = true;
        winner = other;
        s.ballsOnTable.delete(8);
      } else {
      ...
      if (!shooterGroup && (isReference || (!wasBreak && !safety && calledPot))) {
        const groupsPotted = new Set(objectPotted.map((id) => groupForBall(id)).filter(Boolean));
        if ((hasCall && calledGroup) || groupsPotted.size === 1) {
          const chosen = hasCall ? calledGroup : groupsPotted.has('SOLID') ? 'SOLID' : 'STRIPE';
      ...
      if (pottedEight && !wasBreak) {
        ...
        if (!scoredOwnGroup || safety || !calledPot) nextPlayer = other;
      }
    }
    for (const id of offTable) {
      if (id !== 8 || frameOver) s.ballsOnTable.delete(id);
    }
    s.currentPlayer = nextPlayer;

    s.ballInHand = ballInHandNext;
    s.mustPlayFromBaulk = Boolean(!isReference && wasBreak && ballInHandNext);
