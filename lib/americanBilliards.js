export class AmericanBilliards {
  ...
  shotTaken (shot = {}) {
    ...
    const explicitBreakLegal = shot.breakLegal
    const breakRailContacts = Number.isFinite(shot.breakRailContacts)
      ? shot.breakRailContacts
      : null
    ...
    if (
      ...
      !foul &&
      (
        explicitBreakLegal === false ||
        (breakRailContacts != null && breakRailContacts < 4 && pottedBalls.length === 0)
      )
    ) {
      foul = true
    ...
    if (foul) {
      ...
      if (isOpenTable && !s.breakInProgress) {
        const firstGroup = idToGroup(first)
        const legalFirstPot = pottedBalls.find(id => id === first && idToGroup(id) === firstGroup)
        const groupBall = legalFirstPot ?? pottedBalls.find(id => idToGroup(id))
        if (groupBall) {
