...
const EASY_POT_PROBABILITY = 0.45
const STRAIGHT_SHOT_THRESHOLD = Math.PI / 15 // ~12 degrees
const EASY_POT_FORCE_ATTACK_PROBABILITY = 0.52
const EASY_POT_FORCE_ATTACK_RISK = 0.22
const ATTACK_IF_COMPETITIVE_PROBABILITY = 0.42
const ATTACK_IF_COMPETITIVE_RISK = 0.38
const ATTACK_EV_MARGIN = 0.14
...
function estimatePotProbability (shot, state) {
  ...
  let P = angleScore * 0.58 + distScore * 0.12 + clearanceScore * 0.24 + entranceBonus * 0.06
...
function foulRisk (shot, state) {
  ...
  const cueAfter = simulateCueRollout(state, shot)
  let risk = 0
  if (state.pockets.some(p => dist(cueAfter, p) < state.ballRadius * 1.45)) {
    risk += 0.75
  }
  const cue = state.balls.find(b => b.colour === 'cue')
  if (shot.targetBall) {
    if (pocketBetween(cue, shot.targetBall, state.pockets, state.ballRadius)) {
      risk += 0.45
    }
...
function evaluateCandidates (state, colour, candidates) {
  ...
  if (bestPot) {
    if (bestPot.Ppot >= 0.4 && bestPot.risk <= 0.28) {
      return bestPot
    }
