...
function evaluateCandidates (state, colour, candidates) {
  let best = null
  const maxD = Math.hypot(state.width, state.height) || 1
  for (const c of candidates) {
    ...
    if (c.actionType === 'safety') {
      ...
      const baseValue = 1.08
      const straightBoost = typeof c.angle === 'number' ? Math.max(0, (Math.PI / 9 - c.angle) / (Math.PI / 9)) * 0.28 : 0
      const positionWeight = 1.55
      const shapeWindow = nextShapeWindow(nextState, colour)
      const ballsRemaining = nextState.balls.filter(b => b.colour === colour && !b.pocketed).length
      const finishWeight = ballsRemaining <= 2 ? 0.2 : 0.08
      const routeQuality = Math.max(0, Math.min(1, 1 - (c.distCT + c.distTP) / (maxD * 0.9)))
      EV =
        Ppot *
          (baseValue +
            nextBest * positionWeight +
            straightBoost +
            shapeWindow * 0.9 +
            routeQuality * 0.35 +
            finishWeight) -
