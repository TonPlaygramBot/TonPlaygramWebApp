...
function valueOfPositionAfter (nc, state, colour) {
  ...
  const shapeQuality = shapeScore(state, colour, cueAfter)
  return (P2 + straightBonus) * base + centerBonus + shapeQuality * 0.45
}
...
function evaluateCandidates (state, colour, candidates) {
  ...
  for (const c of candidates) {
    ...
    if (c.actionType === 'safety') {
      ...
      const shapeQuality = shapeScore(nextState, colour, cuePos)
      EV = Ppot * (baseValue + nextBest + shapeQuality * 0.6) - riskPenalty(risk)
    }
    if (!best || EV > best.EV) {
      best = { c, EV }
    }
...
function shapeScore (state, colour, cuePosition) {
  const cue = { x: cuePosition.x, y: cuePosition.y }
  const remaining = state.balls.filter(b => b.colour === colour && !b.pocketed)
  if (remaining.length === 0) return 1
  const cueBallId = state.balls.find(b => b.colour === 'cue')?.id
  let best = 0
  for (const ball of remaining) {
    const laneClear = !pathBlocked(cue, ball, state.balls, cueBallId != null ? [cueBallId, ball.id] : [ball.id], state.ballRadius)
    if (!laneClear) continue
    const d = dist(cue, ball)
    const pocketBias = Math.min(dist(ball, { x: state.width / 2, y: state.height / 2 }) / Math.hypot(state.width, state.height), 1)
    const proximity = 1 - Math.min(d / Math.hypot(state.width, state.height), 1)
    const straightView = typeof ball.angle === 'number' ? Math.max(0, 1 - ball.angle / (Math.PI / 3)) : 0
    best = Math.max(best, proximity * 0.6 + straightView * 0.2 + (1 - pocketBias) * 0.2)
  }
  return best
}
