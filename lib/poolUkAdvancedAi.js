...
function resolveLegalColours (state) {
  const rawBallOn = normaliseBallOn(state?.ballOn)
  if (rawBallOn === 'red' || rawBallOn === 'blue' || rawBallOn === 'black') {
    return [rawBallOn]
  }
  if (state?.isOpenTable) {
    const hasBlue = state.balls.some(b => !b.pocketed && b.colour === 'blue')
    const hasRed = state.balls.some(b => !b.pocketed && b.colour === 'red')
    const colours = []
    if (hasBlue) colours.push('blue')
    if (hasRed) colours.push('red')
    return colours
  }
  const hasBlue = state.balls.some(b => !b.pocketed && b.colour === 'blue')
  const hasRed = state.balls.some(b => !b.pocketed && b.colour === 'red')
  if (!hasBlue && !hasRed) return ['black']
  const colours = []
  if (hasBlue) colours.push('blue')
  if (hasRed) colours.push('red')
  return colours
...
function foulRisk (shot, state) {
  ...
  const cue = state.balls.find(b => b.colour === 'cue')
  if (shot.targetBall) {
    const legalColours = resolveLegalColours(state)
    if (legalColours.length > 0 && !legalColours.includes(shot.targetBall.colour)) {
      risk += 0.95
    }
    if (pathBlocked(cue, shot.targetBall, state.balls, [cue.id, shot.targetBall.id], state.ballRadius)) {
      risk += 0.9
    }
...
function chooseBestAction (state, colour) {
  ...
  const legalColours = resolveLegalColours(state)
  if (!legalColours.includes(colour)) {
    const fallbackColour = legalColours[0]
    if (!fallbackColour) return null
...
export function selectShot (state, rules = {}) {
  ...
  if (!state.isOpenTable && colour) {
    ...
    if (!hasOwn) colour = legalColours.includes('black') ? 'black' : legalColours[0] ?? colour
  }
  if (!colour && legalColours.length === 1) colour = legalColours[0]
  if (state.isOpenTable) {
    const planY = legalColours.includes('blue')
      ? chooseBestAction({ ...state, ballOn: 'blue', isOpenTable: false }, 'blue')
      : null
    const planR = legalColours.includes('red')
      ? chooseBestAction({ ...state, ballOn: 'red', isOpenTable: false }, 'red')
      : null
