...
const MIN_POT_PROBABILITY = 0.33
...
function chooseBestAction (state, colour) {
  ...
  if (straightShots.length > 0) {
    // Keep straight shots preferred, but do not discard higher-EV cut pots.
    candidates = candidates.filter(c => {
      if (c.actionType !== 'pot') return true
      const isStraight = typeof c.angle === 'number' && c.angle <= STRAIGHT_SHOT_THRESHOLD
      return isStraight || estimatePotProbability(c, state) >= 0.62
    })
  }
