...
export class UkPool {
  ...
  shotTaken (shot) {
    ...
    if (s.lastEvent === 'BREAK_START' && shot.potted.includes('black') && this.rules.blackOnBreak === 're-rack') {
      s.ballsOnTable = initialBalls();
      s.assignments = { A: null, B: null };
      s.isOpenTable = true;
      s.shotsRemaining = 1;
      this.startBreak();
      return { legal: true, foul: false, potted: [], nextPlayer: current,
        shotsRemainingNext: 1, frameOver: false, rerack: true };
    }
    ...
    // Pots stay down on a foul too; rule counts must match the visible table.
    {
