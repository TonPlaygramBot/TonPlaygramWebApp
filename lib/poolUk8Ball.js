...
export class UkPool {
  ...
  shotTaken (shot) {
    ...
    const blackOnBreak = isBreak && blackPotted;
    if (blackOnBreak && this.rules.blackOnBreak === 're-rack') {
      this.state.ballsOnTable = initialBalls();
      this.state.assignments = { A: null, B: null };
      this.state.isOpenTable = true;
      this.state.shotsRemaining = 1;
      this.state.mustPlayFromBaulk = true;
      this.state.lastEvent = 'BREAK_RERACK';
      return {
        legal: true,
        foul: false,
        potted: [],
    ...
    const spotBlackOnBreak = blackOnBreak && this.rules.blackOnBreak === 'spot';
    if (spotBlackOnBreak) {
      blackPotted = false;
    }
    if (!foul && blackPotted) {
