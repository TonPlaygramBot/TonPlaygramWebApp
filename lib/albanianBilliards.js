...
export class AlbanianBilliards {
  ...
  shotTaken (shot = {}) {
    ...
    // Object balls remain pocketed. In Albanian 61 a ball made during a foul
    // is credited at face value to the incoming player, rather than being
    // silently discarded from the score.
    objectPots.forEach((id) => state.ballsOnTable.delete(id))
    const foul = Boolean(reason)
    const pointsScored = objectPots.reduce((total, id) => total + id, 0)
    const scoringPlayer = foul ? opponent : shooter
    if (pointsScored) state.scores[scoringPlayer] += pointsScored

    if (state.scores[scoringPlayer] >= state.targetScore) {
    ...
    return this.result(foul, reason, uniquePots, pointsScored, keepTurn, scoringPlayer)
  }

  result (foul, reason, potted, pointsScored, keepTurn = false, scoringPlayer = null) {
    return {
      legal: !foul,
      foul,
      ...
      potted,
      pointsScored,
      pointsAwardedTo: pointsScored > 0 ? scoringPlayer : null,
      keepTurn,
