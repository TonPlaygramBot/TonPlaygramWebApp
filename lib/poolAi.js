...
export function planShot (req) {
  ...
  const spinMagnitude = (spin = {}) => Math.abs(spin.top || 0) + Math.abs(spin.side || 0) + Math.abs(spin.back || 0)
  const shouldReplaceBest = (candidate, currentBest) => {
    if (!currentBest) return true
    const qDelta = (candidate?.quality ?? 0) - (currentBest?.quality ?? 0)
    if (qDelta > 1e-3) return true
    if (Math.abs(qDelta) <= 0.02) {
      const candSpin = spinMagnitude(candidate?.spin)
      const bestSpin = spinMagnitude(currentBest?.spin)
      if (candSpin + 1e-6 < bestSpin) return true
      if (Math.abs(candSpin - bestSpin) <= 1e-3) {
        return (candidate?.power ?? Infinity) < (currentBest?.power ?? Infinity)
      }
    }
    return false
  }
  ...
  const powers = req.state?.breakInProgress
    ? [1 * POWER_SCALE]
    : [0.42 * POWER_SCALE, 0.58 * POWER_SCALE, 0.74 * POWER_SCALE, 0.9 * POWER_SCALE]
  const defaultSpins = [
    { top: 0, side: 0, back: 0 },
    ...
    for (const { target, pocket } of candidatePairs) {
      ...
      for (const cuePos of placements) {
        ...
        if (baseCand) {
          ...
          if (shouldReplaceBest(rest, best)) {
