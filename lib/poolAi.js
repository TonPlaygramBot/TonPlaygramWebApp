...
const EIGHT_BALL_LOOKAHEAD_DEPTH = 3
const EIGHT_BALL_MONTE_CARLO_MULTIPLIER = 1.35
...
function isEightBall (req) {
  return req?.game === 'EIGHT_POOL_UK'
}
...
function monteCarloPotChance (req, cue, target, entry, ghost, balls, samples = 20, rng = Math.random) {
  ...
  const gameMultiplier = isEightBall(req) ? EIGHT_BALL_MONTE_CARLO_MULTIPLIER : 1
  const sampleCount = Math.max(
    samples,
    Math.round(MONTE_CARLO_BASE_SAMPLES * gameMultiplier * (1 + distanceFactor))
  )
...
function evaluate (req, cue, target, pocket, power, spin, ballsOverride, strict = false, options = {}) {
  ...
  const pocketLaneClearance = clearanceMargin(target, entry, balls, [0, target.id], r, 1.2)
  if (laneClearance < 0.5) {
    return null
  }
  if (pocketLaneClearance < 0.45) {
    return null
  }
  ...
  const lookaheadDepth = Number.isFinite(options.lookaheadDepth)
    ...
    : isEightBall(req)
      ? EIGHT_BALL_LOOKAHEAD_DEPTH
      : LOOKAHEAD_DEPTH
  const runoutPotential = options.skipLookahead
    ? 0
    : estimateRunoutPotential(req, cueAfter, target.id, balls, lookaheadDepth, rng)
  const clusterPenalty = isEightBall(req)
    ? Math.max(0, (0.55 - pocketLaneClearance) * 0.22)
    : 0
  const positionReward = isEightBall(req) ? 0.1 * runoutPotential + 0.08 * nextScore : 0
...
export function planShot (req) {
  ...
  const tacticalEightBall = isEightBall(req)
  const powers = tacticalEightBall ? [0.42, 0.56, 0.7, 0.82, 0.92] : [0.5, 0.7, 0.85]
  const spins = tacticalEightBall
    ? [
        { top: 0, side: 0, back: 0 },
        { top: 0.22, side: 0, back: -0.2 },
        { top: 0.34, side: 0, back: -0.12 },
        { top: -0.2, side: 0.2, back: 0 },
        { top: -0.2, side: -0.2, back: 0 },
        { top: 0.12, side: 0.22, back: 0 },
        { top: 0.12, side: -0.22, back: 0 }
      ]
    : [
        { top: 0, side: 0, back: 0 },
    ...
    for (const { target, pocket } of candidatePairs) {
      ...
      for (const cuePos of placements) {
        ...
        const baseCand = evaluate(
          ...
          strict,
          {
            lookaheadDepth: tacticalEightBall ? EIGHT_BALL_LOOKAHEAD_DEPTH : LOOKAHEAD_DEPTH,
            rng
          }
        )
        if (baseCand) {
