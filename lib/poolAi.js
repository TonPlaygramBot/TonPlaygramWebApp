...
function evaluate (req, cue, target, pocket, power, spin, ballsOverride, strict = false, options = {}) {
  ...
  const postImpactScratch = scratchRiskAlongLine(target, cueAfter, req.state.pockets || [], r)
  ...
  const riskNearPocket = req.state.pockets.some(p => dist(cueAfter, p) < r * 1.2) ? 1 : 0
  const spinRisk = Math.min(1, Math.abs(spin.side) * 0.6 + Math.max(0, spin.top + spin.back) * 0.3)
  const powerRisk = Math.max(0, (power - 0.65) / 0.35)
  const foulRisk = Math.min(1, 0.55 * riskNearPocket + 0.3 * (postImpactScratch ? 1 : 0) + 0.1 * spinRisk + 0.05 * powerRisk)
  const shotVec = { x: target.x - cue.x, y: target.y - cue.y }
  const potVec = { x: entry.x - target.x, y: entry.y - target.y }
  ...
  if (strict && (centerAlign < 0.5 || pocketOpen < 0.3 || foulRisk > 0.55)) {
    return null
  }
  if (!strict && foulRisk > 0.8) {
    return null
  }
  ...
  return {
    ...
    aimPoint: ghost,
    quality,
    rationale: `target=${target.id} pocket=(${pocket.x.toFixed(0)},${pocket.y.toFixed(0)}) angle=${angle.toFixed(2)} power=${power.toFixed(2)} spin=${spin.top.toFixed(2)},${spin.side.toFixed(2)},${spin.back.toFixed(2)} pc=${potChance.toFixed(2)} ca=${centerAlign.toFixed(2)} nh=${nearHole.toFixed(2)} np=${nextScore.toFixed(2)} fr=${foulRisk.toFixed(2)}`,
    nextScore,
    hasNext
  }
}
...
function legalTargetSuggestion (req) {
  const cueBallId = resolveCueBallId(req.state)
  const cue = req.state.balls.find(b => b.id === cueBallId)
  if (!cue) return null

  const strictCandidates = clearShotCandidates(req)
  const preferred = strictCandidates[0]
  if (preferred) {
    const entry = pocketEntry(preferred.pocket, req.state.ballRadius, req.state.width, req.state.height, preferred.target)
    const ghost = {
      x: preferred.target.x - (entry.x - preferred.target.x) * (req.state.ballRadius * 2 / dist(preferred.target, entry)),
      y: preferred.target.y - (entry.y - preferred.target.y) * (req.state.ballRadius * 2 / dist(preferred.target, entry))
    }
    return {
      suggestedTargetBallId: preferred.target.id,
      suggestedAimPoint: ghost,
      suggestedPocket: entry
    }
  }

  const fallback = fallbackAimAtTarget(req)
  if (!fallback) return null
  return {
    suggestedTargetBallId: fallback.targetBallId,
    suggestedAimPoint: fallback.aimPoint,
    suggestedPocket: fallback.targetPocket
  }
}

function withSuggestion (decision, suggestion) {
  if (!suggestion) return decision
...
export function planShot (req) {
  ...
  const suggestion = legalTargetSuggestion(req)
  let best = null
  let fallback = null
  ...
  const powers = req.state?.breakInProgress
    ? [1 * AI_POWER_SCALE]
    : [0.5 * AI_POWER_SCALE, 0.7 * AI_POWER_SCALE, 0.85 * AI_POWER_SCALE]
  const spins = [
    { top: 0, side: 0, back: 0 },
          ...
          for (const spin of spins) {
            ...
            if (Date.now() > deadline) {
              return withSuggestion(
                best && best.quality >= 0.1 ? best : fallbackAimAtTarget(req) || safetyShot(req),
                suggestion
              )
            }
            const cand = evaluate(
  ...
  if (best) {
    if (best.quality >= 0.1) return withSuggestion(best, suggestion)
    if (hasViableShot) return withSuggestion(best, suggestion)
  }
  if (!hasViableShot) return withSuggestion(safetyShot(req), suggestion)
  fallback = fallbackAimAtTarget(req)
  if (fallback) return withSuggestion(fallback, suggestion)
  return withSuggestion(safetyShot(req), suggestion)
}
