...
export class NineBall {
  ...
  shotTaken (shot = {}) {
    ...
    const railContactsAfterFirstHit = Array.isArray(shot.objectBallsToRailAfterContact)
      ? new Set(shot.objectBallsToRailAfterContact.map(Number).filter(id => Number.isInteger(id) && id >= 1 && id <= 9)).size
      : Math.max(0, Number(shot.railContactsAfterFirstHit) || 0);
    let foul = false;
    let reason = '';
