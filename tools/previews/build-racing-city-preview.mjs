...
const previewIds=['skanderbeg','farke'];
...
const result=await build({entryPoints:[resolve(here,'racing-city-preview.tsx')],bundle:true,write:false,format:'esm',minify:true,platform:'browser',jsx:'transform',plugins:[{name:'portable-preview',setup(b){
...
let fragment=await readFile(resolve(here,'racing-city-preview.html'),'utf8');
...
const output=process.argv[2]||'/workspace/racing-royal-city-career.html';await writeFile(output,fragment);console.log(JSON.stringify({output,bytes:Buffer.byteLength(fragment),courses:courses.map(c=>({id:c.track.id,buildings:c.buildings.length,trees:c.trees.length}))}));
