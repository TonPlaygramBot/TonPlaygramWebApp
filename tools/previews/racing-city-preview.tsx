...
import {CITY_JOBS,createCityJob,stepCityJob} from '../../webapp/src/games/kartroyale/cityJobs.mjs';
import {CityJobGuide} from '../../webapp/src/games/kartroyale/CityJobGuide';
import {createRacingRoadside} from '../../webapp/src/games/kartroyale/RacingRoadside';
...
function App(){
 ...
 const [course,setCourse]=useState(0),[difficulty,setDifficulty]=useState('street'),[mode,setMode]=useState('career');
 const view=useRef('driver');const [cameraView,setCameraView]=useState('driver');
 ...
 const [hud,setHud]=useState({speed:0,boost:45,lap:1,position:1,drifting:false,time:0,finished:false,turbo:0,driftCharge:0,slipstream:0,bump:0,impact:0,boostEvent:0,reversing:false,impactMaterial:'',waterRecovery:0,airborne:false,impactCooldown:0,cityJob:null});
 useEffect(()=>{
  ...
  const data=DATA.courses[course],free=mode!=='race',track=free?{...data.track,terrainMode:'regional'}:data.track,drivingWorld=free?createDrivingWorld(data):null;
  ...
  const effects=new RaceEffects(camera);world.add(effects.group);const guide=new CityJobGuide();world.add(guide.group);guide.group.visible=false;if(!free)world.add(createRacingRoadside(track));let cityJob=null;const jobDefinition=CITY_JOBS.find(j=>j.track===track.id);
  ...
  const begin=()=>{clear();effects.clear();lastImpacts.clear();motions=models.map(()=>new KartMotion());racers=Array.from({length:free?1:6},(_,i)=>equipKart(createRacer(track,String(i),i?'Rival':'You',i,i>0),selectedId(i)));run=true;orbitControls.enabled=false;time=0;accumulator=0;cityJob=mode==='career'&&jobDefinition?createCityJob(jobDefinition.id,track):null;const r=racers[0];if(free){const p=track.points[0];Object.assign(r,{x:p.x,z:p.z,yaw:p.yaw,velocityYaw:p.yaw,index:0,roamRecovery:{x:p.x,z:p.z,yaw:p.yaw}});resetJump(r,track);}camera.position.set(r.x-Math.sin(r.yaw)*7,3.5+(r.groundY||0),r.z-Math.cos(r.yaw)*7);setPause(false);setRacing(true);};
  ...
  const animate=(now:number)=>{
   ...
   if(run&&!pause&&!racers[0].finished&&(!cityJob||cityJob.status==='active')){accumulator+=dt;while(accumulator>=STEP){Object.assign(racers[0].input,input.current.read());time+=STEP;if(drivingWorld)stepRacer(racers[0],racers[0].input,track,STEP,time,difficulty,drivingWorld);else stepRace(racers,track,STEP,time,difficulty);if(cityJob)stepCityJob(cityJob,racers[0],STEP);accumulator-=STEP;}}
   models.forEach((m,i)=>{const r=run?racers[i]:{steering:0,acceleration:0,yawRate:0,speed:0,throttle:0,braking:false};m.visible=run||i===0;
    ...
    const motion=motions[i];motion.update(r,run&&!pause&&!r.finished&&(!cityJob||cityJob.status==='active')?dt:0,radii[i],matchMedia('(prefers-reduced-motion: reduce)').matches);
    ...
    drivers[i].update(r,time,run&&i===0&&view.current==='driver',matchMedia('(prefers-reduced-motion: reduce)').matches);
    ...
    const wheel=m.getObjectByName('steering_wheel');if(wheel)wheel.rotation.z=r.steering*.65;for(const name of ['pedal_brake','pedal_throttle']){const p=m.getObjectByName(name);if(p)p.rotation.x=(name==='pedal_brake'?Number(r.braking):r.throttle||0)*.23;}
   ...
   world.visible=run;showroom.visible=!run;if(run)effects.update(pause||racers[0].finished||(cityJob&&cityJob.status!=='active')?0:dt,racers,effectVisuals,'0',false);
   if(run&&cityJob)guide.update(cityJob,track,racers[0]);
   if(run&&view.current==='driver'){const r=racers[0],m=models[0];m.updateMatrixWorld(true);const eye=m.getObjectByName('driver_eye');if(eye)eye.getWorldPosition(camera.position);camera.lookAt(camera.position.x+Math.sin(r.yaw)*24,camera.position.y-5.5,camera.position.z+Math.cos(r.yaw)*24);camera.near=.025;camera.fov=T.MathUtils.clamp(T.MathUtils.radToDeg(2*Math.atan(Math.tan(T.MathUtils.degToRad(28))/camera.aspect)),68,96);camera.updateProjectionMatrix();}
   else if(run){const r=racers[0],target=new T.Vector3(r.x-Math.sin(r.yaw)*7,3.5+(r.groundY||0)+(r.jumpHeight||0),r.z-Math.cos(r.yaw)*7);camera.position.lerp(target,1-Math.exp(-dt*7));camera.lookAt(r.x+Math.sin(r.yaw)*3,1.1+(r.groundY||0)+(r.jumpHeight||0),r.z+Math.cos(r.yaw)*3);camera.fov+=(60+(matchMedia('(prefers-reduced-motion: reduce)').matches?0:r.speed*.12)-camera.fov)*(1-Math.exp(-dt*5));camera.updateProjectionMatrix();}
   else{orbitControls.update();}
   if(run&&now-ui>100){ui=now;const r=racers[0];setHud({speed:r.speed,boost:r.boost,lap:Math.max(1,Math.min(3,r.lap)),position:standings(racers).findIndex(r=>r.id==='0')+1,drifting:r.drifting,time,finished:r.finished||(cityJob&&cityJob.status!=='active'),cityJob:cityJob?{...cityJob}:null,turbo:r.turbo,driftCharge:r.driftCharge,slipstream:r.slipstream,bump:r.bumpImpact||0,impact:r.hitFlash||0,boostEvent:r.boostEvent||0,reversing:r.reversing||r.speed<-.1,impactMaterial:r.impactMaterial||'',waterRecovery:r.waterRecovery||0,airborne:!!r.airborne,impactCooldown:r.impactCooldown||0});}
 ...
 return <div className="rr-preview rr-remake">
  ...
  <header><span><b>RACING ROYAL</b><small>{DATA.courses[course].track.name} · {(DATA.courses[course].track.length/1000).toFixed(2)} km</small></span>{racing?<><button onClick={()=>{view.current=view.current==='driver'?'chase':'driver';setCameraView(view.current);}}>{cameraView==='driver'?'CHASE':'DRIVER'}</button><button onClick={()=>controller.current?.pause()}>{paused?'RESUME':'PAUSE'}</button></>:<span className="rr-preview-tag">KART SERIES</span>}</header>
  {!racing&&<div className="rr-preview-garage"><div className="rr-preview-picks" aria-label="Driving mode"><button aria-pressed={mode==='career'} onClick={()=>setMode('career')}>CITY JOB</button><button aria-pressed={mode==='race'} onClick={()=>setMode('race')}>RACE</button><button aria-pressed={mode==='free'} onClick={()=>setMode('free')}>FREE ROAM</button></div><label className="rr-kart-choice">KART<select aria-label="Kart model" value={kart} onChange={e=>setKart(e.target.value)}>{choices.map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><div className="rr-preview-options"><label>CIRCUIT<select aria-label="Preview circuit" value={course} onChange={e=>setCourse(Number(e.target.value))}>{DATA.courses.map((c,i)=><option key={c.track.id} value={i}>{c.track.name}</option>)}</select></label>{mode==='race'&&<label>RIVALS<select aria-label="Rival difficulty" value={difficulty} onChange={e=>setDifficulty(e.target.value)}><option value="street">Street</option><option value="pro">Pro</option></select></label>}</div><button className="rr-preview-start" disabled={!ready} onClick={()=>controller.current?.begin()}>{ready?(mode==='career'?'START CITY JOB':mode==='free'?'EXPLORE TIRANA':'START RACE'):'LOADING KARTS…'}</button></div>}
  {racing&&<><div className="rr-preview-stats">{mode!=='race'?<span>{mode==='career'?(CITY_JOBS.find(j=>j.track===DATA.courses[course].track.id)?.title):'FREE ROAM'}</span>:<><b>{hud.position}/6</b><span>LAP {hud.lap}/3</span><time>{Math.floor(hud.time/60)}:{(hud.time%60).toFixed(1).padStart(4,'0')}</time></>}</div><div className="rr-speed"><b>{hud.reversing&&<small>R </small>}{Math.round(Math.abs(hud.speed)*3.6)}</b><span>KM/H</span></div>
   {hud.cityJob&&<div className="rr-preview-job"><b>{hud.cityJob.targets[hud.cityJob.stage]?.label||hud.cityJob.reason}</b><span>{Math.ceil(CITY_JOBS.find(j=>j.id===hud.cityJob.id)!.seconds-hud.cityJob.elapsed)}s · {Math.round(hud.cityJob.distance)} m{hud.cityJob.status==='active'&&hud.cityJob.distance<8?' · Stop, then release both pedals':''}</span>{hud.cityJob.hold>0&&<progress max={1.5} value={hud.cityJob.hold}/>}</div>}<div className="rr-preview-feedback" aria-live="polite">{hud.waterRecovery>0?'WATER · RECOVERING':hud.impactCooldown>0&&hud.impactMaterial?hud.impactMaterial.toUpperCase()+' IMPACT':hud.airborne?'AIRBORNE':''}</div>
   ...
   {(paused||hud.finished)&&<div className="rr-preview-overlay"><b>{hud.finished?(hud.cityJob?.reason||'FINISH'):'PAUSED'}</b><button onClick={()=>hud.finished?controller.current?.begin():controller.current?.pause()}>{hud.finished?'TRY AGAIN':'RESUME'}</button><button onClick={()=>controller.current?.garage()}>GARAGE</button></div>}
  </>}
  {error&&<div className="rr-preview-overlay" role="alert">{error}</div>}
 </div>;
}
createRoot(document.getElementById('racing-city-career')!).render(<App/>);
