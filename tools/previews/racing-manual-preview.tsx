...
import {createTyreBarrierLayer} from '../../webapp/src/games/kartroyale/TyreBarrierLayer';
...
function App(){
 ...
 useEffect(()=>{
  ...
  const {track,buildings,trees}=DATA.courses[course];
  ...
  for(const b of buildings){const shape=new T.Shape(b.p.map(p=>new T.Vector2(p[0],-p[1]))),g=new T.ExtrudeGeometry(shape,{depth:b.h,bevelEnabled:false});g.rotateX(-Math.PI/2);world.add(new T.Mesh(g,new T.MeshStandardMaterial({color:b.color,roughness:.9})));}
  ...
  world.add(createBoostPadLayer(track),createRoadBumpLayer(track),createTyreBarrierLayer(track));const effects=new RaceEffects(camera);world.add(effects.group);
  ...
  const lastImpacts=new Map<string,number>(),wheelBases=new Map<T.Object3D,number>();
  ...
  const begin=()=>{clear();effects.clear();lastImpacts.clear();motions=models.map(()=>new KartMotion());racers=Array.from({length:6},(_,i)=>equipKart(createRacer(track,String(i),i?'Rival':'You',i,i>0),i?choices[i%3][0]:kart));run=true;time=0;accumulator=0;const r=racers[0];camera.position.set(r.x-Math.sin(r.yaw)*7,3.5,r.z-Math.cos(r.yaw)*7);setPause(false);setRacing(true);};
  ...
  const animate=(now:number)=>{
   ...
   models.forEach((m,i)=>{const r=run?racers[i]:{steering:0,acceleration:0,yawRate:0,speed:0,throttle:0,braking:false};m.visible=run||i===0;
    ...
    for(const name of ['fl','fr','rl','rr']){const w=m.getObjectByName('wheel_'+name);if(w){w.rotation.x=motion.wheelSpin;if(!wheelBases.has(w))wheelBases.set(w,w.position.y);w.position.y=wheelBases.get(w)!+(matchMedia('(prefers-reduced-motion: reduce)').matches?0:r.suspension?.wheels[['fl','fr','rl','rr'].indexOf(name)]||0);}}
  ...
  Promise.all([parse(DATA.models.photon),parse(DATA.models.vortex),parse(DATA.models.aegis),parse(DATA.models.driver)]).then(assets=>{
   if(!alive){assets.forEach(a=>a.scene.traverse(o=>{o.geometry?.dispose();o.material?.dispose();}));return;}
   models=Array.from({length:6},(_,i)=>{const model=assets[i===0?choices.findIndex(c=>c[0]===kart):i%3].scene.clone(true);model.traverse(o=>{if(o.isMesh)o.material=o.material.clone();});model.traverse(o=>{if(o.isMesh&&o.material.name==='paint')o.material.color.set(i?COLORS[i]:'#46c8f0');});const driver=new KartDriver(assets[3].scene,i?COLORS[i]:'#46c8f0');drivers.push(driver);(model.getObjectByName('body')||model).add(driver.root);kartGroup.add(model);return model;});
 ...
 return <div className="rr-preview rr-remake">
  ...
  <header><span><b>RACING ROYAL</b><small>{DATA.courses[course].track.name} · {(DATA.courses[course].track.length/1000).toFixed(2)} km</small></span>{racing?<button onClick={()=>controller.current?.pause()}>{paused?'RESUME':'PAUSE'}</button>:<span className="rr-preview-tag">FUTURE SERIES</span>}</header>
  {!racing&&<div className="rr-preview-garage"><div className="rr-preview-options"><label>CIRCUIT<select aria-label="Preview circuit" value={course} onChange={e=>setCourse(Number(e.target.value))}>{DATA.courses.map((c,i)=><option key={c.track.id} value={i}>{c.track.name}</option>)}</select></label><label>RIVALS<select aria-label="Rival difficulty" value={difficulty} onChange={e=>setDifficulty(e.target.value)}><option value="street">Street</option><option value="pro">Pro</option></select></label></div><div className="rr-preview-picks">{choices.map(([id,name])=><button key={id} aria-pressed={kart===id} onClick={()=>setKart(id)}>{name}</button>)}</div><button className="rr-preview-start" disabled={!ready} onClick={()=>controller.current?.begin()}>{ready?'START RACE':'LOADING KARTS…'}</button></div>}
