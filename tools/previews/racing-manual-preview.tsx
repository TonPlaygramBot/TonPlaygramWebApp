...
function App(){
 ...
 const [hud,setHud]=useState({speed:0,boost:45,lap:1,position:1,drifting:false,time:0,finished:false});
 useEffect(()=>{
  ...
  const camera=new T.PerspectiveCamera(52,1,.08,600),world=new T.Group(),kartGroup=new T.Group();scene.add(world,kartGroup);
  ...
  for(const p of boostPads(track)){const strip=new T.Mesh(new T.PlaneGeometry(p.width||4,3),new T.MeshBasicMaterial({color:'#80ecf0'}));strip.rotation.set(-Math.PI/2,0,-p.yaw);strip.position.set(p.x,.051,p.z);world.add(strip);}
  ...
  let racers=[],models=[],drivers=[],run=false,pause=false,time=0,accumulator=0,last=performance.now(),ui=0,orbit=0;
  const clear=()=>input.current.clear();
  const setPause=(value:boolean)=>{pause=value;clear();setPaused(value);};
  const begin=()=>{clear();racers=Array.from({length:3},(_,i)=>equipKart(createRacer(track,String(i),i?'Rival':'You',i,i>0),i?choices[i][0]:kart));run=true;time=0;accumulator=0;const r=racers[0];camera.position.set(r.x-Math.sin(r.yaw)*7,3.5,r.z-Math.cos(r.yaw)*7);setPause(false);setRacing(true);};
  ...
  const animate=(now:number)=>{
   ...
   models.forEach((m,i)=>{const r=run?racers[i]:{steering:0,acceleration:0,yawRate:0,speed:0,throttle:0,braking:false};m.visible=run||i===0;
    if(run){m.position.set(r.x,.06,r.z);m.rotation.y=r.yaw;}else{m.position.set(0,0,0);m.rotation.y=0;}
    drivers[i].update(r,time,false,matchMedia('(prefers-reduced-motion: reduce)').matches);
    for(const name of ['fl','fr','rl','rr']){const w=m.getObjectByName('wheel_'+name);if(w)w.rotation.x+=r.speed*(pause?0:dt)/.28;}
    for(const name of ['steer_fl','steer_fr']){const w=m.getObjectByName(name);if(w)w.rotation.y=-r.steering*.42;}
   ...
   world.visible=run;
   if(run){const r=racers[0],target=new T.Vector3(r.x-Math.sin(r.yaw)*7,3.5,r.z-Math.cos(r.yaw)*7);camera.position.lerp(target,1-Math.exp(-dt*7));camera.lookAt(r.x+Math.sin(r.yaw)*3,1.1,r.z+Math.cos(r.yaw)*3);}
   else{orbit+=dt*.15;camera.position.set(Math.sin(.65+orbit)*4.5,2.3,Math.cos(.65+orbit)*4.5);camera.lookAt(0,.65,0);}
   if(run&&now-ui>100){ui=now;const r=racers[0];setHud({speed:r.speed,boost:r.boost,lap:Math.max(1,Math.min(3,r.lap)),position:standings(racers).findIndex(r=>r.id==='0')+1,drifting:r.drifting,time,finished:r.finished});}
...
createRoot(document.getElementById('racing-manual-preview')!).render(<App/>);
