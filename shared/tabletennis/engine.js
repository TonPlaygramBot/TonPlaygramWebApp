...
import { HALF_LENGTH, TABLE_HEIGHT, BALL_RADIUS, GRAVITY, NET_HEIGHT, TABLE_GRIP, TABLE_RESTITUTION, initialScore, awardPoint, opponent } from './rules.js';
import { advanceBall, ballAtRest, gravity } from './physics.js';
export * from './rules.js';
export const side = (seat, state) => (seat === 0 ? 1 : -1) * (state?.endsSwapped ? -1 : 1);
...
export function createMatch(config = {}) {
    ...
    c.difficulty = clamp(Number.isFinite(c.difficulty) ? Math.floor(c.difficulty) : 1, 0, 2);
...
function event(s, type, seat, label) {
    s.events.push({ id: ++s.eventId, type, seat, label });
    if (s.events.length > 12)
        s.events.shift();
}
function clearSwings(s) {
    s.players.forEach((p) => {
        p.queued = -10;
        p.queuedInput = null;
    });
}
export function setInput(s, seat, p) {
    if (seat !== 0 && seat !== 1)
        return;
    const old = s.inputs[seat], finite = (v, fallback) => typeof v === 'number' && Number.isFinite(v) ? v : fallback;
    const swing = Number.isSafeInteger(p.swing) && p.swing >= old.swing
        ? p.swing
        : old.swing;
    const raw = p.direction === undefined ? old.direction : p.direction;
    const norm = raw && Number.isFinite(raw.x) && Number.isFinite(raw.z)
    ...
    s.inputs[seat] = {
        ...
        direction: raw && norm > 0.001 && Number.isFinite(norm)
...
function recordHit(s, seat, input, serve) {
    const b = s.ball;
...
function serve(s) {
    const seat = s.score.server, b = s.ball, i = s.serveInput || s.inputs[seat], sign = side(seat, s);
    const heading = i.direction || { x: i.aim * 0.16, z: -sign };
    const norm = Math.hypot(heading.x, heading.z), pace = 2.4 + clamp(i.power, 0.1, 1) * 2.8;
    ...
    if (forwardPace > 0.5) {
        ...
        const end = b.z * sign, depth = 0.4 + i.power * 0.72;
        const e = TABLE_RESTITUTION * TABLE_GRIP, a = 1 + e;
        const c = (2 * e * (b.y - TABLE_HEIGHT - BALL_RADIUS) * forwardPace ** 2) / GRAVITY;
        const root = Math.sqrt(Math.max(0, (end + depth) ** 2 - 4 * a * c));
        const distances = [
            (end + depth - root) / (2 * a),
            (end + depth + root) / (2 * a)
        ];
        const distance = distances.find((d) => end - d > 0.2 && end - d < HALF_LENGTH - 0.03) ||
            distances[1];
...
function shot(s, seat) {
    const b = s.ball, p = s.players[seat], ai = seat === 1 && s.config.ai;
    const i = ai || s.inputs[seat].autoHit
        ? s.inputs[seat]
        : p.queuedInput || s.inputs[seat];
    if (b.bounces !== 1 || b.last === seat || (b.serve && b.serveStage !== 2))
        return;
    const power = clamp(i.power + (seat === 0 ? (s.config.upgrades[1] || 0) * 0.04 : 0), 0.1, 1);
    const depth = i.shot === 'backspin' ? 0.35 + power * 0.45 : 0.45 + power * 0.65;
    const x = i.aim * (i.shot === 'smash' ? 0.65 : 0.61) + (ai ? s.aiError : 0), z = -side(seat, s) * depth;
    const length = Math.hypot(x - b.x, z - b.z), heading = i.direction;
    const dx = heading ? heading.x * length : x - b.x, dz = heading ? heading.z * length : z - b.z;
    const pace = (i.shot === 'backspin' ? 2.5 : 3) + power * (i.shot === 'smash' ? 9 : 7);
    ...
    if (netFraction > 0 && netFraction < 1) {
        const base = b.y * (1 - netFraction) + (TABLE_HEIGHT + BALL_RADIUS) * netFraction;
        const clearance = TABLE_HEIGHT + NET_HEIGHT + BALL_RADIUS + 0.035;
        flight = Math.max(flight, Math.sqrt(Math.max(0, (2 * (clearance - base)) / (g * netFraction * (1 - netFraction)))));
    }
    b.vx = dx / flight;
    b.vz = dz / flight;
    b.vy =
...
function contact(s, c) {
    ...
    if (c.kind === 'table' || c.kind === 'edge') {
        const b = s.ball, landed = c.z * side(0, s) >= 0 ? 0 : 1;
        event(s, 'bounce', landed, c.kind === 'edge' ? 'Top edge · in' : 'Table bounce');
        if (s.phase !== 'rally')
            return;
        if (b.serve) {
            if (b.serveStage >= 2) {
        ...
        if (b.bounces === 1 && b.vy === 0)
            point(s, b.last, 'Unreturned');
    }
    else if (s.phase === 'rally') {
        point(s, s.ball.bounces === 1 ? s.ball.last : opponent(s.ball.last), c.kind === 'side'
            ? 'Vertical side · out'
            : s.ball.bounces === 1
                ? 'Unreturned'
                : s.ball.serve
...
function movePlayers(s, dt) {
    ...
    for (const seat of [0, 1]) {
        const p = s.players[seat], input = s.inputs[seat], ai = seat === 1 && s.config.ai;
        const incoming = b.last !== seat && s.phase === 'rally', sign = side(seat, s);
        if (ai) {
        ...
        if (incoming &&
            ...
            if (b.bounces === 1 && (ai || input.autoHit || queued))
                shot(s, seat);
...
function step(s, dt) {
    if (s.phase === 'over' && ballAtRest(s.ball))
        return;
    s.time += dt;
    if (s.phase === 'point' || s.phase === 'over') {
        advanceBall(s.ball, dt, (c) => contact(s, c));
        if (s.phase !== 'over' && s.time - s.phaseAt >= 1.25)
            resetPoint(s);
        return;
    }
    if (s.phase === 'serve') {
        ...
        if ((server === 1 && s.config.ai && s.time - s.phaseAt > 0.9) ||
    ...
    if (s.phase !== 'rally')
        return;
    const b = s.ball;
    const missedEnd = b.bounces === 0 && b.z * side(b.last, s) < -HALF_LENGTH - BALL_RADIUS;
    if (missedEnd || Math.abs(b.z) > 4 || Math.abs(b.x) > 3.2) {
        point(s, b.bounces === 1 ? b.last : opponent(b.last), b.bounces === 1 ? 'Unreturned' : b.serve ? 'Service fault' : 'Out');
        return;
    }
    movePlayers(s, dt);
}
export function advance(s, elapsed) {
