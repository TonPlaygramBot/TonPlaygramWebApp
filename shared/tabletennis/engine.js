...
export const HALF_WIDTH = 0.7625, HALF_LENGTH = 1.37, TABLE_HEIGHT = 0.76, NET_HEIGHT = 0.1525, BALL_RADIUS = 0.02, GRAVITY = 9.81;
export const side = (s) => (s === 0 ? 1 : -1);
export const opponent = (s) => (s === 0 ? 1 : 0);
...
export function setInput(s, seat, p) {
    if (seat !== 0 && seat !== 1)
        return;
    const old = s.inputs[seat], finite = (v, f) => typeof v === 'number' && Number.isFinite(v) ? v : f;
...
function resetPoint(s) {
    const seat = s.score.server, p = s.players[seat];
    s.phase = 'serve';
    s.phaseAt = s.time;
    s.rally = 0;
...
function serve(s) {
    const seat = s.score.server, b = s.ball, i = s.inputs[seat], t = 0.32;
...
function shot(s, seat) {
    const b = s.ball, i = s.inputs[seat];
    if (b.serve && b.serveStage !== 2)
        return;
    if (b.bounces !== 1 || b.last === seat)
        return;
    ...
    const power = clamp(i.power + (seat === 0 ? (s.config.upgrades[1] || 0) * 0.06 : 0), 0, 1);
    const t = i.shot === 'smash'
        ? 0.44
        : i.shot === 'backspin'
            ? 0.68
            : 0.6 - power * 0.075;
...
function physics(s, dt) {
    const b = s.ball, prev = { x: b.x, y: b.y, z: b.z };
    ...
    if (prev.z * b.z < 0) {
        const t = prev.z / (prev.z - b.z), y = prev.y + (b.y - prev.y) * t, x = prev.x + (b.x - prev.x) * t;
        if (Math.abs(x) <= HALF_WIDTH + 0.1525 &&
    ...
    if (prev.y > TABLE_HEIGHT + BALL_RADIUS &&
        b.y <= TABLE_HEIGHT + BALL_RADIUS &&
        b.vy < 0) {
        const t = (prev.y - TABLE_HEIGHT - BALL_RADIUS) / (prev.y - b.y), x = prev.x + (b.x - prev.x) * t, z = prev.z + (b.z - prev.z) * t;
        if (Math.abs(x) <= HALF_WIDTH && Math.abs(z) <= HALF_LENGTH) {
            const landed = z >= 0 ? 0 : 1;
            if (b.serve) {
    ...
    if (b.y < 0.1 || Math.abs(b.z) > 3.2 || Math.abs(b.x) > 2.6) {
        point(s, b.bounces === 1 ? b.last : opponent(b.last), b.bounces === 1 ? 'Unreturned' : 'Out');
    }
}
function step(s, dt) {
    s.time += dt;
    if (s.phase === 'over')
        return;
    if (s.phase === 'point') {
        if (s.time - s.phaseAt > 1.1)
            resetPoint(s);
        return;
    }
    const b = s.ball;
    for (let n = 0; n < 2; n++) {
        const seat = n, p = s.players[n], input = s.inputs[n], ai = n === 1 && s.config.ai;
        if (ai) {
            input.aim = s.aiAim;
    ...
    if (s.phase === 'serve') {
        ...
        if ((s.score.server === 1 && s.config.ai && s.time - s.phaseAt > 0.8) ||
