...
import { newPlayer, frameIndex, availablePins, recordRoll } from './scoring.js';
...
export function validThrow(input) {
    if (!input || typeof input !== 'object')
        return false;
    const x = input;
    return (Object.keys(x).every((k) => ['power', 'releaseX', 'targetX', 'hook'].includes(k)) &&
        [x.power, x.releaseX, x.targetX, x.hook].every((n) => typeof n === 'number' && Number.isFinite(n)) &&
        x.power >= 0.08 &&
        x.power <= 1 &&
...
export function swipeInput(width, height, sx, sy, x, y) {
...
export class BowlingMatch {
    ...
    aiInput() {
        ...
        const target = standing.length === 10
            ? 0.16
    ...
    launch() {
        ...
        const lane = new Material('lane'), ball = new Material('ball'), pin = new Material('pin');
        world.addContactMaterial(new ContactMaterial(lane, ball, { friction: 0.012, restitution: 0.02 }));
        world.addContactMaterial(new ContactMaterial(lane, pin, { friction: 0.24, restitution: 0.06 }));
        world.addContactMaterial(new ContactMaterial(ball, pin, { friction: 0.2, restitution: 0.38 }));
        world.addContactMaterial(new ContactMaterial(pin, pin, { friction: 0.25, restitution: 0.34 }));
        ...
        const body = new Body({
            ...
            position: new Vec3(input.releaseX, LANE.y + LANE.ballR + 0.012, LANE.foulZ - 0.18),
            linearDamping: 0.018,
            angularDamping: 0.02
        });
        const speed = 8 + input.power * 9.2;
        const dz = LANE.pinZ - (LANE.foulZ - 0.18), dx = input.targetX - input.releaseX, length = Math.hypot(dx, dz);
        body.velocity.set((dx / length) * speed, 0, (dz / length) * speed);
        body.angularVelocity.set(body.velocity.z / LANE.ballR, 0, -body.velocity.x / LANE.ballR);
        world.addBody(body);
        this.ballBody = body;
        this.world = world;
        this.settleAge = 0;
    ...
    resolve() {
        ...
        const pins = s.standing.filter(Boolean).length - remaining.filter(Boolean).length;
    ...
    step(dt) {
        ...
        if (s.phase === 'return') {
            ...
            if (t >= 1) {
                if (s.players.every((p) => frameIndex(p) < 0)) {
                    this.end(s.players[0].total === s.players[1].total
                        ? null
        ...
        const ball = this.ballBody, world = this.world;
        // Once over a gutter edge, a delivery cannot bounce back into the rack.
        if (Math.abs(ball.position.x) > LANE.halfWidth + 0.015)
            s.gutter = true;
        if (s.gutter) {
        ...
        const ballDone = ball.position.z < -13.6 ||
            ball.position.y < -1 ||
            ball.velocity.length() < 0.12;
        if (ballDone)
            this.settleAge += dt;
        const moving = this.pinBodies.some((b) => b &&
            (b.velocity.lengthSquared() > 0.025 ||
                b.angularVelocity.lengthSquared() > 0.04));
        if ((this.settleAge > 0.7 && !moving) ||
            this.settleAge > 2.6 ||
            s.phaseTime > 8)
            this.resolve();
    }
