...
import { BALL_RADIUS, GRAVITY, REVIEW_SECONDS, lineCall, reviewActive } from './court.js';
import { aiSpeed, predictIntercept, planAiShot } from './ai.js';
import { advanceBall, ballAtRest } from './physics.js';
export { BALL_RADIUS, GRAVITY, HALF_WIDTH, HALF_LENGTH, SERVICE, CONTACT_RADIUS, REVIEW_SECONDS, reviewActive } from './court.js';
...
function trajectory(s, seat, input, isServe) {
    const b = s.ball, sign = side(seat), ai = seat === 1 && s.config.ai;
    const power = clamp(input.power, 0.1, 1), shot = isServe ? 'flat' : input.shot;
    if (isServe) {
    ...
    if (!isServe && input.depth !== undefined)
        depth = 1.8 + input.depth * 8.8;
    if (shot === 'slice')
    ...
    const pace = (shot === 'lob' ? 7.5 : shot === 'slice' ? 8.5 : 9) +
        power * (isServe ? 11 : 16);
    let flight = Math.max(0.65, length / pace);
    if (shot === 'lob')
        flight = Math.max(flight, 2.15 - power * 0.45);
    ...
    if (netFraction > 0 && netFraction < 1) {
        ...
        const clearFlight = Math.sqrt(Math.max(0, (2 * (netY - baseY)) / (GRAVITY * netFraction * (1 - netFraction))));
...
export function setInput(s, seat, input) {
    ...
    s.inputs[seat] = {
        ...
        direction: rawDirection && norm > 0.001
...
function movePlayers(s, dt) {
    for (const seat of [0, 1]) {
        ...
        if (incoming && assisted && (!ai || s.time >= s.aiThink)) {
            const intercept = predictIntercept(s, seat, speed * (0.78 + p.stamina * 0.22));
...
export function stepMatch(s, dt = 1 / 120) {
    if (!Number.isFinite(dt) || dt <= 0)
        return;
    if (s.phase === 'over' && !reviewActive(s) && ballAtRest(s.ball))
        return;
    dt = clamp(dt, 0, 1 / 30);
    s.time += dt;
    if (reviewActive(s))
    ...
    if (s.phase === 'over' || s.phase === 'point' || s.phase === 'fault') {
        ...
        if (s.phase !== 'over' && s.time - s.phaseAt > 1.55)
            setupServe(s);
        return;
    }
    ...
    advanceBall(b, dt, s.config.surface, {
        ...
        bounce: (impact, rebound, elapsed, first) => {
            ...
            if (s.phase !== 'rally')
                return !reviewActive(s);
            if (first) {
                const call = lineCall(impact.x, impact.z, b.last, b.serve, s.players[b.last].x);
