...
import { BALL_RADIUS, GRAVITY, groundTime, bounceVelocity } from './court.js';
const REST_SPEED = 0.28;
export function ballAtRest(b) {
    return (b.y <= BALL_RADIUS + 1e-7 && b.vy === 0 && Math.hypot(b.vx, b.vz) === 0);
}
function fly(b, dt) {
    b.x += b.vx * dt;
    b.z += b.vz * dt;
...
function roll(b, dt, surface) {
    const drag = surface === 'clay' ? 3.6 : surface === 'grass' ? 2.8 : 2.2;
    const decay = Math.exp(-drag * dt), distance = (1 - decay) / drag;
    b.x += b.vx * distance;
    b.z += b.vz * distance;
    b.vx *= decay;
    b.vz *= decay;
...
export function advanceBall(b, seconds, surface, contacts = {}) {
    if (!Number.isFinite(seconds) || seconds <= 0)
        return;
    let remaining = Math.min(seconds, 1 / 30), elapsed = 0;
    ...
    for (let collision = 0; remaining > 1e-9 && collision < 8; collision++) {
        ...
        if (net > 1e-9 && net <= travel) {
            ...
            if (Math.abs(x) < 5.6 && y < 0.98 + BALL_RADIUS) {
                ...
                if (contacts.net?.(letCandidate) === false)
                    return;
                continue;
            }
        }
        ...
        const rebound = bounceVelocity(impact, surface, b.spin), first = b.bounces === 0;
        if (rebound.vy < REST_SPEED)
            rebound.vy = 0;
        b.bounces++;
        Object.assign(b, rebound);
        if (contacts.bounce?.(impact, rebound, elapsed, first) === false)
            return;
    }
}
