...
import { clamp, side } from './engine.js';
const WINDOW_MS = 120;
export const TAP_POWER = 0.1;
export function beginSwipe(x, y, time, width) {
    const start = { x, y, time };
    return { width: Math.max(1, width), start, samples: [start] };
}
export function sampleSwipe(swipe, x, y, time) {
    ...
    if (time < last.time)
        return;
    if (time === last.time)
        samples[samples.length - 1] = { x, y, time };
    else
 ...
 */
export function readSwipe(swipe, now = swipe.samples[swipe.samples.length - 1].time) {
    const samples = swipe.samples;
    const last = samples[samples.length - 1];
    ...
    const speed = distance < 0.008 ? 0 : (distance * 1000) / Math.max(16, now - from);
    const power = TAP_POWER +
...
export function swipeShot(swipe) {
    ...
    if (dy < -0.27 && readSwipe(swipe).speed < 1.5)
        return 'lob';
    if (dy < -0.07)
        return 'topspin';
    if (dy > 0.07)
...
export function screenAim(fraction, seat) {
