...
export function App() {
  ...
  useEffect(() => {
    ...
    const pick = (clientX: number, clientY: number) => {
      ...
      const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
      const faceMaterialIndex = Number(hit.face.materialIndex ?? 0);
      const material = materials[Math.max(0, Math.min(faceMaterialIndex, materials.length - 1))] as WorkingMaterial;
      setPickInfo({
