...
type IconName = "back" | "search" | "bag" | "layout" | "grid" | "list" | "star" | "compact" | "editorial" | "camera" | "upload" | "check" | "close" | "home" | "hanger" | "user" | "sparkles" | "heart" | "arrow";
...
export function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="hero" aria-labelledby="store-hero-title">
      <div className="hero-copy"><span className="eyebrow"><Icon name="sparkles" size={13}/> YOUR DIGITAL TWIN</span><h1 id="store-hero-title">Wear it<br/><em>your way.</em></h1><p>Scan once. Preview every look on your own character before you buy.</p><button className={scanned?"scan-button complete":"scan-button"} onClick={()=>setScanOpen(true)}><span><Icon name={scanned?"check":"camera"}/></span><div><b>{scanned?"Face ready":"Create your face"}</b><small>{scanned?"Tap to scan again":"Quick scan · about 10 sec"}</small></div><strong>›</strong></button></div>
    ...
    <section className="store">
      ...
      <div className={`products layout-${layout}`}>{products.map((p,i)=><article className="product" key={p.name}><div className={`product-art ${p.color}`}><span className="product-number">0{i+1}</span><button className="favorite-button" aria-label={`Save ${p.name}`}><Icon name="heart" size={15}/></button>{p.mark&&<b className="tag">{p.mark}</b>}<div className={`fashion-shape shape-${i}`}><i/><i/></div><button className="try-button" onClick={()=>!scanned&&setScanOpen(true)}>{scanned?"Try on your avatar":"Scan to try on"}<Icon name="arrow" size={14}/></button></div><div className="product-info"><div><h3>{p.name}</h3><p>{p.kind}</p></div><b>{p.price}</b></div></article>)}</div>
    </section>
